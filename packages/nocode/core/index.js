...
import { defineComponent as e, useCssVars as t, useTemplateRef as o, computed as l, watch as n, reactive as a, onMounted as i, createElementBlock as r, openBlock as s, normalizeStyle as u, normalizeClass as c, unref as d, createElementVNode as p, createCommentVNode as f, createBlock as h, createVNode as g, ref as y, Fragment as m, renderList as v, nextTick as C, toRaw as b, withCtx as M, withModifiers as k, resolveComponent as w, createTextVNode as x, toDisplayString as S, onUnmounted as _, mergeProps as $, readonly as L, normalizeProps as I, guardReactiveProps as D, shallowRef as T, toHandlers as N, renderSlot as B, onBeforeMount as R, createSlots as q, toRefs as A, withDirectives as O, resolveDynamicComponent as j, vShow as z, markRaw as F, Transition as P } from "vue";
import { theme as E, Input as H, InputNumber as Q, Tooltip as G, Segmented as U, Dropdown as V, Checkbox as W, RadioButton as Z, RadioGroup as Y, Select as K, Image as X, Divider as J, Button as ee, Menu as te, Cascader as oe, Radio as le, Form as ne, DatePicker as ae, RangePicker as ie, Switch as re, Breadcrumb as se, Descriptions as ue, Row as ce, Col as de, Card as pe, Drawer as fe, Alert as he, Tag as ge, ConfigProvider as ye, Watermark as me, Modal as ve, InputGroup as Ce, Empty as be, Textarea as Me, MenuItem as ke } from "ant-design-vue";
import { useDesign as we, useRootSetting as xe, useMessage as Se, useDarkModeTheme as _e } from "@mfish/core/hooks";
import { useEyeDropper as $e, useClipboard as Le, useDebounceFn as Ie, useFocus as De } from "@vueuse/core";
import { Icon as Te, IconFont as Ne } from "@mfish/core/components/Icon";
import { cloneDeep as Be, debounce as Re, once as qe, isElement as Ae, merge as Oe, uniqueId as je, sum as ze, random as Fe, throttle as Pe, omit as Ee } from "lodash-es";
import { ThemeEnum as He } from "@mfish/core/enums";
import { fade as Qe } from "@jiaminghi/color";
import { buildUUID as Ge } from "@mfish/core/utils/Uuid";
import { isFunction as Ue, isArray as Ve, isObject as We, isString as Ze, isNumber as Ye } from "@mfish/core/utils/Is";
import { defHttp as Ke } from "@mfish/core/utils/http/axios";
import { defineStore as Xe } from "pinia";
import { useAppStore as Je } from "@mfish/stores/modules";
import et from "@mfish/core/assets/images/noImage.png";
import { mitt as tt } from "@mfish/core/utils/Mitt";
import { createAsyncComponent as ot } from "@mfish/core/utils/factory/CreateAsyncComponent";
import { router as lt } from "@mfish/core/router";
import { formatToDateTime as nt } from "@mfish/core/utils/DateUtil";
import { imageUrl as at, getLocalFileUrl as it, imageSrc as rt } from "@mfish/core/utils/file/FileUtils";
import { DeleteOutlined as st, CaretRightOutlined as ut, CheckOutlined as ct, FolderOpenTwoTone as dt, FolderTwoTone as pt } from "@ant-design/icons-vue";
import { ScrollContainer as ft } from "@mfish/core/components/Container";
import { DraggableInput as ht } from "@mfish/core/components/Draggable";
import { BasicModal as gt, useModalInner as yt, useModal as mt } from "@mfish/core/components/Modal";
import { BasicForm as vt, useForm as Ct } from "@mfish/core/components/Form";
import { BasicTree as bt } from "@mfish/core/components/Tree";
import { Split as Mt } from "@mfish/core/components/Split";
import { useTable as kt, BasicTable as wt } from "@mfish/core/components/Table";
import { CodeEditor as xt, MODE as St } from "@mfish/core/components/CodeEditor";
import { useLocale as _t } from "@mfish/core/i18n/UseLocale";
import { propTypes as $t } from "@mfish/core/utils/PropTypes";
import { FileUp as Lt } from "@mfish/core/components/FileUpDown";
import It from "vuedraggable";
import { NCollapseTransition as Dt } from "naive-ui";
import { queryCategoryTreeByCode as Tt } from "@mfish/core/api";
const Nt = "color_recently_bi", Bt = "gradient_color_recently_bi", Rt = ["rgba(255, 255, 255, 1)", "rgba(0, 0, 0, 1)", "rgba(35, 69, 231, 1)", "rgba(35, 231, 196, 1)", "rgba(65, 231, 35, 1)", "rgba(231, 35, 35, 1)"], qt = ["linear-gradient(120deg, rgba(243,231,233,1) 0%, rgba(227,238,255,1) 100%)", "linear-gradient(45deg, rgba(161,140,209,1) 0%, rgba(251,194,235,1) 100%)", "linear-gradient(90deg, rgba(255,236,210,1) 0%, rgba(252,182,159,1) 100%)", "linear-gradient(180deg, rgba(204,32,142,1) 0%, rgba(103,19,210,1) 100%)", "linear-gradient(-225deg, rgba(35,21,87,1) 0%, rgba(68,16,122,1) 29%, rgba(255,19,97,1) 67%, rgba(255,248,0,1) 100%)", "linear-gradient(180deg, rgba(30,60,114,1) 0%, rgba(30,60,114,1) 1%, rgba(42,82,152,1) 100%)", "linear-gradient(-20deg, rgba(43,88,118,1) 0%, rgba(78,67,118,1) 100%)", "linear-gradient(45deg, rgba(58,181,176,1) 0%, rgba(61,153,190,1) 31%, rgba(86,49,122,1) 100%)", "linear-gradient(180deg, rgba(106,17,203,1) 0%, rgba(37,117,252,1) 100%)"];
function At(e2) {
...
}, Ft = { 10: "A", 11: "B", 12: "C", 13: "D", 14: "E", 15: "F" }, Pt = (e2) => {
  e2 = Math.min(Math.round(e2), 255);
  const t2 = Math.floor(e2 / 16), o2 = e2 % 16;
  return `${Ft[t2] || t2}${Ft[o2] || o2}`;
}, Et = function({ r: e2, g: t2, b: o2 }) {
  return Number.isNaN(+e2) || Number.isNaN(+t2) || Number.isNaN(+o2) ? "" : `#${Pt(e2)}${Pt(t2)}${Pt(o2)}`;
}, Ht = { A: 10, B: 11, C: 12, D: 13, E: 14, F: 15 }, Qt = function(e2) {
  return 2 === e2.length ? 16 * (Ht[e2[0].toUpperCase()] || +e2[0]) + (Ht[e2[1].toUpperCase()] || +e2[1]) : Ht[e2[1].toUpperCase()] || +e2[1];
}, Gt = (e2, t2, o2) => {
  e2 = zt(e2, 255), t2 = zt(t2, 255), o2 = zt(o2, 255);
...
}, Ut = function(e2, t2, o2) {
  e2 = 6 * zt(e2, 360), t2 = zt(t2, 100), o2 = zt(o2, 100);
...
class Vt {
  ...
  toRgb() {
    return Ut(this._hue, this._saturation, this._value);
  }
  toHex() {
    const e2 = this.toRgb();
    return Et(e2).slice(0, 7);
  }
  fromString(e2) {
    ...
    if (e2.includes("hsl")) {
      ...
      if (4 === o2.length ? this._alpha = 100 * Number.parseFloat(o2[3].toString()) : 3 === o2.length && (this._alpha = 100), o2.length >= 3) {
        const { h: e3, s: l2, v: n2 } = Gt(o2[0], o2[1], o2[2]);
        t2(e3, l2, n2);
      }
    } else if (e2.includes("#")) {
      ...
      let l2, n2, a2;
      3 === o2.length ? (l2 = Qt(o2[0] + o2[0]), n2 = Qt(o2[1] + o2[1]), a2 = Qt(o2[2] + o2[2])) : 6 !== o2.length && 8 !== o2.length || (l2 = Qt(o2.slice(0, 2)), n2 = Qt(o2.slice(2, 4)), a2 = Qt(o2.slice(4, 6))), 8 === o2.length ? this._alpha = Qt(o2.slice(6)) / 255 * 100 : 3 !== o2.length && 6 !== o2.length || (this._alpha = 100);
      const { h: i2, s: r2, v: s2 } = Gt(l2, n2, a2);
      t2(i2, r2, s2);
    }
  }
  compare(e2) {
  ...
  doOnChange() {
    ...
    if (this.enableAlpha) switch (n2) {
      ...
      case "hsv":
        ...
        break;
      case "hex":
        this.value = `${Et(Ut(e2, t2, o2))}${Pt(255 * l2 / 100)}`;
        break;
      default: {
        const { r: l3, g: n3, b: a2 } = Ut(e2, t2, o2);
...
function Zt(e2, t2) {
  const o2 = function(e3) {
    ...
    var _a2;
    document.removeEventListener("mousemove", o2), document.removeEventListener("mouseup", l2), document.removeEventListener("touchmove", o2), document.removeEventListener("touchend", l2), document.onselectstart = null, document.ondragstart = null, Wt = false, (_a2 = t2.end) == null ? void 0 : _a2.call(t2, e3);
  }, n2 = function(e3) {
    var _a2;
    Wt || (e3.preventDefault(), document.onselectstart = () => false, document.ondragstart = () => false, document.addEventListener("mousemove", o2), document.addEventListener("mouseup", l2), document.addEventListener("touchmove", o2), document.addEventListener("touchend", l2), Wt = true, (_a2 = t2.start) == null ? void 0 : _a2.call(t2, e3));
  };
...
const Yt = (e2, t2) => {
  ...
  return o2;
}, Kt = Yt(e({ __name: "ColorPanel", props: { color: { type: Object, required: true }, height: { type: Number, default: 150 }, isShow: { type: Boolean, default: false } }, setup(e2) {
  t((t2) => ({ "003068da": `${e2.height}px` }));
  const h2 = e2, g2 = o("colorPanelRef"), y2 = l(() => ({ hue: h2.color.get("hue"), value: h2.color.get("value") })), { prefixCls: m2 } = we("color-panel");
  n(() => y2, () => {
    C2();
...
} }), [["__scopeId", "data-v-696c5dfb"]]), Xt = Yt(e({ __name: "ColorHueSlider", props: { color: { type: Object, required: true }, vertical: { type: Boolean, default: false } }, emits: ["startEyeDropper", "endEyeDropper"], setup(e2, { emit: t2 }) {
  const y2 = e2, m2 = t2, v2 = a({ left: 0, top: 0, hue: 0 }), C2 = o("barRef"), b2 = o("thumbRef"), M2 = o("colorHueSliderRef"), { prefixCls: k2 } = we("hue-slider"), { isSupported: w2, open: x2, sRGBHex: S2 } = $e(), _2 = l(() => y2.color.get("hue"));
  function $2() {
  ...
  return i(() => {
    ...
    D2();
  }), (e3, t3) => (s(), r("div", { class: c(d(k2)) }, [d(w2) ? (s(), h(d(Te), { key: 0, class: "icon", icon: "ion:eyedrop-outline", onClick: $2 })) : f("", true), p("div", { class: "bar-contain", ref_key: "colorHueSliderRef", ref: M2 }, [p("div", { class: "bar", onClick: L2, ref_key: "barRef", ref: C2 }, null, 512), p("div", { class: "thumb", ref_key: "thumbRef", ref: b2, style: u({ left: `${v2.left}px`, top: `${v2.top}px` }) }, [g(d(Te), { class: "thumb__icon", icon: "ant-design:caret-up-outlined" })], 4)], 512)], 2));
} }), [["__scopeId", "data-v-b2d0e443"]]), Jt = { class: "background" }, eo = { class: "color-text" }, to = { class: "color-input" }, oo = { class: "color-input-percentage" }, lo = { class: "color-input-percentage" }, no = { class: "color-input-percentage" }, ao = { class: "color-input-percentage a" }, io = Yt(e({ __name: "ColorText", props: { color: { type: Object, required: true } }, setup(e2) {
  t((e3) => ({ "09aca737": v2.value }));
  const o2 = e2, i2 = y(""), f2 = a({ r: 0, g: 0, b: 0, a: 100 }), { prefixCls: h2 } = we("color-text"), { token: m2 } = E.useToken(), v2 = l(() => m2.value.colorBorder), C2 = l(() => `rgba(${f2.r}, ${f2.g}, ${f2.b}, ${f2.a / 100})`);
  function b2() {
  ...
  }), (e3, t2) => (s(), r("div", { class: c(d(h2)) }, [p("div", Jt, [p("div", { class: "background-color", style: u({ backgroundColor: C2.value }) }, null, 4)]), p("div", eo, [p("div", to, [g(d(H), { size: "small", value: i2.value, "onUpdate:value": t2[0] || (t2[0] = (e4) => i2.value = e4), onPressEnter: M2, onBlur: M2, maxlength: 7 }, null, 8, ["value"])])]), p("div", oo, [t2[5] || (t2[5] = p("div", { class: "prefix" }, "R", -1)), g(d(Q), { size: "small", value: f2.r, "onUpdate:value": t2[1] || (t2[1] = (e4) => f2.r = e4), bordered: false, controls: false, onPressEnter: k2, onBlur: k2, step: 1, min: 0, max: 255, maxlength: 3 }, null, 8, ["value"])]), p("div", lo, [t2[6] || (t2[6] = p("div", { class: "prefix" }, "G", -1)), g(d(Q), { size: "small", bordered: false, value: f2.g, "onUpdate:value": t2[2] || (t2[2] = (e4) => f2.g = e4), controls: false, onPressEnter: k2, onBlur: k2, step: 1, min: 0, max: 255, maxlength: 3 }, null, 8, ["value"])]), p("div", no, [t2[7] || (t2[7] = p("div", { class: "prefix" }, "B", -1)), g(d(Q), { size: "small", bordered: false, value: f2.b, "onUpdate:value": t2[3] || (t2[3] = (e4) => f2.b = e4), controls: false, onPressEnter: k2, onBlur: k2, step: 1, min: 0, max: 255, maxlength: 3 }, null, 8, ["value"])]), p("div", ao, [t2[8] || (t2[8] = p("div", { class: "prefix" }, "A", -1)), g(d(Q), { size: "small", bordered: false, value: f2.a, "onUpdate:value": t2[4] || (t2[4] = (e4) => f2.a = e4), controls: false, onPressEnter: b2, onChange: b2, step: 1, min: 0, max: 100, maxlength: 3 }, null, 8, ["value"]), t2[9] || (t2[9] = p("div", null, "%", -1))])], 2));
} }), [["__scopeId", "data-v-2bab5fee"]]), ro = { class: "colors" }, so = ["onClick"], uo = e({ __name: "ColorRecently", props: { color: { type: Object, required: true }, isShow: { type: Boolean, default: false } }, setup(e2) {
  t((e3) => ({ "8433eb3a": g2.value }));
  const o2 = e2, a2 = y(Rt), i2 = y([]), { prefixCls: f2 } = we("color-recently"), { token: h2 } = E.useToken(), g2 = l(() => h2.value.colorBorder);
  function C2(e3, t2) {
    return e3.map((e4) => {
      const o3 = new Vt();
  ...
  return n(a2, (e3) => {
    ...
    e3 && function() {
      const e4 = localStorage.getItem(Nt);
      if (!e4) return void (i2.value = C2(d(a2), o2.color));
      const t2 = JSON.parse(e4);
      t2 && t2.length > 0 && (a2.value = [...t2].slice(0, 20));
    }();
  }, { immediate: true }), (e3, t2) => (s(), r("div", { class: c(d(f2)) }, [t2[0] || (t2[0] = p("div", { class: "last-time-text" }, "最近使用", -1)), p("div", ro, [(s(true), r(m, null, v(i2.value, (e4, t3) => (s(), r("div", { class: c(["color-selector", { selected: e4.selected }]), key: t3, onClick: (e5) => function(e6) {
    o2.color.fromString(a2.value[e6]);
  }(t3) }, [p("div", { class: "default", style: u({ "background-color": e4.value }) }, null, 4)], 10, so))), 128))])], 2));
} }), co = Yt(uo, [["__scopeId", "data-v-98ec7ef1"]]), po = { class: "colors" }, fo = ["onClick"], ho = e({ __name: "GradientColorRecently", props: { color: { type: String, default: "" }, isShow: { type: Boolean, default: false } }, emits: ["selectColor"], setup(e2, { emit: o2 }) {
  t((e3) => ({ "7fb04f44": C2.value }));
  const a2 = e2, i2 = o2, f2 = y([]), { prefixCls: h2 } = we("color-recently"), { token: g2 } = E.useToken(), C2 = l(() => g2.value.colorBorder), b2 = y();
  return n(() => a2.isShow, (e3) => {
    e3 && (!function() {
      const e4 = localStorage.getItem(Bt);
      if (!e4) return void (f2.value = qt);
      const t2 = JSON.parse(e4);
  ...
  }, { immediate: true }), (e3, t2) => (s(), r("div", { class: c(d(h2)) }, [t2[0] || (t2[0] = p("div", { class: "last-time-text" }, "最近使用", -1)), p("div", po, [(s(true), r(m, null, v(f2.value, (e4, t3) => (s(), r("div", { class: c(["color-selector", { selected: b2.value === t3 }]), key: t3, onClick: (o3) => function(e5, t4) {
    b2.value = e5;
    const o4 = Ot(t4);
    i2("selectColor", o4);
  }(t3, e4) }, [p("div", { class: "default", style: u({ background: e4 }) }, null, 4)], 10, fo))), 128))])], 2));
} }), go = Yt(ho, [["__scopeId", "data-v-be7486eb"]]), yo = { class: "hue-slider" }, mo = ["onMousedown", "onDblclick"], vo = { class: "color-input-percentage" }, Co = e({ __name: "GradientColor", props: { isShow: { type: Boolean, default: false }, colors: { type: Object, default: () => ({ deg: 45, colors: [{ rgba: "rgba(255, 255, 0, 1)", rgbaLength: "0%" }, { rgba: "rgba(0, 0, 255, 0.9)", rgbaLength: "100%" }] }) } }, emits: ["colorChange", "startEyeDropper", "endEyeDropper"], setup(e2, { emit: a2 }) {
  t((e3) => ({ 40133406: T2.value }));
  const i2 = e2, f2 = a2, h2 = y(new Vt({ enableAlpha: true, format: "rgba" }));
  const w2 = y(0), x2 = y([]), S2 = y(45), _2 = y(), $2 = o("barRef"), L2 = y([]), { prefixCls: I2 } = we("gradient-color"), { token: D2 } = E.useToken(), T2 = l(() => D2.value.colorBorder), N2 = l(() => _2.value ? { background: _2.value } : {});
  function B2(e3) {
    x2.value = Be(b(e3.colors)), _2.value = R2(false), w2.value = 0, h2.value.fromString(x2.value[0].rgba), S2.value = e3.deg;
  }
  function R2(e3 = true) {
  ...
  }, { deep: true }), (t2, o2) => (s(), r("div", null, [p("div", { class: c(d(I2)) }, [p("div", yo, [g(d(G), { title: "点击增加新颜色", placement: "left" }, { default: M(() => [p("div", { class: "bar", ref_key: "barRef", ref: $2, onClick: k(q2, ["stop", "prevent"]), style: u(N2.value) }, null, 4)]), _: 1 }), (s(true), r(m, null, v(x2.value, (e3, t3) => (s(), r("div", { class: "thumb", ref_for: true, ref: (e4) => function(e5, t4) {
    L2.value[t4] = e5;
  ...
  }(t3), style: u({ left: e3.rgbaLength }) }, [g(d(Te), { title: "双击取消", class: c(["thumb__icon", w2.value === t3 ? "selected" : ""]), icon: "ant-design:caret-up-outlined" }, null, 8, ["class"])], 44, mo))), 128))]), p("div", vo, [o2[3] || (o2[3] = p("div", { class: "prefix" }, "D", -1)), g(d(Q), { value: S2.value, "onUpdate:value": o2[0] || (o2[0] = (e3) => S2.value = e3), bordered: false, controls: false, max: 360, min: -360, step: 1, onChange: A2, maxlength: 4 }, null, 8, ["value"]), o2[4] || (o2[4] = p("div", { class: "suffix" }, "°", -1))])], 2), g(Kt, { style: { "margin-top": "4px" }, color: h2.value, height: 118, "is-show": e2.isShow }, null, 8, ["color", "is-show"]), g(Xt, { style: { "margin-top": "8px" }, color: h2.value, onStartEyeDropper: o2[1] || (o2[1] = () => f2("startEyeDropper")), onEndEyeDropper: o2[2] || (o2[2] = (e3) => f2("endEyeDropper", e3)) }, null, 8, ["color"]), g(io, { style: { "margin-top": "15px" }, color: h2.value }, null, 8, ["color"]), g(go, { style: { "margin-top": "8px" }, color: _2.value, "is-show": e2.isShow, onSelectColor: B2 }, null, 8, ["color", "is-show"])]));
} }), bo = Yt(Co, [["__scopeId", "data-v-cf05475f"]]), Mo = { class: "header" }, ko = { key: 0, class: "content" }, wo = { key: 1, class: "content" }, xo = { class: "confirm" }, So = { key: 0 }, _o = { key: 1 }, $o = e({ __name: "PickerDropdown", props: { type: { type: Number, default: 3 }, selectType: { type: Number, default: 1 }, color: { type: Object, required: true }, colors: { type: Object, default: () => ({ deg: 45, colors: [{ rgba: "rgba(255, 255, 0, 1)", rgbaLength: "0%" }, { rgba: "rgba(0, 0, 255, 0.9)", rgbaLength: "100%" }] }) }, isShow: { type: Boolean, default: false }, hideConfirm: { type: Boolean, default: false }, hideCancel: { type: Boolean, default: false } }, emits: ["changeGradientColor", "pickColor", "restColor", "selectTypeChange", "startEyeDropper", "endEyeDropper"], setup(e2, { emit: t2 }) {
  const o2 = e2, l2 = t2, { copy: a2, copied: u2 } = Le({ legacy: true }), m2 = y([{ value: 1, payload: "纯色" }, { value: 2, payload: "渐变色" }]), v2 = y(), C2 = y(o2.selectType);
  function b2(e3) {
  ...
  const { prefixCls: k2 } = we("picker-dropdown");
  function _2() {
  ...
  return (t3, l3) => {
    const n2 = w("AButton");
    return s(), r("div", { class: c(d(k2)) }, [p("div", Mo, [g(d(U), { value: C2.value, options: m2.value, onChange: b2, block: "" }, { label: M(({ payload: e3 }) => [x(S(e3), 1)]), _: 1 }, 8, ["value", "options"])]), 1 !== C2.value || 1 !== e2.type && 3 !== e2.type ? f("", true) : (s(), r("div", ko, [g(Kt, { style: { "margin-top": "4px" }, color: e2.color, "is-show": e2.isShow }, null, 8, ["color", "is-show"]), g(Xt, { style: { "margin-top": "8px" }, color: e2.color, onEndEyeDropper: T2, onStartEyeDropper: D2 }, null, 8, ["color"]), g(io, { style: { "margin-top": "15px" }, color: e2.color }, null, 8, ["color"]), g(co, { style: { "margin-top": "8px" }, color: e2.color, "is-show": e2.isShow }, null, 8, ["color", "is-show"])])), 2 !== C2.value || 2 !== e2.type && 3 !== e2.type ? f("", true) : (s(), r("div", wo, [g(bo, { onColorChange: d(L2), colors: e2.colors, "is-show": e2.isShow, onStartEyeDropper: D2, onEndEyeDropper: T2 }, null, 8, ["onColorChange", "colors", "is-show"])])), p("div", xo, [o2.hideCancel ? f("", true) : (s(), h(n2, { key: 0, onClick: $2 }, { default: M(() => l3[0] || (l3[0] = [x("取消")])), _: 1 })), g(n2, { onClick: I2, color: d(u2) ? "warning" : "primary" }, { default: M(() => [d(u2) ? (s(), r("span", _o, "!已拷贝")) : (s(), r("span", So, "拷贝"))]), _: 1 }, 8, ["color"]), o2.hideConfirm ? f("", true) : (s(), h(n2, { key: 1, onClick: _2, color: "success" }, { default: M(() => l3[1] || (l3[1] = [x("确定")])), _: 1 }))])], 2);
  };
} }), Lo = { key: 0, class: "color-background-text" }, Io = { class: "color-background-show-box" }, Do = { key: 1, style: { display: "flex" } }, To = { class: "color-text" }, No = { class: "color-show-box" }, Bo = ["title"], Ro = e({ name: "MfishColorPicker", __name: "index", props: { value: String, showAlpha: { type: Boolean, default: true }, type: { type: Number, default: 3 }, isDelete: { type: Boolean, default: false }, placeholder: { type: String, default: "请输入颜色，格式#FFFFFF" }, title: { type: String, default: "清空则为无色" }, hideConfirm: { type: Boolean, default: false }, hideCancel: { type: Boolean, default: false } }, emits: ["inputChange", "confirmChange"], setup(e2, { emit: o2 }) {
  t((e3) => ({ "018cd00d": I2.value }));
  const a2 = e2, i2 = o2, { prefixCls: m2 } = we("color-picker"), v2 = y(1), b2 = xe().getDarkMode, w2 = y(false), x2 = y(new Vt({ value: d(b2) === He.DARK ? "#000000" : "#FFFFFF", enableAlpha: a2.showAlpha, format: "rgba" })), S2 = y({ deg: 120, colors: [{ rgba: "rgba(243, 231, 233, 1)", rgbaLength: "0%" }, { rgba: "rgba(227,238,255,1)", rgbaLength: "100%" }] }), _2 = y(""), $2 = y(100), { token: L2 } = E.useToken(), I2 = l(() => L2.value.colorBorder), D2 = y(), T2 = y(), N2 = l(() => ({ background: d(D2), height: "100%" }));
  n(() => a2.value, () => {
    A2();
  }, { immediate: true }), n(x2, (e3) => {
  ...
  const z2 = () => {
    ...
    Z2(x2.value.value), C(() => i2("confirmChange", d(D2), d(_2)));
  };
  function F2() {
  ...
  function G2(e3) {
    e3.startsWith("linear-gradient(") ? (D2.value = e3, S2.value = Ot(e3), i2("inputChange", e3)) : (e3.value.fromString(e3), _2.value = e3.value.toHex());
  }
  function U2(e3) {
    R2.value = false, 2 === v2.value ? (D2.value = e3, S2.value = Ot(e3), Z2(e3), C(() => i2("confirmChange", d(D2), d(_2)))) : z2();
  }
  function W2() {
    R2.value = false, O2(), C(() => i2("confirmChange", d(D2), d(_2)));
  }
  function Z2(e3) {
    let t2 = [];
    const o3 = (e4, o4, l2) => {
    ...
    if (2 === v2.value) {
      return o3(localStorage.getItem(Bt), qt, e3), t2 = t2.slice(0, 14), void localStorage.setItem(Bt, JSON.stringify(t2));
    }
    o3(localStorage.getItem(Nt), Rt, e3), t2 = t2.slice(0, 20), localStorage.setItem(Nt, JSON.stringify(t2));
  }
  function Y2(e3) {
  ...
  return (t2, o3) => (s(), h(d(V), { trigger: ["click"], open: R2.value, onOpenChange: q2, placement: "bottom", arrow: { pointAtCenter: true } }, { overlay: M(() => [g($o, { color: x2.value, colors: S2.value, type: e2.type, "select-type": v2.value, "is-show": R2.value, "hide-cancel": e2.hideCancel, "hide-confirm": e2.hideConfirm, onPickColor: U2, onRestColor: W2, onChangeGradientColor: G2, onSelectTypeChange: Y2, onMouseleave: o3[0] || (o3[0] = (e3) => !K2.value && q2(false)), onStartEyeDropper: X2, onEndEyeDropper: J2 }, null, 8, ["color", "colors", "type", "select-type", "is-show", "hide-cancel", "hide-confirm"])]), default: M(() => [p("div", { class: c(d(m2)), onMouseenter: o3[5] || (o3[5] = () => w2.value = true), onMouseleave: o3[6] || (o3[6] = () => w2.value = false) }, [2 === v2.value ? (s(), r("div", Lo, [p("div", Io, [p("div", { style: u(N2.value) }, null, 4)]), w2.value ? (s(), h(d(Te), { key: 0, onClick: k(ee2, ["stop"]), size: "14", class: "close-icon", icon: "ant-design:close-circle-filled" })) : f("", true)])) : (s(), r("div", Do, [p("div", To, [p("div", No, [p("div", { style: u(N2.value) }, null, 4)]), p("div", { class: "color-input", title: e2.title, onClick: o3[2] || (o3[2] = k(() => {
  }, ["stop"])) }, [g(d(H), { placeholder: e2.placeholder, value: _2.value, "onUpdate:value": o3[1] || (o3[1] = (e3) => _2.value = e3), onBlur: F2, onPressEnter: F2, bordered: false, maxlength: 7 }, null, 8, ["placeholder", "value"]), w2.value && _2.value ? (s(), h(d(Te), { key: 0, onClick: k(ee2, ["stop"]), size: "14", class: "close-icon", icon: "ant-design:close-circle-filled" })) : f("", true)], 8, Bo)]), e2.showAlpha ? (s(), r("div", { key: 0, class: "color-input-percentage", onClick: o3[4] || (o3[4] = k(() => {
  }, ["stop"])) }, [g(d(Q), { class: "percent-input", bordered: false, value: $2.value, "onUpdate:value": o3[3] || (o3[3] = (e3) => $2.value = e3), min: 0, max: 100, step: 1, controls: false, onBlur: P2, onPressEnter: P2, maxlength: 3 }, { prefix: M(() => o3[7] || (o3[7] = [p("div", { class: "prefix" }, "A", -1)])), _: 1 }, 8, ["value"]), o3[8] || (o3[8] = p("div", { style: { "user-select": "none" } }, "%", -1))])) : f("", true)]))], 34)]), _: 1 }, 8, ["open"]));
} }), qo = Yt(Ro, [["__scopeId", "data-v-be499b94"]]);
const Ao = { info(...e2) {
}, warn(...e2) {
}, error(...e2) {
} };
function Oo() {
  const e2 = y(), t2 = [], o2 = a({ width: 0, height: 0 }), l2 = () => {
    !function(e3, t3) {
      ...
      e3 ? o3 && l3 || Ao.warn("Component width or height is 0px, rendering abnormality may occur!") : Ao.warn("Failed to get dom node, component rendering may be abnormal!"), t3.width = o3, t3.height = l3;
    }(e2.value, o2);
  }, n2 = Re(l2, 100), r2 = qe((t3) => {
    const o3 = function(e3) {
      return Ae(e3) ? e3 : Ae(e3.$el) ? e3.$el : null;
    }(t3);
    o3 ? e2.value = o3 : Ao.error("Bind Component Dom Ref Failed!");
  });
  return i(() => {
    l2();
  ...
  function r2(t3) {
    return (o3) => {
      const r3 = document.createElement("style"), s2 = (e2) => {
        r3.innerHTML = ((e3, t5) => {
          const o4 = a2(t5);
          return `${Fo} {${e3.toString()}}`.replaceAll(Fo, o4);
        })(o3, e2);
        const t4 = document.querySelector("head");
        t4 && t4.append(r3);
      };
      return (o4) => {
        const u2 = t3, c2 = je("style");
...
Po.setClassNamePrefix;
const Eo = Po.div`
  position: relative;
  width: 100%;
  height: 100%;
`("border-box"), Ho = Po.div`
  position: relative;
  width: 100%;
  height: 100%;
`("border-box-content"), Qo = ["#2e6099", "#7ce7fd"], Go = ["left-top", "right-top", "left-bottom", "right-bottom"], Uo = Po.svg`
  position: absolute;
  display: block;
...
`("border"), Vo = e({ name: "BorderBox1", props: jo(), setup(e2, { slots: t2, expose: o2 }) {
  const { autoBindRef: l2, refreshLayoutSize: n2, domSize: a2 } = Oo();
  return o2({ refreshLayoutSize: n2 }), () => {
    const { color: o3, backgroundColor: n3 } = e2, { width: i2, height: r2 } = a2, s2 = zo(Qo, o3);
    return g(Eo, { class: Po.getClassNameForBind("border-box-1"), ref: l2 }, { default: () => [g(Uo, { width: i2, height: r2 }, { default: () => [g("polygon", { fill: n3, points: `10, 27 10, ${r2 - 27} 13, ${r2 - 24} 13, ${r2 - 21} 24, ${r2 - 11} 38, ${r2 - 11}
...
`("border-svg-container"), Yo = e({ name: "BorderBox2", props: jo(), setup(e2, { slots: t2, expose: o2 }) {
  const { autoBindRef: l2, refreshLayoutSize: n2, domSize: a2 } = Oo();
  return o2({ refreshLayoutSize: n2 }), () => {
    const { color: o3, backgroundColor: n3 } = e2, { width: i2, height: r2 } = a2, s2 = zo(Wo, o3);
    return g(Eo, { class: Po.getClassNameForBind("border-box-2"), ref: l2 }, { default: () => [g(Zo, { width: i2, height: r2 }, { default: () => [g("polygon", { fill: n3, points: `7, 7 ${i2 - 7}, 7 ${i2 - 7}, ${r2 - 7} 7, ${r2 - 7}` }, null), g("polyline", { stroke: s2[0], points: `2, 2 ${i2 - 2} ,2 ${i2 - 2}, ${r2 - 2} 2, ${r2 - 2} 2, 2` }, null), g("polyline", { stroke: s2[1], points: `6, 6 ${i2 - 6}, 6 ${i2 - 6}, ${r2 - 6} 6, ${r2 - 6} 6, 6` }, null), g("circle", { fill: s2[0], cx: "11", cy: "11", r: "1" }, null), g("circle", { fill: s2[0], cx: i2 - 11, cy: "11", r: "1" }, null), g("circle", { fill: s2[0], cx: i2 - 11, cy: r2 - 11, r: "1" }, null), g("circle", { fill: s2[0], cx: "11", cy: r2 - 11, r: "1" }, null)] }), g(Ho, null, { default: () => {
      var _a2;
      return [(_a2 = t2.default) == null ? void 0 : _a2.call(t2)];
    } })] });
  };
} }), Ko = ["#2e6099", "#7ce7fd"], Xo = Po.svg`
  position: absolute;
  width: 100%;
  height: 100%;
  top: 0;
...
`("border-svg-container"), Jo = e({ name: "BorderBox3", props: jo(), setup(e2, { slots: t2, expose: o2 }) {
  const { autoBindRef: l2, refreshLayoutSize: n2, domSize: a2 } = Oo();
  return o2({ refreshLayoutSize: n2 }), () => {
    const { color: o3, backgroundColor: n3 } = e2, { width: i2, height: r2 } = a2, s2 = zo(Ko, o3);
    return g(Eo, { class: Po.getClassNameForBind("border-box-3"), ref: l2 }, { default: () => [g(Xo, { width: i2, height: r2 }, { default: () => [g("polygon", { fill: n3, points: `23, 23 ${i2 - 24}, 23 ${i2 - 24}, ${r2 - 24} 23, ${r2 - 24}` }, null), g("polyline", { class: "stroke-width-3", stroke: s2[0], points: `4, 4 ${i2 - 22} ,4 ${i2 - 22}, ${r2 - 22} 4, ${r2 - 22} 4, 4` }, null), g("polyline", { class: "stroke-width-1", stroke: s2[1], points: `10, 10 ${i2 - 16}, 10 ${i2 - 16}, ${r2 - 16} 10, ${r2 - 16} 10, 10` }, null), g("polyline", { class: "stroke-width-1", stroke: s2[1], points: `16, 16 ${i2 - 10}, 16 ${i2 - 10}, ${r2 - 10} 16, ${r2 - 10} 16, 16` }, null), g("polyline", { class: "stroke-width-1", stroke: s2[1], points: `22, 22 ${i2 - 4}, 22 ${i2 - 4}, ${r2 - 4} 22, ${r2 - 4} 22, 22` }, null)] }), g(Ho, null, { default: () => {
      var _a2;
      return [(_a2 = t2.default) == null ? void 0 : _a2.call(t2)];
    } })] });
  };
} }), el = ["#2e6099", "#7ce7fd"], tl = Po.svg`
  position: absolute;
  width: 100%;
  height: 100%;
  top: 0;
...
`("border-svg-container"), al = e({ name: "BorderBox5", props: { ...jo(), reverse: { type: Boolean, default: false } }, setup(e2, { slots: t2, expose: o2 }) {
  const { autoBindRef: l2, refreshLayoutSize: n2, domSize: a2 } = Oo();
  return o2({ refreshLayoutSize: n2 }), () => {
    const { color: o3, backgroundColor: n3, reverse: i2 } = e2, { width: r2, height: s2 } = a2, u2 = zo(ll, o3);
    return g(Eo, { class: Po.getClassNameForBind("border-box-5"), ref: l2 }, { default: () => [g(nl, { class: i2 ? "reverse" : "", width: r2, height: s2 }, { default: () => [g("polygon", { fill: n3, points: `
...
`("border-svg-container"), sl = e({ name: "BorderBox6", props: jo(), setup(e2, { slots: t2, expose: o2 }) {
  const { autoBindRef: l2, refreshLayoutSize: n2, domSize: a2 } = Oo();
  return o2({ refreshLayoutSize: n2 }), () => {
    const { color: o3, backgroundColor: n3 } = e2, { width: i2, height: r2 } = a2, s2 = zo(il, o3);
    return g(Eo, { class: Po.getClassNameForBind("border-box-6"), ref: l2 }, { default: () => [g(rl, { width: i2, height: r2 }, { default: () => [g("polygon", { fill: n3, points: `
              9, 7 ${i2 - 9}, 7 ${i2 - 9}, ${r2 - 7} 9, ${r2 - 7}` }, null), g("circle", { fill: s2[1], cx: "5", cy: "5", r: "2" }, null), g("circle", { fill: s2[1], cx: i2 - 5, cy: "5", r: "2" }, null), g("circle", { fill: s2[1], cx: i2 - 5, cy: r2 - 5, r: "2" }, null), g("circle", { fill: s2[1], cx: "5", cy: r2 - 5, r: "2" }, null), g("polyline", { stroke: s2[0], points: `10, 4 ${i2 - 10}, 4` }, null), g("polyline", { stroke: s2[0], points: `10, ${r2 - 4} ${i2 - 10}, ${r2 - 4}` }, null), g("polyline", { stroke: s2[0], points: "5, 70 5, " + (r2 - 70) }, null), g("polyline", { stroke: s2[0], points: `${i2 - 5}, 70 ${i2 - 5}, ${r2 - 70}` }, null), g("polyline", { stroke: s2[0], points: "3, 10, 3, 50" }, null), g("polyline", { stroke: s2[0], points: "7, 30 7, 80" }, null), g("polyline", { stroke: s2[0], points: `${i2 - 3}, 10 ${i2 - 3}, 50` }, null), g("polyline", { stroke: s2[0], points: `${i2 - 7}, 30 ${i2 - 7}, 80` }, null), g("polyline", { stroke: s2[0], points: `3, ${r2 - 10} 3, ${r2 - 50}` }, null), g("polyline", { stroke: s2[0], points: `7, ${r2 - 30} 7, ${r2 - 80}` }, null), g("polyline", { stroke: s2[0], points: `${i2 - 3}, ${r2 - 10} ${i2 - 3}, ${r2 - 50}` }, null), g("polyline", { stroke: s2[0], points: `${i2 - 7}, ${r2 - 30} ${i2 - 7}, ${r2 - 80}` }, null)] }), g(Ho, null, { default: () => {
      var _a2;
      return [(_a2 = t2.default) == null ? void 0 : _a2.call(t2)];
    } })] });
  };
} }), ul = ["#2e6099", "#7ce7fd"], cl = Po.svg`
  position: absolute;
  top: 0;
  left: 0;
  width: 100%;
...
function pl() {
  return L(a({ id: je("datav_uuid") }));
}
const fl = ["#2e6099", "#7ce7fd"], hl = Po.svg`
  position: absolute;
  width: 100%;
  height: 100%;
  left: 0;
  top: 0;
`("border-svg-container"), gl = e({ name: "BorderBox8", props: { ...jo(), duration: { type: Number, default: 3 }, reverse: { type: Boolean, default: false } }, setup(e2, { slots: t2, expose: o2 }) {
  const { autoBindRef: l2, refreshLayoutSize: n2, domSize: a2 } = Oo();
  o2({ refreshLayoutSize: n2 });
  const i2 = pl();
  return () => {
    const { color: o3, backgroundColor: n3, duration: r2, reverse: s2 } = e2, { width: u2, height: c2 } = a2, d2 = zo(fl, o3), p2 = `border-box-8-path-${i2.id}`, f2 = `border-box-8-gradient-${i2.id}`, h2 = `border-box-8-mask-${i2.id}`, y2 = s2 ? `M 2.5, 2.5 L 2.5, ${c2 - 2.5} L ${u2 - 2.5}, ${c2 - 2.5} L ${u2 - 2.5}, 2.5 L 2.5, 2.5` : `M2.5, 2.5 L${u2 - 2.5}, 2.5 L${u2 - 2.5}, ${c2 - 2.5} L2.5, ${c2 - 2.5} L2.5, 2.5`;
    return g(Eo, { class: Po.getClassNameForBind("border-box-8"), ref: l2 }, { default: () => [g(hl, { width: u2, height: c2 }, { default: () => [g("defs", null, [g("path", { id: p2, d: y2, fill: "transparent" }, null), g("radialGradient", { id: f2, cx: "50%", cy: "50%", r: "50%" }, [g("stop", { offset: "0%", "stop-color": "#fff", "stop-opacity": "1" }, null), g("stop", { offset: "100%", "stop-color": "#fff", "stop-opacity": "0" }, null)]), g("mask", { id: h2 }, [g("circle", { cx: "0", cy: "0", r: "150", fill: `url(#${f2})` }, [g("animateMotion", { dur: `${r2}s`, path: y2, rotate: "auto", repeatCount: "indefinite" }, null)])])]), g("polygon", { fill: n3, points: `5, 5 ${u2 - 5}, 5 ${u2 - 5} ${c2 - 5} 5, ${c2 - 5}` }, null), g("use", { stroke: d2[0], "stroke-width": "1", "xlink:href": `#${p2}` }, null), g("use", { stroke: d2[1], "stroke-width": "3", "xlink:href": `#${p2}`, mask: `url(#${h2})` }, [g("animate", { attributeName: "stroke-dasharray", from: `0, ${length}`, to: `${length}, 0`, dur: `${r2}s`, repeatCount: "indefinite" }, null)])] }), g(Ho, null, { default: () => {
      var _a2;
      return [(_a2 = t2.default) == null ? void 0 : _a2.call(t2)];
    } })] });
  };
} }), yl = ["#2e6099", "#7ce7fd"], ml = Po.svg`
  position: absolute;
  width: 100%;
  height: 100%;
  left: 0;
  top: 0;
`("border-svg-container"), vl = e({ name: "BorderBox9", props: jo(), setup(e2, { slots: t2, expose: o2 }) {
  const { autoBindRef: l2, refreshLayoutSize: n2, domSize: a2 } = Oo();
  o2({ refreshLayoutSize: n2 });
  const i2 = pl();
  return () => {
    const { color: o3, backgroundColor: n3 } = e2, { width: r2, height: s2 } = a2, u2 = zo(yl, o3), c2 = `border-box-9-gradient-${i2.id}`, d2 = `border-box-9-mask-${i2.id}`;
    return g(Eo, { class: Po.getClassNameForBind("border-box-9"), ref: l2 }, { default: () => [g(ml, { width: r2, height: s2 }, { default: () => [g("defs", null, [g("linearGradient", { id: c2, x1: "0%", y1: "0%", x2: "100%", y2: "100%" }, [g("animate", { attributeName: "x1", values: "0%;100%;0%", dur: "10s", begin: "0s", repeatCount: "indefinite" }, null), g("animate", { attributeName: "x2", values: "100%;0%;100%", dur: "10s", begin: "0s", repeatCount: "indefinite" }, null), g("stop", { offset: "0%", "stop-color": u2[0] }, [g("animate", { attributeName: "stop-color", values: `${u2[0]};${u2[1]};${u2[0]}`, dur: "10s", begin: "0s", repeatCount: "indefinite" }, null)]), g("stop", { offset: "100%", "stop-color": u2[1] }, [g("animate", { attributeName: "stop-color", values: `${u2[1]};${u2[0]};${u2[1]}`, dur: "10s", begin: "0s", repeatCount: "indefinite" }, null)])]), g("mask", { id: d2 }, [g("polyline", { stroke: "#fff", "stroke-width": "3", fill: "transparent", points: `8, ${0.4 * s2} 8, 3, ${0.4 * r2 + 7}, 3` }, null), g("polyline", { fill: "#fff", points: `
                ...
                ` }, null), g("rect", { x: "0", y: "0", width: r2, height: s2, fill: `url(#${c2})`, mask: `url(#${d2})` }, null)] }), g(Ho, null, { default: () => {
      var _a2;
      return [(_a2 = t2.default) == null ? void 0 : _a2.call(t2)];
    } })] });
  };
} }), Cl = ["#2e6099", "#7ce7fd"], bl = ["left-top", "right-top", "left-bottom", "right-bottom"], Ml = Po.svg`
  position: absolute;
  display: block;
...
`("border-svg-container"), kl = e({ name: "BorderBox10", props: jo(), setup(e2, { slots: t2, expose: o2 }) {
  const { autoBindRef: l2, refreshLayoutSize: n2, domSize: a2 } = Oo();
  return o2({ refreshLayoutSize: n2 }), () => {
    const { width: o3, height: n3 } = a2, { backgroundColor: i2, color: r2 } = e2, s2 = zo(Cl, r2);
    return g(Eo, { class: Po.getClassNameForBind("border-box-10"), ref: l2, style: { boxShadow: `inset 0 0 25px 3px ${s2[0]}` } }, { default: () => [g(Ml, { width: o3, height: n3 }, { default: () => [g("polygon", { fill: i2, points: `
                  4, 0 ${o3 - 4}, 0 ${o3}, 4 ${o3}, ${n3 - 4} ${o3 - 4}, ${n3}
                  4, ${n3} 0, ${n3 - 4} 0, 4
                ` }, null)] }), bl.map((e3) => g(Ml, { width: "150px", height: "150px", key: e3, class: e3 }, { default: () => [g("polygon", { fill: s2[1], points: "40, 0 5, 0 0, 5 0, 16 3, 19 3, 7 7, 3 35, 3" }, null)] })), g(Ho, null, { default: () => {
      var _a2;
      return [(_a2 = t2.default) == null ? void 0 : _a2.call(t2)];
    } })] });
  };
} }), wl = ["#2e6099", "#7ce7fd"], xl = Po.svg`
  position: absolute;
  width: 100%;
  height: 100%;
  top: 0;
...
`("border-svg-container"), Sl = e({ name: "BorderBox11", props: { ...jo(), titleWidth: { type: Number, default: 250 }, title: { type: String, default: "" } }, setup(e2, { slots: t2, expose: o2 }) {
  const { autoBindRef: l2, refreshLayoutSize: n2, domSize: a2 } = Oo();
  o2({ refreshLayoutSize: n2 });
  const i2 = pl();
  return () => {
    const { color: o3, backgroundColor: n3, titleWidth: r2, title: s2 } = e2, { width: u2, height: c2 } = a2, d2 = zo(wl, o3), p2 = `border-box-11-filterId-${i2}`;
    return g(Eo, { class: Po.getClassNameForBind("border-box-11"), ref: l2 }, { default: () => [g(xl, { width: u2, height: c2 }, { default: () => [g("defs", null, [g("filter", { id: p2, height: "150%", width: "150%", x: "-25%", y: "-25%" }, [g("feMorphology", { operator: "dilate", radius: "2", in: "SourceAlpha", result: "thicken" }, null), g("feGaussianBlur", { in: "thicken", stdDeviation: "3", result: "blurred" }, null), g("feFlood", { "flood-color": d2[1], result: "glowColor" }, null), g("feComposite", { in: "glowColor", in2: "blurred", operator: "in", result: "softGlowColored" }, null), g("feMerge", null, [g("feMergeNode", { in: "softGlowColored" }, null), g("feMergeNode", { in: "SourceGraphic" }, null)])])]), g("polygon", { fill: n3, points: `
                ...
                ` }, null), g("polygon", { stroke: d2[0], fill: Qe(d2[1] || wl[1], 30), filter: `url(#${p2})`, points: `
                ...
                ` }, null)] }), g(Ho, null, { default: () => {
      var _a2;
      return [(_a2 = t2.default) == null ? void 0 : _a2.call(t2)];
    } })] });
  };
} }), _l = ["#2e6099", "#7ce7fd"], $l = Po.svg`
  position: absolute;
  width: 100%;
  height: 100%;
  top: 0;
  left: 0;
`("border-svg-container"), Ll = e({ name: "BorderBox12", props: jo(), setup(e2, { slots: t2, expose: o2 }) {
  const { autoBindRef: l2, refreshLayoutSize: n2, domSize: a2 } = Oo();
  o2({ refreshLayoutSize: n2 });
  const i2 = pl();
  return () => {
    const { color: o3, backgroundColor: n3 } = e2, { width: r2, height: s2 } = a2, u2 = zo(_l, o3), c2 = `border-box-12-filterId-${i2}`;
    return g(Eo, { class: Po.getClassNameForBind("border-box-12"), ref: l2 }, { default: () => [g($l, { width: r2, height: s2 }, { default: () => [g("defs", null, [g("filter", { id: c2, height: "150%", width: "150%", x: "-25%", y: "-25%" }, [g("feMorphology", { operator: "dilate", radius: "1", in: "SourceAlpha", result: "thicken" }, null), g("feGaussianBlur", { in: "thicken", stdDeviation: "2", result: "blurred" }, null), g("feFlood", { "flood-color": Qe(u2[1] || _l[1], 70), result: "glowColor" }, [g("animate", { attributeName: "flood-color", values: `
                        ${Qe(u2[1] || _l[1], 70)};
                        ${Qe(u2[1] || _l[1], 30)};
                        ${Qe(u2[1] || _l[1], 70)};
...
const un = ["#2e6099", "#7ce7fd"], cn = 2.5, dn = function({ width: e2, height: t2, rowPoints: o2, rowCount: l2 }) {
...
}({ width: 200, height: 50, rowPoints: 20, rowCount: 4 }), pn = dn[39], fn = dn[37], hn = Po.div`
  width: 100%;
  height: 100%;
...
`("decoration-1"), gn = e({ name: "Decoration1", props: an(), setup(e2, { expose: t2 }) {
  const { autoBindRef: o2, refreshLayoutSize: l2, domSize: n2 } = Oo();
  return t2({ refreshLayoutSize: l2 }), () => {
    const { color: t3 } = e2, { width: l3, height: a2 } = n2, i2 = zo(un, t3), r2 = { transform: `scale(${l3 / 200},${a2 / 50})` };
    return g(hn, { ref: o2 }, { default: () => [g("svg", { width: 200, height: 50, style: r2 }, [dn.map(([e3, t4], o3) => {
      const l4 = e3 - 1.25, n3 = t4 - 1.25;
      return Math.random() > 0.6 ? g("rect", { key: o3, x: l4, y: n3, width: cn, height: cn, fill: i2[0] }, [Math.random() > 0.6 && g("animate", { attributeName: "fill", values: `${i2[0]};transparent`, dur: "1s", begin: 2 * Math.random(), repeatCount: "indefinite" }, null)]) : null;
    }), g("rect", { fill: i2[1], x: pn[0] - cn, y: pn[1] - cn, width: 5, height: 5 }, [g("animate", { attributeName: "width", values: "0;5", dur: "2s", repeatCount: "indefinite" }, null), g("animate", { attributeName: "height", values: "0;5", dur: "2s", repeatCount: "indefinite" }, null), g("animate", { attributeName: "x", values: `${pn[0]};${pn[0] - cn}`, dur: "2s", repeatCount: "indefinite" }, null), g("animate", { attributeName: "y", values: `${pn[1]};${pn[1] - cn}`, dur: "2s", repeatCount: "indefinite" }, null)]), g("rect", { fill: i2[1], x: fn[0] - cn, y: fn[1] - cn, width: 5, height: 5 }, [g("animate", { attributeName: "width", values: "0;40;0", dur: "2s", repeatCount: "indefinite" }, null), g("animate", { attributeName: "x", values: `${fn[0]};${fn[0] - 40};${fn[0]}`, dur: "2s", repeatCount: "indefinite" }, null)])])] });
  };
} }), yn = ["#2e6099", "#7ce7fd"];
const mn = Po.div`
  display: flex;
...
`("decoration-3"), kn = e({ name: "Decoration3", props: an(), setup(e2, { expose: t2 }) {
  const { autoBindRef: o2, refreshLayoutSize: l2, domSize: n2 } = Oo();
  return t2({ refreshLayoutSize: l2 }), () => {
    const { width: t3, height: l3 } = n2, { color: a2 } = e2, i2 = zo(Cn, a2);
    return g(Mn, { ref: o2 }, { default: () => [g("svg", { width: 300, height: 35, style: { transform: `scale(${t3 / 300},${l3 / 35})` } }, [bn.map(([e3, t4], o3) => {
...
} }), wn = ["width", "height"], xn = ["stroke", "points"], Sn = ["stroke", "points"], _n = e({ name: "Decoration4", __name: "index", props: { ...an(), ...rn(), ...sn(3) }, setup(e2) {
  ...
  return (e3, t3) => (s(), r("div", { class: "dv-decoration-4", ref_key: "decoration4Ref", ref: n2 }, [p("div", { class: c("container " + (e3.reverse ? "reverse" : "normal")), style: u(e3.reverse ? `width:${a2.value}px;height:5px;animation-duration:${e3.duration}s` : `width:5px;height:${i2.value}px;animation-duration:${e3.duration}s`) }, [(s(), r("svg", { width: e3.reverse ? a2.value : 5, height: e3.reverse ? 5 : i2.value }, [p("polyline", { stroke: d2.value[0], points: e3.reverse ? `0, 2.5 ${a2.value}, 2.5` : `2.5, 0 2.5, ${i2.value}` }, null, 8, xn), p("polyline", { class: "bold-line", stroke: d2.value[1], "stroke-width": "3", "stroke-dasharray": "20, 80", "stroke-dashoffset": "-30", points: e3.reverse ? `0, 2.5 ${a2.value}, 2.5` : `2.5, 0 2.5, ${i2.value}` }, null, 8, Sn)], 8, wn))], 6)], 512));
} }), $n = Yt(_n, [["__scopeId", "data-v-97e7e3ce"]]), Ln = ["#2e6099", "#7ce7fd"];
function In(e2) {
...
const Dn = Po.div`
  width: 100%;
  height: 100%;
`("decoration-5"), Tn = e({ name: "Decoration5", props: { ...an(), ...sn(1.2) }, setup(e2, { expose: t2 }) {
  const { autoBindRef: o2, refreshLayoutSize: l2, domSize: n2 } = Oo();
  return t2({ refreshLayoutSize: l2 }), () => {
    const { width: t3, height: l3 } = n2, { color: a2, duration: i2 } = e2, r2 = zo(Ln, a2), { line1Sum: s2, line2Sum: u2, line1Point: c2, line2Point: d2 } = function(e3, t4) {
...
`("decoration-6");
const { points: Rn, heights: qn, minHeights: An, randoms: On } = function({ width: e2, height: t2, rowPoints: o2, rowCount: l2 }) {
  const n2 = e2 / (o2 + 1), a2 = t2 / (l2 + 1), i2 = Array.from({ length: l2 }).fill(0).flatMap((e3, t3) => Array.from({ length: o2 }).fill(0).map((e4, o3) => [n2 * (o3 + 1), a2 * (t3 + 1)])), r2 = Array.from({ length: l2 * o2 }).fill(0).map(() => Math.random() > 0.8 ? Fe(0.7 * t2, t2) : Fe(0.2 * t2, 0.5 * t2)), s2 = Array.from({ length: l2 * o2 }).fill(0).map((e3, t3) => r2[t3] * Math.random()), u2 = Array.from({ length: l2 * o2 }).fill(0).map(() => Math.random() + 1.5);
  return { points: i2, heights: r2, minHeights: s2, randoms: u2 };
}({ width: 300, height: 35, rowPoints: 40, rowCount: 1 }), jn = e({ name: "Decoration6", props: an(), setup(e2, { expose: t2 }) {
  const { autoBindRef: o2, refreshLayoutSize: l2, domSize: n2 } = Oo();
  return t2({ refreshLayoutSize: l2 }), () => {
    const { width: t3, height: l3 } = n2, { color: a2 } = e2, i2 = zo(Nn, a2), r2 = { transform: `scale(${t3 / 300},${l3 / 35})` };
    return g(Bn, { ref: o2 }, { default: () => [g("svg", { class: "svg-origin", width: t3, height: l3, style: r2 }, [Rn.map(([e3, t4], o3) => g("rect", { key: `rect${o3}`, fill: i2[Math.random() > 0.5 ? 0 : 1], x: e3 - 3.5, y: t4 - qn[o3], width: 7, height: qn[o3] }, [g("animate", { attributeName: "y", values: `${t4 - An[o3] / 2};${t4 - qn[o3] / 2};${t4 - An[o3] / 2}`, dur: On[o3], keyTimes: "0;0.5;1", calcMode: "spline", keySplines: "0.42,0,0.58,1;0.42,0,0.58,1", begin: "0s", repeatCount: "indefinite" }, null), g("animate", { attributeName: "height", values: `${An[o3]};${qn[o3]};${An[o3]}`, dur: On[o3], keyTimes: "0;0.5;1", calcMode: "spline", keySplines: "0.42,0,0.58,1;0.42,0,0.58,1", begin: "0s", repeatCount: "indefinite" }, null)]))])] });
  };
} }), zn = Po.div`
  display: flex;
  width: 100%;
  ...
  justify-content: center;
  align-items: center;
`("decoration-7"), Fn = ["#2e6099", "#7ce7fd"], Pn = e({ name: "Decoration7", props: an(), setup(e2, { slots: t2, expose: o2 }) {
  const { autoBindRef: l2, refreshLayoutSize: n2 } = Oo();
  return o2({ refreshLayoutSize: n2 }), () => {
    const { color: o3 } = e2, n3 = zo(Fn, o3);
    return g(zn, { ref: l2 }, { default: () => {
      var _a2;
...
const Hn = Po.div`
  ...
  width: 100%;
  height: 100%;
`("decoration-8"), Qn = e({ name: "Decoration8", props: { ...an(), ...rn() }, setup(e2, { expose: t2 }) {
  const { autoBindRef: o2, refreshLayoutSize: l2, domSize: n2 } = Oo();
  return t2({ refreshLayoutSize: l2 }), () => {
    const { color: t3, reverse: l3 } = e2, { width: a2, height: i2 } = n2, r2 = (e3) => l3 ? a2 - e3 : e3, s2 = zo(En, t3);
    return g(Hn, { ref: o2 }, { default: () => [g("svg", { width: a2, height: i2 }, [g("polyline", { stroke: s2[0], "stroke-width": "2", fill: "transparent", points: `${r2(0)}, 0 ${r2(30)}, ${i2 / 2}` }, null), g("polyline", { stroke: s2[0], "stroke-width": "2", fill: "transparent", points: `${r2(20)}, 0 ${r2(50)}, ${i2 / 2} ${r2(a2)}, ${i2 / 2}` }, null), g("polyline", { stroke: s2[1], fill: "transparent", "stroke-width": "3", points: `${r2(0)}, ${i2 - 3}, ${r2(200)}, ${i2 - 3}` }, null)])] });
  };
} });
const Gn = ["#2e6099", "#7ce7fd"], Un = Po.div`
  position: relative;
  width: 100%;
  height: 100%;
  display: flex;
...
`("decoration-9"), Vn = e({ name: "Decoration9", props: { ...an(), ...sn(3) }, setup(e2, { slots: t2, expose: o2 }) {
  const { autoBindRef: l2, refreshLayoutSize: n2, domSize: a2 } = Oo();
  o2({ refreshLayoutSize: n2 });
  const i2 = `decoration-9-polygon-${pl().id}`;
  return () => {
    const { width: o3, height: n3 } = a2, { color: r2, duration: s2 } = e2, u2 = zo(Gn, r2), c2 = { transform: `scale(${o3 / 100},${n3 / 100})` };
    return g(Un, { ref: l2 }, { default: () => {
      var _a2;
      return [g("svg", { class: "svg-origin", width: 100, height: 100, style: c2 }, [g("defs", null, [g("polygon", { id: i2, points: "15, 46.5, 21, 47.5, 21, 52.5, 15, 53.5" }, null)]), g("circle", { cx: "50", cy: "50", r: "45", fill: "transparent", stroke: u2[1], "stroke-width": "10", "stroke-dasharray": "80, 100, 30, 100" }, [g("animateTransform", { attributeName: "transform", type: "rotate", values: "0 50 50;360 50 50", dur: `${s2}s`, repeatCount: "indefinite" }, null)]), g("circle", { cx: "50", cy: "50", r: "45", fill: "transparent", stroke: u2[0], "stroke-width": "6", "stroke-dasharray": "50, 66, 100, 66" }, [g("animateTransform", { attributeName: "transform", type: "rotate", values: "0 50 50;-360 50 50", dur: `${s2}s`, repeatCount: "indefinite" }, null)]), g("circle", { cx: "50", cy: "50", r: "38", fill: "transparent", stroke: Qe(u2[1] || Gn[1], 30), "stroke-width": "1", "stroke-dasharray": "5, 1" }, null), Array.from({ length: 20 }).fill(0).map((e3, t3) => g("use", { key: t3, "xlink:href": `#${i2}`, stroke: u2[1], fill: Math.random() > 0.4 ? "transparent" : u2[0] }, [g("animateTransform", { attributeName: "transform", type: "rotate", values: "0 50 50;360 50 50", dur: `${s2}s`, begin: t3 * s2 / 20 + "s", repeatCount: "indefinite" }, null)])), g("circle", { cx: "50", cy: "50", r: "26", fill: "transparent", stroke: Qe(u2[1] || Gn[1], 30), "stroke-width": "1", "stroke-dasharray": "5, 1" }, null)]), (_a2 = t2.default) == null ? void 0 : _a2.call(t2)];
    } });
  };
} }), Wn = Po.div`
  width: 100%;
  height: 100%;
  display: flex;
`("decoration-10"), Zn = ["#2e6099", "#7ce7fd"], Yn = e({ name: "Decoration10", props: an(), setup(e2, { expose: t2 }) {
  const { autoBindRef: o2, refreshLayoutSize: l2, domSize: n2 } = Oo();
  t2({ refreshLayoutSize: l2 });
  const a2 = pl(), i2 = `d10ani${a2.id}1`, r2 = `d10ani${a2.id}2`, s2 = `d10ani${a2.id}3`, u2 = `d10ani${a2.id}4`, c2 = `d10ani${a2.id}5`, d2 = `d10ani${a2.id}6`, p2 = `d10ani${a2.id}7`;
  return () => {
    const { width: t3, height: l3 } = n2, { color: a3 } = e2, f2 = zo(Zn, a3);
...
`("decoration-11"), Xn = Po.div`
  ...
  align-items: center;
  justify-content: center;
`("decoration-content"), Jn = ["#2e6099", "#7ce7fd"], ea = e({ name: "Decoration11", props: an(), setup(e2, { slots: t2, expose: o2 }) {
  const { autoBindRef: l2, refreshLayoutSize: n2, domSize: a2 } = Oo();
  return o2({ refreshLayoutSize: n2 }), () => {
    const { width: o3, height: n3 } = a2, { color: i2 } = e2, r2 = zo(Jn, i2);
    return g(Kn, { ref: l2 }, { default: () => [g("svg", { width: o3, height: n3 }, [g("polygon", { fill: Qe(r2[1] || Jn[1], 10), stroke: r2[1], points: "20 10, 25 4, 55 4 60 10" }, null), g("polygon", { fill: Qe(r2[1] || Jn[1], 10), stroke: r2[1], points: `20 ${n3 - 10}, 25 ${n3 - 4}, 55 ${n3 - 4} 60 ${n3 - 10}` }, null), g("polygon", { fill: Qe(r2[1] || Jn[1], 10), stroke: r2[1], points: `${o3 - 20} 10, ${o3 - 25} 4, ${o3 - 55} 4 ${o3 - 60} 10` }, null), g("polygon", { fill: Qe(r2[1] || Jn[1], 10), stroke: r2[1], points: `${o3 - 20} ${n3 - 10}, ${o3 - 25} ${n3 - 4}, ${o3 - 55} ${n3 - 4} ${o3 - 60} ${n3 - 10}` }, null), g("polygon", { fill: Qe(r2[0] || Jn[0], 20), stroke: r2[0], points: `
        20 10, 5 ${n3 / 2} 20 ${n3 - 10}
        ${o3 - 20} ${n3 - 10} ${o3 - 5} ${n3 / 2} ${o3 - 20} 10
      ` }, null), g("polyline", { fill: "transparent", stroke: Qe(r2[0] || Jn[0], 70), points: `25 18, 15 ${n3 / 2} 25 ${n3 - 18}` }, null), g("polyline", { fill: "transparent", stroke: Qe(r2[0] || Jn[0], 70), points: `${o3 - 25} 18, ${o3 - 15} ${n3 / 2} ${o3 - 25} ${n3 - 18}` }, null)]), g(Xn, null, { default: () => {
      var _a2;
      return [(_a2 = t2.default) == null ? void 0 : _a2.call(t2)];
    } })] });
  };
...
`("decoration-12"), oa = Po.div`
  ...
  align-items: center;
  justify-content: center;
`("decoration-content"), la = ["#2e6099", "#7ce7fd"], na = Math.PI / 3;
const aa = (e2, t2, o2, l2) => [e2 + Math.cos(l2) * o2, t2 + Math.sin(l2) * o2], ia = e({ name: "Decoration12", props: { ...an(), scanDuration: { type: Number, default: 3 }, haloDuration: { type: Number, default: 2 } }, setup(e2, { slots: t2, expose: o2 }) {
  const { autoBindRef: l2, refreshLayoutSize: n2, domSize: a2 } = Oo();
  o2({ refreshLayoutSize: n2 });
  const i2 = pl();
  return () => {
    const { width: o3, height: n3 } = a2, r2 = o3 / 2, s2 = n3 / 2, { color: u2, scanDuration: c2, haloDuration: d2 } = e2, p2 = `decoration-12-g-${i2.id}`, f2 = `decoration-12-gradient-${i2.id}`, h2 = zo(la, u2), y2 = ((e3, t3) => {
      const o4 = 100 / (t3 - 1);
      return Array.from({ length: t3 }).fill(e3).map((t4, l3) => Qe(e3, 100 - l3 * o4));
    })(h2[0], 30), m2 = (({ x: e3, y: t3, width: o4, segment: l3, sectorAngle: n4 }) => {
      const a3 = -Math.PI / 2, i3 = n4 / l3, r3 = o4 / 4;
      let s3 = aa(e3, t3, r3, a3);
      return Array.from({ length: l3 }).fill("").map((o5, l4) => {
        const n5 = aa(e3, t3, r3, a3 - (l4 + 1) * i3).map((e4) => e4.toFixed(5)), u3 = `M${s3.join(",")} A${r3}, ${r3} 0 0 0 ${n5.join(",")}`;
        return s3 = n5, u3;
      });
    })({ x: r2, y: s2, width: o3, segment: 30, sectorAngle: na }), v2 = (({ ringNum: e3, width: t3, ringWidth: o4 }) => {
      ...
      return Array.from({ length: 6 }).fill("").map((o5, a3) => {
        const i3 = l3 * (a3 + 1), r3 = i3 + Math.PI, s3 = aa(e3, t3, n4, i3), u3 = aa(e3, t3, n4, r3);
...
`("header-svg-container"), xa = e({ name: "Header6", props: jo(), setup(e2, { expose: t2 }) {
  ...
  const R2 = y(0), q2 = y(0), A2 = "#303030", O2 = "#3A3A3A", j2 = "#4A4A4A", z2 = "#F0F0F0", F2 = "#FFFFFF", P2 = "#FAFAFA";
  function E2() {
    var _a2, _b;
  ...
  function U2() {
    ...
    const l2 = e3.length;
    l2 > o2 && l2 < 2 * o2 && (e3 = [...e3, ...e3]), e3 = e3.map((e4, t4) => ({ ...e4, scroll: t4 })), M2 = e3, k2.value = e3;
  }
  function V2() {
  ...
  async function K2(e3 = false) {
    ...
    const t3 = N2;
    let o2 = T2;
    const { waitTime: l2, carousel: n2, rowNum: a3 } = C2.value, i2 = M2.length;
    if (a3 >= i2) return;
    if (e3 && (await new Promise((e4) => setTimeout(e4, l2)), t3 !== N2)) return;
    const r2 = "single" === n2 ? 1 : a3, s2 = M2.slice(o2);
    if (s2.push(...M2.slice(0, o2)), k2.value = s2.slice(0, "page" === n2 ? 2 * a3 : a3 + 1), $2.value = Array.from({ length: i2 }).fill(L2), await new Promise((e4) => setTimeout(e4, 300)), t3 !== N2) return;
    $2.value.splice(0, r2, ...Array.from({ length: r2 }).fill(0)), o2 += r2;
    const u2 = o2 - i2;
    u2 >= 0 && (o2 = u2), T2 = o2, D2 = setTimeout(K2, l2 - 300);
  }
  return n(() => c2.config, () => {
    H2(), T2 = 0, Q2();
  }), n(() => {
    ...
    e3 === He.DARK ? (C2.value.headerBGC === z2 && (C2.value.headerBGC = A2), C2.value.oddRowBGC === F2 && (C2.value.oddRowBGC = O2), C2.value.evenRowBGC === P2 && (C2.value.evenRowBGC = j2)) : (C2.value.headerBGC === A2 && (C2.value.headerBGC = z2), C2.value.oddRowBGC === O2 && (C2.value.oddRowBGC = F2), C2.value.evenRowBGC === j2 && (C2.value.evenRowBGC = P2));
  }, { immediate: true }), i(() => {
    var _a2, _b;
    q2.value = ((_a2 = p2.value) == null ? void 0 : _a2.clientWidth) || 0, R2.value = ((_b = p2.value) == null ? void 0 : _b.clientHeight) || 0, Q2(), window.addEventListener("resize", E2);
  }), _(() => {
    H2(), window.removeEventListener("resize", E2);
  }), t2({ resize: E2, updateRows: function(e3, t3) {
    C2.value = { ...C2.value, data: [...e3] }, B2 = true, T2 = t3, D2 || K2(true);
      ...
      ${h2.value ? "" : "border-top-left-radius: 8px;border-top-right-radius: 8px;"}`) }, [(s(true), r(m, null, v(k2.value, (e4, t4) => (s(), r("div", { class: "row-item", key: `${e4.toString()}${e4.scroll}`, style: u(`
  ...
  }(l2, e4, o2) }, [C2.value.index && 0 === l2 ? (s(), r("span", { key: 0, class: "index", style: u(`background: ${C2.value.headerBGC};`) }, S(o2), 5)) : (s(), r(m, { key: 1 }, [x(S(o2), 1)], 64))], 44, Sa))), 128))], 4))), 128))], 4)) : f("", true)], 512));
} }), $a = Yt(_a, [["__scopeId", "data-v-ccb14245"]]), La = ["onClick"], Ia = Yt(e({ __name: "MacButton", props: { size: { type: String, default: () => "default" }, disabled: { request: false, type: Boolean, default: false }, hidden: { request: false, type: Array, default: () => [] }, narrow: { request: false, type: Boolean, default: false } }, emits: ["close", "remove", "resize", "fullResize"], setup(e2, { emit: t2 }) {
  const o2 = e2, n2 = t2, { prefixCls: a2 } = we("mac-btn"), i2 = l(() => p2.filter((e3) => !o2.hidden.includes(e3.key))), u2 = l(() => o2.narrow), p2 = [{ title: "关闭", key: "close", icon: "ant-design:close-outlined" }, { title: "缩小", key: "remove", icon: "ant-design:shrink-outlined" }, { title: u2.value ? "缩小" : "放大", key: o2.narrow ? "fullResize" : "resize", icon: u2.value ? "ant-design:shrink-outlined" : "ant-design:arrows-alt-outlined" }];
  return (t3, o3) => (s(), r("div", { class: c(d(a2)) }, [(s(true), r(m, null, v(i2.value, (t4) => (s(), r("div", { key: t4.key, class: c(["btn", [t4.key, e2.disabled && "disabled", e2.size]]), onClick: k((e3) => {
    return o4 = t4.key, void n2(o4);
    var o4;
  }, ["stop"]) }, [g(d(Te), { icon: t4.icon, size: "10", class: c(["icon-base", { hover: !e2.disabled }]) }, null, 8, ["icon", "class"])], 10, La))), 128))], 2));
} }), [["__scopeId", "data-v-5c20d434"]]), Da = e({ name: "ScreenCheckbox", __name: "index", props: { title: { type: String }, tooltip: { type: String } }, setup: (e2) => (t2, o2) => (s(), r("div", null, [g(d(W), I(D(t2.$attrs)), { default: M(() => [x(S(e2.title), 1)]), _: 1 }, 16), e2.tooltip ? (s(), h(d(G), { key: 0, title: e2.tooltip }, { default: M(() => [g(d(Te), { icon: "ant-design:exclamation-circle-outlined" })]), _: 1 }, 8, ["title"])) : f("", true)])) }), Ta = { key: 0, class: "prefix" }, Na = { key: 3 }, Ba = Yt(e({ name: "ScreenInput", __name: "index", props: { isNumber: { type: Boolean, default: true }, prefix: { type: String }, suffix: { type: String } }, emits: ["change"], setup(e2, { emit: o2 }) {
  t((e3) => ({ cd77f8d8: u2.value }));
  const n2 = o2, { prefixCls: a2 } = we("screen-input"), { token: i2 } = E.useToken(), u2 = l(() => i2.value.colorBorder);
  return (t2, o3) => (s(), r("div", { class: c(d(a2)) }, [e2.prefix ? (s(), r("div", Ta, S(e2.prefix), 1)) : f("", true), e2.isNumber ? (s(), h(d(Q), $({ key: 1, class: "input" }, t2.$attrs, { onChange: o3[0] || (o3[0] = (e3) => n2("change", e3)), bordered: false, controls: false }), null, 16)) : (s(), h(d(H), $({ key: 2, class: "input" }, t2.$attrs, { bordered: false, onChange: o3[1] || (o3[1] = (e3) => n2("change", e3)) }), null, 16)), e2.suffix ? (s(), r("div", Na, S(e2.suffix), 1)) : f("", true)], 2));
} }), [["__scopeId", "data-v-107119ae"]]), Ra = ({ CtrlShiftKeysEnum: e2, CtrlKeysEnum: t2, ShiftKeysEnum: o2, KeysEnum: l2 }) => {
  const n2 = (e3, t3) => {
    if (!t3) return;
    const o3 = t3[1 === e3.key.length ? e3.key.toLowerCase() : e3.key];
    o3 && Ue(o3) && (e3.preventDefault(), e3.stopPropagation(), o3());
  }, { prefixCls: a2 } = we("chart-contain");
  function i2(e3) {
    var _a2;
...
const Aa = Yt(e({ __name: "TextFontStyle", props: { bold: { type: Boolean, default: false }, italic: { type: Boolean, default: false }, strikethrough: { type: Boolean, default: false }, underline: { type: Boolean, default: false }, hide: { type: Array, default: () => [] } }, emits: ["fontStyleChange"], setup(e2, { emit: t2 }) {
  const o2 = e2, l2 = t2, i2 = a({ [Za.Bold]: { name: "加粗", icon: "carbon:text-bold", value: false }, [Za.Italic]: { name: "斜体", icon: "carbon:text-italic", value: false }, [Za.Underline]: { name: "下划线", icon: "carbon:text-underline", value: false }, [Za.Strikethrough]: { name: "删除线", icon: "carbon:text-strikethrough", value: false } }), u2 = (e3) => !o2.hide.includes(e3);
  ...
  const { prefixCls: p2 } = we("font-style");
  return (e3, t3) => (s(), r("div", { class: c(d(p2)) }, [(s(true), r(m, null, v(Object.keys(i2), (e4) => (s(), r(m, { key: e4 }, [u2(e4) ? (s(), h(d(G), { key: 0, placement: "bottom", title: `${i2[e4].value ? "关闭" : "打开"}${i2[e4].name}` }, { default: M(() => [g(d(Z), { class: "radio-button", checked: i2[e4].value, onMousedown: (t4) => function(e5) {
    if (e5) {
      i2[e5].value = !i2[e5].value, i2[e5].value && (e5 === Za.Underline && (i2[Za.Strikethrough].value = false), e5 === Za.Strikethrough && (i2[Za.Underline].value = false));
      const t5 = {};
      Object.keys(i2).forEach((e6) => {
        t5[e6] = i2[e6].value;
      }), l2("fontStyleChange", t5);
    }
  }(e4) }, { default: M(() => [g(d(Te), { icon: i2[e4].icon }, null, 8, ["icon"])]), _: 2 }, 1032, ["checked", "onMousedown"])]), _: 2 }, 1032, ["title"])) : f("", true)], 64))), 128))], 2));
} }), [["__scopeId", "data-v-3b6dd2cf"]]), Oa = Yt(e({ __name: "TextHAlignStyle", props: { alignType: { type: String, default: void 0 } }, emits: ["alignChange"], setup(e2, { emit: t2 }) {
  const o2 = e2, l2 = t2, i2 = y();
  n(() => o2.alignType, (e3) => {
    i2.value = e3;
  }, { immediate: true });
  const { prefixCls: u2 } = we("h-align-style"), p2 = a({ [Va.Left]: { name: "水平居左", icon: "carbon:text-align-left", description: "文字水平居左" }, [Va.Center]: { name: "水平居中", icon: "carbon:text-align-center", description: "文字水平居中" }, [Va.Right]: { name: "水平居右", icon: "carbon:text-align-right", description: "文字水平居右" } });
  function f2(e3) {
    const t3 = e3.target.value;
    l2("alignChange", t3);
  }
  return (e3, t3) => (s(), r("div", { class: c(d(u2)) }, [g(d(Y), { value: i2.value, "onUpdate:value": t3[0] || (t3[0] = (e4) => i2.value = e4), onChange: f2 }, { default: M(() => [(s(true), r(m, null, v(Object.keys(p2), (e4) => (s(), h(d(G), { key: e4, placement: "top", title: p2[e4].description }, { default: M(() => [g(d(Z), { class: "radio-button", value: e4 }, { default: M(() => [g(d(Te), { icon: p2[e4].icon }, null, 8, ["icon"])]), _: 2 }, 1032, ["value"])]), _: 2 }, 1032, ["title"]))), 128))]), _: 1 }, 8, ["value"])], 2));
} }), [["__scopeId", "data-v-63ee2427"]]), ja = Yt(e({ __name: "TextVAlignStyle", props: { alignType: { type: String, default: void 0 } }, emits: ["alignChange"], setup(e2, { emit: t2 }) {
  const o2 = e2, l2 = t2, i2 = y();
  n(() => o2.alignType, (e3) => {
    i2.value = e3;
  }, { immediate: true });
  const { prefixCls: u2 } = we("v-align-style"), p2 = a({ [Wa.Top]: { name: "垂直居上", icon: "carbon:align-vertical-top", description: "文字垂直居上" }, [Wa.Center]: { name: "垂直居中", icon: "carbon:align-vertical-center", description: "文字垂直居中" }, [Wa.Bottom]: { name: "垂直居下", icon: "carbon:align-vertical-bottom", description: "文字垂直居下" } });
  function f2(e3) {
    const t3 = e3.target.value;
    l2("alignChange", t3);
  }
  return (e3, t3) => (s(), r("div", { class: c(d(u2)) }, [g(d(Y), { value: i2.value, "onUpdate:value": t3[0] || (t3[0] = (e4) => i2.value = e4), onChange: f2 }, { default: M(() => [(s(true), r(m, null, v(Object.keys(p2), (e4) => (s(), h(d(G), { key: e4, placement: "top", title: p2[e4].description }, { default: M(() => [g(d(Z), { class: "radio-button", value: e4 }, { default: M(() => [g(d(Te), { icon: p2[e4].icon }, null, 8, ["icon"])]), _: 2 }, 1032, ["value"])]), _: 2 }, 1032, ["title"]))), 128))]), _: 1 }, 8, ["value"])], 2));
} }), [["__scopeId", "data-v-a40ac773"]]), za = { class: "line" }, Fa = { class: "input" }, Pa = { class: "line" }, Ea = { style: { flex: "1" } }, Ha = { key: 0, class: "line" }, Qa = e({ name: "TextStyle", __name: "index", props: { verticalAlign: { type: Boolean, default: false }, textStyle: { type: Object, default: () => {
  ...
  const a2 = e2, i2 = o2, { prefixCls: u2 } = we("text-style"), { token: m2 } = E.useToken(), v2 = l(() => m2.value.colorBorder), C2 = y(12), b2 = y(Ua[0].value), M2 = y(), k2 = y();
  function w2(e3) {
    const t2 = x2(e3);
  ...
  function _2(e3, t2) {
    if (!e3) return;
    const o3 = t2 == null ? void 0 : t2.startsWith("linear-gradient"), l2 = Math.round((C2.value ?? 12) / 10), n2 = (e3 == null ? void 0 : e3.underline) && o3 ? { borderBottom: `${l2}px solid`, borderImage: `${t2} 1` } : (e3 == null ? void 0 : e3.underline) ? { borderBottom: `${l2}px solid`, borderColor: `${t2}` } : {}, a3 = e3[Za.Bold] ? { fontWeight: "bold" } : {}, r2 = e3[Za.Italic] ? { fontStyle: "italic" } : {};
    if (o3) {
      const e4 = t2 == null ? void 0 : t2.match(new RegExp("(?<=linear-gradient.*?)(rgba\\(.*?\\))", "g"));
      e4 && e4.length > 0 && (t2 = e4[0]);
    }
    const s2 = e3[Za.Strikethrough] ? { textDecoration: "line-through", textDecorationColor: t2 } : {};
    i2("fontStyleChange", { value: e3, style: { ...a3, ...r2, ...n2, ...s2 } });
  }
  function L2(e3) {
    i2("hAlignChange", { value: e3, style: { justifyContent: e3 === Va.Left ? "flex-start" : e3 === Va.Center ? "center" : e3 === Va.Right ? "flex-end" : "center" } });
  }
  function I2(e3) {
    i2("vAlignChange", { value: e3, style: { alignItems: e3 === Wa.Top ? "flex-start" : e3 === Wa.Center ? "center" : e3 === Wa.Bottom ? "flex-end" : "unset" } });
  }
  function D2(e3) {
  ...
  return n(() => a2.textStyle, (e3) => {
    var _a2, _b, _c2, _d2;
    e3 && (C2.value = ((_a2 = a2.textStyle) == null ? void 0 : _a2.fontSize) ?? 12, b2.value = ((_b = a2.textStyle) == null ? void 0 : _b.font) ?? Ua[0].value, M2.value = (_c2 = a2.textStyle) == null ? void 0 : _c2.textColor, k2.value = (_d2 = a2.textStyle) == null ? void 0 : _d2.fontStyle);
  }, { immediate: true, deep: true }), (t2, o3) => {
    var _a2, _b;
    return s(), r("div", { class: c(d(u2)) }, [p("div", za, [g(d(K), { style: { flex: "1" }, value: b2.value, "onUpdate:value": o3[0] || (o3[0] = (e3) => b2.value = e3), options: d(Ua), onChange: D2 }, null, 8, ["value", "options"]), p("div", Fa, [g(d(Q), { value: C2.value, "onUpdate:value": o3[1] || (o3[1] = (e3) => C2.value = e3), bordered: false, placeholder: "大小", controls: false, min: 6, maxlength: 4, onChange: T2 }, null, 8, ["value"]), o3[2] || (o3[2] = p("div", { class: "suffix" }, "px", -1))]), e2.hideAlign ? f("", true) : (s(), h(Oa, { key: 0, "align-type": (_a2 = e2.textStyle) == null ? void 0 : _a2.hAlignType, onAlignChange: L2 }, null, 8, ["align-type"]))]), p("div", Pa, [p("div", Ea, [g(d(qo), { type: e2.colorType, "direct-confirm": true, value: M2.value, onConfirmChange: w2, onInputChange: x2 }, null, 8, ["type", "value"])]), g(Aa, $({ hide: e2.hideFontStyle }, k2.value, { onFontStyleChange: S2 }), null, 16, ["hide"])]), e2.verticalAlign ? (s(), r("div", Ha, [g(ja, { "align-type": (_b = e2.textStyle) == null ? void 0 : _b.vAlignType, onAlignChange: I2 }, null, 8, ["align-type"])])) : f("", true)], 2);
  };
} }), Ga = Yt(Qa, [["__scopeId", "data-v-c65c80c4"]]), Ua = [{ value: "微软雅黑", label: "微软雅黑" }, { value: "宋体", label: "宋体" }, { value: "黑体", label: "黑体" }, { value: "楷体", label: "楷体" }, { value: "仿宋", label: "仿宋" }, { value: "sans-serif", label: "sans-serif" }, { value: "Arial", label: "Arial" }, { value: "PingFangSC", label: "PingFangSC" }, { value: "monospace", label: "monospace" }, { value: "Courier New", label: "Courier New" }, { value: "lcd", label: "lcd" }];
var Va = ((e2) => (e2.Left = "left", e2.Center = "center", e2.Right = "right", e2))(Va || {}), Wa = ((e2) => (e2.Top = "top", e2.Center = "center", e2.Bottom = "bottom", e2))(Wa || {}), Za = ((e2) => (e2.Bold = "bold", e2.Italic = "italic", e2.Underline = "underline", e2.Strikethrough = "strikethrough", e2))(Za || {});
function Ya(e2) {
  switch (e2) {
    case "NUMBER":
...
var Ka = ((e2) => (e2.error = "error", e2.eq = "=", e2.ne = "!=", e2.gt = ">", e2.ge = ">=", e2.lt = "<", e2.le = "<=", e2.between = "between", e2.isNull = "isNull", e2.isNotNull = "isNotNull", e2.like = "like", e2.notLike = "notLike", e2.likeLeft = "likeLeft", e2.likeRight = "likeRight", e2))(Ka || {}), Xa = ((e2) => (e2.count = "计数", e2.sum = "求和", e2.avg = "平均", e2.max = "最大", e2.min = "最小", e2))(Xa || {}), Ja = ((e2) => (e2.formula = "formula", e2.operator = "operator", e2.field = "field", e2.param = "param", e2))(Ja || {});
const ei = (e2) => Ke.get({ url: "/nocode/apiFolder/tree", params: e2 }), ti = (e2) => Ke.get({ url: "/nocode/apiFolder", params: e2 });
function oi(e2) {
  return Ke.download({ url: "/nocode/apiFolder/export", params: e2 });
}
function li(e2) {
  return Ke.post({ url: "/nocode/apiFolder", params: e2 }, { successMessageMode: "message" });
}
function ni(e2) {
  return Ke.put({ url: "/nocode/apiFolder", params: e2 }, { successMessageMode: "message" });
}
function ai(e2) {
  return Ke.put({ url: "/nocode/apiFolder/drag", params: e2 }, { successMessageMode: "message" });
}
function ii(e2) {
  return Ke.delete({ url: `/nocode/apiFolder/${e2}` }, { successMessageMode: "message" });
}
const ri = (e2) => Ke.get({ url: "/nocode/apiParams", params: e2 });
function si(e2) {
  return Ke.download({ url: "/nocode/apiParams/export", params: e2 });
}
function ui(e2) {
  return Ke.post({ url: "/nocode/apiParams", params: e2 }, { successMessageMode: "message" });
}
function ci(e2) {
  return Ke.put({ url: "/nocode/apiParams", params: e2 }, { successMessageMode: "message" });
}
function di(e2) {
  return Ke.delete({ url: `/nocode/apiParams/${e2}` }, { successMessageMode: "message" });
}
const pi = (e2) => Ke.get({ url: "/nocode/formulaInfo/tree", params: e2 }), fi = (e2) => Ke.get({ url: `/nocode/formulaParam/${e2}` }), hi = (e2) => Ke.get({ url: "/nocode/formulaInfo", params: e2 });
function gi(e2) {
  return Ke.get({ url: `/nocode/formulaInfo/${e2}` });
}
function yi(e2) {
  return Ke.download({ url: "/nocode/formulaInfo/export", params: e2 });
}
function mi(e2) {
  return Ke.post({ url: "/nocode/formulaInfo", params: e2 }, { successMessageMode: "message" });
}
function vi(e2) {
  return Ke.put({ url: "/nocode/formulaInfo", params: e2 }, { successMessageMode: "message" });
}
function Ci(e2) {
  return Ke.delete({ url: `/nocode/formulaInfo/${e2}` }, { successMessageMode: "message" });
}
function bi(e2) {
  return Ke.delete({ url: "/nocode/formulaInfo/batch/" + e2 }, { successMessageMode: "message" });
}
const Mi = () => "/nocode/mfApi/api", ki = (e2) => Ke.get({ url: "/nocode/mfApi", params: e2 });
function wi(e2) {
  return Ke.download({ url: "/nocode/mfApi/export", params: e2 });
}
function xi(e2) {
  return Ke.post({ url: "/nocode/mfApi", params: e2 }, { successMessageMode: "message" });
}
function Si(e2) {
  return Ke.put({ url: "/nocode/mfApi", params: e2 }, { successMessageMode: "message" });
}
function _i(e2) {
  return Ke.delete({ url: `/nocode/mfApi/${e2}` }, { successMessageMode: "message" });
}
function $i(e2) {
  return Ke.get({ url: `/nocode/mfApi/${e2}` });
}
function Li(e2) {
  return Ke.post({ url: "/nocode/mfApi/sql", params: e2 }, { errorMessageMode: "message" });
}
function Ii(e2, t2) {
  return Ke.post({ url: `/nocode/mfApi/data?pageNum=${t2.pageNum}&pageSize=${t2.pageSize}`, params: e2 }, { errorMessageMode: "message" });
}
function Di(e2, t2) {
  return Ke.post({ url: `/nocode/mfApi/formulaTest?pageNum=${t2.pageNum}&pageSize=${t2.pageSize}`, params: e2 }, { errorMessageMode: "message" });
}
function Ti(e2) {
  return Ke.post({ url: "/nocode/mfApi/fields", params: e2 }, { errorMessageMode: "message" });
}
const Ni = (e2) => Ke.get({ url: "/nocode/mfApi/headers", params: e2 });
function Bi(e2) {
  return Ke.get({ url: `/nocode/mfApi/fields/${e2}` }, { errorMessageMode: "message" });
}
function Ri(e2, t2, o2) {
  return Ke.get({ url: `/nocode/mfApi/table/${e2}?pageNum=${t2.pageNum}&pageSize=${t2.pageSize}`, params: o2 }, { errorMessageMode: "message" });
}
function qi(e2, t2, o2) {
  return Ke.get({ url: `/nocode/mfApi/api/${e2}`, params: { ...t2, ...o2 } }, { errorMessageMode: "message" });
}
function Ai(e2, t2, o2) {
  return Ke.put({ url: "/nocode/mfApi/field/rename", params: { id: e2, colName: t2, newName: o2 } }, { successMessageMode: "message" });
}
const Oi = (e2) => Ke.get({ url: "/nocode/mfScreen", params: e2 });
function ji(e2, t2) {
  return Ke.get({ url: `/nocode/mfScreen/${e2}`, params: { _shareToken: t2 } });
}
function zi(e2) {
  return Ke.post({ url: "/nocode/mfScreen", params: e2 }, { successMessageMode: "message" });
}
function Fi(e2) {
  return Ke.put({ url: "/nocode/mfScreen", params: e2 }, { successMessageMode: "message" });
}
function Pi(e2) {
  return Ke.delete({ url: `/nocode/mfScreen/${e2}` }, { successMessageMode: "message" });
}
function Ei(e2, t2) {
  return Ke.get({ url: `/nocode/mfScreen/share/${e2}`, params: { duration: t2 } });
}
function Hi(e2, t2, o2) {
  return Ke.get({ url: `/nocode/mfScreen/share/code/${e2}`, params: { path: t2, duration: o2 } });
}
const Qi = (e2) => Ke.get({ url: "/nocode/screenCharts", params: e2 });
function Gi(e2) {
  return Ke.get({ url: `/nocode/screenCharts/${e2}` });
}
function Ui(e2) {
  return Ke.download({ url: "/nocode/screenCharts/export", params: e2 });
}
function Vi(e2) {
  return Ke.post({ url: "/nocode/screenCharts", params: e2 }, { successMessageMode: "message" });
}
function Wi(e2) {
  return Ke.put({ url: "/nocode/screenCharts", params: e2 }, { successMessageMode: "message" });
}
function Zi(e2) {
  return Ke.delete({ url: `/nocode/screenCharts/${e2}` }, { successMessageMode: "message" });
}
function Yi(e2) {
  return Ke.delete({ url: `/nocode/screenCharts/batch/${e2}` }, { successMessageMode: "message" });
}
const Ki = (e2) => Ke.get({ url: "/nocode/screenFolder/tree", params: e2 }), Xi = (e2) => Ke.get({ url: "/nocode/screenFolder", params: e2 });
function Ji(e2) {
  return Ke.download({ url: "/nocode/screenFolder/export", params: e2 });
}
function er(e2) {
  return Ke.post({ url: "/nocode/screenFolder", params: e2 }, { successMessageMode: "message" });
}
function tr(e2) {
  return Ke.put({ url: "/nocode/screenFolder", params: e2 }, { successMessageMode: "message" });
}
function or(e2) {
  return Ke.put({ url: "/nocode/screenFolder/drag", params: e2 }, { successMessageMode: "message" });
}
function lr(e2) {
  return Ke.delete({ url: `/nocode/screenFolder/${e2}` }, { successMessageMode: "message" });
}
const nr = (e2) => Ke.get({ url: "/nocode/screenResourceApi", params: e2 });
function ar(e2) {
  return Ke.get({ url: `/nocode/screenResourceApi/${e2}` });
}
function ir(e2) {
  return Ke.post({ url: "/nocode/screenResourceApi", params: e2 }, { successMessageMode: "message" });
}
function rr(e2) {
  return Ke.put({ url: "/nocode/screenResourceApi", params: e2 }, { successMessageMode: "message" });
}
function sr(e2) {
  return Ke.delete({ url: `/nocode/screenResourceApi/${e2}` }, { successMessageMode: "message" });
}
function ur(e2, t2, o2) {
  return Ke.get({ url: `/nocode/screenResourceApi/table/${e2}?pageNum=${t2.pageNum}&pageSize=${t2.pageSize}`, params: { ...o2 } }, { errorMessageMode: "message" });
}
function cr(e2) {
  return Ke.get({ url: `/nocode/screenResourceApi/tables/${e2}` }, { errorMessageMode: "message" });
}
function dr(e2) {
  return Ke.get({ url: `/nocode/screenResourceApi/fields/${e2}` }, { errorMessageMode: "message" });
}
function pr(e2) {
  return Ke.get({ url: `/nocode/screenResourceApi/api/${e2}` });
}
function fr(e2, t2) {
  return Ke.get({ url: "/nocode/screenLayers", params: { ...e2, _shareToken: t2 } });
}
const hr = (e2) => Ke.get({ url: "/nocode/screenResource", params: e2 });
function gr(e2) {
  return Ke.get({ url: `/nocode/screenResource/${e2}` });
}
function yr(e2) {
  return Ke.post({ url: "/nocode/screenResource", params: e2 }, { successMessageMode: "message" });
}
function mr(e2) {
  return Ke.put({ url: "/nocode/screenResource", params: e2 }, { successMessageMode: "message" });
}
function vr(e2) {
  return Ke.delete({ url: `/nocode/screenResource/${e2}` }, { successMessageMode: "message" });
}
function Cr(e2) {
  return Ke.get({ url: `/nocode/screenResource/layers/${e2}` });
}
const br = (e2, t2, o2) => ({ CtrlKeysEnum: { s: Re(() => o2(), 150), z: Re(() => e2(), 150) }, CtrlShiftKeysEnum: { z: Re(() => t2(), 150) } }), Mr = Xe("custom-api", { state: () => ({ sourceId: "", sourceType: 0, tableName: "", tableFields: [], tableList: [], tableFieldsMap: /* @__PURE__ */ new Map(), variables: [], level: 1, fieldsChange: 1, showData: false, showDataLevel: "", formulaMap: /* @__PURE__ */ new Map() }), getters: { getSourceId() {
  ...
  return t2 ? (this.tableFields = await Ni({ sourceId: e2, tableName: t2, sourceType: this.sourceType }), this.tableFields) : [];
}, setTableList(e2) {
  ...
  (await hi({ pageNum: 1, pageSize: 1e4 })).list.forEach((e2) => {
    const t2 = `${e2.categoryId},${e2.id}`;
    this.formulaMap.set(t2, { ...e2, key: t2 });
  });
}, getFormula(e2) {
  return this.formulaMap.get(e2);
} } }), kr = "__API_SAVE__", wr = "公式结果";
var xr = ((e2) => (e2.QUERY_SQL = "api_config:query_sql", e2.QUERY_DATA = "api_config:query_data", e2))(xr || {});
const Sr = async (e2) => {
  const t2 = Mr();
  if (!(e2 == null ? void 0 : e2.sqlQuery)) return [];
  const o2 = { sourceId: t2.getSourceId, sourceType: t2.getSourceType, sqlQuery: e2.sqlQuery }, l2 = await Ti(o2);
  return l2 && l2.forEach((t3) => t3.tableAlias = e2.sourceTable), l2;
}, _r = (e2) => e2 && ((e2 == null ? void 0 : e2.aggregates) && e2.aggregates.length > 0 || e2.groups && e2.groups.length > 0 || e2.customColumns && e2.customColumns.length > 0), $r = (e2, t2) => t2 === Mr().getLevel && _r(e2), Lr = (e2, t2, o2) => {
  ...
  const t2 = Mr(), o2 = [...Ir(e2)].reduce((e3, t3) => (t3 && (e3[t3] = "1"), e3), {});
  return await Li({ sourceId: t2.getSourceId, sourceType: t2.getSourceType, sqlQuery: e2 || {}, params: o2 });
}, Tr = Xe("screen-edit", { state: () => ({ key: 0, id: "", isResource: false, screenCanvas: null, componentList: [], chartImgCache: /* @__PURE__ */ new Map(), scale: 1, canvasConfig: { width: 1920, height: 1080, theme: He.SYSTEM, backgroundColor: "", backgroundImage: "", screenName: "", screenDesc: "", watermark: "", fitType: jr.FIT_BOX, otherSetting: { globalVariables: [] } }, undoRedoManager: new qa(100), curComponent: null, curComponentIndex: -1, curHoverComponentId: "", selectArea: { show: false, dropInfo: { x: 0, y: 0, width: 0, height: 0 }, components: [] }, guideLine: { lineListX: [], lineListY: [] }, chartNameLabel: /* @__PURE__ */ new Map(), copyConfig: "", shareToken: void 0 }), getters: { getKey() {
  return this.key;
}, getId() {
  return this.id || "";
}, getTheme() {
  return this.canvasConfig.theme === He.SYSTEM ? Je().getDarkMode : this.canvasConfig.theme;
}, getScreenCanvas() {
  ...
  var _a2, _b, _c2, _d2;
  return ((_a2 = this.curComponent) == null ? void 0 : _a2.chart.type) === Jr.MfCombine && ((_c2 = (_b = this.curComponent) == null ? void 0 : _b.chart.options) == null ? void 0 : _c2.selectIndex) >= 0 ? (_d2 = this.curComponent) == null ? void 0 : _d2.chart.options.components[this.curComponent.chart.options.selectIndex] : this.curComponent;
  ...
  return e2 && this.chartImgCache.has(e2) ? this.chartImgCache.get(e2) : et;
}, getComponent(e2) {
  return jd(this.componentList, e2);
}, async initCache(e2) {
  if (e2) {
    const t2 = JSON.parse(e2);
    if (!t2) return;
    this.setComponentList(t2.componentList), this.setCanvasConfig(t2.canvasConfig), zd(this.componentList).then((e3) => {
      Promise.all(e3).then(() => {
  ...
  sessionStorage.setItem(`SCREEN_CONFIG_${this.getId}`, JSON.stringify({ componentList: this.componentList, canvasConfig: this.canvasConfig }));
}, async loadingScreen() {
  ...
  let t2;
  if (t2 = this.isResource ? await gr(this.getId) : await ji(this.getId, this.getShareToken), !t2) {
    const { createMessage: e3 } = Se();
  ...
  let n2;
  if (n2 = this.isResource ? await Cr(this.getId) : await fr({ screenId: this.getId }, this.getShareToken), !n2 || 0 === n2.length) return this.componentList;
  ...
  return a2.clear(), this.setComponentList(i2), zd(this.componentList, this.isResource).then((e3) => {
    Promise.all(e3).then(() => {
      this.setUndoRedoData("初始化");
    });
  }), this.componentList;
  ...
  this.canvasConfig.screenName = `${this.canvasConfig.screenName}_副本`, this.componentList.forEach((e3) => {
    t2(e3), e3.chart.type === Jr.MfCombine && e3.chart.options.components.forEach((e4) => {
      t2(e4);
    });
  });
  const o2 = (t3) => {
  ...
  this.componentList.forEach((e3) => {
    o2(e3), e3.chart.type === Jr.MfCombine && e3.chart.options.components.forEach((e4) => {
      o2(e4);
    });
  });
}, setCopyConfig(e2) {
  ...
  this.shareToken = e2;
} } }), Nr = Xe("screen-layout", { state: () => ({ chartCollapsed: false, layerCollapsed: false, configCollapsed: false }), getters: { getChartCollapsed() {
...
} } }), Br = "menu_chart_drag", Rr = "field_data_drag";
var qr = ((e2) => (e2.All = "all", e2.Chart = "chart", e2.Layer = "layer", e2.Config = "config", e2))(qr || {}), Ar = ((e2) => (e2.CurPage = "当前页", e2.NewTab = "新标签", e2.Dialog = "弹出窗", e2))(Ar || {}), Or = ((e2) => (e2.System = "系统页面", e2.External = "外部页面", e2))(Or || {}), jr = ((e2) => (e2.FIT_ORIGINAL = "fit_original", e2.FIT_WIDTH = "fit_width", e2.FIT_WIDTH_SCALE = "fit_width_scale", e2.FIT_HEIGHT = "fit_height", e2.FIT_HEIGHT_SCALE = "fit_height_scale", e2.FIT_BOX = "fit_box", e2))(jr || {});
const zr = { CREATE_CHART: "create_chart", CHART_CHANGE: "chart_change", MOVE: "chart_move", UN_MOVE: "chart_un_move", PLAY_ANIMATION: "play_animation", STOP_ANIMATION: "stop_animation", PLAY_ANIMATION_COMPLETE: "play_animation_complete", DATA_RENAME: "data_rename", SCREEN_SAVE: "screen_save", SCREEN_PREVIEW: "screen_preview", SHOW_MODAL: "show_modal" }, Fr = { x: "x", y: "y", z: "z", width: "width", height: "height", borderWidth: "borderWidth", borderRadius: "borderRadius", borderColor: "borderColor", borderImage: "borderImage", background: "background", rotate: "rotate", opacity: "opacity", padding: "padding" };
var Pr = ((e2) => (e2[e2.CONSTANT = 0] = "CONSTANT", e2[e2.VARIABLE = 1] = "VARIABLE", e2[e2.EVENT = 2] = "EVENT", e2))(Pr || {});
const Er = { id: "", name: "", data: { type: 0, dataSet: void 0, limit: 100, maxLimit: 9999, priority: 0 }, events: { emits: [], on: [] }, options: {} }, Hr = { id: null, opacity: 1, animations: [], lock: false, show: true, dropInfo: { x: 0, y: 0, zIndex: 0, width: 300, height: 300, rotate: 0, proportion: 1 }, groupStyle: {}, border: { show: false, color: void 0, width: 1, radius: 6 }, background: "", padding: "1px" };
function Qr(e2) {
  return e2 * Math.PI / 180;
}
function Gr(e2, t2, o2) {
  return { x: (e2.x - t2.x) * Math.cos(Qr(o2)) - (e2.y - t2.y) * Math.sin(Qr(o2)) + t2.x, y: (e2.x - t2.x) * Math.sin(Qr(o2)) + (e2.y - t2.y) * Math.cos(Qr(o2)) + t2.y };
}
function Ur(e2, t2) {
  return { x: e2.x + (t2.x - e2.x) / 2, y: e2.y + (t2.y - e2.y) / 2 };
}
function Vr(e2) {
  return Math.abs(Math.sin(Qr(e2)));
}
function Wr(e2) {
  return Math.abs(Math.cos(Qr(e2)));
}
function Zr(e2) {
  return (e2 + 360) % 360;
}
const Yr = { style: { value: "样式", show: true, payload: { icon: "carbon:color-palette", component: null } }, data: { value: "数据", show: true, payload: { icon: "carbon:data-structured", component: null } }, animation: { value: "动画", show: true, payload: { icon: "carbon:flow-modeler", component: T(ot(() => import("./index2.js"), { loading: true, size: "large" })) } }, advance: { value: "进阶", show: true, payload: { icon: "iconfont:icon-mfish-config", component: T(ot(() => import("./index3.js"), { loading: true, size: "large" })) } } }, Kr = (e2) => {
...
function os() {
  ...
  async function t2(t3) {
    ...
    t3.type && Ld[t3.type].initData(t3, o2);
  }
  return { setParamValue: function(e3, t3) {
    e3.data.paramsValue ? e3.data.paramsValue = { ...e3.data.paramsValue, ...t3 } : e3.data.paramsValue = t3;
  }, getDataTable: e2, setDataTable: t2, eventCallback: function(e3, o2) {
    o2.type === as.PARAM_CHANGE && t2(e3);
  } };
}
const ls = tt();
var ns = ((e2) => (e2.CHART_CLICK = "click", e2.CHART_DBLCLICK = "dblclick", e2.CHART_MOUSE_ENTER = "mouseenter", e2.CHART_MOUSE_LEAVE = "mouseLeave", e2))(ns || {}), as = ((e2) => (e2.PARAM_CHANGE = "param_change", e2.PAGE_JUMP = "param_jump", e2))(as || {});
const is = (e2, t2) => {
  var _a2, _b;
  switch (e2) {
    case "click":
    ...
    case "mouseLeave":
      return "鼠标离开";
    default:
      return t2 ? ((_b = (_a2 = Ld[t2].customEvents) == null ? void 0 : _a2.find((t3) => t3.value === e2)) == null ? void 0 : _b.name) ?? "" : "";
  }
};
function rs(e2, t2, o2) {
  const l2 = Tr(), a2 = (t3, o3) => {
    ((e3) => {
      var _a2;
      if (((_a2 = e3.events) == null ? void 0 : _a2.emits) && e3.events.emits.includes(t3)) {
        const l3 = t3 + e3.id;
        ls.emit(l3, { eventId: l3, e: o3, data: e3.data });
      }
    })(e2);
  }, i2 = { mouseenter: (e3) => {
    a2("mouseenter", e3);
    ...
    if (e3) for (const t3 of Object.keys(e3)) Ve(e3[t3].value) && e3[t3].value.forEach((e4) => {
      ls.off(c2(e4), u2);
    });
  }, { eventCallback: s2 } = os(), u2 = (o3) => {
    if (0 === e2.data.type) return;
    const l3 = e2.data.params;
    if (l3) {
      const t3 = {};
      for (const e3 of Object.keys(l3)) l3[e3].type === Pr.EVENT && Ve(l3[e3].value) && l3[e3].value.forEach((l4) => {
        const n3 = Object.keys(ns).some((e4) => ns[e4] === l4.event);
        if (o3.eventId === c2(l4)) {
          let a3;
          a3 = n3 ? 0 === o3.data.type ? o3.data.dataSet : o3.data.result : o3.e, Ve(a3) ? a3.length > 0 ? t3[e3] = a3[0][l4.param] : t3[e3] = "" : We(a3) ? t3[e3] = a3[l4.param] : Ze(a3) && (t3[e3] = a3);
        }
      });
      e2.data.paramsValue ? e2.data.paramsValue = { ...e2.data.paramsValue, ...t3 } : e2.data.paramsValue = t3;
    }
    const n2 = { type: "param_change", ...o3 };
    s2(e2, n2), t2 && t2(n2);
  };
  function c2(e3) {
  ...
  n(() => e2.data.params, (e3, t3) => {
    !function(e4, t4) {
      if (r2(t4), e4) {
        ...
        for (const o3 of Object.keys(e4)) Ve(e4[o3].value) && e4[o3].value.forEach((e5) => {
          const o4 = c2(e5);
          t5.has(o4) || (t5.add(o4), ls.on(o4, u2));
        });
      }
    }(e3, t3);
  }, { immediate: true, deep: true }), _(() => {
    var _a2;
    r2((_a2 = e2.data) == null ? void 0 : _a2.params);
  });
  const d2 = (t3) => {
    var _a2, _b, _c2;
    ...
    const o3 = (() => {
      var _a3, _b2, _c3, _d2;
      let t4;
      if (((_a3 = e2.jump) == null ? void 0 : _a3.pageType) === Or.System) {
        t4 = lt.resolve({ name: "ScreenPreview", params: { id: (_b2 = e2.jump) == null ? void 0 : _b2.page.value }, query: { shareToken: l2.getShareToken, isResource: (_c3 = l2.getIsResource) == null ? void 0 : _c3.toString() } }).href;
      } else t4 = (_d2 = e2.jump) == null ? void 0 : _d2.page.value, t4.startsWith("http") || (t4 = `http://${t4}`);
      return t4;
    })();
    switch ((_c2 = e2.jump) == null ? void 0 : _c2.type) {
      ...
      case Ar.NewTab:
        ...
        break;
      case Ar.Dialog:
        Id.emit(zr.SHOW_MODAL, { ...t3, href: o3, jump: e2.jump });
    }
  };
  return n(() => e2 == null ? void 0 : e2.jump, (t3) => {
    !function(t4) {
      const o3 = "click" + e2.id;
      ls.off(o3, d2), (t4 == null ? void 0 : t4.open) && ls.on(o3, d2);
    }(t3);
  }, { immediate: true, deep: true }), { emitEvent: a2, commonEvents: i2, customEvents: o2 };
}
const ss = Yt(e({ name: "MfTag", __name: "index", props: { chart: { type: Object, required: true }, chartContain: { type: Object, required: true } }, emits: ["eventHandler"], setup(e2, { emit: t2 }) {
  const o2 = e2, a2 = t2, { prefixCls: i2 } = we("mf-tag"), g2 = l(() => Kr(o2.chart)), { emitEvent: y2, commonEvents: m2 } = rs(o2.chart, function(e3) {
    a2("eventHandler", e3);
  }, ["titleChange"]);
  return n(() => {
    var _a2;
    ...
    return s(), r("div", $({ class: d(i2), style: { ...(_b = (_a2 = e2.chart.options) == null ? void 0 : _a2.hAlign) == null ? void 0 : _b.style } }, N(d(m2), true)), [p("div", { class: "title", style: u({ overflow: e2.chart.options.hide ? "hidden" : "", ...(_d2 = (_c2 = e2.chart.options) == null ? void 0 : _c2.vAlign) == null ? void 0 : _d2.style }) }, [((_e2 = e2.chart.data.dataSet) == null ? void 0 : _e2.icon) ? (s(), h(d(Te), { key: 0, size: g2.value.fontSize, icon: (_f2 = e2.chart.data.dataSet) == null ? void 0 : _f2.icon, color: e2.chart.options.iconColor }, null, 8, ["size", "icon", "color"])) : f("", true), p("span", { class: c({ hide: e2.chart.options.hide }), style: u(g2.value) }, S((_g = e2.chart.data.dataSet) == null ? void 0 : _g.title), 7)], 4)], 16);
  };
} }), [["__scopeId", "data-v-540aeb6f"]]);
function us(e2) {
  return e2.chart.data.dataSet = Be(cs.defaultData), e2.chart.data.headers = cs.defaultHeaders && cs.defaultHeaders(), e2.chart.data.limit = 1, e2.chart.data.maxLimit = 1, e2.chart.options.bordered = true, e2.chartContain.dropInfo.width = 80, e2.chartContain.dropInfo.height = 34, e2;
}
const cs = { type: Jr.MfTag, component: ss, create: us, config: (() => {
  const e2 = Be(Yr);
  return e2.data.show = false, e2.style.payload.component = T(ot(() => import("./MfTagConfig.js"), { loading: true, size: "large" })), e2;
...
}, customEvents: [{ name: "标签值改变", value: "titleChange" }] }, ds = Object.freeze(Object.defineProperty({ __proto__: null, default: cs, mfTagCreate: us }, Symbol.toStringTag, { value: "Module" })), ps = { ...cs, type: Jr.MfDataTag, component: ot(() => import("./index5.js"), { loading: true }), config: (() => {
  const e2 = Be(Yr);
  return e2.style.payload.component = T(ot(() => import("./MfDataTagStyleConfig.js"), { loading: true, size: "large" })), e2.data.payload.component = T(ot(() => import("./MfDataTagDataConfig.js"), { loading: true, size: "large" })), e2;
})(), initData: (e2, t2) => {
  const { setDynamicData: o2 } = fs();
  o2(e2, t2);
} };
function fs() {
  function e2(e3) {
    var _a2;
    ((_a2 = e3 == null ? void 0 : e3.data) == null ? void 0 : _a2.dataSet) && (e3.data.dataSet.title = ps.defaultData.title);
  }
  function t2(e3) {
    var _a2;
    ((_a2 = e3 == null ? void 0 : e3.data) == null ? void 0 : _a2.dataSet) && (e3.data.dataSet.icon = ps.defaultData.icon);
  }
...
const hs = Object.freeze(Object.defineProperty({ __proto__: null, default: ps, useDataTag: fs }, Symbol.toStringTag, { value: "Module" }));
function gs() {
  const e2 = Tr(), { getDataTable: t2, setDataTable: o2 } = os();
...
function ys(e2) {
  return e2.chart.data.dataSet = Be(ms.defaultData), e2.chart.data.headers = ms.defaultHeaders && ms.defaultHeaders(), e2.chart.data.limit = 1, e2.chart.data.maxLimit = 1, e2.chartContain.dropInfo.width = 120, e2.chartContain.dropInfo.height = 60, e2.chart.options.backgroundColor = "transparent", e2;
}
const ms = { type: Jr.MfDigits, component: ot(() => import("./index6.js"), { loading: true }), create: ys, config: (() => {
  const e2 = Be(Yr);
  return e2.style.payload.component = T(ot(() => import("./MfDigitsConfig.js"), { loading: true, size: "large" })), e2.data.payload.component = T(ot(() => import("./MfDigitsDataConfig.js"), { loading: true, size: "large" })), e2;
})(), defaultData: 12345, defaultHeaders: () => Be({ num: { colName: "num", dataType: "number", rename: "数字" } }), initData: (e2, t2) => {
  const { setDynamicData: o2 } = vs();
  o2(e2, t2);
} };
function vs() {
  const { createMessage: e2 } = Se(), { deleteFields: t2 } = gs();
  function o2(e3) {
  ...
  return { resetDefault: o2, setDynamicData: function(l2, n2) {
    ...
    if (l2) if (((_a2 = n2 == null ? void 0 : n2.list) == null ? void 0 : _a2.length) > 0 && l2.data.fields) {
      ...
      if (a2 && a2.length > 0 && a2[0].colName) {
        ...
        if (!Ye(i2)) return e2.warning("显示内容必须为数字"), t2("num"), void o2(l2);
        l2.data.dataSet = i2;
      } else o2(l2);
    } else o2(l2);
  } };
}
const Cs = Object.freeze(Object.defineProperty({ __proto__: null, default: ms, mfDigitsCreate: ys, useDigitsData: vs }, Symbol.toStringTag, { value: "Module" }));
function bs(e2) {
  return e2.chart.data.dataSet = Be(Ms.defaultData), e2.chart.data.headers = Ms.defaultHeaders && Ms.defaultHeaders(), e2.chart.data.limit = 1, e2.chart.data.maxLimit = 1, e2.chartContain.dropInfo.width = 300, e2.chartContain.dropInfo.height = 300, e2.chart.options = { responsive: true, userOptions: { show: false }, style: { chart: { animation: { speed: 0.2 }, backgroundColor: "transparent", layout: { wheel: { ticks: { rounded: true, inactiveColor: "#3A3A3A", activeColor: "#5f8bee", sizeRatio: 0.9, gradient: { show: true, shiftHueIntensity: 100 } } }, innerCircle: { show: true, stroke: "#E1E5E8", strokeWidth: 1 }, percentage: { show: true, fontSize: 48, rounding: 1, bold: true } } } } }, e2;
}
const Ms = { type: Jr.MfWheel, component: ot(() => import("./index7.js"), { loading: true }), create: bs, config: (() => {
  const e2 = Be(Yr);
  return e2.style.payload.component = T(ot(() => import("./MfWheelConfig.js"), { loading: true, size: "large" })), e2.data.payload.component = T(ot(() => import("./MfWheelDataConfig.js"), { loading: true, size: "large" })), e2;
})(), defaultData: { percentage: 68.8 }, defaultHeaders: () => Be({ percentage: { colName: "percentage", dataType: "number", rename: "比列" } }), initData: (e2, t2) => {
  const { setDynamicData: o2 } = ks("percentage");
  o2(e2, t2);
} };
function ks(e2) {
  const { createMessage: t2 } = Se(), { deleteFields: o2 } = gs();
  function l2(e3) {
    e3.data.dataSet = Ms.defaultData;
  }
  return { resetDefault: l2, setDynamicData: function(n2, a2) {
    ...
    if (n2) if (((_a2 = a2 == null ? void 0 : a2.list) == null ? void 0 : _a2.length) > 0 && n2.data.fields) {
      ...
      if (i2 && i2.length > 0 && i2[0].colName) {
        ...
        if (!Ye(r2)) return t2.warning("百分比必须为数字"), o2(e2), void l2(n2);
        n2.data.dataSet = { percentage: r2 };
      } else l2(n2);
    } else l2(n2);
  } };
}
const ws = Object.freeze(Object.defineProperty({ __proto__: null, default: Ms, mfWheelCreate: bs, useWheelData: ks }, Symbol.toStringTag, { value: "Module" }));
function xs(e2) {
  return e2.chart.data.dataSet = Be(Ss.defaultData), e2.chart.data.headers = Ss.defaultHeaders && Ss.defaultHeaders(), e2.chart.data.limit = 1, e2.chart.data.maxLimit = 1, e2.chartContain.dropInfo.width = 400, e2.chartContain.dropInfo.height = 72, e2.chart.options = { responsive: true, userOptions: { show: false }, style: { chart: { animation: { speed: 0.2 }, backgroundColor: "transparent", layout: { display: "horizontal", crescendo: false, curved: false, curveAngleX: 10, curveAngleY: 10, activeColor: "#5F8BEE", inactiveColor: "#3A3A3A", ticks: { gradient: { show: true, shiftHueIntensity: 30 } } }, percentage: { show: true, useGradientColor: false, color: "#CCCCCC", fontSize: 16, bold: true, rounding: 1, verticalPosition: "bottom", horizontalPosition: "left" } } } }, e2;
}
const Ss = { ...Ms, component: ot(() => import("./index8.js"), { loading: true }), type: Jr.MfTireMarks, create: xs, config: (() => {
  const e2 = Be(Yr);
  return e2.style.payload.component = T(ot(() => import("./MfTireMarksConfig.js"), { loading: true, size: "large" })), e2.data.payload.component = T(ot(() => import("./MfWheelDataConfig.js"), { loading: true, size: "large" })), e2;
})() }, _s = Object.freeze(Object.defineProperty({ __proto__: null, default: Ss, mfTireMarksCreate: xs }, Symbol.toStringTag, { value: "Module" }));
function $s(e2) {
...
const Ls = { type: Jr.MfBorder1, component: ot(() => import("./index9.js"), { loading: true }), create: $s, config: (() => {
  const e2 = Be(Yr);
  return e2.data.show = false, e2.style.payload.component = T(ot(() => import("./MfBorderConfig.js"), { loading: true, size: "large" })), e2;
})(), initData: () => {
} }, Is = Object.freeze(Object.defineProperty({ __proto__: null, default: Ls, mfBorder1Create: $s }, Symbol.toStringTag, { value: "Module" })), Ds = { ...Ls, type: Jr.MfBorder10, component: ot(() => import("./index10.js"), { loading: true }) }, Ts = Object.freeze(Object.defineProperty({ __proto__: null, default: Ds }, Symbol.toStringTag, { value: "Module" }));
function Ns(e2) {
...
const Bs = { type: Jr.MfBorder11, component: ot(() => import("./index11.js"), { loading: true }), create: Ns, config: (() => {
  const e2 = Be(Yr);
  return e2.data.show = false, e2.style.payload.component = T(ot(() => import("./MfBorder11Config.js"), { loading: true, size: "large" })), e2;
})(), initData: () => {
} }, Rs = Object.freeze(Object.defineProperty({ __proto__: null, default: Bs, mfBorder11Create: Ns }, Symbol.toStringTag, { value: "Module" })), qs = { ...Ls, type: Jr.MfBorder12, component: ot(() => import("./index12.js"), { loading: true }) }, As = Object.freeze(Object.defineProperty({ __proto__: null, default: qs }, Symbol.toStringTag, { value: "Module" })), Os = { ...Ls, type: Jr.MfBorder13, component: ot(() => import("./index13.js"), { loading: true }) }, js = Object.freeze(Object.defineProperty({ __proto__: null, default: Os }, Symbol.toStringTag, { value: "Module" }));
function zs(e2) {
  const t2 = Ls.create(e2);
  return t2.chartContain.border.radius = 0, t2;
}
const Fs = { ...Ls, type: Jr.MfBorder14, component: ot(() => import("./index14.js"), { loading: true }), create: zs }, Ps = Object.freeze(Object.defineProperty({ __proto__: null, default: Fs, mfBorder14Create: zs }, Symbol.toStringTag, { value: "Module" })), Es = { ...Fs, type: Jr.MfBorder15, component: ot(() => import("./index15.js"), { loading: true }) }, Hs = Object.freeze(Object.defineProperty({ __proto__: null, default: Es }, Symbol.toStringTag, { value: "Module" })), Qs = { ...Fs, type: Jr.MfBorder16, component: ot(() => import("./index16.js"), { loading: true }) }, Gs = Object.freeze(Object.defineProperty({ __proto__: null, default: Qs }, Symbol.toStringTag, { value: "Module" })), Us = { ...Fs, type: Jr.MfBorder17, component: ot(() => import("./index17.js"), { loading: true }) }, Vs = Object.freeze(Object.defineProperty({ __proto__: null, default: Us }, Symbol.toStringTag, { value: "Module" })), Ws = { ...Fs, type: Jr.MfBorder18, component: ot(() => import("./index18.js"), { loading: true }) }, Zs = Object.freeze(Object.defineProperty({ __proto__: null, default: Ws }, Symbol.toStringTag, { value: "Module" })), Ys = { ...Fs, type: Jr.MfBorder19, component: ot(() => import("./index19.js"), { loading: true }) }, Ks = Object.freeze(Object.defineProperty({ __proto__: null, default: Ys }, Symbol.toStringTag, { value: "Module" })), Xs = { ...Ls, type: Jr.MfBorder2, component: ot(() => import("./index20.js"), { loading: true }) }, Js = Object.freeze(Object.defineProperty({ __proto__: null, default: Xs }, Symbol.toStringTag, { value: "Module" })), eu = { ...Fs, type: Jr.MfBorder20, component: ot(() => import("./index21.js"), { loading: true }) }, tu = Object.freeze(Object.defineProperty({ __proto__: null, default: eu }, Symbol.toStringTag, { value: "Module" })), ou = { ...Fs, type: Jr.MfBorder21, component: ot(() => import("./index22.js"), { loading: true }) }, lu = Object.freeze(Object.defineProperty({ __proto__: null, default: ou }, Symbol.toStringTag, { value: "Module" })), nu = { ...Fs, type: Jr.MfBorder22, component: ot(() => import("./index23.js"), { loading: true }) }, au = Object.freeze(Object.defineProperty({ __proto__: null, default: nu }, Symbol.toStringTag, { value: "Module" })), iu = { ...Ls, type: Jr.MfBorder3, component: ot(() => import("./index24.js"), { loading: true }) }, ru = Object.freeze(Object.defineProperty({ __proto__: null, default: iu }, Symbol.toStringTag, { value: "Module" })), su = { ...Ls, type: Jr.MfBorder4, component: ot(() => import("./index25.js"), { loading: true }), config: (() => {
  const e2 = Be(Yr);
  return e2.data.show = false, e2.style.payload.component = T(ot(() => import("./MfBorder4Config.js"), { loading: true, size: "large" })), e2;
})() }, uu = Object.freeze(Object.defineProperty({ __proto__: null, default: su }, Symbol.toStringTag, { value: "Module" })), cu = { ...su, type: Jr.MfBorder5, component: ot(() => import("./index26.js"), { loading: true }) }, du = Object.freeze(Object.defineProperty({ __proto__: null, default: cu }, Symbol.toStringTag, { value: "Module" })), pu = { ...Ls, type: Jr.MfBorder6, component: ot(() => import("./index27.js"), { loading: true }) }, fu = Object.freeze(Object.defineProperty({ __proto__: null, default: pu }, Symbol.toStringTag, { value: "Module" })), hu = { ...Ls, type: Jr.MfBorder7, component: ot(() => import("./index28.js"), { loading: true }) }, gu = Object.freeze(Object.defineProperty({ __proto__: null, default: hu }, Symbol.toStringTag, { value: "Module" }));
function yu(e2) {
...
const mu = { type: Jr.MfBorder8, component: ot(() => import("./index29.js"), { loading: true }), create: yu, config: (() => {
  const e2 = Be(Yr);
  return e2.data.show = false, e2.style.payload.component = T(ot(() => import("./MfBorder8Config.js"), { loading: true, size: "large" })), e2;
})(), initData: () => {
} }, vu = Object.freeze(Object.defineProperty({ __proto__: null, default: mu, mfBorder8Create: yu }, Symbol.toStringTag, { value: "Module" })), Cu = { ...Ls, type: Jr.MfBorder9, component: ot(() => import("./index30.js"), { loading: true }) }, bu = Object.freeze(Object.defineProperty({ __proto__: null, default: Cu }, Symbol.toStringTag, { value: "Module" }));
function Mu(e2) {
...
const ku = { type: Jr.MfDecoration1, component: ot(() => import("./index31.js"), { loading: true }), create: Mu, config: (() => {
  const e2 = Be(Yr);
  return e2.data.show = false, e2.style.payload.component = T(ot(() => import("./MfDecorationConfig.js"), { loading: true, size: "large" })), e2;
})(), initData: () => {
} }, wu = Object.freeze(Object.defineProperty({ __proto__: null, default: ku, mfDecoration1Create: Mu }, Symbol.toStringTag, { value: "Module" }));
function xu(e2) {
...
const Su = { type: Jr.MfDecoration10, component: ot(() => import("./index32.js"), { loading: true }), create: xu, config: (() => {
  const e2 = Be(Yr);
  return e2.data.show = false, e2.style.payload.component = T(ot(() => import("./MfDecorationConfig.js"), { loading: true, size: "large" })), e2;
})(), initData: () => {
} }, _u = Object.freeze(Object.defineProperty({ __proto__: null, default: Su, mfDecoration10Create: xu }, Symbol.toStringTag, { value: "Module" }));
function $u(e2) {
...
const Lu = { type: Jr.MfDecoration11, component: ot(() => import("./index33.js"), { loading: true }), create: $u, config: (() => {
  const e2 = Be(Yr);
  return e2.data.show = false, e2.style.payload.component = T(ot(() => import("./MfDecoration7Config.js"), { loading: true, size: "large" })), e2;
})(), initData: () => {
} }, Iu = Object.freeze(Object.defineProperty({ __proto__: null, default: Lu, mfDecoration11Create: $u }, Symbol.toStringTag, { value: "Module" }));
function Du(e2) {
...
const Tu = { type: Jr.MfDecoration12, component: ot(() => import("./index34.js"), { loading: true }), create: Du, config: (() => {
  const e2 = Be(Yr);
  return e2.data.show = false, e2.style.payload.component = T(ot(() => import("./MfDecoration12Config.js"), { loading: true, size: "large" })), e2;
})(), initData: () => {
} }, Nu = Object.freeze(Object.defineProperty({ __proto__: null, default: Tu, mfDecoration12Create: Du }, Symbol.toStringTag, { value: "Module" }));
function Bu(e2) {
...
const Ru = { type: Jr.MfDecoration2, component: ot(() => import("./index35.js"), { loading: true }), create: Bu, config: (() => {
  const e2 = Be(Yr);
  return e2.data.show = false, e2.style.payload.component = T(ot(() => import("./MfDecoration2Config.js"), { loading: true, size: "large" })), e2;
})(), initData: () => {
} }, qu = Object.freeze(Object.defineProperty({ __proto__: null, default: Ru, mfDecoration2Create: Bu }, Symbol.toStringTag, { value: "Module" }));
function Au(e2) {
...
const Ou = { type: Jr.MfDecoration3, component: ot(() => import("./index36.js"), { loading: true }), create: Au, config: (() => {
  const e2 = Be(Yr);
  return e2.data.show = false, e2.style.payload.component = T(ot(() => import("./MfDecorationConfig.js"), { loading: true, size: "large" })), e2;
})(), initData: () => {
} }, ju = Object.freeze(Object.defineProperty({ __proto__: null, default: Ou, mfDecoration3Create: Au }, Symbol.toStringTag, { value: "Module" }));
function zu(e2) {
...
const Fu = { type: Jr.MfDecoration4, component: ot(() => import("./index37.js"), { loading: true }), create: zu, config: (() => {
  const e2 = Be(Yr);
  return e2.data.show = false, e2.style.payload.component = T(ot(() => import("./MfDecoration2Config.js"), { loading: true, size: "large" })), e2;
})(), initData: () => {
} }, Pu = Object.freeze(Object.defineProperty({ __proto__: null, default: Fu, mfDecoration4Create: zu }, Symbol.toStringTag, { value: "Module" }));
function Eu(e2) {
...
const Hu = { type: Jr.MfDecoration5, component: ot(() => import("./index38.js"), { loading: true }), create: Eu, config: (() => {
  const e2 = Be(Yr);
  return e2.data.show = false, e2.style.payload.component = T(ot(() => import("./MfDecoration5Config.js"), { loading: true, size: "large" })), e2;
})(), initData: () => {
} }, Qu = Object.freeze(Object.defineProperty({ __proto__: null, default: Hu, mfDecoration5Create: Eu }, Symbol.toStringTag, { value: "Module" }));
function Gu(e2) {
...
const Uu = { type: Jr.MfDecoration6, component: ot(() => import("./index39.js"), { loading: true }), create: Gu, config: (() => {
  const e2 = Be(Yr);
  return e2.data.show = false, e2.style.payload.component = T(ot(() => import("./MfDecorationConfig.js"), { loading: true, size: "large" })), e2;
})(), initData: () => {
} }, Vu = Object.freeze(Object.defineProperty({ __proto__: null, default: Uu, mfDecoration6Create: Gu }, Symbol.toStringTag, { value: "Module" }));
function Wu(e2) {
...
const Zu = { type: Jr.MfDecoration7, component: ot(() => import("./index40.js"), { loading: true }), create: Wu, config: (() => {
  const e2 = Be(Yr);
  return e2.data.show = false, e2.style.payload.component = T(ot(() => import("./MfDecoration7Config.js"), { loading: true, size: "large" })), e2;
})(), initData: () => {
} }, Yu = Object.freeze(Object.defineProperty({ __proto__: null, default: Zu, mfDecoration7Create: Wu }, Symbol.toStringTag, { value: "Module" }));
function Ku(e2) {
...
const Xu = { type: Jr.MfDecoration8, component: ot(() => import("./index41.js"), { loading: true }), create: Ku, config: (() => {
  const e2 = Be(Yr);
  return e2.data.show = false, e2.style.payload.component = T(ot(() => import("./MfDecoration8Config.js"), { loading: true, size: "large" })), e2;
})(), initData: () => {
} }, Ju = Object.freeze(Object.defineProperty({ __proto__: null, default: Xu, mfDecoration8Create: Ku }, Symbol.toStringTag, { value: "Module" }));
function ec(e2) {
...
const tc = { type: Jr.MfDecoration9, component: ot(() => import("./index42.js"), { loading: true }), create: ec, config: (() => {
  const e2 = Be(Yr);
  return e2.data.show = false, e2.style.payload.component = T(ot(() => import("./MfDecoration5Config.js"), { loading: true, size: "large" })), e2;
})(), initData: () => {
} }, oc = Object.freeze(Object.defineProperty({ __proto__: null, default: tc, mfDecoration9Create: ec }, Symbol.toStringTag, { value: "Module" }));
function lc(e2) {
...
const nc = { type: Jr.MfHeader1, create: lc, component: ot(() => import("./index43.js"), { loading: true }), config: (() => {
  const e2 = Be(Yr);
  return e2.data.show = false, e2.style.payload.component = T(ot(() => import("./MfDecorationConfig.js"), { loading: true, size: "large" })), e2;
})(), initData: () => {
} }, ac = Object.freeze(Object.defineProperty({ __proto__: null, default: nc, mfHeader1Create: lc }, Symbol.toStringTag, { value: "Module" }));
function ic(e2) {
...
const rc = { type: Jr.MfHeader2, create: ic, component: ot(() => import("./index44.js"), { loading: true }), config: (() => {
  const e2 = Be(Yr);
  return e2.data.show = false, e2.style.payload.component = T(ot(() => import("./MfDecorationConfig.js"), { loading: true, size: "large" })), e2;
})(), initData: () => {
} }, sc = Object.freeze(Object.defineProperty({ __proto__: null, default: rc, mfHeader2Create: ic }, Symbol.toStringTag, { value: "Module" }));
function uc(e2) {
...
const cc = { type: Jr.MfHeader3, create: uc, component: ot(() => import("./index45.js"), { loading: true }), config: (() => {
  const e2 = Be(Yr);
  return e2.data.show = false, e2.style.payload.component = T(ot(() => import("./MfDecorationConfig.js"), { loading: true, size: "large" })), e2;
})(), initData: () => {
} }, dc = Object.freeze(Object.defineProperty({ __proto__: null, default: cc, mfHeader3Create: uc }, Symbol.toStringTag, { value: "Module" }));
function pc(e2) {
...
const fc = { type: Jr.MfHeader4, create: pc, component: ot(() => import("./index46.js"), { loading: true }), config: (() => {
  const e2 = Be(Yr);
  return e2.data.show = false, e2.style.payload.component = T(ot(() => import("./MfDecorationConfig.js"), { loading: true, size: "large" })), e2;
})(), initData: () => {
} }, hc = Object.freeze(Object.defineProperty({ __proto__: null, default: fc, mfHeader4Create: pc }, Symbol.toStringTag, { value: "Module" }));
function gc(e2) {
  return e2.chartContain.dropInfo.width = 1920, e2.chartContain.dropInfo.height = 75, e2.chart.options.mainColor = "#0077CF", e2.chart.options.secondColor = "#0042A2", e2;
}
const yc = { type: Jr.MfHeader5, create: gc, component: ot(() => import("./index47.js"), { loading: true }), config: (() => {
  const e2 = Be(Yr);
  return e2.data.show = false, e2.style.payload.component = T(ot(() => import("./MfDecorationConfig.js"), { loading: true, size: "large" })), e2;
})(), initData: () => {
} }, mc = Object.freeze(Object.defineProperty({ __proto__: null, default: yc, mfHeader5Create: gc }, Symbol.toStringTag, { value: "Module" }));
function vc(e2) {
...
const Cc = { type: Jr.MfHeader6, create: vc, component: ot(() => import("./index48.js"), { loading: true }), config: (() => {
  const e2 = Be(Yr);
  return e2.data.show = false, e2.style.payload.component = T(ot(() => import("./MfDecorationConfig.js"), { loading: true, size: "large" })), e2;
})(), initData: () => {
} }, bc = Object.freeze(Object.defineProperty({ __proto__: null, default: Cc, mfHeader6Create: vc }, Symbol.toStringTag, { value: "Module" })), Mc = e({ name: "MfDateTime", __name: "index", props: { chart: { type: Object, required: true }, chartContain: { type: Object, required: true } }, setup(e2) {
  const t2 = e2, o2 = y(t2.chart), l2 = () => {
    var _a2;
    ((_a2 = t2.chart.options) == null ? void 0 : _a2.format) ? o2.value.data.dataSet.title = nt(/* @__PURE__ */ new Date(), t2.chart.options.format) : o2.value.data.dataSet.title = nt();
  };
  return i(() => {
    l2(), setInterval(l2, 1e3);
  }), (t3, l3) => (s(), h(ss, { "chart-contain": e2.chartContain, chart: o2.value }, null, 8, ["chart-contain", "chart"]));
} });
function kc(e2) {
  return e2.chart.data.dataSet = Be(wc.defaultData), e2.chart.data.headers = wc.defaultHeaders && wc.defaultHeaders(), e2.chart.data.limit = 1, e2.chart.data.maxLimit = 1, e2.chartContain.dropInfo.width = 190, e2.chartContain.dropInfo.height = 34, e2;
}
const wc = { type: Jr.MfDateTime, component: Mc, create: kc, config: (() => {
  const e2 = Be(Yr);
  return e2.data.show = false, e2.style.payload.component = T(ot(() => import("./MfDataTimeConfig.js"), { loading: true, size: "large" })), e2;
})(), defaultData: { title: "" }, defaultHeaders: () => Be({ title: { colName: "title", dataType: "string", rename: "标签" } }), initData: () => {
} }, xc = Object.freeze(Object.defineProperty({ __proto__: null, default: wc, mfDateTimeCreate: kc }, Symbol.toStringTag, { value: "Module" }));
function Sc() {
  const e2 = (e3, t3) => {
    var _a2;
    ...
    for (let e4 = 0; e4 < t3.length; e4++) if (o2 && o2.length > e4) n2.push(o2[e4]);
      ...
      const t4 = Ld[l2].defaultSeries;
      t4 ? t4.length > e4 ? n2.push(Be(t4[e4])) : n2.push(Be(t4[0])) : n2.push({});
    } else n2.push({});
    return n2;
  };
  function t2(t3) {
    var _a2;
    ((_a2 = t3 == null ? void 0 : t3.data) == null ? void 0 : _a2.dataSet) && (t3 == null ? void 0 : t3.type) && (t3.data.dataSet = Be(Ld[t3.type].defaultData), t3.options.series = e2(t3, Ld[t3.type].defaultSeries || []));
  }
  return { resetDefault: t2, setDynamicData: function(o2, l2) {
    ...
    if (o2) {
      if ((l2 == null ? void 0 : l2.list.length) > 0 && o2.data.fields && (((_b = (_a2 = o2.data.fields) == null ? void 0 : _a2.dimensions) == null ? void 0 : _b.length) > 0 || ((_d2 = (_c2 = o2.data.fields) == null ? void 0 : _c2.source) == null ? void 0 : _d2.length) > 0)) {
        const { createMessage: n2 } = Se(), { deleteFieldValue: a2 } = gs(), i2 = o2.data.fields.dimensions ?? [], r2 = o2.data.fields.source ?? [];
...
const _c = { type: Jr.MfBar, component: ot(() => import("./index49.js"), { loading: true, size: "large" }), create: function(e2) {
  return e2.chart.data.dataSet = Be(_c.defaultData), e2.chart.data.headers = _c.defaultHeaders && _c.defaultHeaders(e2.chart), e2.chart.data.limit = 100, e2.chart.data.maxLimit = 1e3, e2.chart.options = { grid: { left: "10%", right: "5%", top: "10%", bottom: "10%" }, xAxis: { show: true, type: "category" }, yAxis: { show: true }, series: Be(_c.defaultSeries) }, e2.chartContain.dropInfo.width = 400, e2.chartContain.dropInfo.height = 300, e2;
}, config: (() => {
  const e2 = Be(Yr);
  return e2.style.payload.component = T(ot(() => import("./MfBarLineConfig.js"), { loading: true, size: "large" })), e2.data.payload.component = T(ot(() => import("./MfBarLineDataConfig.js"), { loading: true, size: "large" })), e2;
})(), defaultData: { source: [["日期", "数据"], ["Mon", 120], ["Tue", 200], ["Wed", 150], ["Thu", 80], ["Fri", 70], ["Sat", 110], ["Sun", 130]] }, defaultHeaders: (e2) => {
  var _a2, _b;
  return Xr((_b = (_a2 = e2 == null ? void 0 : e2.data) == null ? void 0 : _a2.dataSet) == null ? void 0 : _b.source[0]);
}, defaultSeries: [{ type: "bar" }], initData: (e2, t2) => {
  const { setDynamicData: o2 } = Sc();
  o2(e2, t2);
} }, $c = Object.freeze(Object.defineProperty({ __proto__: null, default: _c }, Symbol.toStringTag, { value: "Module" }));
const Lc = { ..._c, type: Jr.MfBarHorizontal, create: function(e2) {
  const t2 = _c.create(e2);
...
const Dc = { ..._c, type: Jr.MfBarStack, create: function(e2) {
  const t2 = _c.create(e2);
...
}, defaultSeries: [{ type: "bar", stack: "total" }, { type: "bar", stack: "total" }], defaultData: { source: [["日期", "数据1", "数据2"], ["Mon", 120, 80], ["Tue", 200, 132], ["Wed", 150, 90], ["Thu", 80, 180], ["Fri", 70, 110], ["Sat", 110, 130], ["Sun", 130, 90]] } }, Tc = Object.freeze(Object.defineProperty({ __proto__: null, default: Dc }, Symbol.toStringTag, { value: "Module" }));
const Nc = { ...Dc, type: Jr.MfBarLine, defaultSeries: [{ type: "bar" }, { type: "line" }], create: function(e2) {
  const t2 = Dc.create(e2);
  return t2.chart.options.series = Be(Nc.defaultSeries), t2;
} }, Bc = Object.freeze(Object.defineProperty({ __proto__: null, default: Nc }, Symbol.toStringTag, { value: "Module" }));
const Rc = { ..._c, type: Jr.MfBarPlusMinus, create: function(e2) {
  const t2 = _c.create(e2);
  return t2.chart.data.dataSet = Be(Rc.defaultData), t2.chart.data.headers = Rc.defaultHeaders && Rc.defaultHeaders(t2.chart), t2.chart.options = { ...t2.chart.options, xAxis: { show: true, type: "value", position: "top", splitLine: { show: true, lineStyle: { type: "dashed" } } }, yAxis: { show: true, type: "category", axisLine: { show: false }, axisLabel: { show: false }, axisTick: { show: false } }, series: Be(Rc.defaultSeries) }, t2;
}, defaultData: { source: [["维度", "数据"], ["ten", -0.07], ["nine", -0.09], ["eight", 0.2], ["seven", -0.44], ["six", -0.23], ["five", 0.08], ["four", -0.17], ["three", 0.47], ["two", -0.36], ["one", 0.18]] } }, qc = Object.freeze(Object.defineProperty({ __proto__: null, default: Rc }, Symbol.toStringTag, { value: "Module" }));
const Ac = { ..._c, type: Jr.MfLine, create: function(e2) {
  return e2.chart.data.dataSet = Be(Ac.defaultData), e2.chart.data.headers = Ac.defaultHeaders && Ac.defaultHeaders(e2.chart), e2.chart.data.limit = 100, e2.chart.data.maxLimit = 1e3, e2.chart.options = { grid: { left: "10%", right: "10%", top: "10%", bottom: "10%" }, xAxis: { show: true, type: "category" }, yAxis: { show: true }, series: Be(Ac.defaultSeries) }, e2.chartContain.dropInfo.width = 400, e2.chartContain.dropInfo.height = 300, e2;
}, defaultSeries: [{ type: "line" }] }, Oc = Object.freeze(Object.defineProperty({ __proto__: null, default: Ac }, Symbol.toStringTag, { value: "Module" }));
const jc = { ...Ac, type: Jr.MfLineArea, create: function(e2) {
  const t2 = Ac.create(e2);
  return t2.chart.options.series = Be(jc.defaultSeries), t2;
}, defaultSeries: [{ type: "line", areaStyle: {} }] }, zc = Object.freeze(Object.defineProperty({ __proto__: null, default: jc }, Symbol.toStringTag, { value: "Module" }));
const Fc = { ...Dc, type: Jr.MfLineAreaStack, create: function(e2) {
  const t2 = Dc.create(e2);
  return t2.chart.options.series = Be(Fc.defaultSeries), t2;
}, defaultSeries: [{ type: "line", areaStyle: {}, stack: "total" }, { type: "line", areaStyle: {}, stack: "total" }] }, Pc = Object.freeze(Object.defineProperty({ __proto__: null, default: Fc }, Symbol.toStringTag, { value: "Module" }));
const Ec = { ...Ac, type: Jr.MfLineSmooth, create: function(e2) {
  const t2 = Ac.create(e2);
  return t2.chart.options.series = Be(Ec.defaultSeries), t2;
}, defaultSeries: [{ type: "line", smooth: true }] }, Hc = Object.freeze(Object.defineProperty({ __proto__: null, default: Ec }, Symbol.toStringTag, { value: "Module" }));
const Qc = { ...Dc, type: Jr.MfLineStack, create: function(e2) {
  const t2 = Dc.create(e2);
  return t2.chart.options.series = Be(Qc.defaultSeries), t2;
}, defaultSeries: [{ type: "line", stack: "total" }, { type: "line", stack: "total" }] }, Gc = Object.freeze(Object.defineProperty({ __proto__: null, default: Qc }, Symbol.toStringTag, { value: "Module" })), Uc = [{ color: ["#5470c6", "#91cc75", "#fac858", "#ee6666", "#73c0de", "#3ba272", "#fc8452", "#9a60b4", "#ea7ccc"], name: "默认" }, { color: ["#d87c7c", "#919e8b", "#d7ab82", "#6e7074", "#61a0a8", "#efa18d", "#787464", "#cc7e63", "#724e58", "#4b565b"], name: "复古" }, { color: ["#dd6b66", "#759aa0", "#e69d87", "#8dc1a9", "#ea7e53", "#eedd78", "#73a373", "#73b9bc", "#7289ab", "#91ca8c", "#f49f42"], name: "深色" }, { color: ["#516b91", "#59c4e6", "#edafda", "#93b7e3", "#a5e7f0", "#cbb0e3"], name: "灰粉" }, { color: ["#893448", "#d95850", "#eb8146", "#ffb248", "#f2d643", "#ebdba4"], name: "橘红" }, { color: ["#4ea397", "#22c3aa", "#7bd9a5", "#d0648a", "#f58db2", "#f2b3c9"], name: "青草" }, { color: ["#3fb1e3", "#6be6c1", "#626c91", "#a0a7e6", "#c4ebad", "#96dee8"], name: "蓝绿" }, { color: ["#fc97af", "#87f7cf", "#f7f494", "#72ccff", "#f7c5a0", "#d4a4eb", "#d2f5a6", "#76f2f2"], name: "粉青" }, { color: ["#c1232b", "#27727b", "#fcce10", "#e87c25", "#b5c334", "#fe8463", "#9bca63", "#fad860", "#f3a43b", "#60c0dd", "#d7504b", "#c6e579", "#f4e001", "#f0805a", "#26c0c0"], name: "鲜明" }, { color: ["#2ec7c9", "#b6a2de", "#5ab1ef", "#ffb980", "#d87a80", "#8d98b3", "#e5cf0d", "#97b552", "#95706d", "#dc69aa", "#07a2a4", "#9a7fd1", "#588dd5", "#f5994e", "#c05050", "#59678c", "#c9ab00", "#7eb00a", "#6f5553", "#c14089"], name: "马卡龙" }, { color: ["#e01f54", "#5e4ea5", "#f5e8c8", "#b8d2c7", "#c6b38e", "#a4d8c2", "#f3d999", "#d3758f", "#dcc392", "#2e4783", "#82b6e9", "#ff6347", "#a092f1", "#0a915d", "#eaf889", "#6699FF", "#ff6666", "#3cb371", "#d5b158", "#38b6b6"], name: "罗马" }, { color: ["#c12e34", "#e6b600", "#0098d9", "#2b821d", "#005eaa", "#339ca8", "#cda819", "#32a487"], name: "光亮" }, { color: ["#9b8bba", "#e098c7", "#8fd3e8", "#71669e", "#cc70af", "#7cb4cc"], name: "深紫" }];
const Vc = { type: Jr.MfMapLine, component: ot(() => import("./index50.js"), { loading: true, size: "large" }), create: function(e2) {
  return e2.chart.data.dataSet = Be(Vc.defaultData), e2.chart.data.headers = Vc.defaultHeaders && Vc.defaultHeaders(e2.chart), e2.chart.data.limit = 100, e2.chart.data.maxLimit = 1e3, e2.chart.options = { tooltip: { show: true }, geo: { map: "china", scale: true, zoom: 1.2, scaleLimit: { min: 0.5, max: 2 }, emphasis: { itemStyle: { color: "#3146B3" } }, itemStyle: { color: "#03133D", borderColor: "#6699FF", shadowColor: "#1773C3", shadowBlur: 10, borderWidth: 1 }, label: { color: "#ffffff", emphasis: { color: "#ffffff" } } }, visualMap: { show: false, min: 0, max: 100, text: ["高", "低"], realtime: false, calculable: true, inRange: { color: Be(Uc[0].color) } }, series: Be(Vc.defaultSeries) }, e2.chartContain.dropInfo.width = 800, e2.chartContain.dropInfo.height = 650, e2;
}, config: (() => {
  const e2 = Be(Yr);
  return e2.style.payload.component = T(ot(() => import("./MfMapConfig.js"), { loading: true, size: "large" })), e2.data.payload.component = T(ot(() => import("./MfMapLineDataConfig.js"), { loading: true, size: "large" })), e2;
  ...
  const { setDynamicData: o2 } = Wc();
  o2(e2, t2);
} };
function Wc() {
  function e2(e3) {
    e3.data.dataSet = Be(Vc.defaultData);
  }
  return { resetDefault: e2, setDynamicData: function(t2, o2) {
    ...
    if (t2) if ((o2 == null ? void 0 : o2.list.length) > 0 && t2.data.fields && (((_b = (_a2 = t2.data.fields) == null ? void 0 : _a2.start) == null ? void 0 : _b.length) > 0 || ((_d2 = (_c2 = t2.data.fields) == null ? void 0 : _c2.end) == null ? void 0 : _d2.length) > 0)) {
      const { createMessage: l2 } = Se(), { deleteFieldValue: n2 } = gs(), a2 = t2.data.fields.start ?? [], i2 = t2.data.fields.end ?? [], r2 = t2.data.fields.value ?? [];
...
const Zc = Object.freeze(Object.defineProperty({ __proto__: null, default: Vc, useEchartsMapLineData: Wc }, Symbol.toStringTag, { value: "Module" }));
const Yc = { ...Vc, type: Jr.MfMapLine3D, component: ot(() => import("./index51.js"), { loading: true, size: "large" }), create: function(e2) {
  var _a2;
  const t2 = Vc.create(e2);
  return (_a2 = t2.chart.options) == null ? true : delete _a2.geo, t2.chart.options = { ...t2.chart.options, geo3D: { map: "china", shading: "lambert", viewControl: { distance: 85, rotateSensitivity: 0, zoomSensitivity: 0 }, light: { main: { intensity: 1.2, shadow: true } }, itemStyle: { color: "#03133D", borderColor: "#6699FF", shadowColor: "#1773C3", shadowBlur: 10, borderWidth: 1 }, emphasis: { itemStyle: { color: "#3146B3" } }, label: { distance: 0, color: "#ffffff", emphasis: { color: "#ffffff" } } }, series: Be(Yc.defaultSeries) }, t2;
}, defaultSeries: [{ name: "飞线图", type: "lines3D", coordinateSystem: "geo3D", effect: { show: true, trailWidth: 2, trailLength: 0.8, trailOpacity: 0.8, trailColor: "#fff" }, lineStyle: { width: 1, curveness: 0.2 } }, { name: "城市", type: "scatter3D", coordinateSystem: "geo3D", rippleEffect: { brushType: "stroke" }, label: { show: true, position: "right", formatter: "{b}" } }] }, Kc = Object.freeze(Object.defineProperty({ __proto__: null, default: Yc }, Symbol.toStringTag, { value: "Module" }));
const Xc = { ..._c, type: Jr.MfPie, create: function(e2) {
  return e2.chart.data.dataSet = Be(Xc.defaultData), e2.chart.data.headers = Xc.defaultHeaders && Xc.defaultHeaders(e2.chart), e2.chart.data.limit = 100, e2.chart.data.maxLimit = 1e3, e2.chartContain.dropInfo.width = 400, e2.chartContain.dropInfo.height = 400, e2.chart.options = { tooltip: { trigger: "item" }, series: Be(Xc.defaultSeries) }, e2;
}, config: (() => {
  const e2 = Be(Yr);
  return e2.style.payload.component = T(ot(() => import("./MfPieConfig.js"), { loading: true, size: "large" })), e2.data.payload.component = T(ot(() => import("./MfPieDataConfig.js"), { loading: true, size: "large" })), e2;
})(), defaultSeries: [{ type: "pie", radius: ["0%", "65%"] }] }, Jc = Object.freeze(Object.defineProperty({ __proto__: null, default: Xc }, Symbol.toStringTag, { value: "Module" }));
const ed = { ...Xc, type: Jr.MfPieCircular, create: function(e2) {
  const t2 = Xc.create(e2);
  return t2.chart.options.series = Be(ed.defaultSeries), t2;
}, defaultSeries: [{ type: "pie", radius: ["30%", "65%"] }] }, td = Object.freeze(Object.defineProperty({ __proto__: null, default: ed }, Symbol.toStringTag, { value: "Module" }));
const od = { ...Xc, type: Jr.MfPieHalf, create: function(e2) {
  const t2 = Xc.create(e2);
  return t2.chart.options.series = Be(od.defaultSeries), t2;
}, defaultSeries: [{ type: "pie", center: ["50%", "70%"], radius: ["30%", "65%"], startAngle: 180, endAngle: 360 }] }, ld = Object.freeze(Object.defineProperty({ __proto__: null, default: od }, Symbol.toStringTag, { value: "Module" }));
const nd = { ...Xc, type: Jr.MfPieRose, create: function(e2) {
  const t2 = Xc.create(e2);
  return t2.chart.options.series = Be(nd.defaultSeries), t2;
}, defaultSeries: [{ type: "pie", roseType: "area", radius: ["10%", "65%"], itemStyle: { borderRadius: 8 } }] }, ad = Object.freeze(Object.defineProperty({ __proto__: null, default: nd }, Symbol.toStringTag, { value: "Module" }));
const id = { component: ot(() => import("./index52.js"), { loading: true, size: "large" }), type: Jr.MfRadar, create: function(e2) {
  return e2.chart.data.dataSet = Be(id.defaultData), e2.chart.data.headers = id.defaultHeaders && id.defaultHeaders(), e2.chart.data.limit = 5, e2.chart.data.maxLimit = 20, e2.chartContain.dropInfo.width = 400, e2.chartContain.dropInfo.height = 400, e2.chart.options = { series: Be(id.defaultSeries) }, e2;
}, config: (() => {
  const e2 = Be(Yr);
  return e2.style.payload.component = T(ot(() => import("./MfRadarConfig.js"), { loading: true, size: "large" })), e2.data.payload.component = T(ot(() => import("./MfRadarDataConfig.js"), { loading: true, size: "large" })), e2;
...
function rd() {
  const { createMessage: e2 } = Se(), { deleteFieldValue: t2 } = gs();
  function o2(e3) {
    e3.data.dataSet = Be(id.defaultData);
  }
  return { resetDefault: o2, setDynamicData: function(l2, n2) {
    var _a2, _b, _c2, _d2, _e2, _f2, _g;
...
const sd = Object.freeze(Object.defineProperty({ __proto__: null, default: id, useRadarData: rd }, Symbol.toStringTag, { value: "Module" }));
const ud = { ...id, type: Jr.MfRadarCircle, create: function(e2) {
  const t2 = id.create(e2);
  return t2.chart.options = { ...t2.chart.options, radar: { shape: "circle" } }, e2;
} }, cd = Object.freeze(Object.defineProperty({ __proto__: null, default: ud }, Symbol.toStringTag, { value: "Module" }));
const dd = { ..._c, type: Jr.MfScatter, create: function(e2) {
  return e2.chart.data.dataSet = Be(dd.defaultData), e2.chart.data.headers = dd.defaultHeaders && dd.defaultHeaders(e2.chart), e2.chart.data.limit = 100, e2.chart.data.maxLimit = 1e3, e2.chartContain.dropInfo.width = 400, e2.chartContain.dropInfo.height = 300, e2.chart.options = { grid: { left: "10%", right: "10%", top: "10%", bottom: "10%" }, xAxis: { show: true }, yAxis: { show: true }, series: Be(dd.defaultSeries) }, e2;
}, config: (() => {
  const e2 = Be(Yr);
  return e2.style.payload.component = T(ot(() => import("./MfScatterConfig.js"), { loading: true, size: "large" })), e2.data.payload.component = T(ot(() => import("./MfScatterDataConfig.js"), { loading: true, size: "large" })), e2;
})(), defaultData: { source: [["数据1", "数据2"], [10, 8.04], [8.07, 6.95], [13, 7.58], [9.05, 8.81], [11, 8.33], [14, 7.66], [13.4, 6.81], [10, 6.33], [14, 8.96], [12.5, 6.82], [9.15, 7.2], [11.5, 7.2], [3.03, 4.23], [12.2, 7.83], [2.02, 4.47], [1.05, 3.33], [4.05, 4.96], [6.03, 7.24], [12, 6.26], [12, 8.84], [7.08, 5.82], [5.02, 5.68]] }, defaultSeries: [{ type: "scatter" }] }, pd = Object.freeze(Object.defineProperty({ __proto__: null, default: dd }, Symbol.toStringTag, { value: "Module" })), fd = e({ name: "MfPicture", __name: "index", props: { chart: { type: Object, required: true }, chartContain: { type: Object, required: true } }, setup(e2) {
  const t2 = e2, { commonEvents: o2 } = rs(t2.chart), n2 = l(() => {
    var _a2;
    return ((_a2 = t2.chart.data.dataSet) == null ? void 0 : _a2.key) ? at(it(t2.chart.data.dataSet.key)) : et;
  });
  return (t3, l2) => {
    var _a2;
    return s(), h(d(X), $({ src: n2.value, alt: ((_a2 = e2.chart.data.dataSet) == null ? void 0 : _a2.alt) || "图片" }, e2.chart.options, { width: e2.chartContain.dropInfo.width - 2, height: e2.chartContain.dropInfo.height - 2, fallback: d(et) }, N(d(o2))), null, 16, ["src", "alt", "width", "height", "fallback"]);
  };
} });
function hd(e2) {
  return e2.chart.data.dataSet = Be(gd.defaultData), e2.chart.data.headers = gd.defaultHeaders && gd.defaultHeaders(), e2.chart.data.limit = 1, e2.chart.data.maxLimit = 1, e2.chart.options.preview = false, e2.chartContain.dropInfo.width = 200, e2.chartContain.dropInfo.height = 200, e2;
}
const gd = { type: Jr.MfPicture, component: fd, create: hd, config: (() => {
  const e2 = Be(Yr);
  return e2.data.show = false, e2.style.payload.component = T(ot(() => import("./MfPictureConfig.js"), { loading: true, size: "large" })), e2;
...
} }, yd = Object.freeze(Object.defineProperty({ __proto__: null, default: gd, mfPictureCreate: hd }, Symbol.toStringTag, { value: "Module" }));
function md(e2) {
  return e2.chart.data.dataSet = Be(vd.defaultData), e2.chart.data.headers = vd.defaultHeaders && vd.defaultHeaders(e2.chart), e2.chart.data.limit = 50, e2.chart.data.maxLimit = 1e3, e2.chartContain.dropInfo.width = 600, e2.chartContain.dropInfo.height = 300, e2.chart.options = { indexHeader: "序号", showHeader: true, index: true }, e2;
}
const vd = { type: Jr.MfScrollTable, component: ot(() => import("./index53.js"), { loading: true }), create: md, config: (() => {
  const e2 = Be(Yr);
  return e2.style.payload.component = T(ot(() => import("./MfScrollTableConfig.js"), { loading: true, size: "large" })), e2.data.payload.component = T(ot(() => import("./MfScrollTableDataConfig.js"), { loading: true, size: "large" })), e2;
})(), defaultData: { header: ["列1", "列2", "列3", "列4", "列5"], data: [[1e4, 1e3, 100, 10, 0], [10001, 1001, 101, 11, 1], [10002, 1002, 102, 12, 2], [10003, 1003, 103, 13, 3], [10004, 1004, 104, 14, 4], [10005, 1005, 105, 15, 5], [10006, 1006, 106, 16, 6], [10007, 1007, 107, 17, 7], [10008, 1008, 108, 18, 8], [10009, 1009, 109, 19, 9]] }, defaultHeaders: (e2) => Xr(e2 == null ? void 0 : e2.data.dataSet.header), initData: (e2, t2) => {
...
function Cd() {
  function e2(e3) {
    var _a2;
    ((_a2 = e3 == null ? void 0 : e3.data) == null ? void 0 : _a2.dataSet) && (e3.data.dataSet = Be(vd.defaultData));
  }
  return { resetDefault: e2, setDynamicData: function(t2, o2) {
    var _a2, _b;
    if (!t2) return;
...
const bd = Object.freeze(Object.defineProperty({ __proto__: null, default: vd, mfScrollTableCreate: md, useDataScrollTable: Cd }, Symbol.toStringTag, { value: "Module" }));
function Md(e2) {
  return e2.chart.data.dataSet = Be(kd.defaultData), e2.chart.data.headers = kd.defaultHeaders && kd.defaultHeaders(e2.chart), e2.chart.data.limit = 10, e2.chart.data.maxLimit = 1e3, e2.chartContain.dropInfo.width = 600, e2.chartContain.dropInfo.height = 50, e2;
}
const kd = { type: Jr.MfSegmented, component: ot(() => import("./index54.js"), { loading: true }), create: Md, config: (() => {
  const e2 = Be(Yr);
  return e2.style.payload.component = T(ot(() => import("./MfSegmentedStyleConfig.js"), { loading: true, size: "large" })), e2.data.payload.component = T(ot(() => import("./MfSegmentedDataConfig.js"), { loading: true, size: "large" })), e2;
  ...
  const { setDynamicData: o2 } = wd();
  o2(e2, t2);
}, customEvents: [{ name: "标签切换", value: "titleChange" }] };
function wd() {
  function e2(e3) {
    var _a2;
    ((_a2 = e3 == null ? void 0 : e3.data) == null ? void 0 : _a2.dataSet) && (e3.data.dataSet = Be(kd.defaultData));
  }
  return { resetDefault: e2, setDynamicData: function(t2, o2) {
    var _a2, _b, _c2, _d2;
    if (t2) {
...
}, $d = Object.assign({ "./MfCombine/mfCombine.data.ts": ts, "./MfDataTag/mfDataTag.data.ts": hs, "./MfDataUi/MfDigits/mfDigits.data.ts": Cs, "./MfDataUi/MfTireMarks/mfTireMarks.data.ts": _s, "./MfDataUi/MfWheel/mfWheel.data.ts": ws, "./MfDataV/MfBorder1/mfBorder1.data.ts": Is, "./MfDataV/MfBorder10/mfBorder10.data.ts": Ts, "./MfDataV/MfBorder11/mfBorder11.data.ts": Rs, "./MfDataV/MfBorder12/mfBorder12.data.ts": As, "./MfDataV/MfBorder13/mfBorder13.data.ts": js, "./MfDataV/MfBorder14/mfBorder14.data.ts": Ps, "./MfDataV/MfBorder15/mfBorder15.data.ts": Hs, "./MfDataV/MfBorder16/mfBorder16.data.ts": Gs, "./MfDataV/MfBorder17/mfBorder17.data.ts": Vs, "./MfDataV/MfBorder18/mfBorder18.data.ts": Zs, "./MfDataV/MfBorder19/mfBorder19.data.ts": Ks, "./MfDataV/MfBorder2/mfBorder2.data.ts": Js, "./MfDataV/MfBorder20/mfBorder20.data.ts": tu, "./MfDataV/MfBorder21/mfBorder21.data.ts": lu, "./MfDataV/MfBorder22/mfBorder22.data.ts": au, "./MfDataV/MfBorder3/mfBorder3.data.ts": ru, "./MfDataV/MfBorder4/mfBorder4.data.ts": uu, "./MfDataV/MfBorder5/mfBorder5.data.ts": du, "./MfDataV/MfBorder6/mfBorder6.data.ts": fu, "./MfDataV/MfBorder7/mfBorder7.data.ts": gu, "./MfDataV/MfBorder8/mfBorder8.data.ts": vu, "./MfDataV/MfBorder9/mfBorder9.data.ts": bu, "./MfDataV/MfDecoration1/mfDecoration.data.ts": wu, "./MfDataV/MfDecoration10/mfDecoration.data.ts": _u, "./MfDataV/MfDecoration11/mfDecoration.data.ts": Iu, "./MfDataV/MfDecoration12/mfDecoration.data.ts": Nu, "./MfDataV/MfDecoration2/mfDecoration.data.ts": qu, "./MfDataV/MfDecoration3/mfDecoration.data.ts": ju, "./MfDataV/MfDecoration4/mfDecoration.data.ts": Pu, "./MfDataV/MfDecoration5/mfDecoration.data.ts": Qu, "./MfDataV/MfDecoration6/mfDecoration.data.ts": Vu, "./MfDataV/MfDecoration7/mfDecoration.data.ts": Yu, "./MfDataV/MfDecoration8/mfDecoration.data.ts": Ju, "./MfDataV/MfDecoration9/mfDecoration.data.ts": oc, "./MfDataV/MfHeader1/mfHeader1.data.ts": ac, "./MfDataV/MfHeader2/mfHeader2.data.ts": sc, "./MfDataV/MfHeader3/mfHeader3.data.ts": dc, "./MfDataV/MfHeader4/mfHeader4.data.ts": hc, "./MfDataV/MfHeader5/mfHeader5.data.ts": mc, "./MfDataV/MfHeader6/mfHeader6.data.ts": bc, "./MfDateTime/mfDateTime.data.ts": xc, "./MfEcharts/MfBar/mfBar.data.ts": $c, "./MfEcharts/MfBarHorizontal/mfBarHorizontal.data.ts": Ic, "./MfEcharts/MfBarLine/mfBarLine.data.ts": Bc, "./MfEcharts/MfBarPlusMinus/mfBarPlusMinus.data.ts": qc, "./MfEcharts/MfBarStack/mfBarStack.data.ts": Tc, "./MfEcharts/MfLine/mfLine.data.ts": Oc, "./MfEcharts/MfLineArea/mfLineArea.data.ts": zc, "./MfEcharts/MfLineAreaStack/mfLineAreaStack.data.ts": Pc, "./MfEcharts/MfLineSmooth/mfLineSmooth.data.ts": Hc, "./MfEcharts/MfLineStack/mfLineStack.data.ts": Gc, "./MfEcharts/MfMapLine/mfMapLine.data.ts": Zc, "./MfEcharts/MfMapLine3D/mfMapLine3D.data.ts": Kc, "./MfEcharts/MfPie/mfPie.data.ts": Jc, "./MfEcharts/MfPieCircular/mfPieCircular.data.ts": td, "./MfEcharts/MfPieHalf/mfPieHalf.data.ts": ld, "./MfEcharts/MfPieRose/mfPieRose.data.ts": ad, "./MfEcharts/MfRadar/mfRadar.data.ts": sd, "./MfEcharts/MfRadarCircle/mfRadarCircle.data.ts": cd, "./MfEcharts/MfScatter/mfScatter.data.ts": pd, "./MfPicture/mfPicture.data.ts": yd, "./MfScrollTable/mfScrollTable.data.ts": bd, "./MfSegmented/mfSegmented.data.ts": xd, "./MfTag/mfTag.data.ts": ds });
const Ld = { ...function() {
  const e2 = {};
  return Object.keys($d).forEach((t2) => {
    const o2 = $d[t2].default;
    e2[o2.type] = o2;
  }), e2;
}() }, Id = tt();
function Dd(e2) {
  let t2, o2;
  return t2 = Tr().getTheme === He.DARK ? "#ffffff" : "#000000", o2 = e2.backgroundImage ? `url(${at(it(e2.backgroundImage))}) 0% 0% / 100% 100% no-repeat` : e2.backgroundColor ?? "", { width: `${e2.width}px`, height: `${e2.height}px`, background: o2, color: t2 };
}
const Td = () => {
  const e2 = Ge();
  return { chartContain: { ...Be(Hr), id: e2 }, chart: { ...Be(Er), id: e2 } };
};
function Nd(e2, t2, o2) {
  const l2 = Be(e2);
  if (!l2.chart.type) return;
  l2.chart.type === Jr.MfCombine && l2.chart.options.components.forEach((e3) => {
    const t3 = Ge();
    e3.chart.id = t3, e3.chartContain.id = t3;
  });
  const n2 = Ge();
  return e2.chartContain.dropInfo.x = t2, e2.chartContain.dropInfo.y = o2, l2.chart.name = Bd(l2.chart.type, l2.chart.name), l2.chart.id = n2, l2.chartContain.id = n2, l2.chartContain.dropInfo.x = t2, l2.chartContain.dropInfo.y = o2, l2.chartContain.dropInfo.zIndex = 0, l2;
}
function Bd(e2, t2) {
  const o2 = Tr().getChartNameLabel;
  let l2 = t2 || e2;
...
const Rd = (e2) => {
  const t2 = Td();
  t2.chart.type = e2.type, t2.chart.name = Bd(e2.type, e2.name);
  const o2 = Ld[e2.type];
...
function qd(e2, t2 = []) {
  var _a2;
  let o2;
  return o2 = ((_a2 = e2.chartContain.border.color) == null ? void 0 : _a2.startsWith("linear-gradient")) ? { [Fr.borderImage]: `${e2.chartContain.border.color} 1` } : { [Fr.borderColor]: e2.chartContain.border.color }, Ad({ ...e2.chartContain.dropInfo, ...o2, [Fr.borderWidth]: e2.chartContain.border.show ? e2.chartContain.border.width : 0, [Fr.borderRadius]: e2.chartContain.border.radius, [Fr.background]: e2.chartContain.background, [Fr.opacity]: e2.chartContain.opacity, padding: e2.chartContain.padding }, t2);
}
function Ad(e2, t2 = []) {
  const o2 = /* @__PURE__ */ new Set([Fr.width, Fr.height, Fr.borderWidth, Fr.borderRadius]), l2 = new Set(t2), n2 = [];
  l2.has(Fr.x) || l2.has(Fr.y) || (n2.push(`translate(${e2.x}px,${e2.y}px)`), l2.add(Fr.x), l2.add(Fr.y)), l2.has(Fr.rotate) || (n2.push(`rotate(${e2.rotate}deg)`), l2.add(Fr.rotate));
  const a2 = {};
...
function Od(e2) {
  const t2 = { ...e2 };
  if (0 !== t2.rotate) {
    const e3 = Math.round(t2.width * Wr(t2.rotate) + t2.height * Vr(t2.rotate)), o2 = (t2.width - e3) / 2;
    t2.x += Math.round(o2), t2.right = t2.x + e3;
    const l2 = Math.round(t2.height * Wr(t2.rotate) + t2.width * Vr(t2.rotate)), n2 = (l2 - t2.height) / 2;
...
async function zd(e2, t2) {
  const o2 = (t3) => {
    ...
    if (o3) {
      const l3 = {}, n3 = (t4, o4) => {
        if (!o4) return;
        if (Ze(o4)) return void (l3[t4] = o4);
        const n4 = jd(e2, o4.id);
        if (!n4) return;
        const a3 = 0 === n4.chart.data.type ? n4.chart.data.dataSet : n4.chart.data.result;
        Ve(a3) ? l3[t4] = a3[0][o4.param] : a3 && (l3[t4] = a3[o4.param]);
      };
      for (const e3 of Object.keys(o3)) Ve(o3[e3].value) ? o3[e3].value.forEach((t4) => {
        n3(e3, t4);
      }) : n3(e3, o3[e3].value);
      t3.data.paramsValue ? t3.data.paramsValue = { ...t3.data.paramsValue, ...l3 } : t3.data.paramsValue = l3;
    }
  }, { getDataTable: l2 } = os(), n2 = [], a2 = [], i2 = (e3) => {
    0 === e3.chart.data.type ? n2.push(e3.chart) : a2.push(e3.chart);
  };
  for (const o3 of e2) if (t2 && (o3.chart.isResource = true), i2(o3), o3.chart.type === Jr.MfCombine) for (const e3 of o3.chart.options.components) t2 && (e3.chart.isResource = true), i2(e3);
  ...
  for (const e3 of a2) e3.data.priority > s2 && (r2.length > 0 && (await Promise.all(r2), r2 = []), s2 = e3.data.priority), o2(e3), r2.push(l2(e3).then((t3) => {
    var _a2;
    e3.type && ((_a2 = Ld[e3.type]) == null ? void 0 : _a2.initData(e3, t3));
  }));
  return r2;
}
function Fd(e2, t2) {
  const o2 = (e3) => {
    delete e3.data.headers, delete e3.data.result, 1 === e3.data.type && e3.data.id && delete e3.data.dataSet, delete e3.data.paramsValue;
  }, l2 = Be(e2);
  Pd(l2);
  const n2 = { canvasConfig: JSON.stringify(t2), contains: [], layers: {} };
  for (const e3 of l2) {
    if (o2(e3.chart), e3.chart.type === Jr.MfCombine) for (const t3 of e3.chart.options.components) o2(t3.chart);
...
function Pd(e2) {
  if (!Tr().getIsResource) return;
  const t2 = (e3) => {
    delete e3.isResource, 0 === e3.data.type && (e3.data.id = "");
  };
  for (const o2 of e2) if (t2(o2.chart), o2.chart.type === Jr.MfCombine) for (const e3 of o2.chart.options.components) t2(e3.chart);
}
function Ed(e2) {
  return 0 === e2.data.type ? e2.data.dataSet : e2.data.result;
}
const Hd = Tr(), Qd = (e2) => e2 <= 0, Gd = (e2, t2) => e2 >= t2.length - 1, Ud = (e2, t2, o2) => {
  const l2 = e2[t2];
  e2[t2] = e2[o2], e2[o2] = l2;
}, Vd = Xe("screen-shortcut", { state: () => ({ showMenu: false, copyData: null, alignType: tp.GROUP }), getters: { getShowMenu() {
  ...
  var _a2, _b;
  ((_a2 = Hd.getSelectArea) == null ? void 0 : _a2.components.length) > 0 ? this.copyData = Be((_b = Hd.getSelectArea) == null ? void 0 : _b.components) : Hd.getCurComponent && (this.copyData = Be(Hd.getCurComponent));
}, pasteComponent() {
  if (this.copyData) {
    if (Ve(this.copyData)) this.copyData.forEach((e2) => {
      const t2 = Nd(e2, e2.chartContain.dropInfo.x + 10, e2.chartContain.dropInfo.y + 10);
      t2 && Hd.addComponent(t2, 0);
    });
    else {
      const e2 = this.copyData.chartContain.dropInfo.x + 10, t2 = this.copyData.chartContain.dropInfo.y + 10, o2 = Nd(this.copyData, e2, t2);
      o2 && (Hd.addComponent(o2, 0), Hd.setCurComponent(o2, 0));
    }
    Hd.setUndoRedoData("组件复制");
  }
}, moveComponent(e2, t2) {
  Hd.getCurComponent && ("x" === e2 && (Hd.getCurComponent.chartContain.dropInfo.x = Hd.getCurComponent.chartContain.dropInfo.x + t2), "y" === e2 && (Hd.getCurComponent.chartContain.dropInfo.y = Hd.getCurComponent.chartContain.dropInfo.y + t2), t2 >= 10 && Hd.setUndoRedoData("组件移动"));
}, upComponent() {
  Qd(Hd.getCurComponentIndex) || (Ud(Hd.getComponentList, Hd.getCurComponentIndex, Hd.getCurComponentIndex - 1), Hd.setCurComponentIndex(Hd.getCurComponentIndex - 1), Hd.setUndoRedoData("组件上移一层"));
}, downComponent() {
  Gd(Hd.getCurComponentIndex, Hd.getComponentList) || (Ud(Hd.getComponentList, Hd.getCurComponentIndex, Hd.getCurComponentIndex + 1), Hd.setCurComponentIndex(Hd.getCurComponentIndex + 1), Hd.setUndoRedoData("组件下移一层"));
}, topComponent() {
  Hd.getCurComponent && !Qd(Hd.getCurComponentIndex) && (Hd.getComponentList.splice(Hd.getCurComponentIndex, 1), Hd.getComponentList.unshift(Hd.getCurComponent), Hd.setCurComponentIndex(0), Hd.setUndoRedoData("组件置顶"));
}, bottomComponent() {
  Hd.getCurComponent && !Gd(Hd.getCurComponentIndex, Hd.getComponentList) && (Hd.getComponentList.splice(Hd.getCurComponentIndex, 1), Hd.getComponentList.push(Hd.getCurComponent), Hd.setCurComponentIndex(Hd.getComponentList.length - 1), Hd.setUndoRedoData("组件置底"));
}, composeComponent() {
  if (Hd.getSelectArea.components.length <= 1) return;
  const e2 = [];
  Hd.getSelectArea.components.forEach((t2) => {
    var _a2;
    if (t2.chart.type === Jr.MfCombine) {
      const o2 = { ...t2.chartContain.dropInfo }, l2 = t2.chart.options.components, n2 = (_a2 = Hd.getScreenCanvas) == null ? void 0 : _a2.getBoundingClientRect();
      l2.forEach((e3) => {
        Yd(e3, n2, o2, Hd.getScale);
      }), e2.push(...t2.chart.options.components);
    } else e2.push(t2);
  }), C(() => {
    const t2 = Rd({ type: Jr.MfCombine, name: "组合" });
    null !== t2 && (t2.chartContain.dropInfo = Object.assign(t2.chartContain.dropInfo, Hd.getSelectArea.dropInfo), t2.chart.options.components = e2, Zd(t2), Hd.batchDeleteComponent(Hd.getSelectArea.components), Hd.addComponent(t2, 0), Hd.cleanSelectArea(), Hd.setCurComponent(t2, 0), Hd.setUndoRedoData("组件组合"));
  });
}, decomposeComponent() {
  if (!Hd.getCurComponent) return;
  if (Hd.getCurComponent.chart.type !== Jr.MfCombine) return;
  Hd.getCurComponent.chartContain.show || (Hd.getCurComponent.chartContain.show = true);
  const e2 = Hd.getCurComponent.chart.options.components, t2 = [];
  for (const o2 of e2) o2.chartContain.show || (o2.chartContain.show = true, t2.push(o2));
  C(() => {
    const o2 = { ...Hd.getCurComponent.chartContain.dropInfo };
    if (!Hd.getScreenCanvas) return;
    const l2 = Hd.getScreenCanvas.getBoundingClientRect(), n2 = Hd.getCurComponentIndex;
    Hd.deleteComponent(n2);
    for (let t3 = e2.length - 1; t3 >= 0; t3--) Yd(e2[t3], l2, o2, Hd.getScale), Hd.addComponent(e2[t3], n2), Hd.getSelectArea.components.push(e2[t3]);
    t2.forEach((e3) => {
      e3.chartContain.show = false;
    }), Jd(Hd.getSelectArea.components), Hd.setUndoRedoData("组件拆分");
  });
}, lockComponent() {
  ...
  var _a2, _b;
  ((_a2 = Hd.getSelectArea) == null ? void 0 : _a2.components.length) > 0 && ((_b = Hd.getSelectArea) == null ? void 0 : _b.components.forEach((t2) => {
    t2.chartContain.lock = e2;
  })), Hd.getCurComponent && (Hd.getCurComponent.chartContain.lock = e2), Hd.setUndoRedoData(e2 ? "组件锁定" : "组件解锁");
}, hideComponent() {
  ...
  })), Hd.getCurComponent && (Hd.getCurComponent.chartContain.show = e2), Hd.setUndoRedoData(e2 ? "组件显示" : "组件隐藏");
}, deleteComponent() {
  Hd.deleteComponent(), Hd.setUndoRedoData("组件删除");
}, selectAllComponent() {
  Jd(Hd.getComponentList);
}, screenSave() {
  Id.emit(zr.SCREEN_SAVE);
}, screenPreview() {
  Id.emit(zr.SCREEN_PREVIEW);
} } });
function Wd(e2) {
  return 100 * e2 + "%";
}
function Zd(e2) {
  ...
  e2.chart.options.components.forEach((e3) => {
    e3.chartContain.groupStyle = qd(e3, [Fr.x, Fr.y]), e3.chartContain.groupStyle.left = Wd((e3.chartContain.dropInfo.x - t2.x) / t2.width), e3.chartContain.groupStyle.top = Wd((e3.chartContain.dropInfo.y - t2.y) / t2.height), e3.chartContain.groupStyle.width = Wd(e3.chartContain.dropInfo.width / t2.width), e3.chartContain.groupStyle.height = Wd(e3.chartContain.dropInfo.height / t2.height);
  });
}
function Yd(e2, t2, o2, l2) {
  ...
  e2.chartContain.dropInfo.rotate = Zr(e2.chartContain.dropInfo.rotate + o2.rotate), e2.chartContain.dropInfo.width = Math.abs(Math.round(Number.parseFloat(e2.chartContain.groupStyle.width) / 100 * o2.width)), e2.chartContain.dropInfo.height = Math.abs(Math.round(Number.parseFloat(e2.chartContain.groupStyle.height) / 100 * o2.height)), e2.chartContain.dropInfo.x = Math.abs(Math.round((i2 - e2.chartContain.dropInfo.width * l2 / 2) / l2)), e2.chartContain.dropInfo.y = Math.abs(Math.round((r2 - e2.chartContain.dropInfo.height * l2 / 2) / l2)), e2.chartContain.groupStyle = {};
}
const Kd = Tr();
function Xd(e2) {
  const t2 = [], { x: o2, y: l2 } = e2;
  return Kd.getComponentList.forEach((n2) => {
    const { x: a2, y: i2, width: r2, height: s2 } = Od(n2.chartContain.dropInfo);
    o2 <= a2 && l2 <= i2 && a2 + r2 <= o2 + e2.width && i2 + s2 <= l2 + e2.height && t2.push(n2);
  }), t2;
}
function Jd(e2) {
  if (!e2 || (e2 == null ? void 0 : e2.length) < 1) return void Kd.cleanSelectArea();
  let [t2, o2, l2, n2] = [1 / 0, 1 / 0, -1 / 0, -1 / 0];
  e2.forEach((e3) => {
    const a3 = Od(e3.chartContain.dropInfo);
    a3.x < o2 && (o2 = a3.x), a3.y < t2 && (t2 = a3.y), a3.right > l2 && (l2 = a3.right), a3.bottom > n2 && (n2 = a3.bottom);
  });
  const a2 = { x: o2, y: t2, width: l2 - o2, height: n2 - t2, rotate: 0 };
  Kd.setSelectArea({ show: false, dropInfo: a2, components: e2 }), 1 === e2.length && Kd.setCurComponent(e2[0]);
}
function ep(e2) {
  if (Kd.getCurComponent) {
    -1 === Kd.getSelectArea.components.findIndex((e3) => e3.chart.id === Kd.getCurComponent.chart.id) && Kd.getSelectArea.components.push(Kd.getCurComponent);
  }
  Kd.cleanCurComponent(), Kd.setSelectedComponents(e2), Kd.getSelectArea.components.length > 1 && Jd(Kd.getSelectArea.components);
}
var tp = ((e2) => (e2.GROUP = "Group", e2.CANVAS = "Canvas", e2))(tp || {});
const op = (e2, t2) => {
  function o2(o3) {
    const a3 = "x" === o3 ? "width" : "height", i3 = (e3) => {
      const t3 = Od(e3.chartContain.dropInfo);
    ...
    }), Jd(e2.getSelectArea.components), e2.setUndoRedoData("x" === o3 ? "横向分布" : "纵向分布");
  }
  function l2() {
  ...
  function a2(t3) {
    const o3 = l2();
    n2().forEach((e3) => t3(e3, o3)), Jd(e2.getSelectArea.components), e2.setUndoRedoData("对齐");
  }
  function i2() {
  ...
  const o2 = 150;
  return { KeysEnum: { ArrowUp: Pe(() => t2.moveComponent("y", -1), o2), ArrowDown: Pe(() => t2.moveComponent("y", 1), o2), ArrowLeft: Pe(() => t2.moveComponent("x", -1), o2), ArrowRight: Pe(() => t2.moveComponent("x", 1), o2), Delete: Re(() => t2.deleteComponent(), o2) }, CtrlKeysEnum: { c: Re(() => t2.copyComponent(), o2), v: Re(() => t2.pasteComponent(), o2), ArrowUp: Pe(() => t2.moveComponent("y", -10), o2), ArrowDown: Pe(() => t2.moveComponent("y", 10), o2), ArrowLeft: Pe(() => t2.moveComponent("x", -10), o2), ArrowRight: Pe(() => t2.moveComponent("x", 10), o2), g: Re(() => t2.composeComponent(), o2), l: Re(() => t2.lockComponent(), o2), h: Re(() => t2.hideComponent(), o2), a: Re(() => t2.selectAllComponent(), o2), p: Re(() => t2.screenPreview(), o2), s: Re(() => t2.screenSave(), o2), z: Re(() => e2.undo(), o2) }, ShiftKeysEnum: { ArrowUp: Pe(() => t2.upComponent(), o2), ArrowDown: Pe(() => t2.downComponent(), o2) }, CtrlShiftKeysEnum: { ArrowUp: Re(() => t2.topComponent(), o2), ArrowDown: Re(() => t2.bottomComponent(), o2), g: Re(() => t2.decomposeComponent(), o2), l: Re(() => t2.unLockComponent(), o2), h: Re(() => t2.showComponent(), o2), z: Re(() => e2.redo(), o2) }, NoneShortcutKeyEnum: op(e2, t2) };
}, np = { class: "title" }, ap = Yt(e({ __name: "ConfigWrapper", props: { iconfont: { type: String, default: void 0 }, icon: { type: String, default: void 0 }, title: { type: String, default: "" }, allowDelete: { type: Boolean, default: true }, full: { type: Boolean, default: false }, highlight: { type: Boolean, default: false }, queryMode: { type: Boolean, default: false } }, emits: ["deleteConfig", "execute"], setup(e2, { emit: t2 }) {
  const o2 = t2, l2 = xe().getThemeColor, { prefixCls: n2 } = we("config-wrapper"), a2 = Ge(), i2 = Mr(), m2 = y(false);
  function v2(e3) {
    o2("deleteConfig", e3);
  }
  function C2() {
  ...
  return (t3, o3) => (s(), r("div", { class: c([d(n2), d(i2).getShowData && d(i2).getShowDataLevel === d(a2) ? "active" : ""]), style: u(e2.highlight ? { border: `1px solid ${d(l2)}` } : "") }, [d(i2).getShowData && d(i2).getShowDataLevel === d(a2) ? (s(), r("div", { key: 0, class: "select", style: u({ backgroundColor: d(l2) }) }, [g(d(Te), { icon: "ant-design:check-outlined", color: "#FFFFFF", size: 14 })], 4)) : f("", true), void 0 !== e2.iconfont ? (s(), h(d(Te), { key: 1, color: d(l2), icon: `iconfont:${e2.iconfont}`, size: 14 }, null, 8, ["color", "icon"])) : f("", true), void 0 !== e2.icon ? (s(), h(d(Te), { key: 2, color: d(l2), icon: e2.icon, size: 14 }, null, 8, ["color", "icon"])) : f("", true), p("div", np, S(e2.title), 1), e2.allowDelete && !e2.queryMode ? (s(), h(d(J), { key: 3, type: "vertical" })) : f("", true), e2.queryMode ? f("", true) : (s(), h(d(G), { key: 4, title: "删除" }, { default: M(() => [e2.allowDelete ? (s(), h(d(ee), { key: 0, size: "small", type: "link", onClick: v2 }, { icon: M(() => [g(d(st))]), _: 1 })) : f("", true)]), _: 1 })), p("div", { class: c(["config", e2.full ? "full" : ""]) }, [B(t3.$slots, "config", {}, void 0, true)], 2), g(d(G), { title: "执行" }, { default: M(() => [g(d(ee), { size: "small", type: "link", style: { margin: "6px" }, onMouseenter: o3[0] || (o3[0] = (e3) => m2.value = true), onMouseleave: o3[1] || (o3[1] = (e3) => m2.value = false), onClick: C2 }, { icon: M(() => [!d(i2).getShowData || d(i2).getShowDataLevel !== d(a2) || m2.value ? (s(), h(d(ut), { key: 0 })) : (s(), h(d(ct), { key: 1 }))]), _: 1 })]), _: 1 })], 6));
} }), [["__scopeId", "data-v-c5dcf0b5"]]), ip = Yt(e({ __name: "SelectFields", props: { tableFields: { type: Object, require: true, default: void 0 }, fields: { type: Object, require: true, default: void 0 }, queryMode: { type: Boolean, default: false } }, emits: ["fieldChange"], setup(e2, { emit: t2 }) {
  const o2 = e2, a2 = t2, i2 = y([]), p2 = y(false), { prefixCls: f2 } = we("select-fields"), C2 = l(() => {
    var _a2, _b;
  ...
  let k2 = false;
  const w2 = (e3) => {
    b2.value = e3, !e3 && k2 && (a2("fieldChange", i2.value.filter((e4) => e4.checked).map((e4) => ({ colName: e4.colName, fieldName: e4.fieldName, comment: e4.comment, expression: e4.expression, dataType: e4.dataType, tableAlias: e4.tableAlias, targetType: e4.targetType }))), k2 = false);
  }, _2 = (e3) => {
  ...
  return (t3, o3) => (s(), h(d(V), { trigger: ["click"], onOpenChange: w2, placement: "bottom", arrow: { pointAtCenter: true } }, { overlay: M(() => [g(d(te), null, { default: M(() => [g(d(te).Item, { disabled: "", style: { padding: "0", cursor: "default" } }, { default: M(() => [(s(), r("div", { class: c(`${d(f2)}-check-wrapper`), key: `all_check_${p2.value}` }, [g(d(W), { checked: p2.value, onChange: _2, disabled: e2.queryMode }, { default: M(() => o3[0] || (o3[0] = [x("全选")])), _: 1 }, 8, ["checked", "disabled"])], 2)), g(d(J), { style: { margin: "4px 0" } }), g(d(ft), { style: u({ height: `${C2.value}px` }) }, { default: M(() => [(s(true), r(m, null, v(i2.value, (t4) => (s(), r("div", { key: t4.colName + t4.checked, class: c(`${d(f2)}-check-wrapper`) }, [g(d(W), { checked: t4.checked, id: t4.colName, onChange: $2, disabled: e2.queryMode }, { default: M(() => [x(S(t4.colName + (t4.comment ? `[${t4.comment}]` : "")), 1)]), _: 2 }, 1032, ["checked", "id", "disabled"])], 2))), 128))]), _: 1 }, 8, ["style"])]), _: 1 })]), _: 1 })]), default: M(() => [g(d(ee), { type: "primary" }, { default: M(() => [o3[1] || (o3[1] = x(" 字段 ")), g(d(Te), { icon: b2.value ? "ant-design:caret-up-outlined" : "ant-design:caret-down-outlined" }, null, 8, ["icon"])]), _: 1 })]), _: 1 }));
} }), [["__scopeId", "data-v-88afb51f"]]), rp = Yt(e({ __name: "TableFields", props: { sqlQuery: { type: Object, require: true, default: void 0 }, configMitt: { type: Object, default: void 0 }, queryMode: { type: Boolean, default: false } }, emits: ["fieldChange"], setup(e2, { emit: t2 }) {
  const o2 = e2, l2 = t2, a2 = H, r2 = H.Group, { prefixCls: u2 } = we("table-fields"), p2 = y([]), f2 = Mr();
  function m2(e3) {
    Lr(o2.configMitt, { ...o2.sqlQuery, aggregates: [], groups: [], joins: [], customColumns: [], filters: [], orders: [], limit: void 0 }, e3);
  }
  function v2(e3) {
    l2("fieldChange", e3);
  }
  return n(() => {
    ...
    e3 && (e3 === f2.getTableName ? (f2.addTableFieldsMap(e3, f2.getTableFields), p2.value = f2.getTableFields) : Sr(o2.sqlQuery).then((t3) => {
      f2.addTableFieldsMap(e3, t3), p2.value = t3;
    }));
  }, { immediate: true }), i(() => {
    var _a2, _b;
    ((_a2 = o2.sqlQuery) == null ? void 0 : _a2.sqlQuery) && ((_b = o2.sqlQuery) == null ? void 0 : _b.sourceTable) && Sr(o2.sqlQuery).then((e3) => {
  ...
  }), (t3, l3) => (s(), h(ap, { iconfont: "icon-mfish-table", title: "数据源", "allow-delete": false, onExecute: m2, "query-mode": e2.queryMode }, { config: M(() => [g(d(r2), { class: c(d(u2)), compact: "" }, { default: M(() => {
    var _a2, _b;
    return [g(d(a2), { value: (_a2 = e2.sqlQuery) == null ? void 0 : _a2.sourceTable, disabled: "" }, null, 8, ["value"]), g(ip, $(t3.$attrs, { "table-fields": p2.value, fields: (_b = o2.sqlQuery) == null ? void 0 : _b.fields, onFieldChange: v2, "query-mode": e2.queryMode }), null, 16, ["table-fields", "fields", "query-mode"])];
  }), _: 1 }, 8, ["class"])]), _: 1 }, 8, ["query-mode"]));
} }), [["__scopeId", "data-v-5708821c"]]), sp = async (e2, t2) => {
  var _a2, _b;
  const o2 = Mr(), l2 = [];
  ...
  for (; ; ) return o2.has(t2) ? (t2.endsWith(`_${e2}`) && (t2 = t2.slice(0, Math.max(0, t2.length - `${e2}`.length - 1))), up(++e2, `${t2}_${e2}`, o2)) : t2;
}, cp = (e2) => {
  var _a2, _b, _c2, _d2;
  ...
  e2 && e2.push(t2);
}, pp = Yt(e({ __name: "ConfigButton", props: { level: { type: Number, default: 1 }, icon: { type: String, default: void 0 }, iconfont: { type: String, default: void 0 }, title: { type: String, default: "按钮" } }, setup(e2) {
  const t2 = e2, { prefixCls: o2 } = we("config-button"), n2 = xe().getThemeColor, a2 = Mr(), i2 = l(() => t2.level < a2.getLevel ? 12 : 14);
  return (t3, l2) => (s(), h(d(ee), $(t3.$attrs, { size: "small", shape: "round", class: d(o2), style: { "--hover-color--": d(n2) }, type: "link" }), { icon: M(() => [e2.iconfont ? (s(), h(d(Te), { key: 0, icon: `iconfont:${e2.iconfont}`, style: { "margin-right": "8px" }, size: i2.value }, null, 8, ["icon", "size"])) : (s(), h(d(Te), { key: 1, icon: e2.icon, size: i2.value }, null, 8, ["icon", "size"]))]), default: M(() => [p("span", { class: c(e2.level < d(a2).getLevel ? "show-title" : ""), style: u({ fontSize: `${i2.value}px` }) }, S(e2.title), 7)]), _: 1 }, 16, ["class", "style"]));
} }), [["__scopeId", "data-v-749209b7"]]), fp = { key: 0, class: "placeholder" }, hp = { class: "condition" }, gp = e({ __name: "TableJoins", props: { sqlQuery: { type: Object, require: true }, level: { type: Number, default: 1 }, configMitt: { type: Object }, queryMode: { type: Boolean, default: false } }, emits: ["joinChange"], setup(e2, { emit: t2 }) {
  const o2 = e2, a2 = t2, i2 = te.Item, C2 = H, { prefixCls: _2 } = we("table-joins"), $2 = Mr(), L2 = y([]), I2 = y([]), D2 = y([]), T2 = xe().getThemeColor, N2 = { left: "左联", inner: "内联", right: "右联", full: "全联" }, B2 = l(() => {
    var _a2;
    if (1 === $2.getSourceType || o2.queryMode) return false;
    if (o2.sqlQuery && ((_a2 = o2.sqlQuery) == null ? void 0 : _a2.joins)) {
      for (const e3 of o2.sqlQuery.joins) if (!cp(e3)) return false;
    }
    return $r(o2.sqlQuery, o2.level);
  });
  n(() => {
    var _a2;
  ...
  const R2 = l(() => {
    ...
    return [];
  }), q2 = (e3) => I2.value.length > e3 ? I2.value[e3] : [], A2 = (e3) => {
    var _a2, _b;
    ...
    return [];
  }, j2 = (e3, t3) => q2(e3).find((e4) => e4.value === t3[0]) ? t3[1] : "", z2 = (e3) => D2.value.length > e3 ? b(D2.value[e3]) : [], F2 = (e3) => {
    var _a2, _b, _c2, _d2;
    if (!((_a2 = o2.sqlQuery) == null ? void 0 : _a2.joins)) return [];
  ...
  function Q2(e3, t3) {
    ...
    const n2 = new Set(l2), a3 = up(0, t3, n2);
  ...
  function U2() {
    ...
    const e3 = async (e4) => {
      ...
      const t4 = await sp(e4, o2.sqlQuery);
      ...
      if (t4) if (t4.table) {
        ...
        o3 && (D2 == null ? void 0 : D2.value.splice(e4, 1, o3));
      } else D2 == null ? void 0 : D2.value.splice(e4, 1, []);
    };
  ...
  return (t3, l2) => {
    ...
    return s(), r("div", { class: c(d(_2)) }, [(s(true), r(m, null, v((_a2 = e2.sqlQuery) == null ? void 0 : _a2.joins, (t4, c2) => (s(), h(ap, { iconfont: "icon-mfish-join", title: "关联", key: c2, "query-mode": e2.queryMode, onDeleteConfig: (e3) => function(e4) {
      L2 == null ? void 0 : L2.value.splice(e4, 1), a2("joinChange", [...L2.value]);
    }(c2), onExecute: (e3) => function(e4, t5) {
      Lr(o2.configMitt, { ...o2.sqlQuery, joins: L2.value.slice(0, t5 + 1), aggregates: [], groups: [], customColumns: [], filters: [], orders: [], limit: void 0 }, e4);
    }(e3, c2) }, { config: M(() => {
      var _a3, _b, _c2;
      return [0 === c2 ? (s(), h(d(C2), { key: 0, class: "left-table", value: (_a3 = e2.sqlQuery) == null ? void 0 : _a3.sourceTable, disabled: "" }, null, 8, ["value"])) : (s(), h(d(C2), { key: 1, class: "left-table", value: (_b = t4.condition) == null ? void 0 : _b.field.tableAlias, disabled: "" }, null, 8, ["value"])), g(d(V), { class: "condition", placement: "bottom", arrow: { pointAtCenter: true }, disabled: e2.queryMode }, { overlay: M(() => [g(d(te), { onClick: (e3) => function(e4, t5) {
        t5.type = e4.key;
      }(e3, t4), "selected-keys": [t4.type] }, { default: M(() => [(s(true), r(m, null, v(Object.keys(N2), (e3) => (s(), h(d(i2), { key: e3 }, { default: M(() => [p("div", null, [x(S(N2[e3]) + " ", 1), g(d(Te), { icon: `iconfont:icon-mfish-${e3}-join`, color: d(T2), size: 14 }, null, 8, ["icon", "color"])])]), _: 2 }, 1024))), 128))]), _: 2 }, 1032, ["onClick", "selected-keys"])]), default: M(() => [p("a", { onClick: l2[0] || (l2[0] = k(() => {
      }, ["prevent"])) }, [x(S(N2[t4.type]) + " ", 1), g(d(Te), { icon: `iconfont:icon-mfish-${t4.type}-join`, color: d(T2), size: 14 }, null, 8, ["icon", "color"])])]), _: 2 }, 1032, ["disabled"]), g(n2, { class: "right-table", compact: "" }, { default: M(() => {
        return [g(d(oe), { disabled: e2.queryMode, "show-search": "", "allow-clear": false, class: "input-select", "option-filter-prop": "label", value: [A2(c2) || ""], options: R2.value, onChange: (e3) => function(e4, t5) {
          ...
          Ni({ sourceId: Mr().getSourceId, tableName: t5.table, sourceType: 0 }).then((e5) => {
          ...
          }), _: 2 }, 1032, ["title"])) : (s(), r("div", fp, "选择关联表"))];
        }), _: 2 }, 1032, ["disabled", "value", "options", "onChange"]), g(ip, { "query-mode": e2.queryMode, "table-fields": (l3 = t4.table, l3 && $2.getTableFieldsMap.has(l3) ? $2.getTableFieldsMap.get(l3) : []), fields: t4.fields, onFieldChange: (e3) => function(e4, t5) {
          var _a4;
          ((_a4 = o2.sqlQuery) == null ? void 0 : _a4.joins) && o2.sqlQuery.joins.length > 0 && (L2.value[t5].fields = e4), E2(t5);
        ...
        o4.condition && (o4.condition.field = e4[1].realValue, o4.condition.field.tableAlias = e4[0].value, E2(t5));
      }(o3, c2, t4) }, { displayRender: M(({ labels: e3 }) => [g(d(G), { title: j2(c2, e3) }, { default: M(() => [x(S(j2(c2, e3)), 1)]), _: 2 }, 1032, ["title"])]), _: 2 }, 1032, ["disabled", "options", "value", "placeholder", "onChange"]), p("div", hp, [g(d(Te), { icon: "iconfont:icon-mfish-equals", color: d(T2), size: 14 }, null, 8, ["color"])]), g(d(K), { disabled: e2.queryMode, "show-search": "", "allow-clear": false, class: "input-select", "option-filter-prop": "label", value: F2(c2), options: z2(c2), placeholder: `${t4.tableAlias || "关联表"}字段`, onSelect: (e3, o3) => function(e4, t5, o4) {
        o4.condition && (o4.condition.joinedField = e4.realValue, E2(t5));
      }(o3, c2, t4) }, null, 8, ["disabled", "value", "options", "placeholder", "onSelect"])];
    }), _: 2 }, 1032, ["query-mode", "onDeleteConfig", "onExecute"]))), 128)), B2.value ? (s(), h(pp, { key: 0, class: "config-button", title: "关联", iconfont: "icon-mfish-join", level: 0, onClick: P2 })) : f("", true)], 2);
  };
} }), yp = Yt(gp, [["__scopeId", "data-v-d960e99c"]]), mp = e({ __name: "TableLimit", props: { sqlQuery: { type: Object, require: true }, configMitt: { type: Object }, queryMode: { type: Boolean, default: false } }, emits: ["limitChange"], setup(e2, { emit: t2 }) {
  ...
  function c2(e3) {
    Lr(o2.configMitt, { ...o2.sqlQuery, limit: a2.value }, e3);
  }
  return n(() => {
    var _a2;
    ...
    var _a2;
    return void 0 !== ((_a2 = e2.sqlQuery) == null ? void 0 : _a2.limit) ? (s(), h(ap, { key: 0, "query-mode": e2.queryMode, icon: "ant-design:column-height-outlined", title: "行数", onDeleteConfig: u2, onExecute: c2 }, { config: M(() => [g(d(Q), { disabled: e2.queryMode, "default-value": a2.value, "addon-after": "行", step: "10", onChange: i2, onPressEnter: r2, onBlur: r2 }, null, 8, ["disabled", "default-value"])]), _: 1 }, 8, ["query-mode"])) : f("", true);
  };
} }), vp = [{ field: "id", label: "唯一ID", component: "Input", show: false }, { field: "name", label: "参数名称", component: "Input", dynamicRules: () => [{ required: true, validator: (e2, t2) => /^[a-z]\w*$/i.test(t2) ? Promise.resolve() : Promise.reject("错误：参数格式不正确！必须字符开头，不允许包含特殊字符!"), trigger: "blur" }] }, { field: "required", label: "是否必须", component: "RadioButtonGroup", defaultValue: 1, componentProps: { options: [{ label: "是", value: 1 }, { label: "否", value: 0 }] }, required: true }, { field: "defaultValue", label: "默认值", component: "Input" }, { field: "remark", label: "参数描述", component: "InputTextArea" }], Cp = { name: "ApiParamsModal", components: { BasicModal: gt, BasicForm: vt }, emits: ["addSuccess", "updateSuccess", "register", "variableChange"], setup(e2, { emit: t2 }) {
  const o2 = y(true), [n2, { resetFields: a2, setFieldsValue: i2, validate: r2 }] = Ct({ name: "model_form_item", labelWidth: 100, baseColProps: { span: 24 }, schemas: vp, showActionButtonGroup: false, autoSubmitOnEnter: true }), s2 = y(), [u2, { setModalProps: c2, closeModal: p2 }] = yt(async (e3) => {
    a2().then(), c2({ confirmLoading: false, width: "600px" }), o2.value = !!(e3 == null ? void 0 : e3.isUpdate), d(o2) && (s2.value = e3.index, i2({ ...e3.param }).then());
  }), f2 = Mr(), h2 = l(() => d(o2) ? "编辑参数" : "新增参数"), { createMessage: g2 } = Se();
...
const bp = Yt(Cp, [["render", function(e2, t2, o2, l2, n2, a2) {
  const i2 = w("BasicForm"), r2 = w("BasicModal");
  return s(), h(r2, $(e2.$attrs, { onRegister: l2.registerModal, title: l2.getTitle, onOk: l2.handleSubmit }), { default: M(() => [g(i2, { onRegister: l2.registerForm, onSubmit: l2.handleSubmit }, null, 8, ["onRegister", "onSubmit"])]), _: 1 }, 16, ["onRegister", "title", "onOk"]);
}]]), Mp = { key: 0 }, kp = e({ __name: "FilterVariable", props: { value: { type: String }, queryMode: { type: Boolean, default: false } }, emits: ["variableChange"], setup(e2, { emit: t2 }) {
  const o2 = e2, l2 = t2, a2 = K.Option, i2 = xe().getThemeColor, { prefixCls: C2 } = we("filter-variable"), [b2, { openModal: w2 }] = mt(), _2 = Mr(), $2 = y();
  n(() => o2.value, (e3) => {
    $2.value = e3;
  }, { immediate: true });
  ...
  return (t3, o3) => (s(), r(m, null, [g(d(K), { value: $2.value, "onUpdate:value": o3[0] || (o3[0] = (e3) => $2.value = e3), onChange: N2, disabled: e2.queryMode }, { dropdownRender: M(({ menuNode: e3 }) => [g(L2, { vnodes: e3 }, null, 8, ["vnodes"]), g(d(J), { style: { margin: "4px 0" } }), p("div", { style: { padding: "4px 8px", cursor: "pointer" }, onClick: I2 }, [g(d(Te), { icon: "ant-design:plus-outlined", color: d(i2) }, null, 8, ["color"]), p("span", { style: u(`color:${d(i2)};margin-left:6px`) }, "增加变量参数", 4)])]), default: M(() => [(s(true), r(m, null, v(d(_2).getVariables, (t4, o4) => (s(), h(d(a2), { label: t4.name, value: t4.name, key: o4 }, { default: M(() => [p("div", { class: c(`${d(C2)}-select-option`) }, [x(S(t4.name) + " ", 1), p("div", null, S(t4.remark), 1), e2.queryMode ? f("", true) : (s(), r("div", Mp, [g(d(Te), { icon: "ant-design:edit-outlined", color: d(i2), onClick: k((e3) => function(e4, t5) {
    w2(true, { index: e4, param: t5, isUpdate: true });
  }(o4, t4), ["stop"]) }, null, 8, ["color", "onClick"]), g(d(J), { type: "vertical" }), g(d(Te), { icon: "ant-design:delete-outlined", color: d(i2), onClick: k((e3) => function(e4) {
    _2.getVariables.splice(e4, 1), _2.getVariables.some((e5) => e5.name === $2.value) || ($2.value = "");
  }(o4), ["stop"]) }, null, 8, ["color", "onClick"])]))], 2)]), _: 2 }, 1032, ["label", "value"]))), 128))]), _: 1 }, 8, ["value", "disabled"]), g(bp, { onRegister: d(b2), onAddSuccess: D2, onUpdateSuccess: T2, onVariableChange: B2 }, null, 8, ["onRegister"])], 64));
} }), wp = Yt(kp, [["__scopeId", "data-v-68d91e73"]]), xp = e({ __name: "TableField", props: { tableFields: { type: Object }, value: { type: Object }, queryMode: { type: Boolean, default: false } }, emits: ["change"], setup(e2, { emit: t2 }) {
  const o2 = e2, l2 = t2, a2 = y({ dataType: "STRING" }), i2 = y([]), r2 = xe().getThemeColor, u2 = y();
  function c2(e3) {
    u2.value = Ya(e3);
  }
  ...
  }(t4)) }, { displayRender: M(({ labels: e3 }) => [g(d(G), { title: `${e3[0]} / ${e3[1]}` }, { default: M(() => [g(d(Te), { icon: u2.value, color: d(r2) }, null, 8, ["icon", "color"]), x(" " + S(`${e3[0]} / ${e3[1]}`), 1)]), _: 2 }, 1032, ["title"])]), _: 1 }, 8, ["disabled", "options", "value"]));
} }), Sp = e({ __name: "FilterValue", props: { fieldType: { type: String, default: "STRING" }, valueType: { type: String, default: "0" }, operator: { type: String, default: "" }, tableFields: { type: Array, default: () => [] }, queryMode: { type: Boolean, default: false } }, emits: ["typeChange", "variableChange", "fieldChange"], setup(e2, { emit: t2 }) {
  const o2 = e2, l2 = t2, a2 = H, i2 = le.Group, u2 = le.Button, p2 = ne.ItemRest, { prefixCls: m2 } = we("filter-value"), v2 = y("0"), C2 = Mr();
  function b2() {
  ...
  return (t3, o3) => (s(), h(d(gt), $(t3.$attrs, { onRegister: d(C2), title: "设置过滤条件", onOk: _2 }), { default: M(() => [g(d(ne), { ref_key: "formRef", ref: i2, model: n2, "label-col": { span: 6 }, "wrapper-col": { span: 16 } }, { default: M(() => {
    var _a2;
    return [g(d(l2), { label: "字段", name: "field", rules: [{ required: true, message: "请选择字段！" }] }, { default: M(() => [g(xp, { "query-mode": e2.queryMode, "table-fields": r2.value, value: u2.field, onChange: w2 }, null, 8, ["query-mode", "table-fields", "value"])]), _: 1 }), g(d(l2), { label: "条件", name: "operator", rules: [{ required: true, message: "请选择条件！" }] }, { default: M(() => [g(d(K), { disabled: e2.queryMode, "show-search": "", "allow-clear": false, class: "input-select", "option-filter-prop": "label", value: n2.operator, "onUpdate:value": o3[0] || (o3[0] = (e3) => n2.operator = e3), options: c2.value, placeholder: "选择条件", onSelect: S2 }, null, 8, ["disabled", "value", "options"])]), _: 1 }), p2.value ? (s(), h(d(l2), { key: 0, label: "值", name: "value", rules: [{ required: "STRING" !== ((_a2 = u2.field) == null ? void 0 : _a2.dataType), message: "请选择值！" }] }, { default: M(() => {
      var _a3;
      return [g(_p, { "query-mode": e2.queryMode, placeholder: "输入值", value: n2.value, "onUpdate:value": o3[1] || (o3[1] = (e3) => n2.value = e3), "value-type": u2.valueType, "field-type": (_a3 = u2.field) == null ? void 0 : _a3.dataType, operator: n2.operator, "table-fields": r2.value, onTypeChange: L2, onVariableChange: I2, onFieldChange: D2 }, null, 8, ["query-mode", "value", "value-type", "field-type", "operator", "table-fields"])];
    }), _: 1 }, 8, ["rules"])) : f("", true)];
  }), _: 1 }, 8, ["model"])]), _: 1 }, 16, ["onRegister"]));
} }), Lp = e({ __name: "TableFilter", props: { sqlQuery: { type: Object }, level: { type: Number, default: 1 }, configMitt: { type: Object }, queryMode: { type: Boolean, default: false } }, emits: ["filterChange"], setup(e2, { emit: t2 }) {
  const o2 = e2, a2 = t2, i2 = y([]), u2 = l(() => {
    var _a2;
    return void 0 === ((_a2 = o2.sqlQuery) == null ? void 0 : _a2.filters) && $r(o2.sqlQuery, o2.level) && !o2.queryMode;
  });
  n(() => {
    var _a2;
  ...
  const c2 = xe().getThemeColor, p2 = y([{ name: "添加", icon: "icon-mfish-left-bracket", color: c2, value: { type: "0", name: "(", value: "(" } }, { name: "添加", icon: "icon-mfish-right-bracket", color: c2, value: { type: "0", name: ")", value: ")" } }]), m2 = { type: "0", slot: true, hideClose: true, value: "and" }, [v2, { openModal: C2 }] = mt();
  function b2() {
    a2("filterChange");
  }
  function w2() {
  ...
  function I2(e3) {
    Lr(o2.configMitt, { ...o2.sqlQuery, filters: i2.value, aggregates: [], groups: [], customColumns: [], orders: [], limit: void 0 }, e3);
  }
  return (t3, o3) => {
    var _a2;
    return s(), r("div", null, [void 0 !== ((_a2 = e2.sqlQuery) == null ? void 0 : _a2.filters) ? (s(), h(ap, { key: 0, "query-mode": e2.queryMode, icon: "ant-design:filter-outlined", title: "过滤", onDeleteConfig: b2, full: true, onExecute: I2 }, { config: M(() => [g(d(ht), { "query-mode": e2.queryMode, items: i2.value, menus: p2.value, "add-title": "添加过滤条件", onAddBlock: x2, onEditBlock: S2, onCloseBlock: $2, onDragChange: L2 }, { tag: M(({ index: t4, element: o4 }) => [g(d(re), { disabled: e2.queryMode, style: { "margin-right": "8px" }, checked: o4 == null ? void 0 : o4.value, "checked-children": "且", "un-checked-children": "或", "checked-value": "and", "un-checked-value": "or", size: "small", onChange: (e3) => function(e4, t5) {
      var _a3;
      i2.value && ((_a3 = i2.value) == null ? void 0 : _a3.length) > t5 && (i2.value[t5].value = e4, setTimeout(() => {
        i2.value && a2("filterChange", [...i2.value]);
      }, 300));
    }(e3, t4) }, null, 8, ["disabled", "checked", "onChange"])]), _: 1 }, 8, ["query-mode", "items", "menus"])]), _: 1 }, 8, ["query-mode"])) : f("", true), u2.value ? (s(), h(pp, { key: 1, class: "config-button", title: "过滤", icon: "ant-design:filter-outlined", level: 0, onClick: k(w2, ["prevent"]) })) : f("", true), g($p, { "query-mode": e2.queryMode, onRegister: d(v2), onSubmit: _2 }, null, 8, ["query-mode", "onRegister"])]);
  };
} }), Ip = e({ __name: "AggregateModal", props: { queryMode: { type: Boolean, default: false } }, emits: ["submit", "register"], setup(e2, { emit: t2 }) {
  const o2 = t2, l2 = ne.Item, n2 = a({ field: void 0, aggFunc: "" }), i2 = y(), r2 = y([]), u2 = a({ field: { dataType: "STRING" } }), c2 = Object.keys(Xa).map((e3) => ({ value: e3, label: Xa[e3] })), p2 = y(true), m2 = y(), [v2, { setModalProps: C2, closeModal: b2 }] = yt(async (e3) => {
    if (C2({ confirmLoading: false, width: "600px" }), sp(void 0, e3.sqlQuery).then((e4) => {
      var _a2, _b;
      ...
      r2.value = e4;
    }), e3.isUpdate) return n2.field = [e3.field.tableAlias, e3.field.colName], n2.aggFunc = e3.aggFunc, u2.field = e3.field, p2.value = Xa[n2.aggFunc] !== Xa.count, void (m2.value = e3.index);
  ...
  function w2(e3) {
    p2.value = Xa[e3] !== Xa.count;
  }
  async function x2() {
    var _a2, _b;
    try {
      const e3 = await ((_a2 = i2.value) == null ? void 0 : _a2.validateFields());
      b2(), o2("submit", m2.value, { ...u2, name: Xa[n2.aggFunc] + (Xa[n2.aggFunc] === Xa.count ? "" : `(${(_b = u2.field) == null ? void 0 : _b.colName})`), aggFunc: e3.aggFunc });
    } catch (e3) {
    }
  }
  return (t3, o3) => (s(), h(d(gt), $(t3.$attrs, { onRegister: d(v2), title: "选择聚合字段", onOk: x2 }), { default: M(() => [g(d(ne), { ref_key: "formRef", ref: i2, model: n2, "label-col": { span: 6 }, "wrapper-col": { span: 16 } }, { default: M(() => [g(d(l2), { label: "聚合方式", name: "aggFunc", rules: [{ required: true, message: "请选择聚合方式！" }] }, { default: M(() => [g(d(K), { disabled: e2.queryMode, "show-search": "", "allow-clear": false, "option-filter-prop": "label", value: n2.aggFunc, "onUpdate:value": o3[0] || (o3[0] = (e3) => n2.aggFunc = e3), options: d(c2), placeholder: "选择聚合方式", onSelect: w2 }, null, 8, ["disabled", "value", "options"])]), _: 1 }), p2.value ? (s(), h(d(l2), { key: 0, label: "字段", name: "field", rules: [{ required: true, message: "请选择字段！" }] }, { default: M(() => [g(xp, { "query-mode": e2.queryMode, "table-fields": r2.value, value: u2.field, onChange: k2 }, null, 8, ["query-mode", "table-fields", "value"])]), _: 1 })) : f("", true)]), _: 1 }, 8, ["model"])]), _: 1 }, 16, ["onRegister"]));
} }), Dp = e({ __name: "GroupModal", props: { queryMode: { type: Boolean, default: false } }, emits: ["submit", "register"], setup(e2, { emit: t2 }) {
  const o2 = t2, l2 = ne.Item, n2 = a({ field: void 0 }), i2 = a({ field: { dataType: "STRING" } }), r2 = y(), u2 = y([]), c2 = y(), [p2, { setModalProps: f2, closeModal: m2 }] = yt(async (e3) => {
    if (f2({ confirmLoading: false, width: "600px" }), sp(void 0, e3.sqlQuery).then((e4) => {
      u2.value = e4;
  ...
  return (t3, o3) => (s(), h(d(gt), $(t3.$attrs, { onRegister: d(p2), title: "选择分组字段", onOk: C2 }), { default: M(() => [g(d(ne), { ref_key: "formRef", ref: r2, model: n2, "label-col": { span: 6 }, "wrapper-col": { span: 16 } }, { default: M(() => [g(d(l2), { label: "字段", name: "field", rules: [{ required: true, message: "请选择字段！" }] }, { default: M(() => [g(xp, { "query-mode": e2.queryMode, "table-fields": u2.value, value: i2.field, onChange: v2 }, null, 8, ["query-mode", "table-fields", "value"])]), _: 1 })]), _: 1 }, 8, ["model"])]), _: 1 }, 16, ["onRegister"]));
} }), Tp = e({ __name: "TableAggregate", props: { sqlQuery: { type: Object }, configMitt: { type: Object }, queryMode: { type: Boolean, default: false } }, emits: ["aggregateChange", "groupChange", "deleteAggregate"], setup(e2, { emit: t2 }) {
  const o2 = e2, l2 = t2, a2 = xe().getThemeColor, { prefixCls: i2 } = we("table-aggregate"), u2 = y(), m2 = y(), [v2, { openModal: C2 }] = mt(), [b2, { openModal: k2 }] = mt();
  async function w2() {
    l2("deleteAggregate");
  }
  function x2() {
  ...
  function N2(e3) {
    Lr(o2.configMitt, { ...o2.sqlQuery, aggregates: u2.value, groups: m2.value, orders: [], limit: void 0 }, e3);
  }
  return n(() => {
    var _a2;
    ...
    var _a2;
    return s(), r("div", { class: c(d(i2)) }, [void 0 !== ((_a2 = e2.sqlQuery) == null ? void 0 : _a2.aggregates) ? (s(), h(ap, { key: 0, "query-mode": e2.queryMode, iconfont: "icon-mfish-sum", title: "聚合", onDeleteConfig: w2, onExecute: N2, full: true }, { config: M(() => [g(d(ht), { "query-mode": e2.queryMode, items: u2.value, "add-title": "添加聚合指标", onAddBlock: x2, onEditBlock: _2, onDragChange: $2 }, null, 8, ["query-mode", "items"]), g(d(Te), { class: "groupIcon", icon: "ant-design:group-outlined", color: d(a2), style: { "font-size": "14px" } }, null, 8, ["color"]), o3[0] || (o3[0] = p("div", { class: "group" }, "分组", -1)), g(d(ht), { "query-mode": e2.queryMode, items: m2.value, "add-title": "添加分组指标", onAddBlock: L2, onEditBlock: D2, onDragChange: T2 }, null, 8, ["query-mode", "items"])]), _: 1 }, 8, ["query-mode"])) : f("", true), g(Ip, { "query-mode": e2.queryMode, onRegister: d(v2), onSubmit: S2 }, null, 8, ["query-mode", "onRegister"]), g(Dp, { "query-mode": e2.queryMode, onRegister: d(b2), onSubmit: I2 }, null, 8, ["query-mode", "onRegister"])], 2);
  };
} }), Np = Yt(Tp, [["__scopeId", "data-v-a3fb541b"]]), Bp = "其他", Rp = e({ __name: "OrderModal", emits: ["submit", "register"], setup(e2, { emit: t2 }) {
  const o2 = t2, l2 = ne.Item, n2 = a({ field: void 0 }), i2 = a({ field: { dataType: "STRING" }, order: "ASC" }), r2 = y(), u2 = y([]), c2 = y(), p2 = Mr(), [f2, { setModalProps: m2, closeModal: v2 }] = yt(async (e3) => {
    if (m2({ confirmLoading: false, width: "600px" }), _r(e3.sqlQuery) ? Ti({ sourceId: p2.getSourceId, sourceType: p2.getSourceType, sqlQuery: e3.sqlQuery }).then((e4) => {
      u2.value = [{ value: Bp, label: Bp, realValue: Bp, children: e4.map((e5) => ({ value: e5.colName, label: e5.colName + (e5.comment ? `[${e5.comment}]` : ""), realValue: { ...e5 } })) }];
    }) : u2.value = await sp(void 0, e3.sqlQuery), e3.isUpdate) return n2.field = [e3.field.tableAlias, e3.field.colName], i2.field = e3.field, void (c2.value = e3.index);
    n2.field = [], c2.value = void 0, i2.field = {};
  });
  function C2(e3) {
    e3.tableAlias && e3.colName && (n2.field = [e3.tableAlias, e3.colName], i2.field = e3);
  ...
  async function b2() {
    var _a2, _b;
    try {
      await ((_a2 = r2.value) == null ? void 0 : _a2.validateFields()), v2(), i2.field.tableAlias === Bp && (i2.field.tableAlias = ""), o2("submit", c2.value, { ...i2, name: (_b = i2.field) == null ? void 0 : _b.colName });
    } catch (e3) {
    }
  }
  return (e3, t3) => (s(), h(d(gt), $(e3.$attrs, { onRegister: d(f2), title: "选择排序字段", onOk: b2 }), { default: M(() => [g(d(ne), { ref_key: "formRef", ref: r2, model: n2, "label-col": { span: 6 }, "wrapper-col": { span: 16 } }, { default: M(() => [g(d(l2), { label: "字段", name: "field", rules: [{ required: true, message: "请选择字段！" }] }, { default: M(() => [g(xp, { "table-fields": u2.value, value: i2.field, onChange: C2 }, null, 8, ["table-fields", "value"])]), _: 1 })]), _: 1 }, 8, ["model"])]), _: 1 }, 16, ["onRegister"]));
} }), qp = e({ __name: "TableOrder", props: { sqlQuery: { type: Object, default: void 0 }, configMitt: { type: Object, default: void 0 }, queryMode: { type: Boolean, default: false } }, emits: ["orderChange"], setup(e2, { emit: t2 }) {
  const o2 = e2, l2 = t2, a2 = y([]), [i2, { openModal: u2 }] = mt();
  function c2() {
  ...
  function b2(e3) {
    Lr(o2.configMitt, { ...o2.sqlQuery, orders: a2.value, limit: void 0 }, e3);
  }
  return n(() => {
    var _a2;
    ...
    var _a2;
    return s(), r("div", null, [void 0 !== ((_a2 = e2.sqlQuery) == null ? void 0 : _a2.orders) ? (s(), h(ap, { key: 0, "query-mode": e2.queryMode, icon: "ant-design:sort-ascending-outlined", title: "排序", onDeleteConfig: v2, full: true, onExecute: b2 }, { config: M(() => [g(d(ht), { "query-mode": e2.queryMode, items: a2.value, "add-title": "添加排序指标", onAddBlock: c2, onEditBlock: p2, onDragChange: m2 }, { tagIcon: M(({ element: e3 }) => [g(d(Te), { style: { "font-weight": "bold", "font-size": "20px" }, icon: "ASC" === e3.order ? "ant-design:sort-descending-outlined" : "ant-design:sort-ascending-outlined" }, null, 8, ["icon"])]), _: 1 }, 8, ["query-mode", "items"])]), _: 1 }, 8, ["query-mode"])) : f("", true), g(Rp, { onRegister: d(i2), onSubmit: C2 }, null, 8, ["onRegister"])]);
  };
} }), Ap = e({ __name: "FieldColModal", emits: ["submit", "register"], setup(e2, { emit: t2 }) {
  const o2 = t2, l2 = ne.Item, n2 = a({ field: void 0 }), i2 = y(), r2 = y(), u2 = y([]), c2 = y(), [p2, { setModalProps: f2, closeModal: m2 }] = yt(async (e3) => {
    if (f2({ confirmLoading: false, width: "600px" }), sp(void 0, e3.sqlQuery).then((e4) => {
      u2.value = e4;
    }), e3.isUpdate) {
  ...
  async function C2() {
    var _a2, _b;
    try {
      await ((_a2 = r2.value) == null ? void 0 : _a2.validateFields()), m2(), o2("submit", c2.value, { type: Ja.field, value: i2.value, name: (_b = i2.value) == null ? void 0 : _b.colName });
    } catch (e3) {
    }
  }
  return (e3, t3) => (s(), h(d(gt), $(e3.$attrs, { onRegister: d(p2), title: "选择字段", onOk: C2 }), { default: M(() => [g(d(ne), { ref_key: "formRef", ref: r2, model: n2, "label-col": { span: 6 }, "wrapper-col": { span: 16 } }, { default: M(() => [g(d(l2), { label: "字段", name: "field", rules: [{ required: true, message: "请选择字段！" }] }, { default: M(() => [g(xp, { "table-fields": u2.value, value: i2.value, onChange: v2 }, null, 8, ["table-fields", "value"])]), _: 1 })]), _: 1 }, 8, ["model"])]), _: 1 }, 16, ["onRegister"]));
} }), Op = ["onClick"], jp = ["onClick"], zp = e({ __name: "FormulaManagement", props: { isShow: { type: Boolean, default: false }, curKey: { type: String, default: "" } }, emits: ["select"], setup(e2, { emit: t2 }) {
  const o2 = e2, a2 = t2, i2 = se.Item, C2 = ue.Item, b2 = y([]), { prefixCls: k2 } = we("formula-management"), w2 = y(null), _2 = xe().getThemeColor, $2 = y([]), L2 = y(false), I2 = y([]), D2 = y(), T2 = y(0.4), N2 = y([]), B2 = y(), q2 = l(() => {
    var _a2, _b;
    if ("" === ((_a2 = D2.value) == null ? void 0 : _a2.parentId)) return;
    const e3 = (_b = D2.value) == null ? void 0 : _b.formulaInfo;
    if (!e3) return;
    let t3;
    try {
  ...
  function A2(e3, t3) {
    const o3 = [];
    t3 ? o3.push({ icon: t3.icon, title: t3.name, key: t3.key }) : b2.value = e3 ? e3.children : [], e3 && o3.push({ icon: e3.icon, title: e3.name, key: e3.key }), B2.value = o3, D2.value = e3, a2("select", e3);
  }
  function O2(e3, t3) {
    var _a2, _b;
  ...
  return R(async () => {
  ...
  }), (e3, t3) => (s(), h(d(Mt), { "default-size": T2.value, min: 0.1, max: 0.5, class: c(d(k2)) }, { 1: M(() => [g(d(bt), { title: "", toolbar: "", search: "", ref_key: "asyncTreeRef", ref: w2, "tree-wrapper-class-name": "h-[calc(100%-35px)] overflow-auto", "selected-keys": $2.value, "onUpdate:selectedKeys": t3[0] || (t3[0] = (e4) => $2.value = e4), "tree-data": N2.value || [], onSelect: O2 }, null, 8, ["selected-keys", "tree-data"])]), 2: M(() => [p("div", { class: c(`${d(k2)}-right h-full pt-2`) }, [g(d(se), { separator: ">", class: "breadcrumb" }, { default: M(() => [(s(true), r(m, null, v(B2.value, (e4, t4) => (s(), h(d(i2), { key: t4 }, { default: M(() => [g(d(Te), { icon: e4.icon }, null, 8, ["icon"]), p("a", { onClick: (t5) => j2(e4.key), class: "fw-bold text-decoration-none" }, S(e4.title), 9, Op)]), _: 2 }, 1024))), 128))]), _: 1 }), D2.value && "" === D2.value.parentId ? (s(), h(d(ft), { key: 0 }, { default: M(() => [g(d(ce), { class: "ml-6" }, { default: M(() => [(s(true), r(m, null, v(z2.value, (e4, t4) => (s(), h(d(de), { span: 22, key: t4, class: c(`${d(k2)}-card`) }, { default: M(() => {
    var _a2, _b;
    return [p("div", { class: c(`${d(k2)}-card-item`), onClick: (t5) => j2(e4.key) }, [p("div", { class: c(`${d(k2)}-card-item-info`) }, [g(d(G), { title: (_a2 = e4.formulaInfo) == null ? void 0 : _a2.cnName }, { default: M(() => {
      var _a3, _b2;
      return [p("span", { style: u(`color: ${d(_2)};font-weight: bold`) }, [g(d(Te), { icon: "ant-design:function-outlined", color: d(_2) }, null, 8, ["color"]), x(" " + S((_a3 = e4.formulaInfo) == null ? void 0 : _a3.cnName) + " (" + S((_b2 = e4.formulaInfo) == null ? void 0 : _b2.enName) + ") ", 1)], 4)];
    ...
    }), _: 2 }, 1032, ["title"])], 2)], 10, jp)];
  }), _: 2 }, 1032, ["class"]))), 128))]), _: 1 })]), _: 1 })) : f("", true), D2.value && D2.value.formulaInfo ? (s(), h(d(ft), { key: 1 }, { default: M(() => [g(d(ue), { class: "pt-3 pr-2", column: 1, bordered: true }, { default: M(() => [g(d(C2), { label: "描述" }, { default: M(() => {
    var _a2;
    return [x(S((_a2 = q2.value) == null ? void 0 : _a2.description), 1)];
  }), _: 1 }), g(d(C2), { label: "参数" }, { default: M(() => {
    var _a2;
    return [(s(true), r(m, null, v((_a2 = q2.value) == null ? void 0 : _a2.paramDesc, (e4, t4) => (s(), r("div", { key: t4 }, S(e4), 1))), 128))];
...
} }), Fp = Yt(zp, [["__scopeId", "data-v-6dc5ed8e"]]), Pp = { key: 0, class: "content" }, Ep = { key: 1, class: "content placeholder" }, Hp = Yt(e({ __name: "FormulaSelect", props: { formulaKey: { type: String, default: "" }, queryMode: { type: Boolean, default: false } }, emits: ["formulaChange"], setup(e2, { emit: t2 }) {
  const o2 = e2, a2 = t2, i2 = l(() => xe().getDarkMode.value === He.DARK), { prefixCls: u2 } = we("select-formula"), f2 = y(false);
  let m2;
  const { createMessage: v2 } = Se(), C2 = Mr(), k2 = y(), w2 = (e3) => {
    f2.value = e3;
  };
  function _2() {
    f2.value = false, k2.value = void 0, m2 = void 0, a2("formulaChange", k2.value);
  }
  function $2() {
    var _a2;
    m2 ? (f2.value = false, ((_a2 = k2.value) == null ? void 0 : _a2.key) !== m2.key && (k2.value = { key: m2.key, ...m2.formulaInfo }, a2("formulaChange", k2.value))) : v2.error("请选择公式");
  }
  function L2(e3) {
  ...
  return n(() => o2.formulaKey, (e3) => {
    ...
    var _a2;
    return [g(Fp, { style: { height: "350px" }, "is-show": f2.value, "cur-key": (_a2 = k2.value) == null ? void 0 : _a2.key, onSelect: L2 }, null, 8, ["is-show", "cur-key"]), p("div", { class: c(`${d(u2)}-ok-button`) }, [g(d(ee), { class: "mr-2", onClick: _2 }, { default: M(() => o3[0] || (o3[0] = [x("清空")])), _: 1 }), g(d(ee), { type: "primary", onClick: $2 }, { default: M(() => o3[1] || (o3[1] = [x("确定")])), _: 1 })], 2)];
  }), _: 1 })]), _: 1 })]), default: M(() => [g(d(ee), { class: c(`${d(u2)}-drop-button`) }, { default: M(() => {
    var _a2, _b;
    return [((_a2 = k2.value) == null ? void 0 : _a2.cnName) ? (s(), r("span", Pp, S((_b = k2.value) == null ? void 0 : _b.cnName), 1)) : (s(), r("span", Ep, "选择公式")), g(d(Te), { size: 12, icon: f2.value ? "ant-design:up-outlined" : "ant-design:down-outlined", color: i2.value ? "rgba(255, 255, 255, 0.25)" : "rgba(0, 0, 0, 0.25)" }, null, 8, ["icon", "color"])];
  }), _: 1 }, 8, ["class"])]), _: 1 }, 8, ["disabled", "open"]));
} }), [["__scopeId", "data-v-4015d717"]]), Qp = "displayName", Gp = { config: { type: Object, default: () => ({ type: Ja.param, value: "" }) }, comType: { type: String, default: "string" }, paramType: { type: String, default: "string" }, paramValue: { type: String, default: "" }, showRadio: { type: Boolean, default: true }, sqlQuery: { type: Object }, index: { type: Number, default: 0 }, queryMode: { type: Boolean, default: false } }, Up = (e2) => {
  if (!e2) return { type: Ja.formula, value: null };
  const t2 = ((e2 == null ? void 0 : e2.targetParam) ? JSON.parse(e2.targetParam) : []).map((e3) => e3.paramType.startsWith("list") ? { type: Ja.param, value: [{ type: Ja.param, value: "" }] } : { type: Ja.param, value: "" });
  return { type: Ja.formula, value: { key: e2.key, cnName: e2.cnName, enName: e2.enName, targetObject: e2.targetObject, param: t2 } };
}, Vp = e({ __name: "FormulaCompact", props: Gp, emits: ["typeChange", "formulaSelect", "inputChange"], setup(e2, { emit: t2 }) {
  const o2 = e2, l2 = t2, { prefixCls: a2 } = we("formula-compact"), i2 = le.Group, u2 = le.Button, m2 = y("string"), v2 = y("string"), C2 = y(), b2 = y(Ja.param), k2 = y(), w2 = y([]), x2 = y(), S2 = y(false), _2 = y([]), $2 = y();
  function L2(e3) {
    l2("formulaSelect", e3 || "");
  }
  function I2() {
  ...
  function D2(e3) {
    e3.tableAlias && e3.colName && (x2.value = e3, l2("inputChange", { ...e3, [Qp]: "colName" }));
  }
  function T2(e3, t3) {
  ...
  return n(() => o2.config, (e3) => {
    var _a2;
    e3 && (b2.value = e3.type, e3.type === Ja.formula ? k2.value = (_a2 = e3.value) == null ? void 0 : _a2.key : C2.value = e3.value);
    ...
    var _a2;
    if (e3 === Ja.param && "list" === m2.value) {
      const e4 = /@\{(?<value>.*?)\}/u, t3 = (_a2 = o2.paramValue) == null ? void 0 : _a2.replace(e4, "$<value>");
      S2.value = "fieldName" === t3, S2.value && o2.sqlQuery ? sp(void 0, o2.sqlQuery).then((e5) => {
        ...
        if (_2.value = [], e5 && e5.value) {
          ...
          e5.value.forEach((e6) => {
            const o3 = e6[t4];
            _2.value.push({ label: o3, value: o3, realValue: { ...e6, [Qp]: t4 } });
          }), ((_a3 = o2.config) == null ? void 0 : _a3.value) && ($2.value = o2.config.value[o2.config.value[Qp]]);
        }
      });
    }
  }, { immediate: true }), (e3, t3) => (s(), r("div", { class: c(d(a2)) }, [o2.showRadio ? (s(), h(d(i2), { key: 0, disabled: o2.queryMode, class: "radio-group", value: b2.value, "onUpdate:value": t3[0] || (t3[0] = (e4) => b2.value = e4), "button-style": "solid", onChange: N2 }, { default: M(() => ["number" === v2.value ? (s(), h(d(u2), { key: 0, value: d(Ja).param }, { default: M(() => [g(d(G), { placement: "left", title: "数字" }, { default: M(() => [g(d(Te), { icon: "ant-design:field-number-outlined" })]), _: 1 })]), _: 1 }, 8, ["value"])) : (s(), h(d(u2), { key: 1, value: d(Ja).param }, { default: M(() => [g(d(G), { placement: "left", title: "字符" }, { default: M(() => [g(d(Te), { icon: "ant-design:field-string-outlined" })]), _: 1 })]), _: 1 }, 8, ["value"])), g(d(u2), { value: d(Ja).formula }, { default: M(() => [g(d(G), { placement: "left", title: "公式" }, { default: M(() => [g(d(Te), { icon: "ant-design:function-outlined" })]), _: 1 })]), _: 1 }, 8, ["value"])]), _: 1 }, 8, ["disabled", "value"])) : f("", true), p("div", { class: c(o2.showRadio ? "formula-select" : "formula-select-show") }, [b2.value === d(Ja).param && "string" === m2.value ? (s(), h(d(H), { key: 0, disabled: o2.queryMode, value: C2.value, "onUpdate:value": t3[1] || (t3[1] = (e4) => C2.value = e4), class: "input-com", placeholder: "请输入字符常量", onChange: I2 }, null, 8, ["disabled", "value"])) : b2.value === d(Ja).param && "number" === m2.value ? (s(), h(d(Q), { key: 1, disabled: o2.queryMode, value: C2.value, "onUpdate:value": t3[2] || (t3[2] = (e4) => C2.value = e4), class: "input-com", placeholder: "请输入数字常量", onChange: I2 }, null, 8, ["disabled", "value"])) : b2.value === d(Ja).param && "list" === m2.value && S2.value ? (s(), h(xp, { key: 2, "query-mode": o2.queryMode, class: "select-com", "table-fields": w2.value, value: x2.value, onChange: D2 }, null, 8, ["query-mode", "table-fields", "value"])) : b2.value !== d(Ja).param || "list" !== m2.value || S2.value ? (s(), h(Hp, { key: 4, "query-mode": o2.queryMode, "formula-key": k2.value, onFormulaChange: L2 }, null, 8, ["query-mode", "formula-key"])) : (s(), h(d(K), { key: 3, disabled: o2.queryMode, class: "select-com", value: $2.value, "onUpdate:value": t3[3] || (t3[3] = (e4) => $2.value = e4), options: _2.value, placeholder: "请选择参数", onSelect: T2 }, null, 8, ["disabled", "value", "options"]))], 2)], 2));
} }), Wp = Yt(Vp, [["__scopeId", "data-v-8048866c"]]), Zp = { style: { "margin-top": "6px", display: "flex", "justify-content": "flex-end" } }, Yp = e({ __name: "FormulaAround", emits: ["submit"], setup(e2, { emit: t2 }) {
  const o2 = t2, l2 = y([]), n2 = y(), a2 = y(false);
  let i2;
  const { createMessage: r2 } = Se(), u2 = (e3) => {
    a2.value = e3;
  };
  function c2(e3) {
  ...
  return (e3, t3) => (s(), h(d(V), { trigger: ["click"], open: a2.value, placement: "bottom", arrow: { pointAtCenter: true }, onOpenChange: u2 }, { overlay: M(() => [g(d(te), null, { default: M(() => [g(d(te).Item, { disabled: "", style: { padding: "0", cursor: "default" } }, { default: M(() => [g(d(pe), { title: "上层公式设置", bordered: false, style: { width: "240px" } }, { default: M(() => [g(Hp, { onFormulaChange: c2 }), g(d(K), { style: { width: "100%", "margin-top": "6px" }, placeholder: "选择作为哪个参数", class: "select-com", value: n2.value, "onUpdate:value": t3[0] || (t3[0] = (e4) => n2.value = e4), options: l2.value }, null, 8, ["value", "options"]), p("div", Zp, [g(d(ee), { size: "small", onClick: f2 }, { default: M(() => t3[1] || (t3[1] = [x(" 取消")])), _: 1 }), g(d(ee), { size: "small", type: "primary", style: { "margin-left": "6px" }, onClick: m2 }, { default: M(() => t3[2] || (t3[2] = [x(" 确定")])), _: 1 })])]), _: 1 })]), _: 1 })]), _: 1 })]), default: M(() => [g(d(ee), { style: { "margin-top": "-6px" }, size: "small", type: "link" }, { icon: M(() => [g(d(G), { placement: "left", title: "添加外层公式，当前层作为公式参数" }, { default: M(() => [g(d(Te), { icon: "ant-design:node-collapse-outlined" })]), _: 1 })]), _: 1 })]), _: 1 }, 8, ["open"]));
} }), Kp = { class: "formula" }, Xp = { class: "title" }, Jp = { key: 0, class: "add" }, ef = e({ __name: "FormulaConfig", props: Gp, emits: ["formulaChange"], setup(e2, { emit: t2 }) {
  const o2 = e2, l2 = t2, i2 = y(), u2 = y(), { prefixCls: C2 } = we("formula-config"), b2 = Mr(), k2 = y(), _2 = a({ type: Ja.param, value: "" }), $2 = y(0), L2 = (e3) => e3.startsWith("list"), I2 = (e3) => {
    var _a2, _b;
  ...
  function N2(e3) {
    T2(e3), k2.value = Up(e3), $2.value++, l2("formulaChange", k2.value);
  }
  function B2(e3) {
  ...
  function R2(e3) {
    ...
    T2(t3), l2("formulaChange", t3, true);
  }
  return n(() => o2.config, (e3) => {
    e3 && (k2.value = e3, function() {
      var _a2, _b, _c2;
      if (((_a2 = k2.value) == null ? void 0 : _a2.type) === Ja.formula) {
    ...
    return s(), r("div", { class: c(d(C2)) }, [p("div", Kp, [g(Wp, { "query-mode": o2.queryMode, class: "main", index: o2.index, "com-type": o2.comType, "param-type": o2.paramType, "param-value": o2.paramValue, config: k2.value, "sql-query": o2.sqlQuery, "show-radio": o2.showRadio, onInputChange: D2, onFormulaSelect: N2, onTypeChange: B2 }, null, 8, ["query-mode", "index", "com-type", "param-type", "param-value", "config", "sql-query", "show-radio"]), ((_a2 = k2.value) == null ? void 0 : _a2.type) !== d(Ja).formula || o2.queryMode ? f("", true) : (s(), h(Yp, { key: 0, onSubmit: R2 }))]), (s(true), r(m, null, v(i2.value, (e4, a2) => (s(), r("div", { key: `${$2.value}${a2}` }, [(s(true), r(m, null, v(I2(a2), (t4, i3) => {
      var _a3, _b, _c2, _d2, _e2, _f2, _g, _h2, _i2, _j;
      return s(), r("div", { class: "param", key: i3 }, [g(d(Te), { class: "icon", icon: "ant-design:arrow-up-outlined" }), p("span", Xp, [g(d(G), { placement: "left", title: u2.value ? u2.value[a2] : "" }, { default: M(() => [x(S(e4.name), 1)]), _: 2 }, 1032, ["title"])]), g(n2, { "query-mode": o2.queryMode, index: i3, "com-type": e4.comType, "param-type": e4.paramType, "param-value": e4.value, config: L2(e4.paramType) && ((_b = (_a3 = k2.value) == null ? void 0 : _a3.value) == null ? void 0 : _b.param) && ((_e2 = (_d2 = (_c2 = k2.value) == null ? void 0 : _c2.value) == null ? void 0 : _d2.param[a2]) == null ? void 0 : _e2.value) ? (_h2 = (_g = (_f2 = k2.value) == null ? void 0 : _f2.value) == null ? void 0 : _g.param[a2]) == null ? void 0 : _h2.value[i3] : (_j = (_i2 = k2.value) == null ? void 0 : _i2.value) == null ? void 0 : _j.param[a2], "sql-query": o2.sqlQuery, onFormulaChange: (t5) => function(e5, t6, o3, n3) {
        var _a4;
        ((_a4 = k2.value) == null ? void 0 : _a4.type) === Ja.formula && k2.value.value && k2.value.value.param && (L2(o3) ? k2.value.value.param[t6].value ? k2.value.value.param[t6].value[n3] = e5 : k2.value.value.param[t6].value = [e5] : k2.value.value.param[t6] = e5), l2("formulaChange", k2.value);
        ...
        var _a4;
        ((_a4 = k2.value) == null ? void 0 : _a4.type) === Ja.formula && k2.value.value && k2.value.value.param && (k2.value.value.param[e6].value.splice(t5, 1), $2.value++, l2("formulaChange", k2.value));
      }(a2, i3) }, { icon: M(() => [g(d(Te), { icon: "ant-design:delete-outlined" })]), _: 2 }, 1032, ["onClick"])) : f("", true)]);
    }), 128)), L2(e4.paramType) ? (s(), r("div", Jp, [g(d(ee), { type: "primary", onClick: (e5) => function(e6) {
      k2.value && (k2.value.value.param[e6].value ? k2.value.value.param[e6].value.push({ type: Ja.param, value: "" }) : k2.value.value.param[e6].value = [{ type: Ja.param, value: "" }]);
    }(a2) }, { default: M(() => t3[0] || (t3[0] = [x("增加参数")])), _: 2 }, 1032, ["onClick"])])) : f("", true)]))), 128))], 2);
  };
} }), tf = Yt(ef, [["__scopeId", "data-v-aad21227"]]), of = e({ __name: "ParamsModal", emits: ["register", "submit"], setup(e2, { emit: t2 }) {
  const o2 = t2, [l2, { resetFields: n2, setFieldsValue: a2, validate: i2, resetSchema: r2 }] = Ct({ name: "model_form_item", labelWidth: 180, baseColProps: { span: 24 }, showActionButtonGroup: false, autoSubmitOnEnter: true }), u2 = Mr(), c2 = y(), [p2, { setModalProps: f2, closeModal: m2 }] = yt(async (e3) => {
  ...
  return (e3, t3) => (s(), h(d(gt), $(e3.$attrs, { onRegister: d(p2), title: "输入参数值", onOk: v2 }), { default: M(() => [g(d(vt), { onRegister: d(l2), onSubmit: v2 }, null, 8, ["onRegister"])]), _: 1 }, 16, ["onRegister"]));
} }), lf = e({ __name: "FormulaTest", props: { open: { type: Boolean, default: false }, curQuery: { type: Object }, curParams: { type: Object } }, emits: ["close"], setup(e2, { emit: t2 }) {
  const o2 = e2, l2 = t2, a2 = xe().getThemeColor, i2 = Mr(), c2 = y(false);
  n(() => o2.open, (e3) => {
    c2.value = e3, e3 && p2();
  });
  const p2 = Ie((e3) => {
    let t3 = 1, l3 = 10;
    e3 && (t3 = e3.current, l3 = e3.pageSize), Di({ sourceId: i2.getSourceId, sourceType: i2.getSourceType, sqlQuery: o2.curQuery || {}, params: o2.curParams }, { pageNum: t3, pageSize: l3 }).then((e4) => {
  ...
  }, 200), [m2, { setColumns: v2, setTableData: C2, setPagination: b2 }] = kt({ bordered: true, showIndexColumn: false, isCanResizeParent: true, canResize: true, onChange: p2 }), k2 = () => {
    c2.value = false, l2("close", true);
  };
  return (e3, t3) => (s(), h(d(fe), { title: "测试结果", placement: "bottom", open: c2.value, onClose: k2, height: 450 }, { default: M(() => [g(d(wt), { onRegister: d(m2) }, { bodyCell: M(({ column: e4, record: t4 }) => [(e4 == null ? void 0 : e4.key) === d(wr) ? (s(), r("div", { key: 0, style: u({ color: d(a2) }) }, S(t4[d(wr)]), 5)) : f("", true)]), _: 1 }, 8, ["onRegister"])]), _: 1 }, 8, ["open"]));
} }), nf = e({ __name: "FormulaColModal", props: { queryMode: { type: Boolean, default: false } }, emits: ["submit", "register"], setup(e2, { emit: t2 }) {
  const o2 = t2, l2 = y(), n2 = y(), i2 = y(), u2 = a({ type: Ja.formula, value: null });
  let c2;
  const p2 = y(0), v2 = Mr(), [C2, { openModal: b2 }] = mt(), k2 = y(), w2 = y(), S2 = y(false), _2 = (e3) => {
    let t3 = "";
    return Array.isArray(e3.value) ? (e3.value.forEach((e4) => {
      t3 += `${_2(e4)},`;
    }), t3.endsWith(",") ? t3 = t3.slice(0, Math.max(0, t3.lastIndexOf(","))) : t3) : e3.type === Ja.formula && e3.value ? (t3 = `${e3.value.cnName}(`, e3.value.param.forEach((e4) => {
      t3 += `${_2(e4)},`;
    }), t3.endsWith(",") && (t3 = t3.slice(0, Math.max(0, t3.lastIndexOf(",")))), t3 += ")", t3) : (e3.type === Ja.param && (t3 += e3.value instanceof Object ? e3.value[e3.value[Qp]] : e3.value), t3);
  }, [L2, { setModalProps: I2, closeModal: D2 }] = yt(async (e3) => {
  ...
  async function T2() {
    try {
      if (!u2.value) return;
      D2(), o2("submit", n2.value, l2.value, { ...u2, name: i2.value });
    } catch (e3) {
    }
  }
  function N2(e3, t3) {
  ...
  function B2() {
    k2.value = { schema: c2.schema, sourceTable: c2.sourceTable, joins: c2.joins, sqlQuery: c2 == null ? void 0 : c2.sqlQuery, customColumns: [{ name: wr, expressions: [{ type: Ja.formula, value: u2.value }] }] };
    const e3 = Ir(k2.value || {});
    e3 && e3.size > 0 ? b2(true, { params: e3, level: 1 }) : R2({});
  }
  function R2(e3) {
    w2.value = e3, S2.value = true;
  }
  return (t3, o3) => (s(), r(m, null, [g(d(gt), $(t3.$attrs, { onRegister: d(L2), title: "公式配置", onOk: T2 }), { centerFooter: M(() => [g(d(ee), { onClick: B2, type: "primary", danger: "" }, { default: M(() => o3[1] || (o3[1] = [x("测试")])), _: 1 })]), default: M(() => [i2.value ? (s(), h(d(he), { key: 0, class: "mb-2", message: i2.value, type: "info", "show-icon": "" }, { icon: M(() => [g(d(Te), { icon: "ant-design:function-outlined" })]), _: 1 }, 8, ["message"])) : f("", true), (s(), h(tf, { "query-mode": e2.queryMode, key: p2.value, "sql-query": d(c2), "show-radio": false, config: u2, onFormulaChange: N2 }, null, 8, ["query-mode", "sql-query", "config"])), g(lf, { "cur-query": k2.value, "cur-params": w2.value, open: S2.value, onClose: o3[0] || (o3[0] = () => S2.value = false) }, null, 8, ["cur-query", "cur-params", "open"])]), _: 1 }, 16, ["onRegister"]), g(of, { onRegister: d(C2), onSubmit: R2 }, null, 8, ["onRegister"])], 64));
} }), af = e({ __name: "CustomColModal", props: { sqlQuery: { type: Object }, queryMode: { type: Boolean, default: false } }, emits: ["submit", "register"], setup(e2, { emit: t2 }) {
  const o2 = e2, l2 = t2, n2 = y(true), i2 = xe().getThemeColor, { prefixCls: u2 } = we("col-modal"), f2 = y([]), C2 = a({ formula: { key: "formula", name: "公式", icon: "icon-mfish-sum", color: "#cd201f" }, add: { key: "add", name: "加号", icon: "icon-mfish-add", color: i2 }, subtract: { key: "subtract", name: "减号", icon: "icon-mfish-subtract", color: i2, slot: true }, multiply: { key: "multiply", name: "乘号", icon: "icon-mfish-multiply", color: i2, slot: true }, divide: { key: "divide", name: "除号", icon: "icon-mfish-divide", color: i2, slot: true }, leftBracket: { key: "leftBracket", name: "左括号", icon: "icon-mfish-left-bracket", color: i2, slot: true }, rightBracket: { key: "rightBracket", name: "右括号", icon: "icon-mfish-right-bracket", color: i2, slot: true } }), b2 = a({ formula: { type: Ja.formula, value: "" }, add: { type: Ja.operator, name: "add", value: "+", slot: true }, subtract: { type: Ja.operator, name: "subtract", value: "-", slot: true }, multiply: { type: Ja.operator, name: "multiply", value: "*", slot: true }, divide: { type: Ja.operator, name: "divide", value: "/", slot: true }, leftBracket: { type: Ja.operator, name: "leftBracket", value: "(", slot: true }, rightBracket: { type: Ja.operator, name: "rightBracket", value: ")", slot: true } }), k2 = [{ field: "customCol", label: "自定义列", component: "Input", slot: "customCol" }, { field: "name", label: "名称", component: "Input", componentProps: { disabled: o2.queryMode, placeholder: "请输入名称", maxlength: 30, showCount: true }, required: true }], [S2, { resetFields: _2, setFieldsValue: L2, validate: I2 }] = Ct({ name: "model_form_item", labelWidth: 100, baseColProps: { span: 24 }, schemas: k2, showActionButtonGroup: false, autoSubmitOnEnter: true }), [D2, { openModal: T2 }] = mt(), [N2, { openModal: B2 }] = mt(), R2 = y();
  const [A2, { setModalProps: O2, closeModal: j2 }] = yt(async (e3) => {
  ...
  }), [z2, { openModal: F2 }] = mt(), P2 = y(), E2 = y(), H2 = y(false);
  async function Q2() {
    try {
      const e3 = await I2();
  ...
  function U2(e3, t3, l3) {
    t3.type !== Ja.operator ? (l3(false, t3), B2(true, { index: e3, sqlQuery: { ...o2.sqlQuery }, isUpdate: false })) : l3(true, t3);
  }
  function V2() {
    T2(true, { sqlQuery: { ...o2.sqlQuery }, isUpdate: false });
  }
  function W2(e3, t3) {
    t3.type === Ja.field && T2(true, { index: e3, field: t3, sqlQuery: { ...o2.sqlQuery }, isUpdate: true }), t3.type === Ja.formula && B2(true, { index: e3, formulaConfig: t3, sqlQuery: { ...o2.sqlQuery }, isUpdate: true });
  }
  function Z2(e3, t3) {
  ...
  function K2() {
    var _a2, _b, _c2, _d2;
    P2.value = { schema: (_a2 = o2.sqlQuery) == null ? void 0 : _a2.schema, sourceTable: (_b = o2.sqlQuery) == null ? void 0 : _b.sourceTable, joins: (_c2 = o2.sqlQuery) == null ? void 0 : _c2.joins, sqlQuery: (_d2 = o2.sqlQuery) == null ? void 0 : _d2.sqlQuery, customColumns: [{ name: wr, expressions: f2.value }] };
    const e3 = Ir(P2.value || {});
    e3 && e3.size > 0 ? F2(true, { params: e3, level: 1 }) : X2({});
  }
  function X2(e3) {
    E2.value = e3, H2.value = true;
  }
  return (t3, l3) => {
    const n3 = w("AButton");
    return s(), r(m, null, [g(d(gt), $(t3.$attrs, { onRegister: d(A2), title: "自定义列配置", onOk: Q2 }), { centerFooter: M(() => [g(n3, { onClick: K2, type: "primary", danger: "" }, { default: M(() => l3[1] || (l3[1] = [x("测试")])), _: 1 })]), default: M(() => [g(d(vt), { onRegister: d(S2), onSubmit: Q2 }, { customCol: M(() => [g(d(ht), { "query-mode": e2.queryMode, "add-title": "添加字段", items: f2.value, onBlockBuild: U2, onAddBlock: V2, onEditBlock: W2 }, q({ tag: M(({ element: e3 }) => [p("div", { class: c(`${d(u2)}-slot-tag`) }, [g(d(Ne), { icon: C2[e3.name].icon, color: d(i2) }, null, 8, ["icon", "color"])], 2)]), _: 2 }, [e2.queryMode ? void 0 : { name: "headerBar", fn: M(({ headerCallBack: e3 }) => [(s(true), r(m, null, v(C2, (t4, l4) => (s(), h(d(G), { key: l4, title: t4.name }, { default: M(() => [g(d(ge), { style: { cursor: "pointer" }, color: t4.color, draggable: "true", onClick: (l5) => function(e4, t5) {
      t5.type !== Ja.operator ? B2(true, { index: void 0, sqlQuery: o2.sqlQuery, isUpdate: false }) : e4(t5);
    }(e3, b2[t4.key]), onDragstart: (e4) => function(e5, t5) {
      e5.dataTransfer.setData("Text", JSON.stringify(b2[t5.key])), e5.stopPropagation();
    }(e4, t4) }, { icon: M(() => [g(d(Ne), { icon: t4.icon }, null, 8, ["icon"])]), _: 2 }, 1032, ["color", "onClick", "onDragstart"])]), _: 2 }, 1032, ["title"]))), 128))]), key: "0" }]), 1032, ["query-mode", "items"])]), _: 1 }, 8, ["onRegister"]), g(Ap, { onRegister: d(D2), onSubmit: Z2 }, null, 8, ["onRegister"]), g(nf, { "query-mode": e2.queryMode, onRegister: d(N2), onSubmit: Y2 }, null, 8, ["query-mode", "onRegister"]), g(lf, { "cur-query": P2.value, "cur-params": E2.value, open: H2.value, onClose: l3[0] || (l3[0] = () => H2.value = false) }, null, 8, ["cur-query", "cur-params", "open"])]), _: 1 }, 16, ["onRegister"]), g(of, { onRegister: d(z2), onSubmit: X2 }, null, 8, ["onRegister"])], 64);
  };
} }), rf = Yt(af, [["__scopeId", "data-v-93a6f597"]]), sf = e({ __name: "TableCustomCol", props: { sqlQuery: { type: Object }, level: { type: Number, default: 1 }, configMitt: { type: Object }, queryMode: { type: Boolean, default: false } }, emits: ["customColChange"], setup(e2, { emit: t2 }) {
  const o2 = e2, a2 = t2, i2 = y([]);
  function u2() {
    a2("customColChange");
  }
  const [c2, { openModal: p2 }] = mt(), m2 = l(() => {
    var _a2;
    return void 0 === ((_a2 = o2.sqlQuery) == null ? void 0 : _a2.customColumns) && $r(o2.sqlQuery, o2.level) && !o2.queryMode;
  });
  function v2() {
  ...
  function x2(e3) {
    Lr(o2.configMitt, { ...o2.sqlQuery, customColumns: i2.value, aggregates: [], groups: [], orders: [], limit: void 0 }, e3);
  }
  function S2(e3, t3) {
  ...
  return n(() => {
    ...
    var _a2;
    return s(), r("div", null, [void 0 !== ((_a2 = e2.sqlQuery) == null ? void 0 : _a2.customColumns) ? (s(), h(ap, { key: 0, "query-mode": e2.queryMode, icon: "ant-design:insert-row-right-outlined", title: "自定义列", onDeleteConfig: u2, onExecute: x2, full: true }, { config: M(() => [g(d(ht), { "query-mode": e2.queryMode, items: i2.value, "add-title": "添加自定义列", onAddBlock: v2, onEditBlock: C2, onDragChange: b2 }, null, 8, ["query-mode", "items"])]), _: 1 }, 8, ["query-mode"])) : f("", true), m2.value ? (s(), h(pp, { key: 1, class: "config-button", title: "自定义列", icon: "ant-design:insert-row-right-outlined", level: 0, onClick: k(w2, ["prevent"]) })) : f("", true), g(rf, { "query-mode": e2.queryMode, onRegister: d(c2), "sql-query": e2.sqlQuery, onSubmit: S2 }, null, 8, ["query-mode", "onRegister", "sql-query"])]);
  };
} }), uf = { class: "btn-bar" }, cf = Yt(e({ __name: "ApiConfig", props: { configKey: { type: Number, default: 0, require: true }, sqlQuery: { type: Object, require: true, default: void 0 }, level: { type: Number, default: 1 }, configMitt: { type: Object, default: void 0 }, queryMode: { type: Boolean, default: false } }, emits: ["configChange"], setup(e2, { emit: t2 }) {
  const o2 = e2, n2 = t2, { sqlQuery: a2 } = A(o2), { prefixCls: i2 } = we("api-config"), m2 = xe().getThemeColor, v2 = y(), C2 = y(), { focused: b2 } = De(v2), { focused: M2 } = De(C2), k2 = Mr(), x2 = y(false), S2 = l(() => {
    var _a2;
    ...
    var _a2;
    return (void 0 === ((_a2 = o2.sqlQuery) == null ? void 0 : _a2.aggregates) || $r(o2.sqlQuery, o2.level)) && !o2.queryMode;
  }), I2 = l(() => {
    var _a2;
  ...
  function N2(e3) {
    ...
    E2(t3) || (n2("configChange", t3), k2.setFieldsChange(o2.level));
  }
  function B2(e3) {
    const t3 = { ...o2.sqlQuery, filters: e3 };
    E2(t3) || n2("configChange", t3);
  }
  function R2(e3) {
  ...
  function j2() {
    ...
    E2(e3) || (n2("configChange", e3), k2.setFieldsChange(o2.level));
  }
  function z2(e3) {
  ...
  async function H2() {
    ...
    return e3.fields = await Sr(e3), e3;
  }
  async function Q2() {
    var _a2, _b;
    if ($r(o2.sqlQuery, o2.level)) {
      const e3 = await H2();
      return e3.joins = [], dp(e3.joins), void n2("configChange", e3, true);
    }
    ((_a2 = a2 == null ? void 0 : a2.value) == null ? void 0 : _a2.joins) && dp((_b = a2 == null ? void 0 : a2.value) == null ? void 0 : _b.joins);
  }
  function G2() {
    W2(["filters"]);
  }
  ...
  async function W2(e3) {
    ...
    if ($r(o2.sqlQuery, o2.level)) return t3 = await H2(), void l2(t3, true);
  ...
  function K2() {
    var _a2;
    (_a2 = o2.configMitt) == null ? void 0 : _a2.emit(xr.QUERY_SQL, o2.sqlQuery), M2.value = false;
  }
  function X2() {
    Lr(o2.configMitt, o2.sqlQuery, o2.level), b2.value = false;
  }
  return (t3, o3) => {
    var _a2;
    const l2 = w("ApiConfig", true);
    return s(), r("div", { class: c([d(i2), e2.level < d(k2).getLevel ? "inner" : "", d(k2).getShowData && d(k2).getShowDataLevel === e2.level ? "active" : ""]) }, [d(k2).getShowData && d(k2).getShowDataLevel === e2.level ? (s(), r("div", { key: 0, class: "select", style: u({ backgroundColor: d(m2) }) }, [g(d(Te), { icon: "ant-design:check-outlined", color: "#FFFFFF", size: 12 })], 4)) : f("", true), void 0 !== ((_a2 = d(a2)) == null ? void 0 : _a2.sqlQuery) ? (s(), h(l2, { key: 1, "config-key": e2.configKey, "query-mode": e2.queryMode, "sql-query": d(a2).sqlQuery, level: e2.level - 1, "config-mitt": e2.configMitt, onConfigChange: P2 }, null, 8, ["config-key", "query-mode", "sql-query", "level", "config-mitt"])) : f("", true), (s(), h(rp, { key: e2.configKey + d(k2).getFieldsChange, class: "config", "sql-query": d(a2), "query-mode": e2.queryMode, "config-mitt": e2.configMitt, onFieldChange: T2 }, null, 8, ["sql-query", "query-mode", "config-mitt"])), (s(), h(yp, { level: e2.level, key: e2.configKey, class: "config", "sql-query": d(a2), "config-mitt": e2.configMitt, "query-mode": e2.queryMode, onJoinChange: N2 }, null, 8, ["level", "sql-query", "config-mitt", "query-mode"])), (s(), h(Lp, { "query-mode": e2.queryMode, level: e2.level, key: e2.configKey, class: "config", "sql-query": d(a2), "config-mitt": e2.configMitt, onFilterChange: B2 }, null, 8, ["query-mode", "level", "sql-query", "config-mitt"])), (s(), h(sf, { "query-mode": e2.queryMode, level: e2.level, key: e2.configKey, class: "config", "sql-query": d(a2), "config-mitt": e2.configMitt, onCustomColChange: R2 }, null, 8, ["query-mode", "level", "sql-query", "config-mitt"])), (s(), h(Np, { "query-mode": e2.queryMode, key: e2.configKey, class: "config", "sql-query": d(a2), "config-mitt": e2.configMitt, onAggregateChange: q2, onGroupChange: O2, onDeleteAggregate: j2 }, null, 8, ["query-mode", "sql-query", "config-mitt"])), (s(), h(qp, { "query-mode": e2.queryMode, key: e2.configKey, class: "config", "sql-query": d(a2), "config-mitt": e2.configMitt, onOrderChange: z2 }, null, 8, ["query-mode", "sql-query", "config-mitt"])), (s(), h(mp, { "query-mode": e2.queryMode, key: e2.configKey, class: "config", "sql-query": d(a2), "config-mitt": e2.configMitt, onLimitChange: F2 }, null, 8, ["query-mode", "sql-query", "config-mitt"])), p("div", uf, [S2.value ? (s(), h(pp, { key: 0, class: "config-button", title: "关联", iconfont: "icon-mfish-join", level: e2.level, onClick: Q2 }, null, 8, ["level"])) : f("", true), _2.value ? (s(), h(pp, { key: 1, class: "config-button", title: "过滤", icon: "ant-design:filter-outlined", level: e2.level, onClick: G2 }, null, 8, ["level"])) : f("", true), $2.value ? (s(), h(pp, { key: 2, class: "config-button", title: "自定义列", icon: "ant-design:insert-row-right-outlined", level: e2.level, onClick: U2 }, null, 8, ["level"])) : f("", true), L2.value ? (s(), h(pp, { key: 3, class: "config-button", title: "聚合", iconfont: "icon-mfish-sum", level: e2.level, onClick: V2 }, null, 8, ["level"])) : f("", true), I2.value ? (s(), h(pp, { key: 4, class: "config-button", title: "排序", icon: "ant-design:sort-ascending-outlined", level: e2.level, onClick: Z2 }, null, 8, ["level"])) : f("", true), D2.value ? (s(), h(pp, { key: 5, class: "config-button", title: "行数", icon: "ant-design:column-height-outlined", level: e2.level, onClick: Y2 }, null, 8, ["level"])) : f("", true), g(pp, { class: "config-button", ref_key: "querySqlRef", ref: C2, style: u({ fontWeight: "bold", border: `1px dashed ${d(m2)}` }), title: "查看SQL", icon: "ant-design:console-sql-outlined", level: e2.level, onClick: K2 }, null, 8, ["style", "level"]), g(pp, { class: "config-button", style: u({ fontWeight: "bold", border: `1px dashed ${d(m2)}` }), ref_key: "queryDataRef", ref: v2, title: "执行", onMouseenter: o3[0] || (o3[0] = (e3) => x2.value = true), onMouseleave: o3[1] || (o3[1] = (e3) => x2.value = false), icon: !d(k2).getShowData || d(k2).getShowDataLevel !== e2.level || x2.value ? "ant-design:caret-right-outlined" : "ant-design:check-outlined", level: e2.level, onClick: X2 }, null, 8, ["style", "icon", "level"])])], 2);
  };
} }), [["__scopeId", "data-v-32695b70"]]), df = { key: 0 }, pf = { key: 1 }, ff = e({ __name: "SqlQueryModal", setup(e2) {
  const t2 = y(500), o2 = y(""), [l2, { setModalProps: n2 }] = yt(async (e3) => {
    n2({ width: "800px" }), o2.value = e3;
  }), { copy: a2, copied: i2 } = Le({ legacy: true });
  function c2(e3) {
    t2.value = e3;
  }
  return (e3, n3) => {
    const p2 = w("AButton");
    return s(), h(d(gt), $(e3.$attrs, { onRegister: d(l2), title: "SQL查看", "show-ok-btn": false, "cancel-text": "关闭", onHeightChange: c2 }), { insertFooter: M(() => [g(p2, { onClick: n3[0] || (n3[0] = (e4) => d(a2)(o2.value)), color: d(i2) ? "success" : "primary" }, { default: M(() => [d(i2) ? (s(), r("span", pf, "已拷贝")) : (s(), r("span", df, "拷贝"))]), _: 1 }, 8, ["color"])]), default: M(() => [g(d(xt), { style: u({ height: `${t2.value}px` }), mode: d(St).JS, value: o2.value, "onUpdate:value": n3[1] || (n3[1] = (e4) => o2.value = e4), readonly: true }, null, 8, ["style", "mode", "value"])]), _: 1 }, 16, ["onRegister"]);
  };
} });
const hf = ["src"], gf = Yt(e({ name: "PreviewFrame", __name: "PreviewFrame", props: { frameSrc: $t.string.def(""), frameWidth: $t.number.def(400), frameHeight: $t.number.def(300) }, emits: ["message"], setup(e2, { expose: t2, emit: o2 }) {
  const n2 = e2, a2 = o2, r2 = y(true), f2 = y(), { prefixCls: g2 } = we("preview-frame"), m2 = l(() => ({ width: `${n2.frameWidth}px`, height: `${n2.frameHeight}px` }));
  function v2() {
    r2.value = false;
  }
  const C2 = (e3) => {
  ...
  } }), (t3, o3) => (s(), h(d(ft), null, { default: M(() => [p("iframe", { style: u(m2.value), src: e2.frameSrc, class: c(`${d(g2)}__main`), ref_key: "frameRef", ref: f2, onLoad: v2 }, null, 46, hf)]), _: 1 }));
} }), [["__scopeId", "data-v-1bb3fd25"]]), yf = { class: "share-title" }, mf = e({ __name: "ScreenPreview", props: { fixScale: { type: Number, default: 1 }, borderRadius: { type: String } }, setup(e2) {
  const t2 = e2, { prefixCls: n2 } = we("screen-preview"), { token: C2 } = E.useToken(), b2 = l(() => C2.value.colorBorder), k2 = o("fitDomRef"), w2 = y([]), { getAntdLocale: x2 } = _t(), $2 = Tr(), { antTheme: L2 } = _e(l(() => $2.getTheme)), I2 = y(false), D2 = y(""), T2 = y(window.innerWidth), N2 = y(window.innerHeight), B2 = y(), R2 = l(() => [...$2.getComponentList].reverse()), q2 = a({ width: 0, height: 0 }), A2 = l(() => ({ ...Dd($2.getCanvasConfig), width: $2.getCanvasConfig.width * q2.width * t2.fixScale + "px", height: $2.getCanvasConfig.height * q2.height * t2.fixScale + "px" })), F2 = (e3) => Ad({ ...e3.chartContain.dropInfo }), P2 = (e3) => {
    var _a2, _b;
    if (((_b = (_a2 = e3.chart) == null ? void 0 : _a2.events) == null ? void 0 : _b.emits) && e3.chart.events.emits.length > 0) {
      for (const t3 of e3.chart.events.emits) if (t3 === ns.CHART_CLICK || t3 === ns.CHART_DBLCLICK) return "allow-pointer";
    }
  }, H2 = (e3) => {
    const t3 = qd(e3);
    return t3.borderColor || (t3.borderColor = b2.value), Ee(t3, ["transform"]);
  ...
  return i(() => {
    setTimeout(() => {
      $2.loadingScreen().then(() => {
        ...
        }($2.getCanvasConfig.width, $2.getCanvasConfig.height, $2.getCanvasConfig.fitType, k2.value, U2, t2.fixScale);
        e3(), o2(), G2 = l2;
        const n3 = /* @__PURE__ */ new Map();
        for (let e4 = 0; e4 < R2.value.length; e4++) {
          const t3 = R2.value[e4];
          !t3.chartContain.animations || 0 === t3.chartContain.animations.length || w2.value.length <= e4 || (R2.value[e4].chartContain.show && R2.value[e4].chart.type !== Jr.MfCombine && (R2.value[e4].chartContain.show = false, n3.set(e4, R2.value[e4])), setTimeout(() => {
            n3.has(e4) && (n3.get(e4).chartContain.show = true), _d(w2.value[e4], t3.chartContain.animations).then();
          }, 600));
        }
      });
    }, 10), Id.on(zr.SHOW_MODAL, (e3) => {
      var _a2, _b, _c2, _d2, _e2, _f2;
      D2.value = e3.href, B2.value = ((_b = (_a2 = e3.jump) == null ? void 0 : _a2.page) == null ? void 0 : _b.name) || e3.href, T2.value = (_d2 = (_c2 = e3.jump) == null ? void 0 : _c2.page) == null ? void 0 : _d2.width, N2.value = (_f2 = (_e2 = e3.jump) == null ? void 0 : _e2.page) == null ? void 0 : _f2.height, I2.value = true;
    });
  }), _(() => {
    G2 && G2();
  }), (t3, o2) => (s(), h(d(ye), { locale: d(x2), theme: d(L2) }, { default: M(() => [g(d(ft), { class: c(d(n2)), style: u({ borderRadius: e2.borderRadius }) }, { default: M(() => [p("div", { class: "screen-wrap", style: u(A2.value) }, [p("div", { class: "screen-canvas", ref_key: "fitDomRef", ref: k2 }, [d($2).getCanvasConfig.watermark ? (s(), h(d(me), { key: 0, content: d($2).getCanvasConfig.watermark, style: u({ position: "absolute", width: `${d($2).getCanvasConfig.width}px`, height: `${d($2).getCanvasConfig.height}px` }) }, null, 8, ["content", "style"])) : f("", true), (s(true), r(m, null, v(R2.value, (e3, t4) => O((s(), r("div", { class: c(["chart-contain", P2(e3)]), key: e3 == null ? void 0 : e3.chart.id, style: u(F2(e3)) }, [p("div", { ref_for: true, ref: (e4) => function(e5, t5) {
    w2.value[t5] = e5;
  }(e4, t4), style: u(H2(e3)) }, [(s(), h(j((e3 == null ? void 0 : e3.chart) && e3.chart.type && d(Ld)[e3.chart.type].component), { class: "component", id: `com${e3 == null ? void 0 : e3.chart.id}`, chart: e3 == null ? void 0 : e3.chart, "chart-contain": e3 == null ? void 0 : e3.chartContain }, null, 8, ["id", "chart", "chart-contain"]))], 4)], 6)), [[z, e3 == null ? void 0 : e3.chartContain.show]])), 128))], 512)], 4)]), _: 1 }, 8, ["class", "style"]), g(d(ve), { open: I2.value, "onUpdate:open": o2[0] || (o2[0] = (e3) => I2.value = e3), "get-container": Q2, width: T2.value + 48, footer: null, mask: true, centered: true }, { title: M(() => [p("div", yf, S(B2.value), 1)]), default: M(() => [g(gf, { "frame-src": D2.value, "frame-width": T2.value, "frame-height": N2.value }, null, 8, ["frame-src", "frame-width", "frame-height"])]), _: 1 }, 8, ["open", "width"])]), _: 1 }, 8, ["locale", "theme"]));
} }), vf = Yt(mf, [["__scopeId", "data-v-8233c66d"]]), Cf = e({ __name: "ScreenSizeConfig", setup(e2) {
  const t2 = y("1k"), o2 = y([{ value: "720", label: "1280*720" }, { value: "1k", label: "1920*1080" }, { value: "2k", label: "2048*1152" }, { value: "4k", label: "4096*2160" }, { value: "自定义", label: "自定义" }]), l2 = Tr(), a2 = (e3) => {
    var _a2;
  ...
  const { prefixCls: i2 } = we("size-config");
  return (e3, n2) => {
    const r2 = w("AInput");
    return s(), h(d(Ce), { compact: "", class: c(d(i2)) }, { default: M(() => [g(d(K), { class: "select-size", value: t2.value, "onUpdate:value": n2[0] || (n2[0] = (e4) => t2.value = e4), options: o2.value, onChange: a2 }, null, 8, ["value", "options"]), g(d(Q), { value: d(l2).getCanvasConfig.width, "onUpdate:value": n2[1] || (n2[1] = (e4) => d(l2).getCanvasConfig.width = e4), class: "input-left", placeholder: "宽度", min: 50, max: 1e4, controls: false, maxlength: 5 }, { prefix: M(() => n2[3] || (n2[3] = [p("div", { class: "prefix" }, "W", -1)])), _: 1 }, 8, ["value"]), g(r2, { class: "input-split", placeholder: "*", disabled: "" }), g(d(Q), { value: d(l2).getCanvasConfig.height, "onUpdate:value": n2[2] || (n2[2] = (e4) => d(l2).getCanvasConfig.height = e4), class: "input-right", placeholder: "高度", min: 50, max: 1e4, controls: false, maxlength: 5 }, { prefix: M(() => n2[4] || (n2[4] = [p("div", { class: "prefix" }, "H", -1)])), _: 1 }, 8, ["value"])]), _: 1 }, 8, ["class"]);
  };
} }), bf = Yt(Cf, [["__scopeId", "data-v-148656f7"]]), Mf = e({ __name: "ParamsConfig", setup: (e2) => (e3, t2) => (s(), h(d(be), { description: "暂未开发", image: d(be).PRESENTED_IMAGE_SIMPLE }, null, 8, ["image"])) }), kf = { class: "title" }, wf = { class: "title" }, xf = { class: "title" }, Sf = e({ __name: "CanvasConfig", setup(e2) {
  const { prefixCls: t2 } = we("canvas-config"), o2 = Tr(), l2 = y(), n2 = [{ title: "深色", value: He.DARK }, { title: "浅色", value: He.LIGHT }, { title: "跟随平台", value: He.SYSTEM }], u2 = a({ [jr.FIT_BOX]: { name: "全屏铺满", icon: "carbon:fit-to-screen", description: "全屏铺满（设备与配置大屏比例不同时会变形）" }, [jr.FIT_ORIGINAL]: { name: "原始比例", icon: "carbon:center-to-fit", description: "原始比例（保持配置大屏比例，不填充空白处）" }, [jr.FIT_WIDTH_SCALE]: { name: "横向适配", icon: "carbon:fit-to-width", description: "横向适配-保持纵横比（横向铺满，允许纵向滚动，保持原有画布比例）" }, [jr.FIT_HEIGHT_SCALE]: { name: "纵向适配", icon: "carbon:fit-to-height", description: "纵向适配-保持纵横比（纵向铺满，允许横向滚动，保持原有画布比例）" }, [jr.FIT_WIDTH]: { name: "横向适配", icon: "carbon:fit-to-width", description: "横向适配（横向铺满，允许纵向滚动，会变形）" }, [jr.FIT_HEIGHT]: { name: "纵向适配", icon: "carbon:fit-to-height", description: "纵向适配（纵向铺满，允许横向滚动，会变形）" } });
  function f2(e3) {
    C2(e3), l2.value = e3;
  }
  function C2(e3) {
  ...
  return i(() => {
    ...
    const i2 = w("AInput");
    return s(), h(d(ft), null, { default: M(() => [p("div", { class: c(d(t2)) }, [a2[8] || (a2[8] = p("div", { class: "title" }, "大屏名称", -1)), g(i2, { value: d(o2).getCanvasConfig.screenName, "onUpdate:value": a2[0] || (a2[0] = (e4) => d(o2).getCanvasConfig.screenName = e4), "show-count": "", maxlength: 30 }, null, 8, ["value"]), a2[9] || (a2[9] = p("div", { class: "title" }, "大屏尺寸", -1)), g(bf), p("div", kf, [a2[5] || (a2[5] = x(" 主题 ")), g(d(Y), { size: "small", value: d(o2).getCanvasConfig.theme, "onUpdate:value": a2[1] || (a2[1] = (e4) => d(o2).getCanvasConfig.theme = e4) }, { default: M(() => [(s(), r(m, null, v(n2, (e4, t3) => g(d(Z), { key: t3, value: e4.value }, { default: M(() => [x(S(e4.title), 1)]), _: 2 }, 1032, ["value"])), 64))]), _: 1 }, 8, ["value"])]), a2[10] || (a2[10] = p("div", { class: "title" }, "背景颜色", -1)), g(d(qo), { value: l2.value, onConfirmChange: f2, onInputChange: C2 }, null, 8, ["value"]), a2[11] || (a2[11] = p("div", { class: "title" }, "背景图片", -1)), g(d(Lt), { "file-keys": d(o2).getCanvasConfig.backgroundImage, accepts: ".jpg,.jpeg,.png,.svg", "button-text": "上传背景", "max-count": 1, "is-pic": true, onSuccess: b2, onRemove: k2 }, null, 8, ["file-keys"]), p("div", wf, [a2[6] || (a2[6] = x(" 水印 ")), p("div", null, [g(i2, { value: d(o2).getCanvasConfig.watermark, "onUpdate:value": a2[2] || (a2[2] = (e4) => d(o2).getCanvasConfig.watermark = e4), placeholder: "清空无水印", "show-count": "", maxlength: 50 }, null, 8, ["value"])])]), p("div", xf, [a2[7] || (a2[7] = x(" 适配方式 ")), p("div", null, [g(d(Y), { value: d(o2).getCanvasConfig.fitType, "onUpdate:value": a2[3] || (a2[3] = (e4) => d(o2).getCanvasConfig.fitType = e4) }, { default: M(() => [(s(true), r(m, null, v(Object.keys(u2), (e4) => (s(), h(d(G), { key: e4, placement: "bottomRight", title: u2[e4].description }, { default: M(() => [g(d(Z), { class: c(`${d(t2)}-radio-button`), value: e4 }, { default: M(() => [g(d(Te), { icon: u2[e4].icon }, null, 8, ["icon"])]), _: 2 }, 1032, ["class", "value"])]), _: 2 }, 1032, ["title"]))), 128))]), _: 1 }, 8, ["value"])])]), a2[12] || (a2[12] = p("div", { class: "title" }, "全局参数", -1)), g(Mf), a2[13] || (a2[13] = p("div", { class: "title" }, " 描述", -1)), g(d(Me), { value: d(o2).getCanvasConfig.screenDesc, "onUpdate:value": a2[4] || (a2[4] = (e4) => d(o2).getCanvasConfig.screenDesc = e4), "show-count": "", maxlength: 200 }, null, 8, ["value"])], 2)]), _: 1 });
  };
} }), _f = Yt(Sf, [["__scopeId", "data-v-b101a035"]]), $f = { class: "header" }, Lf = e({ name: "FrameConfig", __name: "index", props: { collapsed: $t.bool.def(false) }, setup(e2, { expose: t2 }) {
  const o2 = e2, { prefixCls: a2 } = we("frame-config"), i2 = l(() => [a2, { collapsed: o2.collapsed }]), u2 = y(), m2 = y(), v2 = y(), C2 = Tr(), b2 = y();
  function k2(e3) {
    if (e3) {
      const t3 = Ld[e3.chart.type].config;
      u2.value = Object.keys(t3).filter((e4) => t3[e4].show).map((e4) => t3[e4]);
    } else u2.value = [{ value: "画布配置", payload: { icon: "carbon:gui-management", component: T(_f) } }];
  ...
  return n([() => C2.getCurComponent, () => {
    ...
    b2.value = e3;
  } }), (e3, t3) => (s(), r("div", { class: c(i2.value) }, [p("div", $f, [g(d(U), { value: m2.value, options: u2.value, onChange: w2, block: "" }, { label: M(({ value: e4, payload: t4 }) => [t4.icon ? (s(), h(d(Te), { key: 0, icon: t4.icon }, null, 8, ["icon"])) : f("", true), x(" " + S(e4), 1)]), _: 1 }, 8, ["value", "options"])]), (s(), h(j(v2.value), { "select-data": b2.value }, { "data-select": M(() => [B(e3.$slots, "data-select", { curSelect: b2.value })]), _: 3 }, 8, ["select-data"]))], 2));
} }), If = { class: "action" }, Df = { class: "ac-int" }, Tf = e({ __name: "CanvasRulerLine", props: { lineValue: { type: Number, default: 0 }, lineList: { type: Array, default: () => [] } }, setup(e2) {
  const t2 = e2, { lineList: o2 } = t2, l2 = Tr(), { prefixCls: n2 } = we("canvas-ruler-line"), a2 = xe().getThemeColor;
  return (e3, t3) => (s(), r("div", { class: c(d(n2)) }, [(s(true), r(m, null, v(d(o2), (e4, t4) => (s(), r("div", { class: c(["line-item", { x: "x" === e4.position, y: "y" === e4.position }]), style: u({ left: "x" === e4.position ? `${Math.round(e4.offset * d(l2).getScale)}px` : 0, top: "y" === e4.position ? `${Math.round(e4.offset * d(l2).getScale)}px` : 0 }), key: `${e4}${t4}` }, [p("div", If, [g(d(Te), { class: "ac-close", icon: "ant-design:close-outline", color: d(a2), onClick: (l3) => function(e5, t5) {
    o2.splice(t5, 1);
  }(e4.position, t4) }, null, 8, ["color", "onClick"]), p("span", Df, S(e4.value), 1)])], 6))), 128))], 2));
} }), Nf = Yt(Tf, [["__scopeId", "data-v-91bb6b29"]]), Bf = { id: "horn", class: "horn" }, Rf = ["id", "width", "height"], qf = { class: "indicator-value" }, Af = ["id", "height", "width"], Of = { class: "indicator-value" }, jf = e({ __name: "CanvasRuler", props: { rulerXLeft: { type: Number, default: 0 }, rulerYTop: { type: Number, default: 0 }, rulerX: { type: Number, default: 5e3 }, rulerY: { type: Number, default: 3e3 }, drift: { type: Number, default: 0 }, rulerHeight: { type: Number, default: 28 }, rulerWidth: { type: Number, default: 38 } }, setup(e2) {
  const t2 = e2, a2 = Tr(), { prefixCls: i2 } = we("canvas-ruler"), f2 = y(0), m2 = y(true), v2 = T({}), b2 = T({}), k2 = l(() => `${t2.rulerXLeft}px`), w2 = l(() => `${t2.rulerYTop}px`), x2 = o("rulerXRef"), _2 = o("rulerYRef"), $2 = l(() => xe().getDarkMode.value === He.DARK ? "#999" : "#666");
  n(() => xe().getDarkMode.value, (e3, t3) => {
    e3 !== t3 && C(() => {
      j2();
    });
  }), n([() => a2.getScale, () => t2.rulerX, () => t2.rulerY], ([e3, t3, o2], [l2, n2, a3]) => {
    l2 ? (e3 !== l2 && C(() => j2()), t3 !== n2 && C(() => E2()), o2 !== a3 && C(() => H2())) : C(() => j2());
  }, { immediate: true });
  const L2 = l(() => t2.rulerX * a2.getScale), I2 = l(() => t2.rulerY * a2.getScale), D2 = (e3) => {
  ...
  const F2 = (e3, t3, o2, l2, n2) => {
    P2(e3, t3, $2.value, a2.getScale, o2, l2, n2);
  }, P2 = (e3, t3, o2, l2, n2, a3, i3) => {
  ...
  return (t3, o2) => (s(), r("div", { class: c(d(i2)) }, [p("div", Bf, [g(d(G), { title: (m2.value ? "隐藏" : "显示") + "辅助线" }, { default: M(() => [m2.value ? (s(), h(d(Te), { key: 0, class: "icon", icon: "ant-design:eye-outlined", onClick: Q2 })) : (s(), h(d(Te), { key: 1, class: "icon", icon: "ant-design:eye-invisible-outlined", onClick: Q2 }))]), _: 1 }, 8, ["title"])]), p("div", { class: "ruler rulerX", style: u({ width: `${L2.value}px`, left: k2.value }) }, [p("canvas", { ref_key: "rulerXRef", ref: x2, id: `${d(i2)}-x`, width: L2.value, height: e2.rulerHeight, onMousemove: D2, onMouseleave: N2, onClick: B2 }, null, 40, Rf), p("div", { class: "indicator x", style: u(v2.value) }, [p("div", qf, S(f2.value), 1)], 4), O(g(Nf, { "line-value": f2.value, "line-list": d(a2).getGuideLine.lineListY }, null, 8, ["line-value", "line-list"]), [[z, m2.value]])], 4), p("div", { class: "ruler rulerY", style: u({ height: `${I2.value}px`, top: w2.value }) }, [p("canvas", { ref_key: "rulerYRef", ref: _2, id: `${d(i2)}-y`, height: I2.value, width: e2.rulerWidth, onMousemove: R2, onMouseleave: q2, onClick: A2 }, null, 40, Af), p("div", { class: "indicator y", style: u(b2.value) }, [p("div", Of, S(f2.value), 1)], 4), O(g(Nf, { "line-value": f2.value, "line-list": d(a2).getGuideLine.lineListX }, null, 8, ["line-value", "line-list"]), [[z, m2.value]])], 4)], 2));
} }), zf = Yt(jf, [["__scopeId", "data-v-eeb86e63"]]), Ff = e({ __name: "CanvasBackground", props: { drift: { type: Number, default: 0 }, width: { type: Number, default: 9e3 }, height: { type: Number, default: 6e3 }, ruler: { type: Object, default: () => ({ height: 28, width: 38 }) } }, setup(e2) {
  const t2 = e2, o2 = Tr(), n2 = Vd(), a2 = l(() => t2.width * o2.getScale + "px"), i2 = l(() => t2.height * o2.getScale + "px"), { prefixCls: p2 } = we("canvas-background");
  function f2(e3) {
    o2.cleanCurComponent(), 2 !== e3.button && n2.setShowMenu(false);
  }
  return (t3, l2) => (s(), r("div", { class: c(d(p2)), style: u({ width: a2.value, height: i2.value, padding: `${e2.drift * d(o2).getScale}px 0 0 ${e2.drift * d(o2).getScale}px` }), onMousedown: f2 }, [B(t3.$slots, "default", {}, void 0, true)], 38));
} }), Pf = Yt(Ff, [["__scopeId", "data-v-2efcd248"]]), Ef = { class: "scale-add-reduce" }, Hf = { class: "scale-lock" }, Qf = e({ __name: "CanvasScale", props: { scale: { type: Number, default: 100 }, minScale: { type: Number, default: 20 }, maxScale: { type: Number, default: 500 } }, emits: ["addScale", "reduceScale", "changeScale", "autoScale"], setup(e2, { emit: t2 }) {
  const o2 = t2, l2 = y(true), { prefixCls: n2 } = we("canvas-scale"), a2 = () => {
    o2("reduceScale");
  }, i2 = () => {
    o2("addScale");
  }, u2 = (e3) => {
  ...
  return (t3, o3) => (s(), r("div", { class: c(d(n2)) }, [p("div", Ef, [g(d(Te), { class: "scale-icon", title: "放大", size: 18, icon: "ant-design:plus-outlined", onClick: i2 }), g(d(Q), { size: "small", bordered: false, value: e2.scale, max: e2.maxScale, min: e2.minScale, step: 5, controls: false, formatter: (e3) => `${Math.round(100 * e3)}`, onChange: u2 }, null, 8, ["value", "max", "min", "formatter"]), o3[2] || (o3[2] = p("div", { class: "scale-sign" }, "%", -1)), g(d(Te), { class: "scale-icon", title: "缩小", size: 18, icon: "ant-design:minus-outlined", onClick: a2 })]), p("div", Hf, [g(d(G), { title: l2.value ? "锁定自动比例" : "解锁自动比例", placement: "right" }, { default: M(() => [l2.value ? (s(), h(d(Te), { key: 0, icon: "ant-design:unlock-outlined", onClick: o3[0] || (o3[0] = (e3) => f2(false)) })) : (s(), h(d(Te), { key: 1, icon: "ant-design:lock-outlined", onClick: o3[1] || (o3[1] = (e3) => f2(true)) }))]), _: 1 }, 8, ["title"])])], 2));
} }), Gf = e({ __name: "ShortcutViewModal", setup(e2) {
  const [t2, { setModalProps: o2 }] = yt(async () => {
    o2({ confirmLoading: false, width: "800px", cancelText: "关闭", showOkBtn: false });
  }), [l2] = kt({ columns: [{ title: "功能", dataIndex: "label", width: 120 }, { title: "Win快捷键", dataIndex: "win", width: 120 }, { title: "Mac快捷键", dataIndex: "mac", width: 120 }], dataSource: [{ label: "拖拽 / 放大缩小画布", win: "CTRL + 🖱️拖动 / 滚动", mac: "⌘ + 🖱️拖动 / 滚动", macSource: true }, { label: "向 上/右/下/左 移动", win: "CTRL + ↑ / → / ↓ / ←", mac: "⌘ + ↑" }, { label: "锁定/解锁", win: "CTRL + L / CTRL + SHIFT+ L", mac: "⌘ + L / ⌘ + ⇧ + L" }, { label: "隐藏/显示", win: "CTRL + H / CTRL + SHIFT + H", mac: "⌘ + H / ⌘ + ⇧ + H" }, { label: "组合/拆分", win: "CTRL + G / CTRL + SHIFT + G", mac: "⌘ + G / ⌘ + SHIFT + G" }, { label: "后退/前进", win: "CTRL + Z / CTRL + SHIFT + Z", mac: "⌘ + Z / ⌘ + ⇧ + Z" }, { label: "复制/粘贴", win: "CTRL + C / V", mac: "⌘ + C / V" }, { label: "多选", win: "拖动🖱️框选 / CTRL + 🖱️点击", mac: "拖动🖱️框选 / ⌘ + 🖱️点击" }, { label: "保存", win: "CTRL + S", mac: "⌘ + S" }, { label: "预览", win: "CTRL + P", mac: "⌘ + P" }, { label: "全选", win: "CTRL + A", mac: "⌘ + A" }, { label: "删除", win: "Delete", mac: "Delete" }], bordered: true, showIndexColumn: false, pagination: false, canResize: false });
  return (e3, o3) => (s(), h(d(gt), { onRegister: d(t2), title: "快捷操作" }, { default: M(() => [g(d(wt), { onRegister: d(l2) }, null, 8, ["onRegister"])]), _: 1 }, 8, ["onRegister"]));
} }), Uf = Yt(e({ __name: "CanvasOperator", setup(e2) {
  const { prefixCls: t2 } = we("canvas-operator"), [o2, { openModal: l2 }] = mt();
  function n2() {
    l2(true, {});
  }
  return (e3, l3) => (s(), r(m, null, [p("div", { class: c(d(t2)) }, [g(d(G), { title: "快捷方式", placement: "right" }, { default: M(() => [g(d(Te), { icon: "carbon:table-shortcut", onClick: n2 })]), _: 1 })], 2), g(Gf, { onRegister: d(o2) }, null, 8, ["onRegister"])], 64));
} }), [["__scopeId", "data-v-2148ffa6"]]), Vf = { lt: function(e2, t2, o2, l2, n2, a2) {
  const { symmetricPoint: i2 } = n2;
  let r2 = Ur(t2, i2), s2 = Gr(t2, r2, -e2.rotate), u2 = Gr(i2, r2, -e2.rotate), c2 = u2.x - s2.x, d2 = u2.y - s2.y;
  if (l2) {
    c2 / d2 > o2 ? (s2.x += Math.abs(c2 - d2 * o2), c2 = d2 * o2) : (s2.y += Math.abs(d2 - c2 / o2), d2 = c2 / o2);
    const t3 = Gr(s2, r2, e2.rotate);
    r2 = Ur(t3, i2), s2 = Gr(t3, r2, -e2.rotate), u2 = Gr(i2, r2, -e2.rotate), c2 = u2.x - s2.x, d2 = u2.y - s2.y;
  }
  c2 > 10 && d2 > 10 && (e2.width = Math.round(c2 / a2), e2.height = Math.round(d2 / a2), e2.x = Math.round(s2.x / a2), e2.y = Math.round(s2.y / a2));
}, t: Wf, rt: function(e2, t2, o2, l2, n2, a2) {
  const { symmetricPoint: i2 } = n2;
  let r2 = Ur(t2, i2), s2 = Gr(t2, r2, -e2.rotate), u2 = Gr(i2, r2, -e2.rotate), c2 = s2.x - u2.x, d2 = u2.y - s2.y;
  if (l2) {
    c2 / d2 > o2 ? (s2.x -= Math.abs(c2 - d2 * o2), c2 = d2 * o2) : (s2.y += Math.abs(d2 - c2 / o2), d2 = c2 / o2);
    const t3 = Gr(s2, r2, e2.rotate);
    r2 = Ur(t3, i2), s2 = Gr(t3, r2, -e2.rotate), u2 = Gr(i2, r2, -e2.rotate), c2 = s2.x - u2.x, d2 = u2.y - s2.y;
  }
  c2 > 10 && d2 > 10 && (e2.width = Math.round(c2 / a2), e2.height = Math.round(d2 / a2), e2.x = Math.round(u2.x / a2), e2.y = Math.round(s2.y / a2));
}, r: Zf, rb: function(e2, t2, o2, l2, n2, a2) {
  const { symmetricPoint: i2 } = n2;
  let r2 = Ur(t2, i2), s2 = Gr(i2, r2, -e2.rotate), u2 = Gr(t2, r2, -e2.rotate), c2 = u2.x - s2.x, d2 = u2.y - s2.y;
  if (l2) {
    c2 / d2 > o2 ? (u2.x -= Math.abs(c2 - d2 * o2), c2 = d2 * o2) : (u2.y -= Math.abs(d2 - c2 / o2), d2 = c2 / o2);
    const t3 = Gr(u2, r2, e2.rotate);
    r2 = Ur(t3, i2), s2 = Gr(i2, r2, -e2.rotate), u2 = Gr(t3, r2, -e2.rotate), c2 = u2.x - s2.x, d2 = u2.y - s2.y;
  }
  c2 > 10 && d2 > 10 && (e2.width = Math.round(c2 / a2), e2.height = Math.round(d2 / a2), e2.x = Math.round(s2.x / a2), e2.y = Math.round(s2.y / a2));
}, b: Wf, lb: function(e2, t2, o2, l2, n2, a2) {
  const { symmetricPoint: i2 } = n2;
  let r2 = Ur(t2, i2), s2 = Gr(i2, r2, -e2.rotate), u2 = Gr(t2, r2, -e2.rotate), c2 = s2.x - u2.x, d2 = u2.y - s2.y;
  if (l2) {
    c2 / d2 > o2 ? (u2.x += Math.abs(c2 - d2 * o2), c2 = d2 * o2) : (u2.y -= Math.abs(d2 - c2 / o2), d2 = c2 / o2);
    const t3 = Gr(u2, r2, e2.rotate);
    r2 = Ur(t3, i2), s2 = Gr(i2, r2, -e2.rotate), u2 = Gr(t3, r2, -e2.rotate), c2 = s2.x - u2.x, d2 = u2.y - s2.y;
  }
  c2 > 10 && d2 > 10 && (e2.width = Math.round(c2 / a2), e2.height = Math.round(d2 / a2), e2.x = Math.round(u2.x / a2), e2.y = Math.round(s2.y / a2));
}, l: Zf };
function Wf(e2, t2, o2, l2, n2, a2) {
  const { symmetricPoint: i2, curPoint: r2 } = n2, s2 = Gr(t2, r2, -e2.rotate), u2 = Gr({ x: r2.x, y: s2.y }, r2, e2.rotate), c2 = Math.hypot(u2.x - i2.x, u2.y - i2.y), d2 = u2.x - (u2.x - i2.x) / 2, p2 = u2.y + (i2.y - u2.y) / 2;
  let f2 = e2.width;
...
function Zf(e2, t2, o2, l2, n2, a2) {
  const { symmetricPoint: i2, curPoint: r2 } = n2, s2 = Gr(t2, r2, -e2.rotate), u2 = Gr({ x: s2.x, y: r2.y }, r2, e2.rotate), c2 = Math.hypot(u2.x - i2.x, u2.y - i2.y), d2 = u2.x - (u2.x - i2.x) / 2, p2 = u2.y + (i2.y - u2.y) / 2;
  let f2 = e2.height;
...
const Yf = ["onMousedown"], Kf = { key: 2, class: "cover-layer-out" }, Xf = { key: 0, class: "cover-hover" }, Jf = e({ name: "ChartContain", __name: "index", props: { comOption: { require: true, type: Object, default: () => ({ chart: {}, chartContain: {} }) }, index: { require: true, type: Number, default: 0 } }, setup(e2) {
  t((e3) => ({ "5851a81c": g2.value }));
  const n2 = e2, { token: a2 } = E.useToken(), g2 = l(() => a2.value.colorInfoBg), C2 = l(() => a2.value.colorBorder), b2 = o("chartContainRef"), M2 = o("chartItemRef"), { prefixCls: w2 } = we("chart-contain"), x2 = { points: ["lt", "t", "rt", "r", "rb", "b", "lb", "l"], initAngle: { lt: 0, t: 45, rt: 90, r: 135, rb: 180, b: 225, lb: 270, l: 315 }, pointCursor: [{ start: 338, end: 23, cursor: "nw" }, { start: 23, end: 68, cursor: "n" }, { start: 68, end: 113, cursor: "ne" }, { start: 113, end: 158, cursor: "e" }, { start: 158, end: 203, cursor: "se" }, { start: 203, end: 248, cursor: "s" }, { start: 248, end: 293, cursor: "sw" }, { start: 293, end: 338, cursor: "w" }] }, S2 = y(false), _2 = y(false), $2 = y({}), L2 = Tr(), I2 = Vd(), D2 = l(() => [w2, { active: q2.value || A2.value }]), T2 = l(() => {
    var _a2, _b, _c2, _d2;
  ...
  }), N2 = l(() => Ad({ ...n2.comOption.chartContain.dropInfo })), R2 = l(() => {
    const e3 = qd(n2.comOption);
    return e3.borderColor || (e3.borderColor = C2.value), Ee(e3, ["transform"]);
  }), q2 = l(() => {
    var _a2, _b;
  ...
  async function F2(e3) {
    if (I2.setShowMenu(false), e3.ctrlKey || e3.metaKey) return void ep(n2.comOption);
    ...
    const a3 = setInterval(() => {
      ...
      d(S2) ? L2.setUndoRedoData("组件移动") : L2.cleanSelectArea(), S2.value = false, clearInterval(a3), t2 && Jd(L2.getSelectArea.components), Id.emit(zr.UN_MOVE), document.removeEventListener("mousemove", i2), document.removeEventListener("mouseup", r2), $2.value = P2(n2.comOption);
    };
    document.addEventListener("mousemove", i2), document.addEventListener("mouseup", r2);
  }
  function P2(e3) {
    if (!e3) return;
    const { pointCursor: t2, initAngle: o2, points: l2 } = x2, n3 = Zr(e3.chartContain.dropInfo.rotate), a3 = {};
    let i2 = -1;
    return l2.forEach((e4) => {
      const l3 = Zr(o2[e4] + n3), r2 = t2.length;
      for (; ; ) {
        i2 = (i2 + 1) % r2;
        const o3 = t2[i2];
  ...
  function U2(e3, t2) {
    ...
    let u2 = true;
    const c2 = function() {
      if (n2.comOption.chart.type !== Jr.MfCombine) return false;
      for (const e4 of n2.comOption.chart.options.components) if (![0, 90, 180, 360].includes(Zr(e4.chartContain.dropInfo.rotate))) return true;
      return false;
    }(), p2 = Pe((t3) => {
      ...
      !function(e4, t4, o3, l3, n4, a4, i4) {
        Vf[e4](t4, o3, l3, n4, a4, i4);
      }(e3, o2, n3, i3, c2, { center: l2, curPoint: r2, symmetricPoint: s2 }, L2.getScale), j2(o2), Id.emit(zr.MOVE, false);
    }, 2), f2 = () => {
      d(S2) && L2.setUndoRedoData("组件大小调整"), S2.value = false, Id.emit(zr.UN_MOVE), document.removeEventListener("mousemove", p2), document.removeEventListener("mouseup", f2);
    };
  ...
  function V2(e3) {
    ...
    const s2 = Pe((e4) => {
      ...
      L2.setUndoRedoData("组件旋转"), document.removeEventListener("mousemove", s2), document.removeEventListener("mouseup", u2), $2.value = P2(n2.comOption);
    };
    document.addEventListener("mousemove", s2), document.addEventListener("mouseup", u2);
  }
  return i(() => {
    n2.comOption && ($2.value = P2(n2.comOption)), Id.on(zr.MOVE, () => {
      n2.comOption && L2.getCurComponent && n2.comOption.chart.id !== L2.getCurComponent.chart.id && !d(_2) && (_2.value = true);
    }), Id.on(zr.UN_MOVE, () => {
      _2.value = false;
    }), Id.on(zr.PLAY_ANIMATION, () => {
      var _a2, _b;
      n2.comOption.chart.id === ((_a2 = L2.getCurComponent) == null ? void 0 : _a2.chart.id) && void 0 === ((_b = n2.comOption.chart.options) == null ? void 0 : _b.selectIndex) && _d(M2.value, n2.comOption.chartContain.animations).then(() => {
        Id.emit(zr.PLAY_ANIMATION_COMPLETE);
      });
    }), Id.on(zr.STOP_ANIMATION, () => {
      var _a2;
      ...
      })(M2.value, n2.comOption.chartContain.animations);
    });
  }), (t2, o2) => {
    var _a2;
    return s(), r("div", { ref_key: "chartContainRef", ref: b2, class: c(D2.value), style: u(N2.value), onMousedown: k(F2, ["stop"]), onMouseenter: k(H2, ["prevent", "stop"]), onMouseleave: k(Q2, ["prevent"]) }, [(s(true), r(m, null, v(q2.value && !e2.comOption.chartContain.lock && e2.comOption.chartContain.show ? x2.points : [], (e3) => (s(), r("div", { key: e3, class: "contain-point", style: u(G2(e3)), onMousedown: k((t3) => U2(e3, t3), ["stop", "prevent"]) }, null, 44, Yf))), 128)), O(p("div", { ref_key: "chartItemRef", ref: M2, class: "chart-item", style: u(R2.value) }, [B(t2.$slots, "default", {}, void 0, true)], 4), [[z, (_a2 = e2.comOption) == null ? void 0 : _a2.chartContain.show]]), q2.value && !e2.comOption.chartContain.lock && e2.comOption.chartContain.show ? (s(), h(d(Te), { key: 0, class: "icon-rotate", icon: "ant-design:sync-outlined", size: "20", onMousedown: k(V2, ["prevent", "stop"]) })) : f("", true), e2.comOption.chartContain.lock && e2.comOption.chartContain.show ? (s(), h(d(Te), { key: 1, class: "icon-lock", icon: "ant-design:lock-outlined" })) : f("", true), q2.value || A2.value || T2.value ? (s(), r("div", Kf, [T2.value ? (s(), r("div", Xf)) : f("", true)])) : f("", true)], 38);
  };
} }), eh = Yt(Jf, [["__scopeId", "data-v-bdd5aa92"]]), th = "横上", oh = "横中", lh = "横下", nh = "纵左", ah = "纵中", ih = "纵右", rh = { class: "pos" }, sh = e({ name: "ChartNearLine", __name: "index", setup(e2) {
  const t2 = a({ adsorb: 10, near: 2, lineStatus: { [th]: { show: false, pos: 0, node: null }, [oh]: { show: false, pos: 0, node: null }, [lh]: { show: false, pos: 0, node: null }, [nh]: { show: false, pos: 0, node: null }, [ah]: { show: false, pos: 0, node: null }, [ih]: { show: false, pos: 0, node: null } } }), o2 = Tr(), { prefixCls: l2 } = we("near-line");
  function n2(e3, { x: o3, y: l3, bottom: n3, right: a2, halfWidth: i2, halfHeight: r2 }) {
    ...
    return { y: [{ isNear: u2(e3.y, l3), isAdsorb: f2(e3.y, l3), lineNode: d2[th].node, line: th, dragShift: l3, lineShift: l3 }, { isNear: u2(e3.bottom, l3), isAdsorb: f2(e3.bottom, l3), lineNode: d2[th].node, line: th, dragShift: l3 - e3.height, lineShift: l3 }, { isNear: u2(e3.y + c2, l3 + r2), isAdsorb: f2(e3.y + c2, l3 + r2), lineNode: d2[oh].node, line: oh, dragShift: l3 + r2 - c2, lineShift: l3 + r2 }, { isNear: u2(e3.y, n3), isAdsorb: f2(e3.y, n3), lineNode: d2[lh].node, line: lh, dragShift: n3, lineShift: n3 }, { isNear: u2(e3.bottom, n3), isAdsorb: f2(e3.bottom, n3), lineNode: d2[lh].node, line: lh, dragShift: n3 - e3.height, lineShift: n3 }], x: [{ isNear: u2(e3.x, o3), isAdsorb: f2(e3.x, o3), lineNode: d2[nh].node, line: nh, dragShift: o3, lineShift: o3 }, { isNear: u2(e3.right, o3), isAdsorb: f2(e3.right, o3), lineNode: d2[nh].node, line: nh, dragShift: o3 - e3.width, lineShift: o3 }, { isNear: u2(e3.x + s2, o3 + i2), isAdsorb: f2(e3.x + s2, o3 + i2), lineNode: d2[ah].node, line: ah, dragShift: o3 + i2 - s2, lineShift: o3 + i2 }, { isNear: u2(e3.x, a2), isAdsorb: f2(e3.x, a2), lineNode: d2[ih].node, line: ih, dragShift: a2, lineShift: a2 }, { isNear: u2(e3.right, a2), isAdsorb: f2(e3.right, a2), lineNode: d2[ih].node, line: ih, dragShift: a2 - e3.width, lineShift: a2 }] };
  }
  function u2(e3, o3) {
    ...
    Id.on(zr.MOVE, (e3) => {
      !async function(e4) {
        ...
        e4 ? o2.getSelectArea.components.forEach((e5) => {
          l3.push({ com: e5, style: Od(e5.chartContain.dropInfo) });
        }) : l3.push({ com: o2.getCurComponent, style: Od(o2.getCurComponent.chartContain.dropInfo) });
        g2().then(), l3.forEach((l4) => {
          !function(e5, l5) {
            ...
            a2(n2(e5.style, { x: o2.getCanvasConfig.width / 2, y: o2.getCanvasConfig.height / 2, bottom: 0, right: 0, halfWidth: 0, halfHeight: 0 })), a2(n2(e5.style, { x: 0, y: 0, bottom: o2.getCanvasConfig.height, right: o2.getCanvasConfig.width, halfWidth: 0, halfHeight: 0 })), o2.getGuideLine.lineListX.forEach((t3) => {
              ...
              const i2 = Od(t3.chartContain.dropInfo), { x: r2, y: s2, bottom: u3, right: c2 } = i2, d2 = i2.width / 2, p2 = i2.height / 2;
  ...
  }), (e3, o3) => (s(), r("div", { class: c(d(l2)) }, [(s(true), r(m, null, v(Object.keys(t2.lineStatus), (e4) => O((s(), r("div", { key: e4, ref_for: true, ref: (o4) => t2.lineStatus[e4].node = o4, class: c(["line", e4.includes("横") ? "xLine" : "yLine"]) }, [p("div", rh, S(t2.lineStatus[e4].pos), 1)], 2)), [[z, t2.lineStatus[e4].show || false]])), 128))], 2));
} }), uh = Yt(sh, [["__scopeId", "data-v-88d9d34f"]]), ch = e({ name: "ChartMultiSelect", __name: "index", props: { selectArea: { type: Object, default: () => {
} } }, setup(e2) {
  const t2 = Tr(), { prefixCls: o2 } = we("select-area");
...
} }), dh = Yt(ch, [["__scopeId", "data-v-e9844525"]]), ph = e({ name: "ScreenCanvas", __name: "index", setup(e2) {
  const { prefixCls: t2 } = we("screen-canvas"), n2 = Tr(), p2 = Vd(), y2 = o("screenCanvasRef"), { getAntdLocale: C2 } = _t(), { antTheme: b2 } = _e(l(() => n2.getTheme)), w2 = l(() => [...n2.getComponentList].reverse()), x2 = a({ canvasX: 0, canvasY: 0, area: { show: false, x: 0, y: 0, width: 0, height: 0 }, comMoving: false, isUp: false }), S2 = l(() => ({ ...Dd(n2.getCanvasConfig), transform: `scale(${n2.getScale})` })), _2 = (e3) => {
    var _a2, _b;
    if (((_b = (_a2 = e3.chart) == null ? void 0 : _a2.events) == null ? void 0 : _b.emits) && e3.chart.events.emits.length > 0) {
      for (const t3 of e3.chart.events.emits) if (t3 === ns.CHART_CLICK || t3 === ns.CHART_DBLCLICK) return "allow-pointer";
    }
  ...
  i(() => {
    n2.setScreenCanvas(y2.value ?? null), Id.on(zr.MOVE, () => {
      x2.comMoving || (x2.comMoving = true);
    }), Id.on(zr.UN_MOVE, () => {
      x2.comMoving = false;
    }), Id.on(zr.CREATE_CHART, (e3) => {
      L2(e3, 0, 0);
    });
  });
  const $2 = (e3) => {
    ...
    const t3 = (_a2 = e3.dataTransfer) == null ? void 0 : _a2.getData(Br);
  ...
  return (e3, o2) => (s(), r("div", { ref_key: "screenCanvasRef", ref: y2, class: c(d(t2)), onDrop: $2, onDragstart: o2[0] || (o2[0] = k(() => {
  }, ["prevent", "stop"])), onMousedown: I2, style: u(S2.value) }, [g(d(ye), { locale: d(C2), theme: d(b2) }, { default: M(() => [d(n2).getCanvasConfig.watermark ? (s(), h(d(me), { key: 0, content: d(n2).getCanvasConfig.watermark, style: u({ position: "absolute", width: `${d(n2).getCanvasConfig.width}px`, height: `${d(n2).getCanvasConfig.height}px` }) }, null, 8, ["content", "style"])) : f("", true), (s(true), r(m, null, v(w2.value, (e4, t3) => {
    var _a2;
    return O((s(), h(eh, { key: ((_a2 = e4.chart) == null ? void 0 : _a2.id) + d(n2).getKey, index: w2.value.length - t3 - 1, "com-option": e4, class: c(_2(e4)) }, { default: M(() => {
      var _a3;
      return [(s(), h(j(e4.chart.type && d(Ld)[e4.chart.type].component), { class: c(["component", { move: x2.comMoving && e4.chart.id !== ((_a3 = d(n2).getCurComponent) == null ? void 0 : _a3.chart.id) }]), id: `com${e4.chart.id}`, chart: e4.chart, "chart-contain": e4.chartContain }, null, 8, ["class", "id", "chart", "chart-contain"]))];
    }), _: 2 }, 1032, ["index", "com-option", "class"])), [[z, l(() => {
      var _a3;
      return (_a3 = e4.chartContain) == null ? void 0 : _a3.show;
    })]]);
  }), 128))]), _: 1 }, 8, ["locale", "theme"]), g(dh, { "select-area": x2.area }, null, 8, ["select-area"]), g(uh)], 38));
} }), fh = Yt(ph, [["__scopeId", "data-v-17c82941"]]), hh = 0.2, gh = e({ name: "FrameCanvas", __name: "index", props: { width: { type: Number, default: 5e3 }, height: { type: Number, default: 3e3 } }, setup(e2) {
  const t2 = y(true), { prefixCls: l2 } = we("frame-canvas"), a2 = y(0), u2 = y(0), f2 = y(1e3), h2 = y(1), m2 = Tr(), v2 = o("frameCanvasRef"), C2 = o("dropContentRef");
  let b2 = [0, 0], w2 = [0, 0];
  ...
  const x2 = Pe(() => {
    t2.value && (!function() {
      if (d(t2) && v2.value) {
        ...
        h2.value = n2 > l3 ? Number.parseFloat((o2 * l3 / m2.getCanvasConfig.width).toFixed(2)) : Number.parseFloat((t3 / l3 / m2.getCanvasConfig.height).toFixed(2)), d(h2) > 5 && (h2.value = 5), d(h2) < hh && (h2.value = hh);
      }
      m2.setScale(d(h2));
    }(), D2());
  }, 30);
  ...
  const $2 = Pe((e3) => {
    ...
    h2.value = "add" === e3 ? Math.min(5, Number.parseFloat((t3 + 0.05).toFixed(2))) : Math.max(hh, Number.parseFloat((t3 - 0.05).toFixed(2))), D2(), m2.setScale(h2.value);
  }, 100), L2 = Pe((e3) => {
    e3 && (h2.value = e3 / 100, D2(), m2.setScale(h2.value));
  }, 100), I2 = (e3) => {
  ...
  return (t3, l3) => (s(), r("div", { class: c(d(n2)) }, [e2.showHeader ? (s(), r("div", mh, [p("div", { class: "icon-title", onClick: C2 }, [e2.icon ? (s(), h(d(Te), { key: 0, class: "title", icon: e2.icon, size: e2.iconSize }, null, 8, ["icon", "size"])) : f("", true), p("div", vh, S(e2.title), 1)]), g(d(Y), { class: "radio-group", size: "small", value: a2.value, "onUpdate:value": l3[0] || (l3[0] = (e3) => a2.value = e3), onChange: u2 }, { default: M(() => [(s(true), r(m, null, v(o2.radioData, (e3) => (s(), h(d(G), { key: e3.value, title: e3.tooltip }, { default: M(() => [g(d(Z), { class: "radio-button", value: e3.value }, { default: M(() => [g(d(Te), { icon: e3.icon }, null, 8, ["icon"])]), _: 2 }, 1032, ["value"])]), _: 2 }, 1032, ["title"]))), 128))]), _: 1 }, 8, ["value"])])) : f("", true), p("div", Ch, [(s(), h(j(e2.chart))), B(t3.$slots, "default", {}, void 0, true)])], 2));
} }), [["__scopeId", "data-v-8929fb33"]]), Mh = { class: "item-content" }, kh = ["src"], wh = { class: "title", style: { flex: "1" } }, xh = { class: "iconContain" }, Sh = e({ __name: "LayerItem", props: { parentItem: { type: Object }, item: { type: Object, required: true }, showEye: { type: Boolean, default: true }, showLock: { type: Boolean, default: true }, index: { type: Number }, expand: { type: Boolean, default: false }, externalSelect: { type: Boolean, default: false }, isSelect: { type: Boolean, default: false } }, emits: ["expand", "select", "enter", "leave"], setup(e2, { emit: o2 }) {
  t((e3) => ({ "56d1bd00": C2.value }));
  const n2 = e2, a2 = o2, { prefixCls: i2 } = we("layer-item"), u2 = Tr(), g2 = Nr(), m2 = l(() => u2.getChartImg(n2.item.chart.type)), { token: v2 } = E.useToken(), C2 = l(() => v2.value.colorInfoBg), b2 = xe().getThemeColor, M2 = l(() => {
  ...
  function w2() {
    k2.value = true, a2("enter");
  }
  function x2() {
    k2.value = false, a2("leave");
  }
  function _2() {
    a2("expand", !n2.expand);
  }
  ...
  return (t2, o3) => {
    var _a2;
    return s(), r("div", { class: c(M2.value), onMouseenter: w2, onMouseleave: x2, onMousedown: $2 }, [p("div", Mh, [e2.item.chart.type === d(Jr).MfCombine ? (s(), r("div", { key: 0, onClick: _2 }, [n2.expand ? (s(), h(d(dt), { key: 0, class: "group", "two-tone-color": d(b2) }, null, 8, ["two-tone-color"])) : (s(), h(d(pt), { key: 1, class: "group", "two-tone-color": d(b2) }, null, 8, ["two-tone-color"]))])) : (s(), r("img", { key: 1, class: "img", alt: "组件", src: m2.value }, null, 8, kh)), p("span", wh, S((_a2 = e2.item) == null ? void 0 : _a2.chart.name), 1), p("div", xh, [!e2.showEye || d(g2).getLayerCollapsed || !k2.value && e2.item.chartContain.show ? f("", true) : (s(), h(d(Te), { key: 0, class: "icon", icon: e2.item.chartContain.show ? "ant-design:eye-outlined" : "ant-design:eye-invisible-outlined", onClick: o3[0] || (o3[0] = () => e2.item.chartContain.show = !e2.item.chartContain.show) }, null, 8, ["icon"])), e2.showLock && !d(g2).getLayerCollapsed && (k2.value || e2.item.chartContain.lock) ? (s(), h(d(Te), { key: 1, class: "icon", icon: e2.item.chartContain.lock ? "ant-design:lock-outlined" : "ant-design:unlock-outlined", onClick: o3[1] || (o3[1] = () => e2.item.chartContain.lock = !e2.item.chartContain.lock) }, null, 8, ["icon"])) : f("", true)])])], 34);
  };
} }), _h = Yt(Sh, [["__scopeId", "data-v-925f45da"]]), $h = e({ __name: "LayerGroup", props: { item: { type: Object, required: true }, expand: { type: Boolean, default: false }, showEye: { type: Boolean, default: true }, showLock: { type: Boolean, default: true }, showChildEye: { type: Boolean, default: true }, showChildLock: { type: Boolean, default: true }, externalSelect: { type: Boolean, default: false }, isSelect: { type: Boolean, default: false }, childSelect: { type: String, default: "" } }, emits: ["parentClick", "childClick"], setup(e2, { emit: t2 }) {
  const o2 = e2, a2 = t2, { prefixCls: i2 } = we("layer-group"), u2 = Tr(), p2 = Nr(), f2 = l(() => ({ "child-item": true, collapsed: p2.getLayerCollapsed })), C2 = y(o2.expand);
  function b2() {
    var _a2;
  ...
  return n(() => o2.expand, (e3) => C2.value = e3), (t3, o3) => (s(), r("div", { class: c(d(i2)), onMouseenter: b2, onMouseleave: k2 }, [g(_h, { item: e2.item, onExpand: x2, expand: C2.value, "show-eye": e2.showEye, "show-lock": e2.showLock, "external-select": e2.externalSelect, "is-select": e2.isSelect, onClick: S2 }, null, 8, ["item", "expand", "show-eye", "show-lock", "external-select", "is-select"]), g(d(Dt), { show: C2.value, class: c(f2.value) }, { default: M(() => [(s(true), r(m, null, v(e2.item.chart.options.components, (t4, o4) => (s(), h(_h, { key: t4.chart.id, "parent-item": e2.item, item: t4, index: o4, "external-select": e2.externalSelect, "is-select": e2.childSelect === t4.chart.id, "show-eye": e2.showChildEye, "show-lock": e2.showChildLock, onSelect: (t5) => function(e3, t6) {
...
} }), Lh = Yt($h, [["__scopeId", "data-v-b118e01c"]]), Ih = ["onDrop"], Dh = e({ name: "FrameLayer", __name: "index", props: { collapsed: $t.bool.def(false) }, setup(e2) {
  const t2 = e2, { prefixCls: o2 } = we("frame-layer"), n2 = l(() => [o2, { collapsed: t2.collapsed }]), a2 = [{ value: "collapse", icon: "iconfont:icon-mfish-collapse-all", tooltip: "折叠分组" }, { value: "expand", icon: "iconfont:icon-mfish-expand-all", tooltip: "展开分组" }], r2 = l(() => f2.getComponentList);
  i(() => {
    const e3 = f2.getComponentList.map((e4) => e4.chart.type).join(",");
    Qi({ types: e3, pageNum: 1, pageSize: 1e4 }).then((e4) => {
      var _a2;
      e4 && ((_a2 = e4.list) == null ? void 0 : _a2.length) > 0 && e4.list.forEach((e5) => {
        f2.hasChartImg(e5.type) || rt(it(e5 == null ? void 0 : e5.picKey), { errorMessageMode: "none" }).then((t3) => {
          f2.setChartImg(e5.type, t3);
        });
      });
    });
  });
  const u2 = y(false), f2 = Tr(), m2 = Nr(), v2 = (e3) => {
  ...
  function b2(e3, t3) {
    var _a2;
    e3.stopPropagation();
    const o3 = (_a2 = e3.dataTransfer) == null ? void 0 : _a2.getData(Br);
    if (o3) {
  ...
  function k2(e3, t3, o3) {
    !o3.ctrlKey && !o3.metaKey || 0 !== o3.button ? 0 === o3.button && (f2.cleanSelectArea(), f2.setCurComponent(e3, t3)) : ep(e3);
  }
  function w2() {
  ...
  return (e3, t3) => (s(), h(bh, { class: c(n2.value), title: d(m2).getLayerCollapsed ? "" : "图层", icon: "iconfont:icon-mfish-layer", "icon-size": d(m2).getLayerCollapsed ? 16 : 12, "radio-data": a2, onTitleClick: C2, onDrop: t3[1] || (t3[1] = (e4) => b2(e4, r2.value.length)), onRadioChange: x2 }, { default: M(() => [0 === r2.value.length ? (s(), h(d(be), { key: 0, "image-style": d(m2).getLayerCollapsed ? { height: "20px" } : {}, description: "暂无图层", image: d(et) }, null, 8, ["image-style", "image"])) : (s(), h(d(ft), { key: 1 }, { default: M(() => [g(d(It), { "item-key": "id", modelValue: r2.value, "onUpdate:modelValue": t3[0] || (t3[0] = (e4) => r2.value = e4), animation: "300", "ghost-class": "ghost", onChange: v2 }, { item: M(({ index: e4, element: t4 }) => [p("div", { onDrop: (t5) => b2(t5, e4), style: { "margin-top": "4px" } }, [t4.chart.type !== d(Jr).MfCombine ? (s(), h(_h, { key: 0, item: t4, onMousedown: (o3) => k2(t4, e4, o3), onEnter: (e5) => {
    return o3 = t4, void f2.setCurHoverComponent(o3 == null ? void 0 : o3.chart.id);
    var o3;
  }, onLeave: w2 }, null, 8, ["item", "onMousedown", "onEnter"])) : (s(), h(Lh, { key: 1, item: t4, expand: u2.value, "show-child-lock": false, onMousedown: (o3) => k2(t4, e4, o3) }, null, 8, ["item", "expand", "onMousedown"]))], 40, Ih)]), _: 1 }, 8, ["modelValue"])]), _: 1 }))]), _: 1 }, 8, ["class", "title", "icon-size"]));
} }), Th = Yt(Dh, [["__scopeId", "data-v-3b66e5f3"]]), Nh = { class: "menu-item-hide-title" }, Bh = { class: "title" }, Rh = Yt(e({ __name: "ChartMenus", props: { itemHeight: { type: Number }, width: { type: Number, default: 68 }, collapsed: { type: Boolean, default: false }, horizontal: { type: Boolean, default: false }, hideTitle: { type: Boolean, default: false }, backgroundColor: { type: Object, default: () => ({ light: "transparent", dark: "transparent" }) }, menus: { type: Array, default: () => [] } }, emits: ["menuClick", "menuChange"], setup(e2, { emit: o2 }) {
  ...
  const { prefixCls: f2 } = we("chart-menus"), C2 = y([]), { backgroundColor: b2, width: k2 } = a2, w2 = l(() => b2.light), x2 = l(() => b2.dark), _2 = (e3) => {
    i2("menuClick", e3, d(C2)), i2("menuChange", e3.key, e3.item.id);
  };
  return (t2, o3) => (s(), r("div", { class: c(d(f2)) }, [g(d(te), { class: c({ collapsed: e2.collapsed }), "selected-keys": C2.value, "onUpdate:selectedKeys": o3[0] || (o3[0] = (e3) => C2.value = e3), onClick: _2 }, { default: M(() => [g(d(ft), null, { default: M(() => [(s(true), r(m, null, v(e2.menus, (t3) => (s(), h(d(ke), { id: t3.name, key: t3.value, style: u({ height: `${e2.itemHeight}px` }) }, { default: M(() => [e2.hideTitle ? (s(), h(d(G), { key: 0, title: t3.name, placement: "right" }, { default: M(() => [p("div", Nh, [g(d(Te), { style: u({ lineHeight: `${e2.itemHeight}px` }), icon: t3.icon }, null, 8, ["style", "icon"])])]), _: 2 }, 1032, ["title"])) : (s(), r("div", { key: 1, class: c(e2.horizontal ? "menu-item-horizontal" : "menu-item-vertical") }, [g(d(Te), { icon: t3.icon }, null, 8, ["icon"]), p("div", Bh, S(t3.name), 1)], 2))]), _: 2 }, 1032, ["id", "style"]))), 128))]), _: 1 })]), _: 1 }, 8, ["class", "selected-keys"])], 2));
} }), [["__scopeId", "data-v-dea86191"]]), qh = ["onDragstart", "onDblclick"], Ah = { class: "box-header" }, Oh = { class: "box-header-text" }, jh = { class: "box-content" }, zh = ["src"], Fh = e({ __name: "ChartItems", props: { charts: { type: Array, default: () => [] }, mode: { type: String, default: "single" } }, setup(e2) {
  const t2 = e2, { prefixCls: o2 } = we("chart-items"), l2 = Tr();
  return n(() => t2.charts, (e3) => {
    e3.forEach((e4) => {
      const t3 = e4 == null ? void 0 : e4.type;
      t3 && (e4 == null ? void 0 : e4.picKey) && !l2.hasChartImg(t3) && rt(it(e4 == null ? void 0 : e4.picKey), { errorMessageMode: "none" }).then((e5) => {
        l2.setChartImg(t3, e5);
      });
    });
  }, { immediate: true }), (t3, n2) => (s(), h(d(ft), { class: c(d(o2)) }, { default: M(() => [p("div", { class: c(["items", "single" === e2.mode ? e2.mode : [e2.mode, "miniAnimation"]]) }, [(s(true), r(m, null, v(e2.charts, (t4) => {
    return s(), r("div", { draggable: "true", key: t4.type, class: "item-box", onDragstart: (e3) => function(e4, t5) {
      var _a2, _b, _c2;
      const o4 = Rd(t5);
      if (!o4) return;
      const l3 = e4.currentTarget;
      "img" === e4.target.className ? (_a2 = e4.dataTransfer) == null ? void 0 : _a2.setDragImage(l3, e4.offsetX + 15, e4.offsetY + 32) : (_b = e4.dataTransfer) == null ? void 0 : _b.setDragImage(l3, e4.offsetX, e4.offsetY), (_c2 = e4.dataTransfer) == null ? void 0 : _c2.setData(Br, JSON.stringify(o4));
    }(e3, t4), onDblclick: (e3) => function(e4) {
      const t5 = Rd(e4);
      t5 && Id.emit(zr.CREATE_CHART, t5);
    }(t4) }, [p("div", Ah, [g(d(Ia), { disabled: true, size: "single" === e2.mode ? "small" : "mini" }, null, 8, ["size"]), p("span", Oh, S(t4.name), 1)]), p("div", jh, [p("img", { class: "img", alt: "组件图片", src: (o3 = t4.type, l2.getChartImg(o3)) }, null, 8, zh)])], 40, qh);
    var o3;
  }), 128))], 2)]), _: 1 }, 8, ["class"]));
} }), Ph = Yt(Fh, [["__scopeId", "data-v-74651cba"]]), Eh = e({ name: "FrameChart", __name: "index", props: { collapsed: $t.bool.def(false) }, setup(e2) {
  const t2 = e2, o2 = Nr(), n2 = y(), a2 = y(), r2 = y(""), u2 = y(/* @__PURE__ */ new Map()), f2 = l(() => a2.value && a2.value.length > 0 ? a2.value.map((e3) => ({ name: e3.categoryName, icon: e3.icon, value: e3.id, children: e3.children })) : []), m2 = l(() => {
    var _a2;
    if (n2.value) {
    ...
    return [];
  }), { prefixCls: v2 } = we("frame-chart"), C2 = y("single"), b2 = [{ value: "single", icon: "ant-design:group-outlined", tooltip: "单列" }, { value: "double", icon: "ant-design:appstore-outlined", tooltip: "双列" }], k2 = l(() => [v2, { collapsed: t2.collapsed }]);
  function w2(e3) {
    C2.value = e3;
  }
  i(() => {
    Tt("screen_charts_type", "down").then((e3) => {
  ...
  function $2(e3, t3) {
    let o3;
    o3 = "所有" === t3 ? m2.value.find((t4) => t4.value === e3).parentId : e3, r2.value = o3, u2.value.has(o3) || Qi({ category: o3, pageNum: 1, pageSize: 1e3 }).then((e4) => {
      u2.value.set(o3, e4.list);
    });
  }
  return (t3, o3) => (s(), h(bh, { title: "组件", icon: "iconfont:icon-mfish-chart", "icon-size": 12, "radio-data": b2, onRadioChange: w2, class: c(k2.value), onTitleClick: S2 }, { default: M(() => [p("div", { class: c(`${d(v2)}-content`) }, [g(Rh, { menus: f2.value, "item-height": 58, "background-color": { light: "#f0f2f5", dark: "#232324" }, onMenuClick: x2, onMenuChange: _2 }, null, 8, ["menus"]), p("div", { class: c(["charts", { collapsed: e2.collapsed }]) }, [g(Rh, { menus: m2.value, collapsed: e2.collapsed, "hide-title": true, "background-color": { light: "#f8f8f9", dark: "#1e1e1f" }, horizontal: true, "item-height": 28, width: 40, onMenuChange: $2 }, null, 8, ["menus", "collapsed"]), g(Ph, { mode: C2.value, charts: u2.value.get(r2.value) }, null, 8, ["mode", "charts"])], 2)], 2)]), _: 1 }, 8, ["class"]));
} }), Hh = Yt(Eh, [["__scopeId", "data-v-3b8e6093"]]), Qh = ["onClick"], Gh = { class: "name" }, Uh = { class: "short-cut" }, Vh = e({ __name: "SubMenu", props: { subData: { type: Array, default: () => [] } }, setup(e2, { expose: t2 }) {
  const { prefixCls: l2 } = we("frame-shortcut"), n2 = y(false), a2 = y(0), i2 = o("subMenuRef"), C2 = xe().getThemeColor;
  return t2({ show: function(e3) {
    n2.value = true;
  }, hidden: function() {
    n2.value = false;
  } }), (t3, o2) => (s(), h(P, { name: "shortcut" }, { default: M(() => [O(p("div", { ref_key: "subMenuRef", ref: i2, class: c(d(l2)), style: u({ top: `${a2.value}px`, left: "160px" }) }, [(s(true), r(m, null, v(e2.subData, (e3) => (s(), r(m, { key: e3.name }, [p("div", { class: "menu-item", onClick: e3.action }, [g(d(Te), { icon: e3.icon, color: d(C2) }, null, 8, ["icon", "color"]), p("span", Gh, S(e3.name), 1), p("span", Uh, S(e3.shortcut), 1)], 8, Qh), e3.divider ? (s(), r("div", { key: 0, class: c(`${d(l2)}__divider`) }, null, 2)) : f("", true)], 64))), 128))], 6), [[z, n2.value]])]), _: 1 }));
} }), Wh = Yt(Vh, [["__scopeId", "data-v-f4307b07"]]), Zh = { class: "short-cut" }, Yh = { class: "short-cut" }, Kh = { class: "short-cut" }, Xh = ["onClick"], Jh = { class: "name" }, eg = { class: "short-cut" }, tg = e({ name: "FrameShortcut", __name: "index", props: { menuTop: { type: Number, default: 0 }, menuLeft: { type: Number, default: 0 }, isUp: { type: Boolean, default: false } }, setup(e2, { expose: t2 }) {
  const n2 = e2, i2 = l(() => ({ top: `${n2.menuTop}px`, left: `${n2.menuLeft}px` })), y2 = xe().getThemeColor, { prefixCls: b2 } = we("frame-shortcut"), w2 = Vd(), x2 = Tr(), _2 = o("frameShortcutRef"), $2 = o("canvasAlignMenuRef"), L2 = o("selectAlignMenuRef"), I2 = o("fitMenuRef"), { NoneShortcutKeyEnum: D2 } = lp(x2, w2);
  t2({ getBoundingClientRect: () => {
    var _a2;
    return (_a2 = _2.value) == null ? void 0 : _a2.getBoundingClientRect();
  } });
  const T2 = l(() => x2.getSelectArea.components.length > 1), N2 = a([{ icon: "iconfont:icon-mfish-copy", name: "复制", shortcut: "CTRL+C", action: w2.copyComponent, show: l(() => x2.getCurComponent || x2.getSelectArea.components.length > 0) }, { icon: "iconfont:icon-mfish-paste", name: "粘贴", shortcut: "CTRL+V", action: w2.pasteComponent, show: l(() => w2.getCopyData), divider: true }, { icon: "iconfont:icon-mfish-up-level", name: "上移一层", shortcut: "SHIFT+↑", action: w2.upComponent, show: l(() => x2.getCurComponent && !d(T2)) }, { icon: "iconfont:icon-mfish-down-level", name: "下移一层", shortcut: "SHIFT+↓", action: w2.downComponent, show: l(() => x2.getCurComponent && !d(T2)) }, { icon: "iconfont:icon-mfish-up-level", name: "置顶", shortcut: "CTRL+SHIFT+↑", action: w2.topComponent, show: l(() => x2.getCurComponent && !d(T2)) }, { icon: "iconfont:icon-mfish-bottom-level", name: "置底", shortcut: "CTRL+SHIFT+↓", action: w2.bottomComponent, show: l(() => x2.getCurComponent && !d(T2)), divider: true }, { icon: "iconfont:icon-mfish-combine", name: "组合", shortcut: "CTRL+G", action: w2.composeComponent, show: l(() => x2.getSelectArea.components.length > 1) }, { icon: "iconfont:icon-mfish-un-combine", name: "拆分", shortcut: "CTRL+SHIFT+G", action: w2.decomposeComponent, show: l(() => {
    var _a2;
    return ((_a2 = x2.getCurComponent) == null ? void 0 : _a2.chart.type) === Jr.MfCombine && !d(T2);
  ...
  function O2() {
    var _a2;
    z2(tp.GROUP), (_a2 = L2.value) == null ? void 0 : _a2.show(n2.isUp);
  }
  function j2() {
    var _a2;
  ...
  function z2(e3) {
    w2.setAlignType(e3), 0 === x2.getSelectArea.components.length && x2.getCurComponent && w2.setAlignType(tp.CANVAS);
  }
  function F2() {
    var _a2;
  ...
  return (e3, t3) => (s(), h(P, { name: "shortcut" }, { default: M(() => [d(w2).getShowMenu ? (s(), r("div", { key: "shortcutMenu", ref_key: "frameShortcutRef", ref: _2, class: c(d(b2)), style: u(i2.value), onMousedown: t3[3] || (t3[3] = k(() => {
  ...
  }, ["stop"])) }, [g(d(Te), { icon: "iconfont:icon-mfish-left-align", color: d(y2) }, null, 8, ["color"]), t3[4] || (t3[4] = p("span", { class: "name" }, "画布对齐", -1)), p("span", Zh, [g(d(Te), { icon: "ant-design:right-outlined" })]), g(Wh, { ref_key: "canvasAlignMenuRef", ref: $2, "sub-data": B2 }, null, 8, ["sub-data"])], 32)) : f("", true), d(x2).getSelectArea.components.length > 1 ? (s(), r("div", { key: 1, class: "menu-item", onMouseenter: O2, onMouseleave: j2, onMouseup: t3[1] || (t3[1] = k(() => {
  }, ["stop"])) }, [g(d(Te), { icon: "iconfont:icon-mfish-left-align", color: d(y2) }, null, 8, ["color"]), t3[5] || (t3[5] = p("span", { class: "name" }, "选中对齐", -1)), p("span", Yh, [g(d(Te), { icon: "ant-design:right-outlined" })]), g(Wh, { ref_key: "selectAlignMenuRef", ref: L2, "sub-data": B2 }, null, 8, ["sub-data"])], 32)) : f("", true), d(x2).getCurComponent ? (s(), r("div", { key: 2, class: "menu-item", onMouseenter: F2, onMouseleave: E2, onMouseup: t3[2] || (t3[2] = k(() => {
  }, ["stop"])) }, [g(d(Te), { icon: "carbon:fit-to-screen", color: d(y2) }, null, 8, ["color"]), t3[6] || (t3[6] = p("span", { class: "name" }, "撑满", -1)), p("span", Kh, [g(d(Te), { icon: "ant-design:right-outlined" })]), g(Wh, { ref_key: "fitMenuRef", ref: I2, "sub-data": R2 }, null, 8, ["sub-data"])], 32)) : f("", true), d(x2).getCurComponent || d(x2).getSelectArea.components.length > 1 ? (s(), r("div", { key: 3, class: c(`${d(b2)}__divider`) }, null, 2)) : f("", true), (s(true), r(m, null, v(N2, (e4) => (s(), r(m, { key: e4.name }, [e4.show ? (s(), r("div", { key: 0, class: "menu-item", onClick: (t4) => {
    return o2 = e4.action, w2.setShowMenu(false), void C(() => {
      o2();
    });
    var o2;
  } }, [g(d(Te), { icon: e4.icon, color: d(y2) }, null, 8, ["icon", "color"]), p("span", Jh, S(e4.name), 1), p("span", eg, S(e4.shortcut), 1)], 8, Xh)) : f("", true), e4.show && e4.divider ? (s(), r("div", { key: 1, class: c(`${d(b2)}__divider`) }, null, 2)) : f("", true)], 64))), 128))], 38)) : f("", true)]), _: 1 }));
} }), og = Yt(tg, [["__scopeId", "data-v-2dcb7415"]]);
export {
  gl as $,
  tn as A,
  Vo as B,
