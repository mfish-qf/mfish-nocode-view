...
import { Button as lt } from "@mfish/core/components/Button";
import { router as it } from "@mfish/core/router";
import { sleep as rt } from "@mfish/core/utils/Utils";
import { formatToDateTime as st, dateUtil as ut } from "@mfish/core/utils/DateUtil";
import { imageUrl as ct, getLocalFileUrl as dt, imageSrc as pt } from "@mfish/core/utils/file/FileUtils";
import { DeleteOutlined as ft, CaretRightOutlined as ht, CheckOutlined as gt, FolderOpenTwoTone as mt, FolderTwoTone as yt } from "@ant-design/icons-vue";
import { ScrollContainer as vt } from "@mfish/core/components/Container";
import { DraggableInput as Ct } from "@mfish/core/components/Draggable";
import { BasicModal as bt, useModalInner as Mt, useModal as kt } from "@mfish/core/components/Modal";
import { BasicForm as wt, useForm as xt } from "@mfish/core/components/Form";
import { BasicTree as St } from "@mfish/core/components/Tree";
import { Split as _t } from "@mfish/core/components/Split";
import { useTable as $t, BasicTable as Lt } from "@mfish/core/components/Table";
import { CodeEditor as It, MODE as Dt } from "@mfish/core/components/CodeEditor";
import { useLocale as Tt } from "@mfish/core/i18n/UseLocale";
import { propTypes as Nt } from "@mfish/core/utils/PropTypes";
import { FileUp as Bt } from "@mfish/core/components/FileUpDown";
import { NCollapseTransition as Rt } from "naive-ui";
import qt from "vuedraggable";
import { queryCategoryTreeByCode as Ot } from "@mfish/core/api";
const At = "color_recently_bi", jt = "gradient_color_recently_bi", zt = ["rgba(255, 255, 255, 1)", "rgba(0, 0, 0, 1)", "rgba(35, 69, 231, 1)", "rgba(35, 231, 196, 1)", "rgba(65, 231, 35, 1)", "rgba(231, 35, 35, 1)"], Et = ["linear-gradient(120deg, rgba(243,231,233,1) 0%, rgba(227,238,255,1) 100%)", "linear-gradient(45deg, rgba(161,140,209,1) 0%, rgba(251,194,235,1) 100%)", "linear-gradient(90deg, rgba(255,236,210,1) 0%, rgba(252,182,159,1) 100%)", "linear-gradient(180deg, rgba(204,32,142,1) 0%, rgba(103,19,210,1) 100%)", "linear-gradient(-225deg, rgba(35,21,87,1) 0%, rgba(68,16,122,1) 29%, rgba(255,19,97,1) 67%, rgba(255,248,0,1) 100%)", "linear-gradient(180deg, rgba(30,60,114,1) 0%, rgba(30,60,114,1) 1%, rgba(42,82,152,1) 100%)", "linear-gradient(-20deg, rgba(43,88,118,1) 0%, rgba(78,67,118,1) 100%)", "linear-gradient(45deg, rgba(58,181,176,1) 0%, rgba(61,153,190,1) 31%, rgba(86,49,122,1) 100%)", "linear-gradient(180deg, rgba(106,17,203,1) 0%, rgba(37,117,252,1) 100%)"];
function Ft(e2) {
...
}, Gt = { 10: "A", 11: "B", 12: "C", 13: "D", 14: "E", 15: "F" }, Ut = (e2) => {
  e2 = Math.min(Math.round(e2), 255);
  const t2 = Math.floor(e2 / 16), o2 = e2 % 16;
  return `${Gt[t2] || t2}${Gt[o2] || o2}`;
}, Vt = function({ r: e2, g: t2, b: o2 }) {
  return Number.isNaN(+e2) || Number.isNaN(+t2) || Number.isNaN(+o2) ? "" : `#${Ut(e2)}${Ut(t2)}${Ut(o2)}`;
}, Wt = { A: 10, B: 11, C: 12, D: 13, E: 14, F: 15 }, Yt = function(e2) {
  return 2 === e2.length ? 16 * (Wt[e2[0].toUpperCase()] || +e2[0]) + (Wt[e2[1].toUpperCase()] || +e2[1]) : Wt[e2[1].toUpperCase()] || +e2[1];
}, Zt = (e2, t2, o2) => {
  e2 = Qt(e2, 255), t2 = Qt(t2, 255), o2 = Qt(o2, 255);
...
}, Kt = function(e2, t2, o2) {
  e2 = 6 * Qt(e2, 360), t2 = Qt(t2, 100), o2 = Qt(o2, 100);
...
class Xt {
  ...
  toRgb() {
    return Kt(this._hue, this._saturation, this._value);
  }
  toHex() {
    const e2 = this.toRgb();
    return Vt(e2).slice(0, 7);
  }
  fromString(e2) {
    ...
    if (e2.includes("hsl")) {
      ...
      if (4 === o2.length ? this._alpha = 100 * Number.parseFloat(o2[3].toString()) : 3 === o2.length && (this._alpha = 100), o2.length >= 3) {
        const { h: e3, s: a2, v: n2 } = Zt(o2[0], o2[1], o2[2]);
        t2(e3, a2, n2);
      }
    } else if (e2.includes("#")) {
      ...
      let a2, n2, l2;
      3 === o2.length ? (a2 = Yt(o2[0] + o2[0]), n2 = Yt(o2[1] + o2[1]), l2 = Yt(o2[2] + o2[2])) : 6 !== o2.length && 8 !== o2.length || (a2 = Yt(o2.slice(0, 2)), n2 = Yt(o2.slice(2, 4)), l2 = Yt(o2.slice(4, 6))), 8 === o2.length ? this._alpha = Yt(o2.slice(6)) / 255 * 100 : 3 !== o2.length && 6 !== o2.length || (this._alpha = 100);
      const { h: i2, s: r2, v: s2 } = Zt(a2, n2, l2);
      t2(i2, r2, s2);
    }
  }
  compare(e2) {
  ...
  doOnChange() {
    ...
    if (this.enableAlpha) switch (n2) {
      ...
      case "hsv":
        ...
        break;
      case "hex":
        this.value = `${Vt(Kt(e2, t2, o2))}${Ut(255 * a2 / 100)}`;
        break;
      default: {
        const { r: a3, g: n3, b: l2 } = Kt(e2, t2, o2);
...
function eo(e2, t2) {
  const o2 = function(e3) {
    ...
    document.removeEventListener("mousemove", o2), document.removeEventListener("mouseup", a2), document.removeEventListener("touchmove", o2), document.removeEventListener("touchend", a2), document.onselectstart = null, document.ondragstart = null, Jt = false, t2.end?.(e3);
  }, n2 = function(e3) {
    Jt || (e3.preventDefault(), document.onselectstart = () => false, document.ondragstart = () => false, document.addEventListener("mousemove", o2), document.addEventListener("mouseup", a2), document.addEventListener("touchmove", o2), document.addEventListener("touchend", a2), Jt = true, t2.start?.(e3));
  };
...
const to = (e2, t2) => {
  ...
  return o2;
}, oo = to(e({ __name: "ColorPanel", props: { color: { type: Object, required: true }, height: { type: Number, default: 150 }, isShow: { type: Boolean, default: false } }, setup(e2) {
...
} }), [["__scopeId", "data-v-696c5dfb"]]), ao = to(e({ __name: "ColorHueSlider", props: { color: { type: Object, required: true }, vertical: { type: Boolean, default: false } }, emits: ["startEyeDropper", "endEyeDropper"], setup(e2, { emit: t2 }) {
  const m2 = e2, y2 = t2, v2 = l({ left: 0, top: 0, hue: 0 }), C2 = o("barRef"), b2 = o("thumbRef"), M2 = o("colorHueSliderRef"), { prefixCls: k2 } = xe("hue-slider"), { isSupported: w2, open: x2, sRGBHex: S2 } = Ie(), _2 = a(() => m2.color.get("hue"));
  function $2() {
...
} }), [["__scopeId", "data-v-b2d0e443"]]), no = { class: "background" }, lo = { class: "color-text" }, io = { class: "color-input" }, ro = { class: "color-input-percentage" }, so = { class: "color-input-percentage" }, uo = { class: "color-input-percentage" }, co = { class: "color-input-percentage a" }, po = to(e({ __name: "ColorText", props: { color: { type: Object, required: true } }, setup(e2) {
  t((e3) => ({ "09aca737": v2.value }));
  const o2 = e2, i2 = m(""), f2 = l({ r: 0, g: 0, b: 0, a: 100 }), { prefixCls: h2 } = xe("color-text"), { token: y2 } = P.useToken(), v2 = a(() => y2.value.colorBorder), C2 = a(() => `rgba(${f2.r}, ${f2.g}, ${f2.b}, ${f2.a / 100})`);
  function b2() {
  ...
  }), (e3, t2) => (s(), r("div", { class: c(d(h2)) }, [p("div", no, [p("div", { class: "background-color", style: u({ backgroundColor: C2.value }) }, null, 4)]), p("div", lo, [p("div", io, [g(d(H), { size: "small", value: i2.value, "onUpdate:value": t2[0] || (t2[0] = (e4) => i2.value = e4), onPressEnter: M2, onBlur: M2, maxlength: 7 }, null, 8, ["value"])])]), p("div", ro, [t2[5] || (t2[5] = p("div", { class: "prefix" }, "R", -1)), g(d(Q), { size: "small", value: f2.r, "onUpdate:value": t2[1] || (t2[1] = (e4) => f2.r = e4), bordered: false, controls: false, onPressEnter: k2, onBlur: k2, step: 1, min: 0, max: 255, maxlength: 3 }, null, 8, ["value"])]), p("div", so, [t2[6] || (t2[6] = p("div", { class: "prefix" }, "G", -1)), g(d(Q), { size: "small", bordered: false, value: f2.g, "onUpdate:value": t2[2] || (t2[2] = (e4) => f2.g = e4), controls: false, onPressEnter: k2, onBlur: k2, step: 1, min: 0, max: 255, maxlength: 3 }, null, 8, ["value"])]), p("div", uo, [t2[7] || (t2[7] = p("div", { class: "prefix" }, "B", -1)), g(d(Q), { size: "small", bordered: false, value: f2.b, "onUpdate:value": t2[3] || (t2[3] = (e4) => f2.b = e4), controls: false, onPressEnter: k2, onBlur: k2, step: 1, min: 0, max: 255, maxlength: 3 }, null, 8, ["value"])]), p("div", co, [t2[8] || (t2[8] = p("div", { class: "prefix" }, "A", -1)), g(d(Q), { size: "small", bordered: false, value: f2.a, "onUpdate:value": t2[4] || (t2[4] = (e4) => f2.a = e4), controls: false, onPressEnter: b2, onChange: b2, step: 1, min: 0, max: 100, maxlength: 3 }, null, 8, ["value"]), t2[9] || (t2[9] = p("div", null, "%", -1))])], 2));
} }), [["__scopeId", "data-v-2bab5fee"]]), fo = { class: "colors" }, ho = ["onClick"], go = e({ __name: "ColorRecently", props: { color: { type: Object, required: true }, isShow: { type: Boolean, default: false } }, setup(e2) {
  t((e3) => ({ "8433eb3a": g2.value }));
  const o2 = e2, l2 = m(zt), i2 = m([]), { prefixCls: f2 } = xe("color-recently"), { token: h2 } = P.useToken(), g2 = a(() => h2.value.colorBorder);
  function C2(e3, t2) {
    return e3.map((e4) => {
      const o3 = new Xt();
  ...
  }, { immediate: true }), (e3, t2) => (s(), r("div", { class: c(d(f2)) }, [t2[0] || (t2[0] = p("div", { class: "last-time-text" }, "最近使用", -1)), p("div", fo, [(s(true), r(y, null, v(i2.value, (e4, t3) => (s(), r("div", { class: c(["color-selector", { selected: e4.selected }]), key: t3, onClick: (e5) => function(e6) {
    o2.color.fromString(l2.value[e6]);
  }(t3) }, [p("div", { class: "default", style: u({ "background-color": e4.value }) }, null, 4)], 10, ho))), 128))])], 2));
} }), mo = to(go, [["__scopeId", "data-v-98ec7ef1"]]), yo = { class: "colors" }, vo = ["onClick"], Co = e({ __name: "GradientColorRecently", props: { color: { type: String, default: "" }, isShow: { type: Boolean, default: false } }, emits: ["selectColor"], setup(e2, { emit: o2 }) {
  t((e3) => ({ "7fb04f44": C2.value }));
  const l2 = e2, i2 = o2, f2 = m([]), { prefixCls: h2 } = xe("color-recently"), { token: g2 } = P.useToken(), C2 = a(() => g2.value.colorBorder), b2 = m();
  return n(() => l2.isShow, (e3) => {
    e3 && (!function() {
      const e4 = localStorage.getItem(jt);
      if (!e4) return void (f2.value = Et);
      const t2 = JSON.parse(e4);
  ...
  }, { immediate: true }), (e3, t2) => (s(), r("div", { class: c(d(h2)) }, [t2[0] || (t2[0] = p("div", { class: "last-time-text" }, "最近使用", -1)), p("div", yo, [(s(true), r(y, null, v(f2.value, (e4, t3) => (s(), r("div", { class: c(["color-selector", { selected: b2.value === t3 }]), key: t3, onClick: (o3) => function(e5, t4) {
    b2.value = e5;
    const o4 = Pt(t4);
    i2("selectColor", o4);
  }(t3, e4) }, [p("div", { class: "default", style: u({ background: e4 }) }, null, 4)], 10, vo))), 128))])], 2));
} }), bo = to(Co, [["__scopeId", "data-v-be7486eb"]]), Mo = { class: "hue-slider" }, ko = ["onMousedown", "onDblclick"], wo = { class: "color-input-percentage" }, xo = e({ __name: "GradientColor", props: { isShow: { type: Boolean, default: false }, colors: { type: Object, default: () => ({ deg: 45, colors: [{ rgba: "rgba(255, 255, 0, 1)", rgbaLength: "0%" }, { rgba: "rgba(0, 0, 255, 0.9)", rgbaLength: "100%" }] }) } }, emits: ["colorChange", "startEyeDropper", "endEyeDropper"], setup(e2, { emit: l2 }) {
  t((e3) => ({ 40133406: T2.value }));
  const i2 = e2, f2 = l2, h2 = m(new Xt({ enableAlpha: true, format: "rgba" }));
  const w2 = m(0), x2 = m([]), S2 = m(45), _2 = m(), $2 = o("barRef"), L2 = m([]), { prefixCls: I2 } = xe("gradient-color"), { token: D2 } = P.useToken(), T2 = a(() => D2.value.colorBorder), N2 = a(() => _2.value ? { background: _2.value } : {});
  function B2(e3) {
  ...
  }, { deep: true }), (t2, o2) => (s(), r("div", null, [p("div", { class: c(d(I2)) }, [p("div", Mo, [g(d(G), { title: "点击增加新颜色", placement: "left" }, { default: M(() => [p("div", { class: "bar", ref_key: "barRef", ref: $2, onClick: k(q2, ["stop", "prevent"]), style: u(N2.value) }, null, 4)]), _: 1 }), (s(true), r(y, null, v(x2.value, (e3, t3) => (s(), r("div", { class: "thumb", ref_for: true, ref: (e4) => function(e5, t4) {
    L2.value[t4] = e5;
  ...
  }(t3), style: u({ left: e3.rgbaLength }) }, [g(d(Be), { title: "双击取消", class: c(["thumb__icon", w2.value === t3 ? "selected" : ""]), icon: "ant-design:caret-up-outlined" }, null, 8, ["class"])], 44, ko))), 128))]), p("div", wo, [o2[3] || (o2[3] = p("div", { class: "prefix" }, "D", -1)), g(d(Q), { value: S2.value, "onUpdate:value": o2[0] || (o2[0] = (e3) => S2.value = e3), bordered: false, controls: false, max: 360, min: -360, step: 1, onChange: O2, maxlength: 4 }, null, 8, ["value"]), o2[4] || (o2[4] = p("div", { class: "suffix" }, "°", -1))])], 2), g(oo, { style: { "margin-top": "4px" }, color: h2.value, height: 118, "is-show": e2.isShow }, null, 8, ["color", "is-show"]), g(ao, { style: { "margin-top": "8px" }, color: h2.value, onStartEyeDropper: o2[1] || (o2[1] = () => f2("startEyeDropper")), onEndEyeDropper: o2[2] || (o2[2] = (e3) => f2("endEyeDropper", e3)) }, null, 8, ["color"]), g(po, { style: { "margin-top": "15px" }, color: h2.value }, null, 8, ["color"]), g(bo, { style: { "margin-top": "8px" }, color: _2.value, "is-show": e2.isShow, onSelectColor: B2 }, null, 8, ["color", "is-show"])]));
} }), So = to(xo, [["__scopeId", "data-v-cf05475f"]]), _o = { class: "header" }, $o = { key: 0, class: "content" }, Lo = { key: 1, class: "content" }, Io = { class: "confirm" }, Do = { key: 0 }, To = { key: 1 }, No = e({ __name: "PickerDropdown", props: { type: { type: Number, default: 3 }, selectType: { type: Number, default: 1 }, color: { type: Object, required: true }, colors: { type: Object, default: () => ({ deg: 45, colors: [{ rgba: "rgba(255, 255, 0, 1)", rgbaLength: "0%" }, { rgba: "rgba(0, 0, 255, 0.9)", rgbaLength: "100%" }] }) }, isShow: { type: Boolean, default: false }, hideConfirm: { type: Boolean, default: false }, hideCancel: { type: Boolean, default: false } }, emits: ["changeGradientColor", "pickColor", "restColor", "selectTypeChange", "startEyeDropper", "endEyeDropper"], setup(e2, { emit: t2 }) {
  const o2 = e2, a2 = t2, { copy: l2, copied: u2 } = De({ legacy: true }), y2 = m([{ value: 1, payload: "纯色" }, { value: 2, payload: "渐变色" }]), v2 = m(), C2 = m(o2.selectType);
  function b2(e3) {
  ...
  return (t3, a3) => {
    const n2 = w("AButton");
    return s(), r("div", { class: c(d(k2)) }, [p("div", _o, [g(d(U), { value: C2.value, options: y2.value, onChange: b2, block: "" }, { label: M(({ payload: e3 }) => [x(S(e3), 1)]), _: 1 }, 8, ["value", "options"])]), 1 !== C2.value || 1 !== e2.type && 3 !== e2.type ? f("", true) : (s(), r("div", $o, [g(oo, { style: { "margin-top": "4px" }, color: e2.color, "is-show": e2.isShow }, null, 8, ["color", "is-show"]), g(ao, { style: { "margin-top": "8px" }, color: e2.color, onEndEyeDropper: T2, onStartEyeDropper: D2 }, null, 8, ["color"]), g(po, { style: { "margin-top": "15px" }, color: e2.color }, null, 8, ["color"]), g(mo, { style: { "margin-top": "8px" }, color: e2.color, "is-show": e2.isShow }, null, 8, ["color", "is-show"])])), 2 !== C2.value || 2 !== e2.type && 3 !== e2.type ? f("", true) : (s(), r("div", Lo, [g(So, { onColorChange: d(L2), colors: e2.colors, "is-show": e2.isShow, onStartEyeDropper: D2, onEndEyeDropper: T2 }, null, 8, ["onColorChange", "colors", "is-show"])])), p("div", Io, [o2.hideCancel ? f("", true) : (s(), h(n2, { key: 0, onClick: $2 }, { default: M(() => a3[0] || (a3[0] = [x("取消")])), _: 1, __: [0] })), g(n2, { onClick: I2, color: d(u2) ? "warning" : "primary" }, { default: M(() => [d(u2) ? (s(), r("span", To, "!已拷贝")) : (s(), r("span", Do, "拷贝"))]), _: 1 }, 8, ["color"]), o2.hideConfirm ? f("", true) : (s(), h(n2, { key: 1, onClick: _2, color: "success" }, { default: M(() => a3[1] || (a3[1] = [x("确定")])), _: 1, __: [1] }))])], 2);
  };
} }), Bo = { key: 0, class: "color-background-text" }, Ro = { class: "color-background-show-box" }, qo = { key: 1, style: { display: "flex" } }, Oo = { class: "color-text" }, Ao = { class: "color-show-box" }, jo = ["title"], zo = e({ name: "MfishColorPicker", __name: "index", props: { value: String, showAlpha: { type: Boolean, default: true }, type: { type: Number, default: 3 }, isDelete: { type: Boolean, default: false }, placeholder: { type: String, default: "请输入颜色，格式#FFFFFF" }, title: { type: String, default: "清空则为无色" }, hideConfirm: { type: Boolean, default: false }, hideCancel: { type: Boolean, default: false } }, emits: ["inputChange", "confirmChange"], setup(e2, { emit: o2 }) {
  t((e3) => ({ "018cd00d": I2.value }));
  const l2 = e2, i2 = o2, { prefixCls: y2 } = xe("color-picker"), v2 = m(1), b2 = Se().getDarkMode, w2 = m(false), x2 = m(new Xt({ value: d(b2) === Ge.DARK ? "#000000" : "#FFFFFF", enableAlpha: l2.showAlpha, format: "rgba" })), S2 = m({ deg: 120, colors: [{ rgba: "rgba(243, 231, 233, 1)", rgbaLength: "0%" }, { rgba: "rgba(227,238,255,1)", rgbaLength: "100%" }] }), _2 = m(""), $2 = m(100), { token: L2 } = P.useToken(), I2 = a(() => L2.value.colorBorder), D2 = m(), T2 = m(), N2 = a(() => ({ background: d(D2), height: "100%" }));
  n(() => l2.value, () => {
    O2();
  }, { immediate: true }), n(x2, (e3) => {
  ...
  function G2(e3) {
    e3.startsWith("linear-gradient(") ? (D2.value = e3, S2.value = Pt(e3), i2("inputChange", e3)) : (e3.value.fromString(e3), _2.value = e3.value.toHex());
  }
  function U2(e3) {
    R2.value = false, 2 === v2.value ? (D2.value = e3, S2.value = Pt(e3), Y2(e3), C(() => i2("confirmChange", d(D2), d(_2)))) : z2();
  }
  function W2() {
  ...
  function Y2(e3) {
    ...
    if (2 === v2.value) {
      return o3(localStorage.getItem(jt), Et, e3), t2 = t2.slice(0, 14), void localStorage.setItem(jt, JSON.stringify(t2));
    }
    o3(localStorage.getItem(At), zt, e3), t2 = t2.slice(0, 20), localStorage.setItem(At, JSON.stringify(t2));
  }
  function Z2(e3) {
  ...
  return (t2, o3) => (s(), h(d(V), { trigger: ["click"], open: R2.value, onOpenChange: q2, placement: "bottom", arrow: { pointAtCenter: true } }, { overlay: M(() => [g(No, { color: x2.value, colors: S2.value, type: e2.type, "select-type": v2.value, "is-show": R2.value, "hide-cancel": e2.hideCancel, "hide-confirm": e2.hideConfirm, onPickColor: U2, onRestColor: W2, onChangeGradientColor: G2, onSelectTypeChange: Z2, onMouseleave: o3[0] || (o3[0] = (e3) => !K2.value && q2(false)), onStartEyeDropper: X2, onEndEyeDropper: J2 }, null, 8, ["color", "colors", "type", "select-type", "is-show", "hide-cancel", "hide-confirm"])]), default: M(() => [p("div", { class: c(d(y2)), onMouseenter: o3[5] || (o3[5] = () => w2.value = true), onMouseleave: o3[6] || (o3[6] = () => w2.value = false) }, [2 === v2.value ? (s(), r("div", Bo, [p("div", Ro, [p("div", { style: u(N2.value) }, null, 4)]), w2.value ? (s(), h(d(Be), { key: 0, onClick: k(ee2, ["stop"]), size: "14", class: "close-icon", icon: "ant-design:close-circle-filled" })) : f("", true)])) : (s(), r("div", qo, [p("div", Oo, [p("div", Ao, [p("div", { style: u(N2.value) }, null, 4)]), p("div", { class: "color-input", title: e2.title, onClick: o3[2] || (o3[2] = k(() => {
  }, ["stop"])) }, [g(d(H), { placeholder: e2.placeholder, value: _2.value, "onUpdate:value": o3[1] || (o3[1] = (e3) => _2.value = e3), onBlur: E2, onPressEnter: E2, bordered: false, maxlength: 7 }, null, 8, ["placeholder", "value"]), w2.value && _2.value ? (s(), h(d(Be), { key: 0, onClick: k(ee2, ["stop"]), size: "14", class: "close-icon", icon: "ant-design:close-circle-filled" })) : f("", true)], 8, jo)]), e2.showAlpha ? (s(), r("div", { key: 0, class: "color-input-percentage", onClick: o3[4] || (o3[4] = k(() => {
...
} }), Eo = to(zo, [["__scopeId", "data-v-be499b94"]]);
const Fo = { info(...e2) {
}, warn(...e2) {
}, error(...e2) {
} };
function Po() {
  const e2 = m(), t2 = [], o2 = l({ width: 0, height: 0 }), a2 = () => {
    !function(e3, t3) {
      ...
      e3 ? o3 && a3 || Fo.warn("Component width or height is 0px, rendering abnormality may occur!") : Fo.warn("Failed to get dom node, component rendering may be abnormal!"), t3.width = o3, t3.height = a3;
    }(e2.value, o2);
  }, n2 = Oe(a2, 100), r2 = Ae((t3) => {
    const o3 = function(e3) {
      return je(e3) ? e3 : je(e3.$el) ? e3.$el : null;
    }(t3);
    o3 ? e2.value = o3 : Fo.error("Bind Component Dom Ref Failed!");
  });
  return i(() => {
    a2();
  ...
  function r2(t3) {
    return (o3) => {
      const r3 = document.createElement("style"), s2 = (e2) => {
        r3.innerHTML = ((e3, t5) => {
          const o4 = l2(t5);
          return `${Go} {${e3.toString()}}`.replaceAll(Go, o4);
        })(o3, e2);
...
Uo.setClassNamePrefix;
const Vo = Uo.div`
  position: relative;
  width: 100%;
  height: 100%;
`("border-box"), Wo = Uo.div`
  position: relative;
  width: 100%;
  height: 100%;
`("border-box-content"), Yo = ["#2e6099", "#7ce7fd"], Zo = ["left-top", "right-top", "left-bottom", "right-bottom"], Ko = Uo.svg`
  position: absolute;
  display: block;
...
`("border"), Xo = e({ name: "BorderBox1", props: Ho(), setup(e2, { slots: t2, expose: o2 }) {
  const { autoBindRef: a2, refreshLayoutSize: n2, domSize: l2 } = Po();
  return o2({ refreshLayoutSize: n2 }), () => {
    const { color: o3, backgroundColor: n3 } = e2, { width: i2, height: r2 } = l2, s2 = Qo(Yo, o3);
    return g(Vo, { class: Uo.getClassNameForBind("border-box-1"), ref: a2 }, { default: () => [g(Ko, { width: i2, height: r2 }, { default: () => [g("polygon", { fill: n3, points: `10, 27 10, ${r2 - 27} 13, ${r2 - 24} 13, ${r2 - 21} 24, ${r2 - 11} 38, ${r2 - 11}
...
`("border-svg-container"), ta = e({ name: "BorderBox2", props: Ho(), setup(e2, { slots: t2, expose: o2 }) {
  const { autoBindRef: a2, refreshLayoutSize: n2, domSize: l2 } = Po();
  return o2({ refreshLayoutSize: n2 }), () => {
    const { color: o3, backgroundColor: n3 } = e2, { width: i2, height: r2 } = l2, s2 = Qo(Jo, o3);
    return g(Vo, { class: Uo.getClassNameForBind("border-box-2"), ref: a2 }, { default: () => [g(ea, { width: i2, height: r2 }, { default: () => [g("polygon", { fill: n3, points: `7, 7 ${i2 - 7}, 7 ${i2 - 7}, ${r2 - 7} 7, ${r2 - 7}` }, null), g("polyline", { stroke: s2[0], points: `2, 2 ${i2 - 2} ,2 ${i2 - 2}, ${r2 - 2} 2, ${r2 - 2} 2, 2` }, null), g("polyline", { stroke: s2[1], points: `6, 6 ${i2 - 6}, 6 ${i2 - 6}, ${r2 - 6} 6, ${r2 - 6} 6, 6` }, null), g("circle", { fill: s2[0], cx: "11", cy: "11", r: "1" }, null), g("circle", { fill: s2[0], cx: i2 - 11, cy: "11", r: "1" }, null), g("circle", { fill: s2[0], cx: i2 - 11, cy: r2 - 11, r: "1" }, null), g("circle", { fill: s2[0], cx: "11", cy: r2 - 11, r: "1" }, null)] }), g(Wo, null, { default: () => [t2.default?.()] })] });
  };
} }), oa = ["#2e6099", "#7ce7fd"], aa = Uo.svg`
  position: absolute;
  width: 100%;
  height: 100%;
  top: 0;
...
`("border-svg-container"), na = e({ name: "BorderBox3", props: Ho(), setup(e2, { slots: t2, expose: o2 }) {
  const { autoBindRef: a2, refreshLayoutSize: n2, domSize: l2 } = Po();
  return o2({ refreshLayoutSize: n2 }), () => {
    const { color: o3, backgroundColor: n3 } = e2, { width: i2, height: r2 } = l2, s2 = Qo(oa, o3);
    return g(Vo, { class: Uo.getClassNameForBind("border-box-3"), ref: a2 }, { default: () => [g(aa, { width: i2, height: r2 }, { default: () => [g("polygon", { fill: n3, points: `23, 23 ${i2 - 24}, 23 ${i2 - 24}, ${r2 - 24} 23, ${r2 - 24}` }, null), g("polyline", { class: "stroke-width-3", stroke: s2[0], points: `4, 4 ${i2 - 22} ,4 ${i2 - 22}, ${r2 - 22} 4, ${r2 - 22} 4, 4` }, null), g("polyline", { class: "stroke-width-1", stroke: s2[1], points: `10, 10 ${i2 - 16}, 10 ${i2 - 16}, ${r2 - 16} 10, ${r2 - 16} 10, 10` }, null), g("polyline", { class: "stroke-width-1", stroke: s2[1], points: `16, 16 ${i2 - 10}, 16 ${i2 - 10}, ${r2 - 10} 16, ${r2 - 10} 16, 16` }, null), g("polyline", { class: "stroke-width-1", stroke: s2[1], points: `22, 22 ${i2 - 4}, 22 ${i2 - 4}, ${r2 - 4} 22, ${r2 - 4} 22, 22` }, null)] }), g(Wo, null, { default: () => [t2.default?.()] })] });
  };
} }), la = ["#2e6099", "#7ce7fd"], ia = Uo.svg`
  position: absolute;
  width: 100%;
  height: 100%;
  top: 0;
...
`("border-svg-container"), ca = e({ name: "BorderBox5", props: { ...Ho(), reverse: { type: Boolean, default: false } }, setup(e2, { slots: t2, expose: o2 }) {
  const { autoBindRef: a2, refreshLayoutSize: n2, domSize: l2 } = Po();
  return o2({ refreshLayoutSize: n2 }), () => {
    const { color: o3, backgroundColor: n3, reverse: i2 } = e2, { width: r2, height: s2 } = l2, u2 = Qo(sa, o3);
    return g(Vo, { class: Uo.getClassNameForBind("border-box-5"), ref: a2 }, { default: () => [g(ua, { class: i2 ? "reverse" : "", width: r2, height: s2 }, { default: () => [g("polygon", { fill: n3, points: `
...
`("border-svg-container"), fa = e({ name: "BorderBox6", props: Ho(), setup(e2, { slots: t2, expose: o2 }) {
  const { autoBindRef: a2, refreshLayoutSize: n2, domSize: l2 } = Po();
  return o2({ refreshLayoutSize: n2 }), () => {
    const { color: o3, backgroundColor: n3 } = e2, { width: i2, height: r2 } = l2, s2 = Qo(da, o3);
    return g(Vo, { class: Uo.getClassNameForBind("border-box-6"), ref: a2 }, { default: () => [g(pa, { width: i2, height: r2 }, { default: () => [g("polygon", { fill: n3, points: `
              9, 7 ${i2 - 9}, 7 ${i2 - 9}, ${r2 - 7} 9, ${r2 - 7}` }, null), g("circle", { fill: s2[1], cx: "5", cy: "5", r: "2" }, null), g("circle", { fill: s2[1], cx: i2 - 5, cy: "5", r: "2" }, null), g("circle", { fill: s2[1], cx: i2 - 5, cy: r2 - 5, r: "2" }, null), g("circle", { fill: s2[1], cx: "5", cy: r2 - 5, r: "2" }, null), g("polyline", { stroke: s2[0], points: `10, 4 ${i2 - 10}, 4` }, null), g("polyline", { stroke: s2[0], points: `10, ${r2 - 4} ${i2 - 10}, ${r2 - 4}` }, null), g("polyline", { stroke: s2[0], points: "5, 70 5, " + (r2 - 70) }, null), g("polyline", { stroke: s2[0], points: `${i2 - 5}, 70 ${i2 - 5}, ${r2 - 70}` }, null), g("polyline", { stroke: s2[0], points: "3, 10, 3, 50" }, null), g("polyline", { stroke: s2[0], points: "7, 30 7, 80" }, null), g("polyline", { stroke: s2[0], points: `${i2 - 3}, 10 ${i2 - 3}, 50` }, null), g("polyline", { stroke: s2[0], points: `${i2 - 7}, 30 ${i2 - 7}, 80` }, null), g("polyline", { stroke: s2[0], points: `3, ${r2 - 10} 3, ${r2 - 50}` }, null), g("polyline", { stroke: s2[0], points: `7, ${r2 - 30} 7, ${r2 - 80}` }, null), g("polyline", { stroke: s2[0], points: `${i2 - 3}, ${r2 - 10} ${i2 - 3}, ${r2 - 50}` }, null), g("polyline", { stroke: s2[0], points: `${i2 - 7}, ${r2 - 30} ${i2 - 7}, ${r2 - 80}` }, null)] }), g(Wo, null, { default: () => [t2.default?.()] })] });
  };
} }), ha = ["#2e6099", "#7ce7fd"], ga = Uo.svg`
  position: absolute;
  top: 0;
  left: 0;
  width: 100%;
...
`("border-svg-container"), ma = e({ name: "BorderBox7", props: Ho(), setup(e2, { slots: t2, expose: o2 }) {
  const { autoBindRef: a2, refreshLayoutSize: n2, domSize: l2 } = Po();
  return o2({ refreshLayoutSize: n2 }), () => {
    const { color: o3, backgroundColor: n3 } = e2, { width: i2, height: r2 } = l2, s2 = Qo(ha, o3);
    return g(Vo, { class: Uo.getClassNameForBind("border-box-7"), ref: a2, style: { boxShadow: `inset 0 0 40px ${s2[0]}`, border: `1px solid ${s2[0]}`, backgroundColor: n3 } }, { default: () => [g(ga, { width: i2, height: r2 }, { default: () => [g("polyline", { class: "stroke-width2", stroke: s2[0], points: "0, 25 0, 0 25, 0" }, null), g("polyline", { class: "stroke-width2", stroke: s2[0], points: `${i2 - 25}, 0 ${i2}, 0 ${i2}, 25` }, null), g("polyline", { class: "stroke-width2", stroke: s2[0], points: `${i2 - 25}, ${r2} ${i2}, ${r2} ${i2}, ${r2 - 25}` }, null), g("polyline", { class: "stroke-width2", stroke: s2[0], points: `0, ${r2 - 25} 0, ${r2} 25, ${r2}` }, null), g("polyline", { class: "stroke-width5", stroke: s2[1], points: "0, 10 0, 0 10, 0" }, null), g("polyline", { class: "stroke-width5", stroke: s2[1], points: `${i2 - 10}, 0 ${i2}, 0 ${i2}, 10` }, null), g("polyline", { class: "stroke-width5", stroke: s2[1], points: `${i2 - 10}, ${r2} ${i2}, ${r2} ${i2}, ${r2 - 10}` }, null), g("polyline", { class: "stroke-width5", stroke: s2[1], points: `0, ${r2 - 10} 0, ${r2} 10, ${r2}` }, null)] }), g(Wo, null, { default: () => [t2.default?.()] })] });
  };
} });
function ya() {
  return L(l({ id: Ee("datav_uuid") }));
}
const va = ["#2e6099", "#7ce7fd"], Ca = Uo.svg`
  position: absolute;
  width: 100%;
  height: 100%;
  left: 0;
  top: 0;
`("border-svg-container"), ba = e({ name: "BorderBox8", props: { ...Ho(), duration: { type: Number, default: 3 }, reverse: { type: Boolean, default: false } }, setup(e2, { slots: t2, expose: o2 }) {
  const { autoBindRef: a2, refreshLayoutSize: n2, domSize: l2 } = Po();
  o2({ refreshLayoutSize: n2 });
  const i2 = ya();
  return () => {
    const { color: o3, backgroundColor: n3, duration: r2, reverse: s2 } = e2, { width: u2, height: c2 } = l2, d2 = Qo(va, o3), p2 = `border-box-8-path-${i2.id}`, f2 = `border-box-8-gradient-${i2.id}`, h2 = `border-box-8-mask-${i2.id}`, m2 = s2 ? `M 2.5, 2.5 L 2.5, ${c2 - 2.5} L ${u2 - 2.5}, ${c2 - 2.5} L ${u2 - 2.5}, 2.5 L 2.5, 2.5` : `M2.5, 2.5 L${u2 - 2.5}, 2.5 L${u2 - 2.5}, ${c2 - 2.5} L2.5, ${c2 - 2.5} L2.5, 2.5`;
    return g(Vo, { class: Uo.getClassNameForBind("border-box-8"), ref: a2 }, { default: () => [g(Ca, { width: u2, height: c2 }, { default: () => [g("defs", null, [g("path", { id: p2, d: m2, fill: "transparent" }, null), g("radialGradient", { id: f2, cx: "50%", cy: "50%", r: "50%" }, [g("stop", { offset: "0%", "stop-color": "#fff", "stop-opacity": "1" }, null), g("stop", { offset: "100%", "stop-color": "#fff", "stop-opacity": "0" }, null)]), g("mask", { id: h2 }, [g("circle", { cx: "0", cy: "0", r: "150", fill: `url(#${f2})` }, [g("animateMotion", { dur: `${r2}s`, path: m2, rotate: "auto", repeatCount: "indefinite" }, null)])])]), g("polygon", { fill: n3, points: `5, 5 ${u2 - 5}, 5 ${u2 - 5} ${c2 - 5} 5, ${c2 - 5}` }, null), g("use", { stroke: d2[0], "stroke-width": "1", "xlink:href": `#${p2}` }, null), g("use", { stroke: d2[1], "stroke-width": "3", "xlink:href": `#${p2}`, mask: `url(#${h2})` }, [g("animate", { attributeName: "stroke-dasharray", from: `0, ${length}`, to: `${length}, 0`, dur: `${r2}s`, repeatCount: "indefinite" }, null)])] }), g(Wo, null, { default: () => [t2.default?.()] })] });
  };
} }), Ma = ["#2e6099", "#7ce7fd"], ka = Uo.svg`
  position: absolute;
  width: 100%;
  height: 100%;
  left: 0;
  top: 0;
`("border-svg-container"), wa = e({ name: "BorderBox9", props: Ho(), setup(e2, { slots: t2, expose: o2 }) {
  const { autoBindRef: a2, refreshLayoutSize: n2, domSize: l2 } = Po();
  o2({ refreshLayoutSize: n2 });
  const i2 = ya();
  return () => {
    const { color: o3, backgroundColor: n3 } = e2, { width: r2, height: s2 } = l2, u2 = Qo(Ma, o3), c2 = `border-box-9-gradient-${i2.id}`, d2 = `border-box-9-mask-${i2.id}`;
    return g(Vo, { class: Uo.getClassNameForBind("border-box-9"), ref: a2 }, { default: () => [g(ka, { width: r2, height: s2 }, { default: () => [g("defs", null, [g("linearGradient", { id: c2, x1: "0%", y1: "0%", x2: "100%", y2: "100%" }, [g("animate", { attributeName: "x1", values: "0%;100%;0%", dur: "10s", begin: "0s", repeatCount: "indefinite" }, null), g("animate", { attributeName: "x2", values: "100%;0%;100%", dur: "10s", begin: "0s", repeatCount: "indefinite" }, null), g("stop", { offset: "0%", "stop-color": u2[0] }, [g("animate", { attributeName: "stop-color", values: `${u2[0]};${u2[1]};${u2[0]}`, dur: "10s", begin: "0s", repeatCount: "indefinite" }, null)]), g("stop", { offset: "100%", "stop-color": u2[1] }, [g("animate", { attributeName: "stop-color", values: `${u2[1]};${u2[0]};${u2[1]}`, dur: "10s", begin: "0s", repeatCount: "indefinite" }, null)])]), g("mask", { id: d2 }, [g("polyline", { stroke: "#fff", "stroke-width": "3", fill: "transparent", points: `8, ${0.4 * s2} 8, 3, ${0.4 * r2 + 7}, 3` }, null), g("polyline", { fill: "#fff", points: `
                ...
                ` }, null), g("rect", { x: "0", y: "0", width: r2, height: s2, fill: `url(#${c2})`, mask: `url(#${d2})` }, null)] }), g(Wo, null, { default: () => [t2.default?.()] })] });
  };
} }), xa = ["#2e6099", "#7ce7fd"], Sa = ["left-top", "right-top", "left-bottom", "right-bottom"], _a = Uo.svg`
  position: absolute;
  display: block;
...
`("border-svg-container"), $a = e({ name: "BorderBox10", props: Ho(), setup(e2, { slots: t2, expose: o2 }) {
  const { autoBindRef: a2, refreshLayoutSize: n2, domSize: l2 } = Po();
  return o2({ refreshLayoutSize: n2 }), () => {
    const { width: o3, height: n3 } = l2, { backgroundColor: i2, color: r2 } = e2, s2 = Qo(xa, r2);
    return g(Vo, { class: Uo.getClassNameForBind("border-box-10"), ref: a2, style: { boxShadow: `inset 0 0 25px 3px ${s2[0]}` } }, { default: () => [g(_a, { width: o3, height: n3 }, { default: () => [g("polygon", { fill: i2, points: `
                  4, 0 ${o3 - 4}, 0 ${o3}, 4 ${o3}, ${n3 - 4} ${o3 - 4}, ${n3}
                  4, ${n3} 0, ${n3 - 4} 0, 4
                ` }, null)] }), Sa.map((e3) => g(_a, { width: "150px", height: "150px", key: e3, class: e3 }, { default: () => [g("polygon", { fill: s2[1], points: "40, 0 5, 0 0, 5 0, 16 3, 19 3, 7 7, 3 35, 3" }, null)] })), g(Wo, null, { default: () => [t2.default?.()] })] });
  };
} }), La = ["#2e6099", "#7ce7fd"], Ia = Uo.svg`
  position: absolute;
  width: 100%;
...
`("border-svg-container"), Da = e({ name: "BorderBox11", props: { ...Ho(), titleWidth: { type: Number, default: 250 }, title: { type: String, default: "" } }, setup(e2, { slots: t2, expose: o2 }) {
  const { autoBindRef: a2, refreshLayoutSize: n2, domSize: l2 } = Po();
  o2({ refreshLayoutSize: n2 });
  const i2 = ya();
  return () => {
    const { color: o3, backgroundColor: n3, titleWidth: r2, title: s2 } = e2, { width: u2, height: c2 } = l2, d2 = Qo(La, o3), p2 = `border-box-11-filterId-${i2}`;
    return g(Vo, { class: Uo.getClassNameForBind("border-box-11"), ref: a2 }, { default: () => [g(Ia, { width: u2, height: c2 }, { default: () => [g("defs", null, [g("filter", { id: p2, height: "150%", width: "150%", x: "-25%", y: "-25%" }, [g("feMorphology", { operator: "dilate", radius: "2", in: "SourceAlpha", result: "thicken" }, null), g("feGaussianBlur", { in: "thicken", stdDeviation: "3", result: "blurred" }, null), g("feFlood", { "flood-color": d2[1], result: "glowColor" }, null), g("feComposite", { in: "glowColor", in2: "blurred", operator: "in", result: "softGlowColored" }, null), g("feMerge", null, [g("feMergeNode", { in: "softGlowColored" }, null), g("feMergeNode", { in: "SourceGraphic" }, null)])])]), g("polygon", { fill: n3, points: `
                ...
                ` }, null), g("polygon", { stroke: d2[0], fill: Ue(d2[1] || La[1], 30), filter: `url(#${p2})`, points: `
                ...
                ` }, null)] }), g(Wo, null, { default: () => [t2.default?.()] })] });
  };
} }), Ta = ["#2e6099", "#7ce7fd"], Na = Uo.svg`
  position: absolute;
  width: 100%;
  height: 100%;
  top: 0;
  left: 0;
`("border-svg-container"), Ba = e({ name: "BorderBox12", props: Ho(), setup(e2, { slots: t2, expose: o2 }) {
  const { autoBindRef: a2, refreshLayoutSize: n2, domSize: l2 } = Po();
  o2({ refreshLayoutSize: n2 });
  const i2 = ya();
  return () => {
    const { color: o3, backgroundColor: n3 } = e2, { width: r2, height: s2 } = l2, u2 = Qo(Ta, o3), c2 = `border-box-12-filterId-${i2}`;
    return g(Vo, { class: Uo.getClassNameForBind("border-box-12"), ref: a2 }, { default: () => [g(Na, { width: r2, height: s2 }, { default: () => [g("defs", null, [g("filter", { id: c2, height: "150%", width: "150%", x: "-25%", y: "-25%" }, [g("feMorphology", { operator: "dilate", radius: "1", in: "SourceAlpha", result: "thicken" }, null), g("feGaussianBlur", { in: "thicken", stdDeviation: "2", result: "blurred" }, null), g("feFlood", { "flood-color": Ue(u2[1] || Ta[1], 70), result: "glowColor" }, [g("animate", { attributeName: "flood-color", values: `
                        ${Ue(u2[1] || Ta[1], 70)};
                        ${Ue(u2[1] || Ta[1], 30)};
                        ${Ue(u2[1] || Ta[1], 70)};
...
const hn = ["#2e6099", "#7ce7fd"], gn = 2.5, mn = function({ width: e2, height: t2, rowPoints: o2, rowCount: a2 }) {
...
}({ width: 200, height: 50, rowPoints: 20, rowCount: 4 }), yn = mn[39], vn = mn[37], Cn = Uo.div`
  width: 100%;
  height: 100%;
...
`("decoration-1"), bn = e({ name: "Decoration1", props: dn(), setup(e2, { expose: t2 }) {
  const { autoBindRef: o2, refreshLayoutSize: a2, domSize: n2 } = Po();
  return t2({ refreshLayoutSize: a2 }), () => {
    const { color: t3 } = e2, { width: a3, height: l2 } = n2, i2 = Qo(hn, t3), r2 = { transform: `scale(${a3 / 200},${l2 / 50})` };
    return g(Cn, { ref: o2 }, { default: () => [g("svg", { width: 200, height: 50, style: r2 }, [mn.map(([e3, t4], o3) => {
      const a4 = e3 - 1.25, n3 = t4 - 1.25;
      return Math.random() > 0.6 ? g("rect", { key: o3, x: a4, y: n3, width: gn, height: gn, fill: i2[0] }, [Math.random() > 0.6 && g("animate", { attributeName: "fill", values: `${i2[0]};transparent`, dur: "1s", begin: 2 * Math.random(), repeatCount: "indefinite" }, null)]) : null;
    }), g("rect", { fill: i2[1], x: yn[0] - gn, y: yn[1] - gn, width: 5, height: 5 }, [g("animate", { attributeName: "width", values: "0;5", dur: "2s", repeatCount: "indefinite" }, null), g("animate", { attributeName: "height", values: "0;5", dur: "2s", repeatCount: "indefinite" }, null), g("animate", { attributeName: "x", values: `${yn[0]};${yn[0] - gn}`, dur: "2s", repeatCount: "indefinite" }, null), g("animate", { attributeName: "y", values: `${yn[1]};${yn[1] - gn}`, dur: "2s", repeatCount: "indefinite" }, null)]), g("rect", { fill: i2[1], x: vn[0] - gn, y: vn[1] - gn, width: 5, height: 5 }, [g("animate", { attributeName: "width", values: "0;40;0", dur: "2s", repeatCount: "indefinite" }, null), g("animate", { attributeName: "x", values: `${vn[0]};${vn[0] - 40};${vn[0]}`, dur: "2s", repeatCount: "indefinite" }, null)])])] });
  };
} }), Mn = ["#2e6099", "#7ce7fd"];
const kn = Uo.div`
  display: flex;
...
`("decoration-3"), $n = e({ name: "Decoration3", props: dn(), setup(e2, { expose: t2 }) {
  const { autoBindRef: o2, refreshLayoutSize: a2, domSize: n2 } = Po();
  return t2({ refreshLayoutSize: a2 }), () => {
    const { width: t3, height: a3 } = n2, { color: l2 } = e2, i2 = Qo(xn, l2);
    return g(_n, { ref: o2 }, { default: () => [g("svg", { width: 300, height: 35, style: { transform: `scale(${t3 / 300},${a3 / 35})` } }, [Sn.map(([e3, t4], o3) => {
...
} }), Ln = ["width", "height"], In = ["stroke", "points"], Dn = ["stroke", "points"], Tn = e({ name: "Decoration4", __name: "index", props: { ...dn(), ...pn(), ...fn(3) }, setup(e2) {
  const t2 = ["#2e6099", "#7ce7fd"], n2 = o("decoration4Ref"), l2 = a(() => n2.value?.clientWidth), i2 = a(() => n2.value?.clientHeight), d2 = a(() => function(e3, t3 = []) {
    return ze(e3, t3);
  }(t2, e2.color));
  return (e3, t3) => (s(), r("div", { class: "dv-decoration-4", ref_key: "decoration4Ref", ref: n2 }, [p("div", { class: c("container " + (e3.reverse ? "reverse" : "normal")), style: u(e3.reverse ? `width:${l2.value}px;height:5px;animation-duration:${e3.duration}s` : `width:5px;height:${i2.value}px;animation-duration:${e3.duration}s`) }, [(s(), r("svg", { width: e3.reverse ? l2.value : 5, height: e3.reverse ? 5 : i2.value }, [p("polyline", { stroke: d2.value[0], points: e3.reverse ? `0, 2.5 ${l2.value}, 2.5` : `2.5, 0 2.5, ${i2.value}` }, null, 8, In), p("polyline", { class: "bold-line", stroke: d2.value[1], "stroke-width": "3", "stroke-dasharray": "20, 80", "stroke-dashoffset": "-30", points: e3.reverse ? `0, 2.5 ${l2.value}, 2.5` : `2.5, 0 2.5, ${i2.value}` }, null, 8, Dn)], 8, Ln))], 6)], 512));
} }), Nn = to(Tn, [["__scopeId", "data-v-97e7e3ce"]]), Bn = ["#2e6099", "#7ce7fd"];
function Rn(e2) {
...
const qn = Uo.div`
  width: 100%;
  height: 100%;
`("decoration-5"), On = e({ name: "Decoration5", props: { ...dn(), ...fn(1.2) }, setup(e2, { expose: t2 }) {
  const { autoBindRef: o2, refreshLayoutSize: a2, domSize: n2 } = Po();
  return t2({ refreshLayoutSize: a2 }), () => {
    const { width: t3, height: a3 } = n2, { color: l2, duration: i2 } = e2, r2 = Qo(Bn, l2), { line1Sum: s2, line2Sum: u2, line1Point: c2, line2Point: d2 } = function(e3, t4) {
      ...
      return { line1Sum: Fe(Rn(o3)), line2Sum: Fe(Rn(a4)), line1Point: o3.map((e4) => e4.join(",")).join(" "), line2Point: a4.map((e4) => e4.join(",")).join(" ") };
    }(t3, a3);
...
`("decoration-6");
const { points: zn, heights: En, minHeights: Fn, randoms: Pn } = function({ width: e2, height: t2, rowPoints: o2, rowCount: a2 }) {
...
}({ width: 300, height: 35, rowPoints: 40, rowCount: 1 }), Hn = e({ name: "Decoration6", props: dn(), setup(e2, { expose: t2 }) {
  const { autoBindRef: o2, refreshLayoutSize: a2, domSize: n2 } = Po();
  return t2({ refreshLayoutSize: a2 }), () => {
    const { width: t3, height: a3 } = n2, { color: l2 } = e2, i2 = Qo(An, l2), r2 = { transform: `scale(${t3 / 300},${a3 / 35})` };
    return g(jn, { ref: o2 }, { default: () => [g("svg", { class: "svg-origin", width: t3, height: a3, style: r2 }, [zn.map(([e3, t4], o3) => g("rect", { key: `rect${o3}`, fill: i2[Math.random() > 0.5 ? 0 : 1], x: e3 - 3.5, y: t4 - En[o3], width: 7, height: En[o3] }, [g("animate", { attributeName: "y", values: `${t4 - Fn[o3] / 2};${t4 - En[o3] / 2};${t4 - Fn[o3] / 2}`, dur: Pn[o3], keyTimes: "0;0.5;1", calcMode: "spline", keySplines: "0.42,0,0.58,1;0.42,0,0.58,1", begin: "0s", repeatCount: "indefinite" }, null), g("animate", { attributeName: "height", values: `${Fn[o3]};${En[o3]};${Fn[o3]}`, dur: Pn[o3], keyTimes: "0;0.5;1", calcMode: "spline", keySplines: "0.42,0,0.58,1;0.42,0,0.58,1", begin: "0s", repeatCount: "indefinite" }, null)]))])] });
  };
} }), Qn = Uo.div`
  display: flex;
  width: 100%;
  ...
  justify-content: center;
  align-items: center;
`("decoration-7"), Gn = ["#2e6099", "#7ce7fd"], Un = e({ name: "Decoration7", props: dn(), setup(e2, { slots: t2, expose: o2 }) {
  const { autoBindRef: a2, refreshLayoutSize: n2 } = Po();
  return o2({ refreshLayoutSize: n2 }), () => {
    const { color: o3 } = e2, n3 = Qo(Gn, o3);
    return g(Qn, { ref: a2 }, { default: () => [g("svg", { width: "21px", height: "20px" }, [g("polyline", { "stroke-width": "4", fill: "transparent", stroke: n3[0], points: "10, 0 19, 10 10, 20" }, null), g("polyline", { "stroke-width": "2", fill: "transparent", stroke: n3[1], points: "2, 0 11, 10 2, 20" }, null)]), t2.default?.(), g("svg", { width: "21px", height: "20px" }, [g("polyline", { "stroke-width": "4", fill: "transparent", stroke: n3[0], points: "11, 0 2, 10 11, 20" }, null), g("polyline", { "stroke-width": "2", fill: "transparent", stroke: n3[1], points: "19, 0 10, 10 19, 20" }, null)])] });
  };
} }), Vn = ["#2e6099", "#7ce7fd"];
const Wn = Uo.div`
  display: flex;
  width: 100%;
  height: 100%;
`("decoration-8"), Yn = e({ name: "Decoration8", props: { ...dn(), ...pn() }, setup(e2, { expose: t2 }) {
  const { autoBindRef: o2, refreshLayoutSize: a2, domSize: n2 } = Po();
  return t2({ refreshLayoutSize: a2 }), () => {
    const { color: t3, reverse: a3 } = e2, { width: l2, height: i2 } = n2, r2 = (e3) => a3 ? l2 - e3 : e3, s2 = Qo(Vn, t3);
    return g(Wn, { ref: o2 }, { default: () => [g("svg", { width: l2, height: i2 }, [g("polyline", { stroke: s2[0], "stroke-width": "2", fill: "transparent", points: `${r2(0)}, 0 ${r2(30)}, ${i2 / 2}` }, null), g("polyline", { stroke: s2[0], "stroke-width": "2", fill: "transparent", points: `${r2(20)}, 0 ${r2(50)}, ${i2 / 2} ${r2(l2)}, ${i2 / 2}` }, null), g("polyline", { stroke: s2[1], fill: "transparent", "stroke-width": "3", points: `${r2(0)}, ${i2 - 3}, ${r2(200)}, ${i2 - 3}` }, null)])] });
  };
} });
const Zn = ["#2e6099", "#7ce7fd"], Kn = Uo.div`
  position: relative;
  width: 100%;
  height: 100%;
  display: flex;
...
`("decoration-9"), Xn = e({ name: "Decoration9", props: { ...dn(), ...fn(3) }, setup(e2, { slots: t2, expose: o2 }) {
  const { autoBindRef: a2, refreshLayoutSize: n2, domSize: l2 } = Po();
  o2({ refreshLayoutSize: n2 });
  const i2 = `decoration-9-polygon-${ya().id}`;
  return () => {
    const { width: o3, height: n3 } = l2, { color: r2, duration: s2 } = e2, u2 = Qo(Zn, r2), c2 = { transform: `scale(${o3 / 100},${n3 / 100})` };
    return g(Kn, { ref: a2 }, { default: () => [g("svg", { class: "svg-origin", width: 100, height: 100, style: c2 }, [g("defs", null, [g("polygon", { id: i2, points: "15, 46.5, 21, 47.5, 21, 52.5, 15, 53.5" }, null)]), g("circle", { cx: "50", cy: "50", r: "45", fill: "transparent", stroke: u2[1], "stroke-width": "10", "stroke-dasharray": "80, 100, 30, 100" }, [g("animateTransform", { attributeName: "transform", type: "rotate", values: "0 50 50;360 50 50", dur: `${s2}s`, repeatCount: "indefinite" }, null)]), g("circle", { cx: "50", cy: "50", r: "45", fill: "transparent", stroke: u2[0], "stroke-width": "6", "stroke-dasharray": "50, 66, 100, 66" }, [g("animateTransform", { attributeName: "transform", type: "rotate", values: "0 50 50;-360 50 50", dur: `${s2}s`, repeatCount: "indefinite" }, null)]), g("circle", { cx: "50", cy: "50", r: "38", fill: "transparent", stroke: Ue(u2[1] || Zn[1], 30), "stroke-width": "1", "stroke-dasharray": "5, 1" }, null), Array.from({ length: 20 }).fill(0).map((e3, t3) => g("use", { key: t3, "xlink:href": `#${i2}`, stroke: u2[1], fill: Math.random() > 0.4 ? "transparent" : u2[0] }, [g("animateTransform", { attributeName: "transform", type: "rotate", values: "0 50 50;360 50 50", dur: `${s2}s`, begin: t3 * s2 / 20 + "s", repeatCount: "indefinite" }, null)])), g("circle", { cx: "50", cy: "50", r: "26", fill: "transparent", stroke: Ue(u2[1] || Zn[1], 30), "stroke-width": "1", "stroke-dasharray": "5, 1" }, null)]), t2.default?.()] });
  };
} }), Jn = Uo.div`
  width: 100%;
  height: 100%;
  display: flex;
`("decoration-10"), el = ["#2e6099", "#7ce7fd"], tl = e({ name: "Decoration10", props: dn(), setup(e2, { expose: t2 }) {
  const { autoBindRef: o2, refreshLayoutSize: a2, domSize: n2 } = Po();
  t2({ refreshLayoutSize: a2 });
  const l2 = ya(), i2 = `d10ani${l2.id}1`, r2 = `d10ani${l2.id}2`, s2 = `d10ani${l2.id}3`, u2 = `d10ani${l2.id}4`, c2 = `d10ani${l2.id}5`, d2 = `d10ani${l2.id}6`, p2 = `d10ani${l2.id}7`;
  return () => {
    const { width: t3, height: a3 } = n2, { color: l3 } = e2, f2 = Qo(el, l3);
...
`("decoration-11"), al = Uo.div`
  ...
  align-items: center;
  justify-content: center;
`("decoration-content"), nl = ["#2e6099", "#7ce7fd"], ll = e({ name: "Decoration11", props: dn(), setup(e2, { slots: t2, expose: o2 }) {
  const { autoBindRef: a2, refreshLayoutSize: n2, domSize: l2 } = Po();
  return o2({ refreshLayoutSize: n2 }), () => {
    const { width: o3, height: n3 } = l2, { color: i2 } = e2, r2 = Qo(nl, i2);
    return g(ol, { ref: a2 }, { default: () => [g("svg", { width: o3, height: n3 }, [g("polygon", { fill: Ue(r2[1] || nl[1], 10), stroke: r2[1], points: "20 10, 25 4, 55 4 60 10" }, null), g("polygon", { fill: Ue(r2[1] || nl[1], 10), stroke: r2[1], points: `20 ${n3 - 10}, 25 ${n3 - 4}, 55 ${n3 - 4} 60 ${n3 - 10}` }, null), g("polygon", { fill: Ue(r2[1] || nl[1], 10), stroke: r2[1], points: `${o3 - 20} 10, ${o3 - 25} 4, ${o3 - 55} 4 ${o3 - 60} 10` }, null), g("polygon", { fill: Ue(r2[1] || nl[1], 10), stroke: r2[1], points: `${o3 - 20} ${n3 - 10}, ${o3 - 25} ${n3 - 4}, ${o3 - 55} ${n3 - 4} ${o3 - 60} ${n3 - 10}` }, null), g("polygon", { fill: Ue(r2[0] || nl[0], 20), stroke: r2[0], points: `
        20 10, 5 ${n3 / 2} 20 ${n3 - 10}
        ${o3 - 20} ${n3 - 10} ${o3 - 5} ${n3 / 2} ${o3 - 20} 10
      ` }, null), g("polyline", { fill: "transparent", stroke: Ue(r2[0] || nl[0], 70), points: `25 18, 15 ${n3 / 2} 25 ${n3 - 18}` }, null), g("polyline", { fill: "transparent", stroke: Ue(r2[0] || nl[0], 70), points: `${o3 - 25} 18, ${o3 - 15} ${n3 / 2} ${o3 - 25} ${n3 - 18}` }, null)]), g(al, null, { default: () => [t2.default?.()] })] });
  };
} }), il = Uo.div`
  position: relative;
  width: 100%;
...
`("decoration-12"), rl = Uo.div`
  ...
  align-items: center;
  justify-content: center;
`("decoration-content"), sl = ["#2e6099", "#7ce7fd"], ul = Math.PI / 3;
const cl = (e2, t2, o2, a2) => [e2 + Math.cos(a2) * o2, t2 + Math.sin(a2) * o2], dl = e({ name: "Decoration12", props: { ...dn(), scanDuration: { type: Number, default: 3 }, haloDuration: { type: Number, default: 2 } }, setup(e2, { slots: t2, expose: o2 }) {
  const { autoBindRef: a2, refreshLayoutSize: n2, domSize: l2 } = Po();
  o2({ refreshLayoutSize: n2 });
  const i2 = ya();
  return () => {
    const { width: o3, height: n3 } = l2, r2 = o3 / 2, s2 = n3 / 2, { color: u2, scanDuration: c2, haloDuration: d2 } = e2, p2 = `decoration-12-g-${i2.id}`, f2 = `decoration-12-gradient-${i2.id}`, h2 = Qo(sl, u2), m2 = ((e3, t3) => {
      const o4 = 100 / (t3 - 1);
      ...
      let s3 = cl(e3, t3, r3, l3);
      return Array.from({ length: a3 }).fill("").map((o5, a4) => {
        const n5 = cl(e3, t3, r3, l3 - (a4 + 1) * i3).map((e4) => e4.toFixed(5)), u3 = `M${s3.join(",")} A${r3}, ${r3} 0 0 0 ${n5.join(",")}`;
        return s3 = n5, u3;
      });
    })({ x: r2, y: s2, width: o3, segment: 30, sectorAngle: ul }), v2 = (({ ringNum: e3, width: t3, ringWidth: o4 }) => {
      ...
      return Array.from({ length: 6 }).fill("").map((o5, l3) => {
        const i3 = a3 * (l3 + 1), r3 = i3 + Math.PI, s3 = cl(e3, t3, n4, i3), u3 = cl(e3, t3, n4, r3);
...
`("header-svg-container"), Il = e({ name: "Header6", props: Ho(), setup(e2, { expose: t2 }) {
  ...
  return n(() => c2.config, () => {
    H2(), T2 = 0, Q2();
  }), n(() => c2.config?.columnWidth, He(() => {
    G2(), V2();
  }, 100), { deep: true }), n(() => c2.config?.align, () => {
    G2(), Y2();
  }, { deep: true }), n(() => c2.theme, (e3) => {
    e3 === Ge.DARK ? (C2.value.headerBGC === z2 && (C2.value.headerBGC = O2), C2.value.oddRowBGC === E2 && (C2.value.oddRowBGC = A2), C2.value.evenRowBGC === F2 && (C2.value.evenRowBGC = j2)) : (C2.value.headerBGC === O2 && (C2.value.headerBGC = z2), C2.value.oddRowBGC === A2 && (C2.value.oddRowBGC = E2), C2.value.evenRowBGC === j2 && (C2.value.evenRowBGC = F2));
  }, { immediate: true }), i(() => {
  ...
  return (t3, o3) => (s(), r("div", { class: c(d(l2)) }, [(s(true), r(y, null, v(i2.value, (t4) => (s(), r("div", { key: t4.key, class: c(["btn", [t4.key, e2.disabled && "disabled", e2.size]]), onClick: k((e3) => {
    ...
    var o4;
  }, ["stop"]) }, [g(d(Be), { icon: t4.icon, size: "10", class: c(["icon-base", { hover: !e2.disabled }]) }, null, 8, ["icon", "class"])], 10, Bl))), 128))], 2));
} }), [["__scopeId", "data-v-5c20d434"]]), ql = e({ name: "ScreenCheckbox", __name: "index", props: { title: { type: String }, tooltip: { type: String } }, setup: (e2) => (t2, o2) => (s(), r("div", null, [g(d(W), I(D(t2.$attrs)), { default: M(() => [x(S(e2.title), 1)]), _: 1 }, 16), e2.tooltip ? (s(), h(d(G), { key: 0, title: e2.tooltip }, { default: M(() => [g(d(Be), { icon: "ant-design:exclamation-circle-outlined" })]), _: 1 }, 8, ["title"])) : f("", true)])) }), Ol = { key: 0, class: "prefix" }, Al = { key: 3 }, jl = to(e({ name: "ScreenInput", __name: "index", props: { isNumber: { type: Boolean, default: true }, prefix: { type: String }, suffix: { type: String } }, emits: ["change"], setup(e2, { emit: o2 }) {
  t((e3) => ({ "236dc44e": u2.value }));
  const n2 = o2, { prefixCls: l2 } = xe("screen-input"), { token: i2 } = P.useToken(), u2 = a(() => i2.value.colorBorder);
  return (t2, o3) => (s(), r("div", { class: c(d(l2)) }, [e2.prefix ? (s(), r("div", Ol, S(e2.prefix), 1)) : f("", true), e2.isNumber ? (s(), h(d(Q), $({ key: 1, class: "input" }, t2.$attrs, { onChange: o3[0] || (o3[0] = (e3) => n2("change", e3)), bordered: false, controls: false }), null, 16)) : (s(), h(d(H), $({ key: 2, class: "input" }, t2.$attrs, { bordered: false, onChange: o3[1] || (o3[1] = (e3) => n2("change", e3)) }), null, 16)), e2.suffix ? (s(), r("div", Al, S(e2.suffix), 1)) : f("", true)], 2));
} }), [["__scopeId", "data-v-00593fb6"]]), zl = ({ CtrlShiftKeysEnum: e2, CtrlKeysEnum: t2, ShiftKeysEnum: o2, KeysEnum: a2 }) => {
  const n2 = (e3, t3) => {
    if (!t3) return;
    const o3 = t3[1 === e3.key.length ? e3.key.toLowerCase() : e3.key];
    o3 && We(o3) && (e3.preventDefault(), e3.stopPropagation(), o3());
  }, { prefixCls: l2 } = xe("chart-contain");
  function i2(e3) {
...
const Fl = to(e({ __name: "TextFontStyle", props: { bold: { type: Boolean, default: false }, italic: { type: Boolean, default: false }, strikethrough: { type: Boolean, default: false }, underline: { type: Boolean, default: false }, hide: { type: Array, default: () => [] } }, emits: ["fontStyleChange"], setup(e2, { emit: t2 }) {
  const o2 = e2, a2 = t2, i2 = l({ [ei.Bold]: { name: "加粗", icon: "carbon:text-bold", value: false }, [ei.Italic]: { name: "斜体", icon: "carbon:text-italic", value: false }, [ei.Underline]: { name: "下划线", icon: "carbon:text-underline", value: false }, [ei.Strikethrough]: { name: "删除线", icon: "carbon:text-strikethrough", value: false } }), u2 = (e3) => !o2.hide.includes(e3);
  ...
  return (e3, t3) => (s(), r("div", { class: c(d(p2)) }, [(s(true), r(y, null, v(Object.keys(i2), (e4) => (s(), r(y, { key: e4 }, [u2(e4) ? (s(), h(d(G), { key: 0, placement: "bottom", title: `${i2[e4].value ? "关闭" : "打开"}${i2[e4].name}` }, { default: M(() => [g(d(Y), { class: "radio-button", checked: i2[e4].value, onMousedown: (t4) => function(e5) {
    if (e5) {
      i2[e5].value = !i2[e5].value, i2[e5].value && (e5 === ei.Underline && (i2[ei.Strikethrough].value = false), e5 === ei.Strikethrough && (i2[ei.Underline].value = false));
      const t5 = {};
...
} }), [["__scopeId", "data-v-3b6dd2cf"]]), Pl = to(e({ __name: "TextHAlignStyle", props: { alignType: { type: String, default: void 0 } }, emits: ["alignChange"], setup(e2, { emit: t2 }) {
  const o2 = e2, a2 = t2, i2 = m();
  n(() => o2.alignType, (e3) => {
    i2.value = e3;
  }, { immediate: true });
  const { prefixCls: u2 } = xe("h-align-style"), p2 = l({ [Xl.Left]: { name: "水平居左", icon: "carbon:text-align-left", description: "文字水平居左" }, [Xl.Center]: { name: "水平居中", icon: "carbon:text-align-center", description: "文字水平居中" }, [Xl.Right]: { name: "水平居右", icon: "carbon:text-align-right", description: "文字水平居右" } });
  function f2(e3) {
...
} }), [["__scopeId", "data-v-63ee2427"]]), Hl = to(e({ __name: "TextVAlignStyle", props: { alignType: { type: String, default: void 0 } }, emits: ["alignChange"], setup(e2, { emit: t2 }) {
  const o2 = e2, a2 = t2, i2 = m();
  n(() => o2.alignType, (e3) => {
    i2.value = e3;
  }, { immediate: true });
  const { prefixCls: u2 } = xe("v-align-style"), p2 = l({ [Jl.Top]: { name: "垂直居上", icon: "carbon:align-vertical-top", description: "文字垂直居上" }, [Jl.Center]: { name: "垂直居中", icon: "carbon:align-vertical-center", description: "文字垂直居中" }, [Jl.Bottom]: { name: "垂直居下", icon: "carbon:align-vertical-bottom", description: "文字垂直居下" } });
  function f2(e3) {
    const t3 = e3.target.value;
    a2("alignChange", t3);
  }
  return (e3, t3) => (s(), r("div", { class: c(d(u2)) }, [g(d(Z), { value: i2.value, "onUpdate:value": t3[0] || (t3[0] = (e4) => i2.value = e4), onChange: f2 }, { default: M(() => [(s(true), r(y, null, v(Object.keys(p2), (e4) => (s(), h(d(G), { key: e4, placement: "top", title: p2[e4].description }, { default: M(() => [g(d(Y), { class: "radio-button", value: e4 }, { default: M(() => [g(d(Be), { icon: p2[e4].icon }, null, 8, ["icon"])]), _: 2 }, 1032, ["value"])]), _: 2 }, 1032, ["title"]))), 128))]), _: 1 }, 8, ["value"])], 2));
} }), [["__scopeId", "data-v-a40ac773"]]), Ql = { class: "line" }, Gl = { class: "input" }, Ul = { class: "line" }, Vl = { style: { flex: "1" } }, Wl = { key: 0, class: "line" }, Yl = e({ name: "TextStyle", __name: "index", props: { verticalAlign: { type: Boolean, default: false }, textStyle: { type: Object, default: () => {
  ...
  const l2 = e2, i2 = o2, { prefixCls: u2 } = xe("text-style"), { token: y2 } = P.useToken(), v2 = a(() => y2.value.colorBorder), C2 = m(12), b2 = m(Kl[0].value), M2 = m(), k2 = m();
  function w2(e3) {
    const t2 = x2(e3);
  ...
  function _2(e3, t2) {
    if (!e3) return;
    const o3 = t2?.startsWith("linear-gradient"), a2 = Math.round((C2.value ?? 12) / 10), n2 = e3?.underline && o3 ? { borderBottom: `${a2}px solid`, borderImage: `${t2} 1` } : e3?.underline ? { borderBottom: `${a2}px solid`, borderColor: `${t2}` } : {}, l3 = e3[ei.Bold] ? { fontWeight: "bold" } : {}, r2 = e3[ei.Italic] ? { fontStyle: "italic" } : {};
    if (o3) {
      const e4 = t2?.match(new RegExp("(?<=linear-gradient.*?)(rgba\\(.*?\\))", "g"));
      e4 && e4.length > 0 && (t2 = e4[0]);
    }
    const s2 = e3[ei.Strikethrough] ? { textDecoration: "line-through", textDecorationColor: t2 } : {};
    i2("fontStyleChange", { value: e3, style: { ...l3, ...r2, ...n2, ...s2 } });
  }
  function L2(e3) {
    i2("hAlignChange", { value: e3, style: { justifyContent: e3 === Xl.Left ? "flex-start" : e3 === Xl.Center ? "center" : e3 === Xl.Right ? "flex-end" : "center" } });
  }
  function I2(e3) {
    i2("vAlignChange", { value: e3, style: { alignItems: e3 === Jl.Top ? "flex-start" : e3 === Jl.Center ? "center" : e3 === Jl.Bottom ? "flex-end" : "unset" } });
  }
  function D2(e3) {
  ...
  return n(() => l2.textStyle, (e3) => {
    e3 && (C2.value = l2.textStyle?.fontSize ?? 12, b2.value = l2.textStyle?.font ?? Kl[0].value, M2.value = l2.textStyle?.textColor, k2.value = l2.textStyle?.fontStyle);
  }, { immediate: true, deep: true }), (t2, o3) => (s(), r("div", { class: c(d(u2)) }, [p("div", Ql, [g(d(K), { style: { flex: "1" }, value: b2.value, "onUpdate:value": o3[0] || (o3[0] = (e3) => b2.value = e3), options: d(Kl), onChange: D2 }, null, 8, ["value", "options"]), p("div", Gl, [g(d(Q), { value: C2.value, "onUpdate:value": o3[1] || (o3[1] = (e3) => C2.value = e3), bordered: false, placeholder: "大小", controls: false, min: 6, maxlength: 4, onChange: T2 }, null, 8, ["value"]), o3[2] || (o3[2] = p("div", { class: "suffix" }, "px", -1))]), e2.hideAlign ? f("", true) : (s(), h(Pl, { key: 0, "align-type": e2.textStyle?.hAlignType, onAlignChange: L2 }, null, 8, ["align-type"]))]), p("div", Ul, [p("div", Vl, [g(d(Eo), { type: e2.colorType, "direct-confirm": true, value: M2.value, onConfirmChange: w2, onInputChange: x2 }, null, 8, ["type", "value"])]), g(Fl, $({ hide: e2.hideFontStyle }, k2.value, { onFontStyleChange: S2 }), null, 16, ["hide"])]), e2.verticalAlign ? (s(), r("div", Wl, [g(Hl, { "align-type": e2.textStyle?.vAlignType, onAlignChange: I2 }, null, 8, ["align-type"])])) : f("", true)], 2));
} }), Zl = to(Yl, [["__scopeId", "data-v-c65c80c4"]]), Kl = [{ value: "微软雅黑", label: "微软雅黑" }, { value: "宋体", label: "宋体" }, { value: "黑体", label: "黑体" }, { value: "楷体", label: "楷体" }, { value: "仿宋", label: "仿宋" }, { value: "sans-serif", label: "sans-serif" }, { value: "Arial", label: "Arial" }, { value: "PingFangSC", label: "PingFangSC" }, { value: "monospace", label: "monospace" }, { value: "Courier New", label: "Courier New" }, { value: "lcd", label: "lcd" }];
var Xl = ((e2) => (e2.Left = "left", e2.Center = "center", e2.Right = "right", e2))(Xl || {}), Jl = ((e2) => (e2.Top = "top", e2.Center = "center", e2.Bottom = "bottom", e2))(Jl || {}), ei = ((e2) => (e2.Bold = "bold", e2.Italic = "italic", e2.Underline = "underline", e2.Strikethrough = "strikethrough", e2))(ei || {});
function ti(e2) {
  switch (e2) {
    case "NUMBER":
...
var oi = ((e2) => (e2.error = "error", e2.eq = "=", e2.ne = "!=", e2.gt = ">", e2.ge = ">=", e2.lt = "<", e2.le = "<=", e2.between = "between", e2.isNull = "isNull", e2.isNotNull = "isNotNull", e2.like = "like", e2.notLike = "notLike", e2.likeLeft = "likeLeft", e2.likeRight = "likeRight", e2))(oi || {}), ai = ((e2) => (e2.count = "计数", e2.sum = "求和", e2.avg = "平均", e2.max = "最大", e2.min = "最小", e2))(ai || {}), ni = ((e2) => (e2.formula = "formula", e2.operator = "operator", e2.field = "field", e2.param = "param", e2))(ni || {});
const li = (e2) => Je.get({ url: "/nocode/apiFolder/tree", params: e2 }), ii = (e2) => Je.get({ url: "/nocode/apiFolder", params: e2 });
function ri(e2) {
...
const pi = (e2) => Je.get({ url: "/nocode/apiParams", params: e2 });
function fi(e2) {
...
const yi = (e2) => Je.get({ url: "/nocode/formulaInfo/tree", params: e2 }), vi = (e2) => Je.get({ url: `/nocode/formulaParam/${e2}` }), Ci = (e2) => Je.get({ url: "/nocode/formulaInfo", params: e2 });
function bi(e2) {
...
const _i = () => "/nocode/mfApi/api", $i = (e2) => Je.get({ url: "/nocode/mfApi", params: e2 });
function Li(e2) {
...
const Pi = (e2) => Je.get({ url: "/nocode/mfScreen", params: e2 });
function Hi(e2, t2) {
...
const or = (e2) => Je.get({ url: "/nocode/screenFolder/tree", params: e2 }), ar = (e2) => Je.get({ url: "/nocode/screenFolder", params: e2 });
function nr(e2) {
...
const ur = (e2) => Je.get({ url: "/nocode/screenResourceApi", params: e2 });
function cr(e2) {
...
const Cr = (e2) => Je.get({ url: "/nocode/screenResource", params: e2 });
function br(e2) {
...
const Sr = (e2, t2, o2) => ({ CtrlKeysEnum: { s: Oe(() => o2(), 150), z: Oe(() => e2(), 150) }, CtrlShiftKeysEnum: { z: Oe(() => t2(), 150) } }), _r = et("custom-api", { state: () => ({ sourceId: "", sourceType: 0, tableName: "", tableFields: [], tableList: [], tableFieldsMap: /* @__PURE__ */ new Map(), variables: [], level: 1, fieldsChange: 1, showData: false, showDataLevel: "", formulaMap: /* @__PURE__ */ new Map() }), getters: { getSourceId() {
  ...
  (await Ci({ pageNum: 1, pageSize: 1e4 })).list.forEach((e2) => {
    const t2 = `${e2.categoryId},${e2.id}`;
    this.formulaMap.set(t2, { ...e2, key: t2 });
  });
}, getFormula(e2) {
  return this.formulaMap.get(e2);
} } }), $r = "__API_SAVE__", Lr = "公式结果";
var Ir = ((e2) => (e2.QUERY_SQL = "api_config:query_sql", e2.QUERY_DATA = "api_config:query_data", e2))(Ir || {});
const Dr = async (e2) => {
  const t2 = _r();
  if (!e2?.sqlQuery) return [];
  const o2 = { sourceId: t2.getSourceId, sourceType: t2.getSourceType, sqlQuery: e2.sqlQuery }, a2 = await Oi(o2);
  return a2 && a2.forEach((t3) => t3.tableAlias = e2.sourceTable), a2;
}, Tr = (e2) => e2 && (e2?.aggregates && e2.aggregates.length > 0 || e2.groups && e2.groups.length > 0 || e2.customColumns && e2.customColumns.length > 0), Nr = (e2, t2) => t2 === _r().getLevel && Tr(e2), Br = (e2, t2, o2) => {
  ...
  return this.curComponent?.chart.type === ns.MfCombine && this.curComponent?.chart.options?.selectIndex >= 0 ? this.curComponent?.chart.options.components[this.curComponent.chart.options.selectIndex] : this.curComponent;
  ...
  return dp(this.componentList, e2);
}, async initCache(e2) {
  if (this.setInitComplete(false), e2) {
    const t2 = JSON.parse(e2);
    if (!t2) return;
    this.setComponentList(t2.componentList), this.setCanvasConfig(t2.canvasConfig), fp(this.componentList).then((e3) => {
      Promise.all(e3).then(() => {
  ...
  e2.forEach((e3) => {
    hp(e3.chart), e3.chart.type === ns.MfCombine && e3.chart.options.components.forEach((e4) => {
      hp(e4.chart);
    });
  ...
  let t2;
  if (this.setInitComplete(false), t2 = this.isResource ? await br(this.getId) : await Hi(this.getId, this.getShareToken), !t2) {
  ...
  let n2;
  if (n2 = this.isResource ? await xr(this.getId) : await vr({ screenId: this.getId }, this.getShareToken), !n2 || 0 === n2.length) return this.componentList;
  ...
  return l2.clear(), this.setComponentList(i2), fp(this.componentList, this.isResource).then((e3) => {
    Promise.all(e3).then(() => {
      this.setUndoRedoData("初始化"), this.setInitComplete(true);
    });
  }), this.componentList;
  ...
  this.canvasConfig.screenName = `${this.canvasConfig.screenName}_副本`, this.componentList.forEach((e3) => {
    t2(e3), e3.chart.type === ns.MfCombine && e3.chart.options.components.forEach((e4) => {
      t2(e4);
    });
  });
  const o2 = (t3) => {
  ...
  this.componentList.forEach((e3) => {
    o2(e3), a2(e3), n2(e3), l2(e3), e3.chart.type === ns.MfCombine && e3.chart.options.components.forEach((e4) => {
...
function ls() {
  ...
  async function t2(t3) {
    ...
    t3.type && tp[t3.type].initData(t3, o2);
  }
...
const ss = (e2, t2) => {
  switch (e2) {
    ...
    case "mouseLeave":
      return "鼠标离开";
    default:
      return t2 ? tp[t2].customEvents?.find((t3) => t3.value === e2)?.name ?? "" : "";
  }
};
function us(e2, t2, o2) {
  const a2 = Or(), l2 = (t3, o3) => {
    ((e3) => {
      if (e3.events?.emits && e3.events.emits.includes(t3)) {
        const a3 = t3 + e3.id;
        is.emit(a3, { eventId: a3, e: o3, data: e3.data });
      }
    })(e2);
  };
  N(() => {
  ...
  const r2 = { mouseenter: (e3) => {
    ...
    if (o3) {
      const a3 = {};
      for (const e3 of Object.keys(o3)) o3[e3].type === Ur.EVENT && Ye(o3[e3].value) && o3[e3].value.forEach((o4) => {
  ...
  function d2(e3, t3) {
    let o3;
    return o3 = Object.keys(rs).some((e4) => rs[e4] === t3.event) ? 0 === e3.data.type ? e3.data.dataSet : e3.data.result : e3.e, Ye(o3) ? o3.length > 0 ? o3[0][t3.param] : "" : Ze(o3) ? o3[t3.param] : Ke(o3) ? o3 : void 0;
  }
  function p2(e3) {
  ...
  n(() => e2.data.params, (e3, t3) => {
    !function(e4, t4) {
      ...
      for (const t5 of Object.keys(e4)) Ye(e4[t5].value) && e4[t5].value.forEach((e5) => {
        const t6 = p2(e5);
        o3.has(t6) || (o3.add(t6), is.on(t6, u2));
      });
    }(e3, t3);
  }, { immediate: true, deep: true });
  const f2 = (t3) => {
    ...
    const o3 = (() => {
      ...
      const o4 = (() => {
        ...
        const t5 = {};
        for (const o5 of e2.jump.params) if (o5.type === Ur.CONSTANT) t5[o5.name] = o5.value;
        else if (o5.type === Ur.VARIABLE) {
          const e3 = pp(a2.getComponentList, o5.value);
          e3 && (t5[o5.name] = e3);
        }
        return t5;
      })();
      if (e2.jump?.pageType === Pr.System) {
        t4 = it.resolve({ name: "ScreenPreview", params: { id: e2.jump?.page.value }, query: { shareToken: a2.getShareToken, isResource: a2.getIsResource?.toString(), ...o4 } }).href;
    ...
    switch (e2.jump?.type) {
      ...
      case Fr.NewTab:
        ...
        break;
      case Fr.Dialog:
        op.emit(Qr.SHOW_MODAL, { ...t3, href: o3, jump: e2.jump });
    }
    c2(t3, "page_jump");
  };
  function h2(e3) {
    if (e3) for (const t3 of e3) is.off(p2(t3.event), g2);
  }
  function g2(t3) {
  ...
  function m2(t3) {
    setTimeout(() => {
      ...
      e2.data.eventRefresh?.open && e2.data.eventRefresh?.events && (o3(e2).then((t4) => {
        e2.type && tp[e2.type]?.initData(e2, t4);
      }), c2(t3, "refresh"));
    }, 10);
  }
  function y2(t3) {
    op.emit(Qr.EVENT_ANIMATION, { ...t3, chart: e2 }), c2(t3, "animation");
  }
  return n(() => e2?.jump, (t3) => {
    !function(t4) {
      const o3 = "click" + e2.id;
      is.off(o3, f2), t4?.open && is.on(o3, f2);
    }(t3);
    ...
    !function(e4, t4) {
      ...
      for (const t5 of e4) {
        ...
        o3.has(e5) || (o3.add(e5), is.on(e5, g2));
      }
    }(e3, t3);
    ...
    !function(e4, t4) {
      if (function(e5) {
        if (!e5) return;
        for (const t5 of e5) is.off(p2(t5), m2);
      }(t4), !e4) return;
      const o3 = /* @__PURE__ */ new Set();
      for (const t5 of e4) {
        const e5 = p2(t5);
        o3.has(e5) || (o3.add(e5), is.on(e5, m2));
      }
    }(e3, t3);
    ...
    !function(e4, t4) {
      if (function(e5) {
        if (!e5) return;
        for (const t5 of e5) if (t5.events) for (const e6 of t5.events) is.off(p2(e6), y2);
      }(t4), !e4) return;
      const o3 = /* @__PURE__ */ new Set();
      for (const t5 of e4) if (t5.events) for (const e5 of t5.events) {
        const t6 = p2(e5);
        o3.has(t6) || (o3.add(t6), is.on(t6, y2));
      }
    }(e3, t3);
...
const cs = e({ name: "MfButton", __name: "index", props: { chart: { type: Object, required: true }, chartContain: { type: Object, required: true } }, setup(e2) {
  const t2 = e2, { commonEvents: o2 } = us(t2.chart), n2 = a(() => t2.chartContain.dropInfo.width > t2.chartContain.dropInfo.height ? t2.chartContain.dropInfo.height / 2.2 + "px" : t2.chartContain.dropInfo.width / 3 + "px");
  return (t3, a2) => (s(), h(d(lt), $({ style: { color: e2.chart.options.iconColor, background: e2.chart.options.buttonColor, fontSize: n2.value } }, B(d(o2)), { type: e2.chart.options.type, ghost: e2.chart.options.ghost, shape: e2.chart.options.shape }), { icon: M(() => [e2.chart.data.dataSet?.icon ? (s(), h(d(Be), { key: 0, size: n2.value, icon: e2.chart.data.dataSet?.icon, color: e2.chart.options.iconColor }, null, 8, ["size", "icon", "color"])) : f("", true)]), default: M(() => [x(" " + S(e2.chart.data.dataSet?.title), 1)]), _: 1 }, 16, ["style", "type", "ghost", "shape"]));
} });
function ds(e2) {
  return e2.chart.data.dataSet = qe(ps.defaultData), e2.chart.data.headers = ps.defaultHeaders && ps.defaultHeaders(), e2.chart.data.limit = 1, e2.chart.data.maxLimit = 1, e2.chart.options.bordered = true, e2.chartContain.dropInfo.width = 80, e2.chartContain.dropInfo.height = 34, e2;
}
const ps = { type: ns.MfButton, component: cs, create: ds, config: (() => {
  const e2 = qe(ts);
  return e2.data.show = false, e2.style.payload.component = T(nt(() => import("./MfButtonConfig.js"), { loading: true, size: "large" })), e2;
...
} }, fs = Object.freeze(Object.defineProperty({ __proto__: null, default: ps, mfButtonCreate: ds }, Symbol.toStringTag, { value: "Module" }));
const hs = { type: ns.MfCombine, component: nt(() => import("./index4.js"), { loading: true }), create: function(e2) {
  return e2.chartContain.padding = "0px", e2;
}, config: (() => {
  const e2 = qe(ts);
  return e2.data.show = false, e2.style.payload.component = T(nt(() => import("./MfCombineConfig.js"), { loading: true, size: "large" })), e2;
})() }, gs = Object.freeze(Object.defineProperty({ __proto__: null, default: hs }, Symbol.toStringTag, { value: "Module" })), ms = to(e({ name: "MfTag", __name: "index", props: { chart: { type: Object, required: true }, chartContain: { type: Object, required: true } }, emits: ["eventHandler"], setup(e2, { emit: t2 }) {
  const o2 = e2, l2 = t2, { prefixCls: i2 } = xe("mf-tag"), g2 = a(() => os(o2.chart)), { emitEvent: m2, commonEvents: y2 } = us(o2.chart, function(e3) {
    l2("eventHandler", e3);
  }, ["titleChange"]);
  return n(() => o2.chart.data.dataSet?.title, (e3) => {
    m2("titleChange", e3);
  }), (t3, o3) => (s(), r("div", $({ class: d(i2), style: { ...e2.chart.options?.hAlign?.style } }, B(d(y2), true)), [p("div", { class: "title", style: u({ overflow: e2.chart.options.hide ? "hidden" : "", ...e2.chart.options?.vAlign?.style }) }, [e2.chart.data.dataSet?.icon ? (s(), h(d(Be), { key: 0, size: g2.value.fontSize, icon: e2.chart.data.dataSet?.icon, color: e2.chart.options.iconColor }, null, 8, ["size", "icon", "color"])) : f("", true), p("span", { class: c({ hide: e2.chart.options.hide }), style: u(g2.value) }, S(e2.chart.data.dataSet?.title), 7)], 4)], 16));
} }), [["__scopeId", "data-v-540aeb6f"]]);
function ys(e2) {
  return e2.chart.data.dataSet = qe(vs.defaultData), e2.chart.data.headers = vs.defaultHeaders && vs.defaultHeaders(), e2.chart.data.limit = 1, e2.chart.data.maxLimit = 1, e2.chart.options.bordered = true, e2.chartContain.dropInfo.width = 80, e2.chartContain.dropInfo.height = 34, e2;
}
const vs = { type: ns.MfTag, component: ms, create: ys, config: (() => {
  const e2 = qe(ts);
  return e2.data.show = false, e2.style.payload.component = T(nt(() => import("./MfTagConfig.js"), { loading: true, size: "large" })), e2;
...
}, customEvents: [{ name: "标签值改变", value: "titleChange" }] }, Cs = Object.freeze(Object.defineProperty({ __proto__: null, default: vs, mfTagCreate: ys }, Symbol.toStringTag, { value: "Module" })), bs = { ...vs, type: ns.MfDataTag, component: nt(() => import("./index5.js"), { loading: true }), config: (() => {
  const e2 = qe(ts);
  return e2.style.payload.component = T(nt(() => import("./MfDataTagStyleConfig.js"), { loading: true, size: "large" })), e2.data.payload.component = T(nt(() => import("./MfDataTagDataConfig.js"), { loading: true, size: "large" })), e2;
})(), initData: (e2, t2) => {
  const { setDynamicData: o2 } = Ms();
  o2(e2, t2);
} };
function Ms() {
  function e2(e3) {
    e3?.data?.dataSet && (e3.data.dataSet.title = bs.defaultData.title);
  }
  function t2(e3) {
    e3?.data?.dataSet && (e3.data.dataSet.icon = bs.defaultData.icon);
  }
...
const ks = Object.freeze(Object.defineProperty({ __proto__: null, default: bs, useDataTag: Ms }, Symbol.toStringTag, { value: "Module" }));
function ws() {
  const e2 = Or(), { getDataTable: t2, setDataTable: o2 } = ls(), a2 = () => {
...
function xs(e2) {
  return e2.chart.data.dataSet = qe(Ss.defaultData), e2.chart.data.headers = Ss.defaultHeaders && Ss.defaultHeaders(), e2.chart.data.limit = 1, e2.chart.data.maxLimit = 1, e2.chartContain.dropInfo.width = 120, e2.chartContain.dropInfo.height = 60, e2.chart.options.backgroundColor = "transparent", e2;
}
const Ss = { type: ns.MfDigits, component: nt(() => import("./index6.js"), { loading: true }), create: xs, config: (() => {
  const e2 = qe(ts);
  return e2.style.payload.component = T(nt(() => import("./MfDigitsConfig.js"), { loading: true, size: "large" })), e2.data.payload.component = T(nt(() => import("./MfDigitsDataConfig.js"), { loading: true, size: "large" })), e2;
})(), defaultData: 12345, defaultHeaders: () => qe({ num: { colName: "num", dataType: "number", rename: "数字" } }), initData: (e2, t2) => {
  const { setDynamicData: o2 } = _s();
  o2(e2, t2);
} };
function _s() {
  const { createMessage: e2 } = _e(), { deleteFields: t2 } = ws();
  function o2(e3) {
    e3.data.dataSet = Ss.defaultData;
  }
...
const $s = Object.freeze(Object.defineProperty({ __proto__: null, default: Ss, mfDigitsCreate: xs, useDigitsData: _s }, Symbol.toStringTag, { value: "Module" }));
function Ls(e2) {
  return e2.chart.data.dataSet = qe(Is.defaultData), e2.chart.data.headers = Is.defaultHeaders && Is.defaultHeaders(), e2.chart.data.limit = 1, e2.chart.data.maxLimit = 1, e2.chartContain.dropInfo.width = 300, e2.chartContain.dropInfo.height = 300, e2.chart.options = { responsive: true, userOptions: { show: false }, style: { chart: { animation: { speed: 0.2 }, backgroundColor: "transparent", layout: { wheel: { ticks: { rounded: true, inactiveColor: "#3A3A3A", activeColor: "#5f8bee", sizeRatio: 0.9, gradient: { show: true, shiftHueIntensity: 100 } } }, innerCircle: { show: true, stroke: "#E1E5E8", strokeWidth: 1 }, percentage: { show: true, fontSize: 48, rounding: 1, bold: true } } } } }, e2;
}
const Is = { type: ns.MfWheel, component: nt(() => import("./index7.js"), { loading: true }), create: Ls, config: (() => {
  const e2 = qe(ts);
  return e2.style.payload.component = T(nt(() => import("./MfWheelConfig.js"), { loading: true, size: "large" })), e2.data.payload.component = T(nt(() => import("./MfWheelDataConfig.js"), { loading: true, size: "large" })), e2;
...
function Ds(e2) {
  const { createMessage: t2 } = _e(), { deleteFields: o2 } = ws();
  function a2(e3) {
    e3.data.dataSet = Is.defaultData;
  }
    ...
    if (n2) {
      if (l2?.list?.length > 0 && n2.data.fields) {
        ...
        if (i2 && i2.length > 0 && i2[0].colName) {
          ...
          if (!Xe(r2)) return t2.warning("百分比必须为数字"), o2(e2), void a2(n2);
...
const Ts = Object.freeze(Object.defineProperty({ __proto__: null, default: Is, mfWheelCreate: Ls, useWheelData: Ds }, Symbol.toStringTag, { value: "Module" }));
function Ns(e2) {
  return e2.chart.data.dataSet = qe(Bs.defaultData), e2.chart.data.headers = Bs.defaultHeaders && Bs.defaultHeaders(), e2.chart.data.limit = 1, e2.chart.data.maxLimit = 1, e2.chartContain.dropInfo.width = 400, e2.chartContain.dropInfo.height = 72, e2.chart.options = { responsive: true, userOptions: { show: false }, style: { chart: { animation: { speed: 0.2 }, backgroundColor: "transparent", layout: { display: "horizontal", crescendo: false, curved: false, curveAngleX: 10, curveAngleY: 10, activeColor: "#5F8BEE", inactiveColor: "#3A3A3A", ticks: { gradient: { show: true, shiftHueIntensity: 30 } } }, percentage: { show: true, useGradientColor: false, color: "#CCCCCC", fontSize: 16, bold: true, rounding: 1, verticalPosition: "bottom", horizontalPosition: "left" } } } }, e2;
}
const Bs = { ...Is, component: nt(() => import("./index8.js"), { loading: true }), type: ns.MfTireMarks, create: Ns, config: (() => {
  const e2 = qe(ts);
  return e2.style.payload.component = T(nt(() => import("./MfTireMarksConfig.js"), { loading: true, size: "large" })), e2.data.payload.component = T(nt(() => import("./MfWheelDataConfig.js"), { loading: true, size: "large" })), e2;
})() }, Rs = Object.freeze(Object.defineProperty({ __proto__: null, default: Bs, mfTireMarksCreate: Ns }, Symbol.toStringTag, { value: "Module" }));
function qs(e2) {
...
const Os = { type: ns.MfBorder1, component: nt(() => import("./index9.js"), { loading: true }), create: qs, config: (() => {
  const e2 = qe(ts);
  return e2.data.show = false, e2.style.payload.component = T(nt(() => import("./MfBorderConfig.js"), { loading: true, size: "large" })), e2;
})(), initData: () => {
} }, As = Object.freeze(Object.defineProperty({ __proto__: null, default: Os, mfBorder1Create: qs }, Symbol.toStringTag, { value: "Module" })), js = { ...Os, type: ns.MfBorder10, component: nt(() => import("./index10.js"), { loading: true }) }, zs = Object.freeze(Object.defineProperty({ __proto__: null, default: js }, Symbol.toStringTag, { value: "Module" }));
function Es(e2) {
...
const Fs = { type: ns.MfBorder11, component: nt(() => import("./index11.js"), { loading: true }), create: Es, config: (() => {
  const e2 = qe(ts);
  return e2.data.show = false, e2.style.payload.component = T(nt(() => import("./MfBorder11Config.js"), { loading: true, size: "large" })), e2;
})(), initData: () => {
} }, Ps = Object.freeze(Object.defineProperty({ __proto__: null, default: Fs, mfBorder11Create: Es }, Symbol.toStringTag, { value: "Module" })), Hs = { ...Os, type: ns.MfBorder12, component: nt(() => import("./index12.js"), { loading: true }) }, Qs = Object.freeze(Object.defineProperty({ __proto__: null, default: Hs }, Symbol.toStringTag, { value: "Module" })), Gs = { ...Os, type: ns.MfBorder13, component: nt(() => import("./index13.js"), { loading: true }) }, Us = Object.freeze(Object.defineProperty({ __proto__: null, default: Gs }, Symbol.toStringTag, { value: "Module" }));
function Vs(e2) {
  const t2 = Os.create(e2);
  return t2.chartContain.border.radius = 0, t2;
}
const Ws = { ...Os, type: ns.MfBorder14, component: nt(() => import("./index14.js"), { loading: true }), create: Vs }, Ys = Object.freeze(Object.defineProperty({ __proto__: null, default: Ws, mfBorder14Create: Vs }, Symbol.toStringTag, { value: "Module" })), Zs = { ...Ws, type: ns.MfBorder15, component: nt(() => import("./index15.js"), { loading: true }) }, Ks = Object.freeze(Object.defineProperty({ __proto__: null, default: Zs }, Symbol.toStringTag, { value: "Module" })), Xs = { ...Ws, type: ns.MfBorder16, component: nt(() => import("./index16.js"), { loading: true }) }, Js = Object.freeze(Object.defineProperty({ __proto__: null, default: Xs }, Symbol.toStringTag, { value: "Module" })), eu = { ...Ws, type: ns.MfBorder17, component: nt(() => import("./index17.js"), { loading: true }) }, tu = Object.freeze(Object.defineProperty({ __proto__: null, default: eu }, Symbol.toStringTag, { value: "Module" })), ou = { ...Ws, type: ns.MfBorder18, component: nt(() => import("./index18.js"), { loading: true }) }, au = Object.freeze(Object.defineProperty({ __proto__: null, default: ou }, Symbol.toStringTag, { value: "Module" })), nu = { ...Ws, type: ns.MfBorder19, component: nt(() => import("./index19.js"), { loading: true }) }, lu = Object.freeze(Object.defineProperty({ __proto__: null, default: nu }, Symbol.toStringTag, { value: "Module" })), iu = { ...Os, type: ns.MfBorder2, component: nt(() => import("./index20.js"), { loading: true }) }, ru = Object.freeze(Object.defineProperty({ __proto__: null, default: iu }, Symbol.toStringTag, { value: "Module" })), su = { ...Ws, type: ns.MfBorder20, component: nt(() => import("./index21.js"), { loading: true }) }, uu = Object.freeze(Object.defineProperty({ __proto__: null, default: su }, Symbol.toStringTag, { value: "Module" })), cu = { ...Ws, type: ns.MfBorder21, component: nt(() => import("./index22.js"), { loading: true }) }, du = Object.freeze(Object.defineProperty({ __proto__: null, default: cu }, Symbol.toStringTag, { value: "Module" })), pu = { ...Ws, type: ns.MfBorder22, component: nt(() => import("./index23.js"), { loading: true }) }, fu = Object.freeze(Object.defineProperty({ __proto__: null, default: pu }, Symbol.toStringTag, { value: "Module" })), hu = { ...Os, type: ns.MfBorder3, component: nt(() => import("./index24.js"), { loading: true }) }, gu = Object.freeze(Object.defineProperty({ __proto__: null, default: hu }, Symbol.toStringTag, { value: "Module" })), mu = { ...Os, type: ns.MfBorder4, component: nt(() => import("./index25.js"), { loading: true }), config: (() => {
  const e2 = qe(ts);
  return e2.data.show = false, e2.style.payload.component = T(nt(() => import("./MfBorder4Config.js"), { loading: true, size: "large" })), e2;
})() }, yu = Object.freeze(Object.defineProperty({ __proto__: null, default: mu }, Symbol.toStringTag, { value: "Module" })), vu = { ...mu, type: ns.MfBorder5, component: nt(() => import("./index26.js"), { loading: true }) }, Cu = Object.freeze(Object.defineProperty({ __proto__: null, default: vu }, Symbol.toStringTag, { value: "Module" })), bu = { ...Os, type: ns.MfBorder6, component: nt(() => import("./index27.js"), { loading: true }) }, Mu = Object.freeze(Object.defineProperty({ __proto__: null, default: bu }, Symbol.toStringTag, { value: "Module" })), ku = { ...Os, type: ns.MfBorder7, component: nt(() => import("./index28.js"), { loading: true }) }, wu = Object.freeze(Object.defineProperty({ __proto__: null, default: ku }, Symbol.toStringTag, { value: "Module" }));
function xu(e2) {
...
const Su = { type: ns.MfBorder8, component: nt(() => import("./index29.js"), { loading: true }), create: xu, config: (() => {
  const e2 = qe(ts);
  return e2.data.show = false, e2.style.payload.component = T(nt(() => import("./MfBorder8Config.js"), { loading: true, size: "large" })), e2;
})(), initData: () => {
} }, _u = Object.freeze(Object.defineProperty({ __proto__: null, default: Su, mfBorder8Create: xu }, Symbol.toStringTag, { value: "Module" })), $u = { ...Os, type: ns.MfBorder9, component: nt(() => import("./index30.js"), { loading: true }) }, Lu = Object.freeze(Object.defineProperty({ __proto__: null, default: $u }, Symbol.toStringTag, { value: "Module" }));
function Iu(e2) {
...
const Du = { type: ns.MfDecoration1, component: nt(() => import("./index31.js"), { loading: true }), create: Iu, config: (() => {
  const e2 = qe(ts);
  return e2.data.show = false, e2.style.payload.component = T(nt(() => import("./MfDecorationConfig.js"), { loading: true, size: "large" })), e2;
})(), initData: () => {
} }, Tu = Object.freeze(Object.defineProperty({ __proto__: null, default: Du, mfDecoration1Create: Iu }, Symbol.toStringTag, { value: "Module" }));
function Nu(e2) {
...
const Bu = { type: ns.MfDecoration10, component: nt(() => import("./index32.js"), { loading: true }), create: Nu, config: (() => {
  const e2 = qe(ts);
  return e2.data.show = false, e2.style.payload.component = T(nt(() => import("./MfDecorationConfig.js"), { loading: true, size: "large" })), e2;
})(), initData: () => {
} }, Ru = Object.freeze(Object.defineProperty({ __proto__: null, default: Bu, mfDecoration10Create: Nu }, Symbol.toStringTag, { value: "Module" }));
function qu(e2) {
...
const Ou = { type: ns.MfDecoration11, component: nt(() => import("./index33.js"), { loading: true }), create: qu, config: (() => {
  const e2 = qe(ts);
  return e2.data.show = false, e2.style.payload.component = T(nt(() => import("./MfDecoration7Config.js"), { loading: true, size: "large" })), e2;
})(), initData: () => {
} }, Au = Object.freeze(Object.defineProperty({ __proto__: null, default: Ou, mfDecoration11Create: qu }, Symbol.toStringTag, { value: "Module" }));
function ju(e2) {
...
const zu = { type: ns.MfDecoration12, component: nt(() => import("./index34.js"), { loading: true }), create: ju, config: (() => {
  const e2 = qe(ts);
  return e2.data.show = false, e2.style.payload.component = T(nt(() => import("./MfDecoration12Config.js"), { loading: true, size: "large" })), e2;
})(), initData: () => {
} }, Eu = Object.freeze(Object.defineProperty({ __proto__: null, default: zu, mfDecoration12Create: ju }, Symbol.toStringTag, { value: "Module" }));
function Fu(e2) {
...
const Pu = { type: ns.MfDecoration2, component: nt(() => import("./index35.js"), { loading: true }), create: Fu, config: (() => {
  const e2 = qe(ts);
  return e2.data.show = false, e2.style.payload.component = T(nt(() => import("./MfDecoration2Config.js"), { loading: true, size: "large" })), e2;
})(), initData: () => {
} }, Hu = Object.freeze(Object.defineProperty({ __proto__: null, default: Pu, mfDecoration2Create: Fu }, Symbol.toStringTag, { value: "Module" }));
function Qu(e2) {
...
const Gu = { type: ns.MfDecoration3, component: nt(() => import("./index36.js"), { loading: true }), create: Qu, config: (() => {
  const e2 = qe(ts);
  return e2.data.show = false, e2.style.payload.component = T(nt(() => import("./MfDecorationConfig.js"), { loading: true, size: "large" })), e2;
})(), initData: () => {
} }, Uu = Object.freeze(Object.defineProperty({ __proto__: null, default: Gu, mfDecoration3Create: Qu }, Symbol.toStringTag, { value: "Module" }));
function Vu(e2) {
...
const Wu = { type: ns.MfDecoration4, component: nt(() => import("./index37.js"), { loading: true }), create: Vu, config: (() => {
  const e2 = qe(ts);
  return e2.data.show = false, e2.style.payload.component = T(nt(() => import("./MfDecoration2Config.js"), { loading: true, size: "large" })), e2;
})(), initData: () => {
} }, Yu = Object.freeze(Object.defineProperty({ __proto__: null, default: Wu, mfDecoration4Create: Vu }, Symbol.toStringTag, { value: "Module" }));
function Zu(e2) {
...
const Ku = { type: ns.MfDecoration5, component: nt(() => import("./index38.js"), { loading: true }), create: Zu, config: (() => {
  const e2 = qe(ts);
  return e2.data.show = false, e2.style.payload.component = T(nt(() => import("./MfDecoration5Config.js"), { loading: true, size: "large" })), e2;
})(), initData: () => {
} }, Xu = Object.freeze(Object.defineProperty({ __proto__: null, default: Ku, mfDecoration5Create: Zu }, Symbol.toStringTag, { value: "Module" }));
function Ju(e2) {
...
const ec = { type: ns.MfDecoration6, component: nt(() => import("./index39.js"), { loading: true }), create: Ju, config: (() => {
  const e2 = qe(ts);
  return e2.data.show = false, e2.style.payload.component = T(nt(() => import("./MfDecorationConfig.js"), { loading: true, size: "large" })), e2;
})(), initData: () => {
} }, tc = Object.freeze(Object.defineProperty({ __proto__: null, default: ec, mfDecoration6Create: Ju }, Symbol.toStringTag, { value: "Module" }));
function oc(e2) {
...
const ac = { type: ns.MfDecoration7, component: nt(() => import("./index40.js"), { loading: true }), create: oc, config: (() => {
  const e2 = qe(ts);
  return e2.data.show = false, e2.style.payload.component = T(nt(() => import("./MfDecoration7Config.js"), { loading: true, size: "large" })), e2;
})(), initData: () => {
} }, nc = Object.freeze(Object.defineProperty({ __proto__: null, default: ac, mfDecoration7Create: oc }, Symbol.toStringTag, { value: "Module" }));
function lc(e2) {
...
const ic = { type: ns.MfDecoration8, component: nt(() => import("./index41.js"), { loading: true }), create: lc, config: (() => {
  const e2 = qe(ts);
  return e2.data.show = false, e2.style.payload.component = T(nt(() => import("./MfDecoration8Config.js"), { loading: true, size: "large" })), e2;
})(), initData: () => {
} }, rc = Object.freeze(Object.defineProperty({ __proto__: null, default: ic, mfDecoration8Create: lc }, Symbol.toStringTag, { value: "Module" }));
function sc(e2) {
...
const uc = { type: ns.MfDecoration9, component: nt(() => import("./index42.js"), { loading: true }), create: sc, config: (() => {
  const e2 = qe(ts);
  return e2.data.show = false, e2.style.payload.component = T(nt(() => import("./MfDecoration5Config.js"), { loading: true, size: "large" })), e2;
})(), initData: () => {
} }, cc = Object.freeze(Object.defineProperty({ __proto__: null, default: uc, mfDecoration9Create: sc }, Symbol.toStringTag, { value: "Module" }));
function dc(e2) {
...
const pc = { type: ns.MfHeader1, create: dc, component: nt(() => import("./index43.js"), { loading: true }), config: (() => {
  const e2 = qe(ts);
  return e2.data.show = false, e2.style.payload.component = T(nt(() => import("./MfDecorationConfig.js"), { loading: true, size: "large" })), e2;
})(), initData: () => {
} }, fc = Object.freeze(Object.defineProperty({ __proto__: null, default: pc, mfHeader1Create: dc }, Symbol.toStringTag, { value: "Module" }));
function hc(e2) {
...
const gc = { type: ns.MfHeader2, create: hc, component: nt(() => import("./index44.js"), { loading: true }), config: (() => {
  const e2 = qe(ts);
  return e2.data.show = false, e2.style.payload.component = T(nt(() => import("./MfDecorationConfig.js"), { loading: true, size: "large" })), e2;
})(), initData: () => {
} }, mc = Object.freeze(Object.defineProperty({ __proto__: null, default: gc, mfHeader2Create: hc }, Symbol.toStringTag, { value: "Module" }));
function yc(e2) {
...
const vc = { type: ns.MfHeader3, create: yc, component: nt(() => import("./index45.js"), { loading: true }), config: (() => {
  const e2 = qe(ts);
  return e2.data.show = false, e2.style.payload.component = T(nt(() => import("./MfDecorationConfig.js"), { loading: true, size: "large" })), e2;
})(), initData: () => {
} }, Cc = Object.freeze(Object.defineProperty({ __proto__: null, default: vc, mfHeader3Create: yc }, Symbol.toStringTag, { value: "Module" }));
function bc(e2) {
...
const Mc = { type: ns.MfHeader4, create: bc, component: nt(() => import("./index46.js"), { loading: true }), config: (() => {
  const e2 = qe(ts);
  return e2.data.show = false, e2.style.payload.component = T(nt(() => import("./MfDecorationConfig.js"), { loading: true, size: "large" })), e2;
})(), initData: () => {
} }, kc = Object.freeze(Object.defineProperty({ __proto__: null, default: Mc, mfHeader4Create: bc }, Symbol.toStringTag, { value: "Module" }));
function wc(e2) {
  return e2.chartContain.dropInfo.width = 1920, e2.chartContain.dropInfo.height = 75, e2.chart.options.mainColor = "#0077CF", e2.chart.options.secondColor = "#0042A2", e2;
}
const xc = { type: ns.MfHeader5, create: wc, component: nt(() => import("./index47.js"), { loading: true }), config: (() => {
  const e2 = qe(ts);
  return e2.data.show = false, e2.style.payload.component = T(nt(() => import("./MfDecorationConfig.js"), { loading: true, size: "large" })), e2;
})(), initData: () => {
} }, Sc = Object.freeze(Object.defineProperty({ __proto__: null, default: xc, mfHeader5Create: wc }, Symbol.toStringTag, { value: "Module" }));
function _c(e2) {
...
const $c = { type: ns.MfHeader6, create: _c, component: nt(() => import("./index48.js"), { loading: true }), config: (() => {
  const e2 = qe(ts);
  return e2.data.show = false, e2.style.payload.component = T(nt(() => import("./MfDecorationConfig.js"), { loading: true, size: "large" })), e2;
})(), initData: () => {
} }, Lc = Object.freeze(Object.defineProperty({ __proto__: null, default: $c, mfHeader6Create: _c }, Symbol.toStringTag, { value: "Module" })), Ic = e({ name: "MfDateTime", __name: "index", props: { chart: { type: Object, required: true }, chartContain: { type: Object, required: true } }, setup(e2) {
  const t2 = e2, o2 = m(t2.chart), a2 = () => {
    t2.chart.options?.format ? o2.value.data.dataSet.title = st(ut(), t2.chart.options.format) : o2.value.data.dataSet.title = st();
  };
  return i(() => {
    a2(), setInterval(a2, 1e3);
  }), (t3, a3) => (s(), h(ms, { "chart-contain": e2.chartContain, chart: o2.value }, null, 8, ["chart-contain", "chart"]));
} });
function Dc(e2) {
  return e2.chart.data.dataSet = qe(Tc.defaultData), e2.chart.data.headers = Tc.defaultHeaders && Tc.defaultHeaders(), e2.chart.data.limit = 1, e2.chart.data.maxLimit = 1, e2.chartContain.dropInfo.width = 190, e2.chartContain.dropInfo.height = 34, e2;
}
const Tc = { type: ns.MfDateTime, component: Ic, create: Dc, config: (() => {
  const e2 = qe(ts);
  return e2.data.show = false, e2.style.payload.component = T(nt(() => import("./MfDataTimeConfig.js"), { loading: true, size: "large" })), e2;
})(), defaultData: { title: "" }, defaultHeaders: () => qe({ title: { colName: "title", dataType: "string", rename: "标签" } }), initData: () => {
} }, Nc = Object.freeze(Object.defineProperty({ __proto__: null, default: Tc, mfDateTimeCreate: Dc }, Symbol.toStringTag, { value: "Module" }));
function Bc() {
  const e2 = (e3, t3) => {
    ...
    for (let e4 = 0; e4 < t3.length; e4++) if (o2 && o2.length > e4) n2.push(o2[e4]);
      ...
      const t4 = tp[a2].defaultSeries;
      t4 ? t4.length > e4 ? n2.push(qe(t4[e4])) : n2.push(qe(t4[0])) : n2.push({});
    } else n2.push({});
    return n2;
  };
  function t2(t3) {
    t3?.data?.dataSet && t3?.type && (t3.data.dataSet = qe(tp[t3.type].defaultData), t3.options.series = e2(t3, tp[t3.type].defaultSeries || []));
  }
  return { resetDefault: t2, setDynamicData: function(o2, a2) {
    if (o2) {
      if (a2?.list.length > 0 && o2.data.fields && (o2.data.fields?.dimensions?.length > 0 || o2.data.fields?.source?.length > 0)) {
        const { createMessage: n2 } = _e(), { deleteFieldValue: l2 } = ws(), i2 = o2.data.fields.dimensions ?? [], r2 = o2.data.fields.source ?? [];
...
const Rc = { type: ns.MfBar, component: nt(() => import("./index49.js"), { loading: true, size: "large" }), create: function(e2) {
  return e2.chart.data.dataSet = qe(Rc.defaultData), e2.chart.data.headers = Rc.defaultHeaders && Rc.defaultHeaders(e2.chart), e2.chart.data.limit = 100, e2.chart.data.maxLimit = 1e3, e2.chart.options = { grid: { left: "10%", right: "5%", top: "10%", bottom: "10%" }, xAxis: { show: true, type: "category" }, yAxis: { show: true }, series: qe(Rc.defaultSeries) }, e2.chartContain.dropInfo.width = 400, e2.chartContain.dropInfo.height = 300, e2;
}, config: (() => {
  const e2 = qe(ts);
  return e2.style.payload.component = T(nt(() => import("./MfBarLineConfig.js"), { loading: true, size: "large" })), e2.data.payload.component = T(nt(() => import("./MfBarLineDataConfig.js"), { loading: true, size: "large" })), e2;
})(), defaultData: { source: [["日期", "数据"], ["Mon", 120], ["Tue", 200], ["Wed", 150], ["Thu", 80], ["Fri", 70], ["Sat", 110], ["Sun", 130]] }, defaultHeaders: (e2) => as(e2?.data?.dataSet?.source[0]), defaultSeries: [{ type: "bar" }], initData: (e2, t2) => {
  const { setDynamicData: o2 } = Bc();
  o2(e2, t2);
} }, qc = Object.freeze(Object.defineProperty({ __proto__: null, default: Rc }, Symbol.toStringTag, { value: "Module" }));
const Oc = { ...Rc, type: ns.MfBarHorizontal, create: function(e2) {
  const t2 = Rc.create(e2);
  return t2.chart.options = { ...t2.chart.options, xAxis: { show: true }, yAxis: { show: true, type: "category" } }, t2;
} }, Ac = Object.freeze(Object.defineProperty({ __proto__: null, default: Oc }, Symbol.toStringTag, { value: "Module" }));
const jc = { ...Rc, type: ns.MfBarStack, create: function(e2) {
  const t2 = Rc.create(e2);
  return t2.chart.data.dataSet = qe(jc.defaultData), t2.chart.data.headers = jc.defaultHeaders && jc.defaultHeaders(t2.chart), t2.chart.options.series = qe(jc.defaultSeries), t2;
}, defaultSeries: [{ type: "bar", stack: "total" }, { type: "bar", stack: "total" }], defaultData: { source: [["日期", "数据1", "数据2"], ["Mon", 120, 80], ["Tue", 200, 132], ["Wed", 150, 90], ["Thu", 80, 180], ["Fri", 70, 110], ["Sat", 110, 130], ["Sun", 130, 90]] } }, zc = Object.freeze(Object.defineProperty({ __proto__: null, default: jc }, Symbol.toStringTag, { value: "Module" }));
const Ec = { ...jc, type: ns.MfBarLine, defaultSeries: [{ type: "bar" }, { type: "line" }], create: function(e2) {
  const t2 = jc.create(e2);
  return t2.chart.options.series = qe(Ec.defaultSeries), t2;
} }, Fc = Object.freeze(Object.defineProperty({ __proto__: null, default: Ec }, Symbol.toStringTag, { value: "Module" }));
const Pc = { ...Rc, type: ns.MfBarPlusMinus, create: function(e2) {
  const t2 = Rc.create(e2);
  return t2.chart.data.dataSet = qe(Pc.defaultData), t2.chart.data.headers = Pc.defaultHeaders && Pc.defaultHeaders(t2.chart), t2.chart.options = { ...t2.chart.options, xAxis: { show: true, type: "value", position: "top", splitLine: { show: true, lineStyle: { type: "dashed" } } }, yAxis: { show: true, type: "category", axisLine: { show: false }, axisLabel: { show: false }, axisTick: { show: false } }, series: qe(Pc.defaultSeries) }, t2;
}, defaultData: { source: [["维度", "数据"], ["ten", -0.07], ["nine", -0.09], ["eight", 0.2], ["seven", -0.44], ["six", -0.23], ["five", 0.08], ["four", -0.17], ["three", 0.47], ["two", -0.36], ["one", 0.18]] } }, Hc = Object.freeze(Object.defineProperty({ __proto__: null, default: Pc }, Symbol.toStringTag, { value: "Module" }));
const Qc = { ...Rc, type: ns.MfLine, create: function(e2) {
  return e2.chart.data.dataSet = qe(Qc.defaultData), e2.chart.data.headers = Qc.defaultHeaders && Qc.defaultHeaders(e2.chart), e2.chart.data.limit = 100, e2.chart.data.maxLimit = 1e3, e2.chart.options = { grid: { left: "10%", right: "10%", top: "10%", bottom: "10%" }, xAxis: { show: true, type: "category" }, yAxis: { show: true }, series: qe(Qc.defaultSeries) }, e2.chartContain.dropInfo.width = 400, e2.chartContain.dropInfo.height = 300, e2;
}, defaultSeries: [{ type: "line" }] }, Gc = Object.freeze(Object.defineProperty({ __proto__: null, default: Qc }, Symbol.toStringTag, { value: "Module" }));
const Uc = { ...Qc, type: ns.MfLineArea, create: function(e2) {
  const t2 = Qc.create(e2);
  return t2.chart.options.series = qe(Uc.defaultSeries), t2;
}, defaultSeries: [{ type: "line", areaStyle: {} }] }, Vc = Object.freeze(Object.defineProperty({ __proto__: null, default: Uc }, Symbol.toStringTag, { value: "Module" }));
const Wc = { ...jc, type: ns.MfLineAreaStack, create: function(e2) {
  const t2 = jc.create(e2);
  return t2.chart.options.series = qe(Wc.defaultSeries), t2;
}, defaultSeries: [{ type: "line", areaStyle: {}, stack: "total" }, { type: "line", areaStyle: {}, stack: "total" }] }, Yc = Object.freeze(Object.defineProperty({ __proto__: null, default: Wc }, Symbol.toStringTag, { value: "Module" }));
const Zc = { ...Qc, type: ns.MfLineSmooth, create: function(e2) {
  const t2 = Qc.create(e2);
  return t2.chart.options.series = qe(Zc.defaultSeries), t2;
}, defaultSeries: [{ type: "line", smooth: true }] }, Kc = Object.freeze(Object.defineProperty({ __proto__: null, default: Zc }, Symbol.toStringTag, { value: "Module" }));
const Xc = { ...jc, type: ns.MfLineStack, create: function(e2) {
  const t2 = jc.create(e2);
  return t2.chart.options.series = qe(Xc.defaultSeries), t2;
}, defaultSeries: [{ type: "line", stack: "total" }, { type: "line", stack: "total" }] }, Jc = Object.freeze(Object.defineProperty({ __proto__: null, default: Xc }, Symbol.toStringTag, { value: "Module" })), ed = [{ color: ["#5470c6", "#91cc75", "#fac858", "#ee6666", "#73c0de", "#3ba272", "#fc8452", "#9a60b4", "#ea7ccc"], name: "默认" }, { color: ["#d87c7c", "#919e8b", "#d7ab82", "#6e7074", "#61a0a8", "#efa18d", "#787464", "#cc7e63", "#724e58", "#4b565b"], name: "复古" }, { color: ["#dd6b66", "#759aa0", "#e69d87", "#8dc1a9", "#ea7e53", "#eedd78", "#73a373", "#73b9bc", "#7289ab", "#91ca8c", "#f49f42"], name: "深色" }, { color: ["#516b91", "#59c4e6", "#edafda", "#93b7e3", "#a5e7f0", "#cbb0e3"], name: "灰粉" }, { color: ["#893448", "#d95850", "#eb8146", "#ffb248", "#f2d643", "#ebdba4"], name: "橘红" }, { color: ["#4ea397", "#22c3aa", "#7bd9a5", "#d0648a", "#f58db2", "#f2b3c9"], name: "青草" }, { color: ["#3fb1e3", "#6be6c1", "#626c91", "#a0a7e6", "#c4ebad", "#96dee8"], name: "蓝绿" }, { color: ["#fc97af", "#87f7cf", "#f7f494", "#72ccff", "#f7c5a0", "#d4a4eb", "#d2f5a6", "#76f2f2"], name: "粉青" }, { color: ["#c1232b", "#27727b", "#fcce10", "#e87c25", "#b5c334", "#fe8463", "#9bca63", "#fad860", "#f3a43b", "#60c0dd", "#d7504b", "#c6e579", "#f4e001", "#f0805a", "#26c0c0"], name: "鲜明" }, { color: ["#2ec7c9", "#b6a2de", "#5ab1ef", "#ffb980", "#d87a80", "#8d98b3", "#e5cf0d", "#97b552", "#95706d", "#dc69aa", "#07a2a4", "#9a7fd1", "#588dd5", "#f5994e", "#c05050", "#59678c", "#c9ab00", "#7eb00a", "#6f5553", "#c14089"], name: "马卡龙" }, { color: ["#e01f54", "#5e4ea5", "#f5e8c8", "#b8d2c7", "#c6b38e", "#a4d8c2", "#f3d999", "#d3758f", "#dcc392", "#2e4783", "#82b6e9", "#ff6347", "#a092f1", "#0a915d", "#eaf889", "#6699FF", "#ff6666", "#3cb371", "#d5b158", "#38b6b6"], name: "罗马" }, { color: ["#c12e34", "#e6b600", "#0098d9", "#2b821d", "#005eaa", "#339ca8", "#cda819", "#32a487"], name: "光亮" }, { color: ["#9b8bba", "#e098c7", "#8fd3e8", "#71669e", "#cc70af", "#7cb4cc"], name: "深紫" }];
const td = { type: ns.MfMapChunks, component: nt(() => import("./index50.js"), { loading: true, size: "large" }), create: function(e2) {
  return e2.chart.data.dataSet = qe(td.defaultData), e2.chart.data.headers = td.defaultHeaders && td.defaultHeaders(e2.chart), e2.chart.data.limit = 100, e2.chart.data.maxLimit = 1e3, e2.chart.options = { tooltip: { show: true }, geo: { show: false, map: "china" }, visualMap: { show: false, min: 0, max: 100, text: ["高", "低"], realtime: false, calculable: true, inRange: { color: qe(ed[0].color) } }, series: qe(td.defaultSeries) }, e2.chartContain.dropInfo.width = 800, e2.chartContain.dropInfo.height = 650, e2;
}, config: (() => {
  const e2 = qe(ts);
  return e2.style.payload.component = T(nt(() => import("./MfMapChunksConfig.js"), { loading: true, size: "large" })), e2.data.payload.component = T(nt(() => import("./MfMapChunksDataConfig.js"), { loading: true, size: "large" })), e2;
})(), defaultData: { source: [["区域", "值"], ["河北省", 90], ["湖北省", 60], ["江苏省", 30], ["北京市", 100], ["陕西省", 50], ["四川省", 20], ["湖南省", 80], ["内蒙古自治区", 40], ["新疆维吾尔自治区", 70], ["安徽省", 10]] }, defaultHeaders: (e2) => as(e2?.data?.dataSet?.source[0]), defaultSeries: [{ name: "分布图", type: "map", map: "china", scale: true, zoom: 1.2, scaleLimit: { min: 0.5, max: 2 }, emphasis: { itemStyle: { areaColor: "#3146B3", color: "#3146B3" } }, itemStyle: { areaColor: "#03133D", color: "#03133D", borderColor: "#6699FF", shadowColor: "#1773C3", shadowBlur: 10, borderWidth: 1 }, label: { color: "#ffffff", emphasis: { color: "#ffffff" } } }], initData: (e2, t2) => {
  const { setDynamicData: o2 } = od();
  o2(e2, t2);
} };
function od() {
  function e2(e3) {
    e3.data.dataSet = qe(td.defaultData);
  }
  return { resetDefault: e2, setDynamicData: function(t2, o2) {
    if (t2) {
      if (o2?.list.length > 0 && t2.data.fields && t2.data.fields?.name?.length > 0) {
        const { createMessage: a2 } = _e(), { deleteFieldValue: n2 } = ws(), l2 = t2.data.fields.name ?? [], i2 = t2.data.fields.value ?? [];
...
const ad = Object.freeze(Object.defineProperty({ __proto__: null, default: td, useEchartsMapChunksData: od }, Symbol.toStringTag, { value: "Module" }));
const nd = { type: ns.MfMapLine, component: nt(() => import("./index51.js"), { loading: true, size: "large" }), create: function(e2) {
  return e2.chart.data.dataSet = qe(nd.defaultData), e2.chart.data.headers = nd.defaultHeaders && nd.defaultHeaders(e2.chart), e2.chart.data.limit = 100, e2.chart.data.maxLimit = 1e3, e2.chart.options = { tooltip: { show: true }, geo: { map: "china", scale: true, zoom: 1.2, scaleLimit: { min: 0.5, max: 2 }, emphasis: { itemStyle: { color: "#3146B3", areaColor: "#3146B3" } }, itemStyle: { color: "#03133D", areaColor: "#03133D", borderColor: "#6699FF", shadowColor: "#1773C3", shadowBlur: 10, borderWidth: 1 }, label: { color: "#ffffff", emphasis: { color: "#ffffff" } } }, visualMap: { show: false, min: 0, max: 100, text: ["高", "低"], realtime: false, calculable: true, inRange: { color: qe(ed[0].color) } }, series: qe(nd.defaultSeries) }, e2.chartContain.dropInfo.width = 800, e2.chartContain.dropInfo.height = 650, e2;
}, config: (() => {
  const e2 = qe(ts);
  return e2.style.payload.component = T(nt(() => import("./MfMapLineConfig.js"), { loading: true, size: "large" })), e2.data.payload.component = T(nt(() => import("./MfMapLineDataConfig.js"), { loading: true, size: "large" })), e2;
})(), defaultData: { source: [["起始", "结束", "值"], ["南京市", "北京市", 40], ["南京市", "西安市", 100], ["北京市", "上海市", 30], ["上海市", "广州市", 50], ["北京市", "武汉市", 20], ["北京市", "成都市", 80], ["长沙市", "北京市", 100]] }, defaultHeaders: (e2) => as(e2?.data?.dataSet?.source[0]), defaultSeries: [{ name: "飞线图", type: "lines", effect: { show: true, period: 6, trailLength: 0.7, color: "#fff", symbolSize: 3 }, lineStyle: { width: 1, curveness: 0.2 } }, { name: "城市", type: "effectScatter", coordinateSystem: "geo", rippleEffect: { brushType: "stroke" }, label: { show: true, position: "right", formatter: "{b}" } }], initData: (e2, t2) => {
  const { setDynamicData: o2 } = ld();
  o2(e2, t2);
} };
function ld() {
  function e2(e3) {
    e3.data.dataSet = qe(nd.defaultData);
  }
  return { resetDefault: e2, setDynamicData: function(t2, o2) {
    if (t2) {
      if (o2?.list.length > 0 && t2.data.fields && (t2.data.fields?.start?.length > 0 || t2.data.fields?.end?.length > 0)) {
        const { createMessage: a2 } = _e(), { deleteFieldValue: n2 } = ws(), l2 = t2.data.fields.start ?? [], i2 = t2.data.fields.end ?? [], r2 = t2.data.fields.value ?? [];
        if (0 === l2.length || 0 === i2.length) return void e2(t2);
        const s2 = [];
...
const id = Object.freeze(Object.defineProperty({ __proto__: null, default: nd, useEchartsMapLineData: ld }, Symbol.toStringTag, { value: "Module" }));
const rd = { ...nd, type: ns.MfMapLine3D, component: nt(() => import("./index52.js"), { loading: true, size: "large" }), create: function(e2) {
  const t2 = nd.create(e2);
  return delete t2.chart.options?.geo, t2.chart.options = { ...t2.chart.options, geo3D: { map: "china", shading: "lambert", viewControl: { distance: 85, rotateSensitivity: 0, zoomSensitivity: 0 }, light: { main: { intensity: 1.2, shadow: true } }, itemStyle: { areaColor: "#03133D", color: "#03133D", borderColor: "#6699FF", shadowColor: "#1773C3", shadowBlur: 10, borderWidth: 1 }, emphasis: { itemStyle: { areaColor: "#3146B3", color: "#3146B3" } }, label: { distance: 0, color: "#ffffff", emphasis: { color: "#ffffff" } } }, series: qe(rd.defaultSeries) }, t2;
}, defaultSeries: [{ name: "飞线图", type: "lines3D", coordinateSystem: "geo3D", effect: { show: true, trailWidth: 2, trailLength: 0.8, trailOpacity: 0.8, trailColor: "#fff" }, lineStyle: { width: 1, curveness: 0.2 } }, { name: "城市", type: "scatter3D", coordinateSystem: "geo3D", rippleEffect: { brushType: "stroke" }, label: { show: true, position: "right", formatter: "{b}" } }] }, sd = Object.freeze(Object.defineProperty({ __proto__: null, default: rd }, Symbol.toStringTag, { value: "Module" }));
const ud = { ...td, type: ns.MfMapScatter, component: nt(() => import("./index53.js"), { loading: true, size: "large" }), create: function(e2) {
  return e2.chart.data.dataSet = qe(ud.defaultData), e2.chart.data.headers = ud.defaultHeaders && ud.defaultHeaders(e2.chart), e2.chart.data.limit = 100, e2.chart.data.maxLimit = 1e3, e2.chart.options = { tooltip: { show: true }, geo: { map: "china", scale: true, zoom: 1.2, scaleLimit: { min: 0.5, max: 2 }, emphasis: { itemStyle: { color: "#3146B3", areaColor: "#3146B3" } }, itemStyle: { color: "#03133D", areaColor: "#03133D", borderColor: "#6699FF", shadowColor: "#1773C3", shadowBlur: 10, borderWidth: 1 }, label: { color: "#ffffff", emphasis: { color: "#ffffff" } } }, visualMap: { show: false, min: 0, max: 100, text: ["高", "低"], realtime: false, calculable: true, inRange: { color: qe(ed[0].color) } }, series: qe(ud.defaultSeries) }, e2.chartContain.dropInfo.width = 800, e2.chartContain.dropInfo.height = 650, e2;
}, config: (() => {
  const e2 = qe(ts);
  return e2.style.payload.component = T(nt(() => import("./MfMapScatterConfig.js"), { loading: true, size: "large" })), e2.data.payload.component = T(nt(() => import("./MfMapChunksDataConfig.js"), { loading: true, size: "large" })), e2;
})(), defaultSeries: [{ name: "城市", type: "effectScatter", symbolSize: 20, coordinateSystem: "geo", rippleEffect: { brushType: "stroke" }, label: { show: true, position: "right", formatter: "{b}" } }] }, cd = Object.freeze(Object.defineProperty({ __proto__: null, default: ud }, Symbol.toStringTag, { value: "Module" }));
const dd = { ...Rc, type: ns.MfPie, create: function(e2) {
  return e2.chart.data.dataSet = qe(dd.defaultData), e2.chart.data.headers = dd.defaultHeaders && dd.defaultHeaders(e2.chart), e2.chart.data.limit = 100, e2.chart.data.maxLimit = 1e3, e2.chartContain.dropInfo.width = 400, e2.chartContain.dropInfo.height = 400, e2.chart.options = { tooltip: { trigger: "item" }, series: qe(dd.defaultSeries) }, e2;
}, config: (() => {
  const e2 = qe(ts);
  return e2.style.payload.component = T(nt(() => import("./MfPieConfig.js"), { loading: true, size: "large" })), e2.data.payload.component = T(nt(() => import("./MfPieDataConfig.js"), { loading: true, size: "large" })), e2;
})(), defaultSeries: [{ type: "pie", radius: ["0%", "65%"] }] }, pd = Object.freeze(Object.defineProperty({ __proto__: null, default: dd }, Symbol.toStringTag, { value: "Module" }));
const fd = { ...dd, type: ns.MfPieCircular, create: function(e2) {
  const t2 = dd.create(e2);
  return t2.chart.options.series = qe(fd.defaultSeries), t2;
}, defaultSeries: [{ type: "pie", radius: ["30%", "65%"] }] }, hd = Object.freeze(Object.defineProperty({ __proto__: null, default: fd }, Symbol.toStringTag, { value: "Module" }));
const gd = { ...dd, type: ns.MfPieHalf, create: function(e2) {
  const t2 = dd.create(e2);
  return t2.chart.options.series = qe(gd.defaultSeries), t2;
}, defaultSeries: [{ type: "pie", center: ["50%", "70%"], radius: ["30%", "65%"], startAngle: 180, endAngle: 360 }] }, md = Object.freeze(Object.defineProperty({ __proto__: null, default: gd }, Symbol.toStringTag, { value: "Module" }));
const yd = { ...dd, type: ns.MfPieRose, create: function(e2) {
  const t2 = dd.create(e2);
  return t2.chart.options.series = qe(yd.defaultSeries), t2;
}, defaultSeries: [{ type: "pie", roseType: "area", radius: ["10%", "65%"], itemStyle: { borderRadius: 8 } }] }, vd = Object.freeze(Object.defineProperty({ __proto__: null, default: yd }, Symbol.toStringTag, { value: "Module" }));
const Cd = { component: nt(() => import("./index54.js"), { loading: true, size: "large" }), type: ns.MfRadar, create: function(e2) {
  return e2.chart.data.dataSet = qe(Cd.defaultData), e2.chart.data.headers = Cd.defaultHeaders && Cd.defaultHeaders(), e2.chart.data.limit = 5, e2.chart.data.maxLimit = 20, e2.chartContain.dropInfo.width = 400, e2.chartContain.dropInfo.height = 400, e2.chart.options = { series: qe(Cd.defaultSeries) }, e2;
}, config: (() => {
  const e2 = qe(ts);
  return e2.style.payload.component = T(nt(() => import("./MfRadarConfig.js"), { loading: true, size: "large" })), e2.data.payload.component = T(nt(() => import("./MfRadarDataConfig.js"), { loading: true, size: "large" })), e2;
...
function bd() {
  const { createMessage: e2 } = _e(), { deleteFieldValue: t2 } = ws();
  function o2(e3) {
    e3.data.dataSet = qe(Cd.defaultData);
  }
  return { resetDefault: o2, setDynamicData: function(a2, n2) {
...
const Md = Object.freeze(Object.defineProperty({ __proto__: null, default: Cd, useRadarData: bd }, Symbol.toStringTag, { value: "Module" }));
const kd = { ...Cd, type: ns.MfRadarCircle, create: function(e2) {
  const t2 = Cd.create(e2);
  return t2.chart.options = { ...t2.chart.options, radar: { shape: "circle" } }, e2;
} }, wd = Object.freeze(Object.defineProperty({ __proto__: null, default: kd }, Symbol.toStringTag, { value: "Module" }));
const xd = { ...Rc, type: ns.MfScatter, create: function(e2) {
  return e2.chart.data.dataSet = qe(xd.defaultData), e2.chart.data.headers = xd.defaultHeaders && xd.defaultHeaders(e2.chart), e2.chart.data.limit = 100, e2.chart.data.maxLimit = 1e3, e2.chartContain.dropInfo.width = 400, e2.chartContain.dropInfo.height = 300, e2.chart.options = { grid: { left: "10%", right: "10%", top: "10%", bottom: "10%" }, xAxis: { show: true }, yAxis: { show: true }, series: qe(xd.defaultSeries) }, e2;
}, config: (() => {
  const e2 = qe(ts);
  return e2.style.payload.component = T(nt(() => import("./MfScatterConfig.js"), { loading: true, size: "large" })), e2.data.payload.component = T(nt(() => import("./MfScatterDataConfig.js"), { loading: true, size: "large" })), e2;
})(), defaultData: { source: [["数据1", "数据2"], [10, 8.04], [8.07, 6.95], [13, 7.58], [9.05, 8.81], [11, 8.33], [14, 7.66], [13.4, 6.81], [10, 6.33], [14, 8.96], [12.5, 6.82], [9.15, 7.2], [11.5, 7.2], [3.03, 4.23], [12.2, 7.83], [2.02, 4.47], [1.05, 3.33], [4.05, 4.96], [6.03, 7.24], [12, 6.26], [12, 8.84], [7.08, 5.82], [5.02, 5.68]] }, defaultSeries: [{ type: "scatter" }] }, Sd = Object.freeze(Object.defineProperty({ __proto__: null, default: xd }, Symbol.toStringTag, { value: "Module" })), _d = e({ name: "MfInput", __name: "index", props: { chart: { type: Object, required: true }, chartContain: { type: Object, required: true } }, setup(e2) {
  const t2 = e2, { emitEvent: o2, commonEvents: a2 } = us(t2.chart, void 0, ["change", "pressEnter", "blur"]), n2 = m();
  function l2() {
    o2("change", d(n2));
  }
  function i2() {
  ...
  return (e3, t3) => (s(), h(d(H), $({ value: n2.value, "onUpdate:value": t3[0] || (t3[0] = (e4) => n2.value = e4) }, B(d(a2)), { onBlur: i2, onPressEnter: r2, onChange: l2 }), null, 16, ["value"]));
} });
function $d(e2) {
  return e2.chart.data.dataSet = qe(Ld.defaultData), e2.chart.data.headers = Ld.defaultHeaders && Ld.defaultHeaders(), e2.chart.data.limit = 1, e2.chart.data.maxLimit = 1, e2.chart.options.bordered = true, e2.chartContain.dropInfo.width = 200, e2.chartContain.dropInfo.height = 34, e2;
}
const Ld = { type: ns.MfInput, component: _d, create: $d, config: (() => {
  const e2 = qe(ts);
  return e2.data.show = false, e2.style.payload.component = T(nt(() => import("./MfTagConfig.js"), { loading: true, size: "large" })), e2;
})(), defaultData: { value: "" }, defaultHeaders: () => qe({ value: { colName: "value", dataType: "string", rename: "值" } }), initData: () => {
}, customEvents: [{ name: "输入框值改变", value: "change" }, { name: "按下回车", value: "pressEnter" }, { name: "离开组件", value: "blur" }] }, Id = Object.freeze(Object.defineProperty({ __proto__: null, default: Ld, mfInputCreate: $d }, Symbol.toStringTag, { value: "Module" })), Dd = e({ name: "MfPicture", __name: "index", props: { chart: { type: Object, required: true }, chartContain: { type: Object, required: true } }, setup(e2) {
  const t2 = e2, { commonEvents: o2 } = us(t2.chart), n2 = a(() => t2.chart.data.dataSet?.key ? ct(dt(t2.chart.data.dataSet.key)) : ot);
  return (t3, a2) => (s(), h(d(X), $({ src: n2.value, alt: e2.chart.data.dataSet?.alt || "图片" }, e2.chart.options, { width: e2.chartContain.dropInfo.width - 2, height: e2.chartContain.dropInfo.height - 2, fallback: d(ot) }, B(d(o2))), null, 16, ["src", "alt", "width", "height", "fallback"]));
} });
function Td(e2) {
  return e2.chart.data.dataSet = qe(Nd.defaultData), e2.chart.data.headers = Nd.defaultHeaders && Nd.defaultHeaders(), e2.chart.data.limit = 1, e2.chart.data.maxLimit = 1, e2.chart.options.preview = false, e2.chartContain.dropInfo.width = 200, e2.chartContain.dropInfo.height = 200, e2;
}
const Nd = { type: ns.MfPicture, component: Dd, create: Td, config: (() => {
  const e2 = qe(ts);
  return e2.data.show = false, e2.style.payload.component = T(nt(() => import("./MfPictureConfig.js"), { loading: true, size: "large" })), e2;
...
} }, Bd = Object.freeze(Object.defineProperty({ __proto__: null, default: Nd, mfPictureCreate: Td }, Symbol.toStringTag, { value: "Module" }));
function Rd(e2) {
  return e2.chart.data.dataSet = qe(qd.defaultData), e2.chart.data.headers = qd.defaultHeaders && qd.defaultHeaders(e2.chart), e2.chart.data.limit = 50, e2.chart.data.maxLimit = 1e3, e2.chartContain.dropInfo.width = 600, e2.chartContain.dropInfo.height = 300, e2.chart.options = { indexHeader: "序号", showHeader: true, index: true }, e2;
}
const qd = { type: ns.MfScrollTable, component: nt(() => import("./index55.js"), { loading: true }), create: Rd, config: (() => {
  const e2 = qe(ts);
  return e2.style.payload.component = T(nt(() => import("./MfScrollTableConfig.js"), { loading: true, size: "large" })), e2.data.payload.component = T(nt(() => import("./MfTableDataConfig.js"), { loading: true, size: "large" })), e2;
})(), defaultData: { header: ["列1", "列2", "列3", "列4", "列5"], data: [[1e4, 1e3, 100, 10, 0], [10001, 1001, 101, 11, 1], [10002, 1002, 102, 12, 2], [10003, 1003, 103, 13, 3], [10004, 1004, 104, 14, 4], [10005, 1005, 105, 15, 5], [10006, 1006, 106, 16, 6], [10007, 1007, 107, 17, 7], [10008, 1008, 108, 18, 8], [10009, 1009, 109, 19, 9]] }, defaultHeaders: (e2) => as(e2?.data.dataSet.header), initData: (e2, t2) => {
  const { setDynamicData: o2 } = Od();
  o2(e2, t2);
}, customEvents: [{ name: "行点击", value: "rowClick" }] };
function Od() {
  function e2(e3) {
    e3.data.dataSet = qe(qd.defaultData);
  }
  return { resetDefault: e2, setDynamicData: function(t2, o2) {
    if (!t2) return;
...
const Ad = Object.freeze(Object.defineProperty({ __proto__: null, default: qd, mfScrollTableCreate: Rd, useDataScrollTable: Od }, Symbol.toStringTag, { value: "Module" }));
function jd(e2) {
  return e2.chart.data.dataSet = qe(zd.defaultData), e2.chart.data.headers = zd.defaultHeaders && zd.defaultHeaders(e2.chart), e2.chart.data.limit = 10, e2.chart.data.maxLimit = 1e3, e2.chartContain.dropInfo.width = 600, e2.chartContain.dropInfo.height = 50, e2;
}
const zd = { type: ns.MfSegmented, component: nt(() => import("./index56.js"), { loading: true }), create: jd, config: (() => {
  const e2 = qe(ts);
  return e2.style.payload.component = T(nt(() => import("./MfSegmentedStyleConfig.js"), { loading: true, size: "large" })), e2.data.payload.component = T(nt(() => import("./MfSegmentedDataConfig.js"), { loading: true, size: "large" })), e2;
  ...
  const { setDynamicData: o2 } = Ed();
  o2(e2, t2);
}, customEvents: [{ name: "标签切换", value: "titleChange" }] };
function Ed() {
  function e2(e3) {
    e3.data.dataSet = qe(zd.defaultData);
  }
  return { resetDefault: e2, setDynamicData: function(t2, o2) {
    if (t2) {
      if (o2?.list?.length > 0) {
...
const Fd = Object.freeze(Object.defineProperty({ __proto__: null, default: zd, mfSegmentedCreate: jd, useSegmented: Ed }, Symbol.toStringTag, { value: "Module" }));
function Pd(e2) {
  return e2.chart.data.dataSet = qe(Hd.defaultData), e2.chartContain.border.show = true, e2.chart.data.headers = Hd.defaultHeaders && Hd.defaultHeaders(e2.chart), e2.chart.data.currentValue = "", e2.chart.data.limit = 10, e2.chart.data.maxLimit = 1e3, e2.chartContain.dropInfo.width = 150, e2.chartContain.dropInfo.height = 33, e2;
}
const Hd = { type: ns.MfSelect, component: nt(() => import("./index57.js"), { loading: true }), create: Pd, config: (() => {
  const e2 = qe(ts);
  return e2.style.payload.component = T(nt(() => import("./MfSelectStyleConfig.js"), { loading: true, size: "large" })), e2.data.payload.component = T(nt(() => import("./MfSelectDataConfig.js"), { loading: true, size: "large" })), e2;
...
function Qd() {
  function e2(e3) {
    e3.data.dataSet = qe(Hd.defaultData);
  }
  return { resetDefault: e2, setDynamicData: function(t2, o2) {
    if (t2) {
      if (o2?.list?.length > 0) {
...
const Gd = Object.freeze(Object.defineProperty({ __proto__: null, default: Hd, mfSelectCreate: Pd, useSelect: Qd }, Symbol.toStringTag, { value: "Module" }));
function Ud(e2) {
  return e2.chart.data.dataSet = qe(Wd.defaultData), e2.chart.data.headers = Wd.defaultHeaders && Wd.defaultHeaders(e2.chart), e2.chart.data.limit = void 0, e2.chart.data.maxLimit = Number.MAX_VALUE, e2.chart.data.pagination = { pageSize: 5 }, e2.chartContain.dropInfo.width = 600, e2.chartContain.dropInfo.height = 380, e2.chart.options = { indexHeader: "序号", columns: [{ ellipsis: true, align: "center", width: 120 }, { ellipsis: true, align: "center", width: 120 }, { ellipsis: true, align: "center", width: 120 }], tableInfo: { scroll: { x: true, y: 330 }, columns: qe(Vd) } }, e2;
}
const Vd = [{ title: "姓名", dataIndex: "name" }, { title: "年龄", dataIndex: "age" }, { title: "地址", dataIndex: "address" }], Wd = { type: ns.MfTable, component: nt(() => import("./index58.js"), { loading: true }), create: Ud, config: (() => {
  const e2 = qe(ts);
  return e2.style.payload.component = T(nt(() => import("./MfTableConfig.js"), { loading: true, size: "large" })), e2.data.payload.component = T(nt(() => import("./MfTableDataConfig.js"), { loading: true, size: "large" })), e2;
})(), defaultData: [{ key: "1", name: "mfish nocode", age: 20, address: "Mfish nocode No. 1" }, { key: "2", name: "Jim Green", age: 42, address: "London No. 1 Lake Park" }, { key: "3", name: "Joe Black", age: 32, address: "Sidney No. 1 Lake Park" }, { key: "4", name: "Disabled User", age: 99, address: "Sidney No. 1 Lake Park" }, { key: "5", name: "John Brown", age: 32, address: "New York No. 1 Lake Park" }], defaultHeaders: (e2) => e2?.data.dataSet?.length > 0 ? as(Object.keys(e2?.data.dataSet[0])) : as([]), initData: (e2, t2) => {
  const { setDynamicData: o2 } = Yd();
  o2(e2, t2);
}, customEvents: [{ name: "行点击", value: "rowClick" }] };
function Yd() {
  function e2(e3) {
    e3.data.dataSet = qe(Wd.defaultData), e3.options.tableInfo.columns = qe(Vd), e3.data.pagination = { pageSize: e3.data.pagination.pageSize }, e3.options.index && e3.options.tableInfo.columns.unshift({ title: e3.options.indexHeader || "序号", width: 80, align: "center", ellipsis: true });
  }
  return { resetDefault: e2, setDynamicData: function(t2, o2) {
    if (!t2) return;
    const a2 = t2.data.fields?.fieldName;
    ...
    const { t: l2 } = $e(), { setDataTable: i2 } = ls();
...
}, ep = Object.assign({ "./MfButton/mfButton.data.ts": fs, "./MfCombine/mfCombine.data.ts": gs, "./MfDataTag/mfDataTag.data.ts": ks, "./MfDataUi/MfDigits/mfDigits.data.ts": $s, "./MfDataUi/MfTireMarks/mfTireMarks.data.ts": Rs, "./MfDataUi/MfWheel/mfWheel.data.ts": Ts, "./MfDataV/MfBorder1/mfBorder1.data.ts": As, "./MfDataV/MfBorder10/mfBorder10.data.ts": zs, "./MfDataV/MfBorder11/mfBorder11.data.ts": Ps, "./MfDataV/MfBorder12/mfBorder12.data.ts": Qs, "./MfDataV/MfBorder13/mfBorder13.data.ts": Us, "./MfDataV/MfBorder14/mfBorder14.data.ts": Ys, "./MfDataV/MfBorder15/mfBorder15.data.ts": Ks, "./MfDataV/MfBorder16/mfBorder16.data.ts": Js, "./MfDataV/MfBorder17/mfBorder17.data.ts": tu, "./MfDataV/MfBorder18/mfBorder18.data.ts": au, "./MfDataV/MfBorder19/mfBorder19.data.ts": lu, "./MfDataV/MfBorder2/mfBorder2.data.ts": ru, "./MfDataV/MfBorder20/mfBorder20.data.ts": uu, "./MfDataV/MfBorder21/mfBorder21.data.ts": du, "./MfDataV/MfBorder22/mfBorder22.data.ts": fu, "./MfDataV/MfBorder3/mfBorder3.data.ts": gu, "./MfDataV/MfBorder4/mfBorder4.data.ts": yu, "./MfDataV/MfBorder5/mfBorder5.data.ts": Cu, "./MfDataV/MfBorder6/mfBorder6.data.ts": Mu, "./MfDataV/MfBorder7/mfBorder7.data.ts": wu, "./MfDataV/MfBorder8/mfBorder8.data.ts": _u, "./MfDataV/MfBorder9/mfBorder9.data.ts": Lu, "./MfDataV/MfDecoration1/mfDecoration.data.ts": Tu, "./MfDataV/MfDecoration10/mfDecoration.data.ts": Ru, "./MfDataV/MfDecoration11/mfDecoration.data.ts": Au, "./MfDataV/MfDecoration12/mfDecoration.data.ts": Eu, "./MfDataV/MfDecoration2/mfDecoration.data.ts": Hu, "./MfDataV/MfDecoration3/mfDecoration.data.ts": Uu, "./MfDataV/MfDecoration4/mfDecoration.data.ts": Yu, "./MfDataV/MfDecoration5/mfDecoration.data.ts": Xu, "./MfDataV/MfDecoration6/mfDecoration.data.ts": tc, "./MfDataV/MfDecoration7/mfDecoration.data.ts": nc, "./MfDataV/MfDecoration8/mfDecoration.data.ts": rc, "./MfDataV/MfDecoration9/mfDecoration.data.ts": cc, "./MfDataV/MfHeader1/mfHeader1.data.ts": fc, "./MfDataV/MfHeader2/mfHeader2.data.ts": mc, "./MfDataV/MfHeader3/mfHeader3.data.ts": Cc, "./MfDataV/MfHeader4/mfHeader4.data.ts": kc, "./MfDataV/MfHeader5/mfHeader5.data.ts": Sc, "./MfDataV/MfHeader6/mfHeader6.data.ts": Lc, "./MfDateTime/mfDateTime.data.ts": Nc, "./MfEcharts/MfBar/mfBar.data.ts": qc, "./MfEcharts/MfBarHorizontal/mfBarHorizontal.data.ts": Ac, "./MfEcharts/MfBarLine/mfBarLine.data.ts": Fc, "./MfEcharts/MfBarPlusMinus/mfBarPlusMinus.data.ts": Hc, "./MfEcharts/MfBarStack/mfBarStack.data.ts": zc, "./MfEcharts/MfLine/mfLine.data.ts": Gc, "./MfEcharts/MfLineArea/mfLineArea.data.ts": Vc, "./MfEcharts/MfLineAreaStack/mfLineAreaStack.data.ts": Yc, "./MfEcharts/MfLineSmooth/mfLineSmooth.data.ts": Kc, "./MfEcharts/MfLineStack/mfLineStack.data.ts": Jc, "./MfEcharts/MfMapChunks/mfMapChunks.data.ts": ad, "./MfEcharts/MfMapLine/mfMapLine.data.ts": id, "./MfEcharts/MfMapLine3D/mfMapLine3D.data.ts": sd, "./MfEcharts/MfMapScatter/mfMapScatter.data.ts": cd, "./MfEcharts/MfPie/mfPie.data.ts": pd, "./MfEcharts/MfPieCircular/mfPieCircular.data.ts": hd, "./MfEcharts/MfPieHalf/mfPieHalf.data.ts": md, "./MfEcharts/MfPieRose/mfPieRose.data.ts": vd, "./MfEcharts/MfRadar/mfRadar.data.ts": Md, "./MfEcharts/MfRadarCircle/mfRadarCircle.data.ts": wd, "./MfEcharts/MfScatter/mfScatter.data.ts": Sd, "./MfInput/mfInput.data.ts": Id, "./MfPicture/mfPicture.data.ts": Bd, "./MfScrollTable/mfScrollTable.data.ts": Ad, "./MfSegmented/mfSegmented.data.ts": Fd, "./MfSelect/mfSelect.data.ts": Gd, "./MfTable/mfTable.data.ts": Zd, "./MfTag/mfTag.data.ts": Cs });
const tp = { ...function() {
  const e2 = {};
  return Object.keys(ep).forEach((t2) => {
    const o2 = ep[t2].default;
    e2[o2.type] = o2;
  }), e2;
}() }, op = at();
function ap(e2) {
  let t2, o2;
  return t2 = Or().getTheme === Ge.DARK ? "#ffffff" : "#000000", o2 = e2.backgroundImage ? `url(${ct(dt(e2.backgroundImage))}) 0% 0% / 100% 100% no-repeat` : e2.backgroundColor ?? "", { width: `${e2.width}px`, height: `${e2.height}px`, background: o2, color: t2 };
}
const np = () => {
  const e2 = Ve();
  return { chartContain: { ...qe(Wr), id: e2 }, chart: { ...qe(Vr), id: e2 } };
};
function lp(e2, t2, o2) {
  const a2 = qe(e2);
  if (!a2.chart.type) return;
  a2.chart.type === ns.MfCombine && a2.chart.options.components.forEach((e3) => {
    const t3 = Ve();
    e3.chart.id = t3, e3.chartContain.id = t3;
  });
  const n2 = Ve();
  return e2.chartContain.dropInfo.x = t2, e2.chartContain.dropInfo.y = o2, a2.chart.name = ip(a2.chart.type, a2.chart.name), a2.chart.id = n2, a2.chartContain.id = n2, a2.chartContain.dropInfo.x = t2, a2.chartContain.dropInfo.y = o2, a2.chartContain.dropInfo.zIndex = 0, a2;
}
function ip(e2, t2) {
  const o2 = Or().getChartNameLabel;
  let a2 = t2 || e2;
...
const rp = (e2) => {
  const t2 = np();
  t2.chart.type = e2.type, t2.chart.name = ip(e2.type, e2.name);
  const o2 = tp[e2.type];
...
function sp(e2, t2 = []) {
  let o2;
  return o2 = e2.chartContain.border.color?.startsWith("linear-gradient") ? { [Gr.borderImage]: `${e2.chartContain.border.color} 1` } : { [Gr.borderColor]: e2.chartContain.border.color }, up({ ...e2.chartContain.dropInfo, ...o2, [Gr.borderWidth]: e2.chartContain.border.show ? e2.chartContain.border.width : 0, [Gr.borderRadius]: e2.chartContain.border.radius, [Gr.background]: e2.chartContain.background, [Gr.opacity]: e2.chartContain.opacity, padding: e2.chartContain.padding }, t2);
}
function up(e2, t2 = []) {
  const o2 = /* @__PURE__ */ new Set([Gr.width, Gr.height, Gr.borderWidth, Gr.borderRadius]), a2 = new Set(t2), n2 = [];
  a2.has(Gr.x) || a2.has(Gr.y) || (n2.push(`translate(${e2.x}px,${e2.y}px)`), a2.add(Gr.x), a2.add(Gr.y)), a2.has(Gr.rotate) || (n2.push(`rotate(${e2.rotate}deg)`), a2.add(Gr.rotate));
  const l2 = {};
...
function cp(e2) {
  const t2 = { ...e2 };
  if (0 !== t2.rotate) {
    const e3 = Math.round(t2.width * Jr(t2.rotate) + t2.height * Xr(t2.rotate)), o2 = (t2.width - e3) / 2;
    t2.x += Math.round(o2), t2.right = t2.x + e3;
    const a2 = Math.round(t2.height * Jr(t2.rotate) + t2.width * Xr(t2.rotate)), n2 = (a2 - t2.height) / 2;
...
function dp(e2, t2) {
  for (const o2 of e2) {
    ...
    if (o2.chart.type === ns.MfCombine) {
      for (const e3 of o2.chart.options.components) if (e3.chart.id === t2) return e3;
    }
  }
}
function pp(e2, t2) {
  const o2 = dp(e2, t2.id);
  if (!o2) return;
  const a2 = 0 === o2.chart.data.type ? o2.chart.data.dataSet : o2.chart.data.result;
  return Ye(a2) ? void 0 !== o2.chart.data.currentValue ? o2.chart.data.currentValue : a2[0][t2.param] : a2 ? a2[t2.param] : void 0;
}
async function fp(e2, t2) {
  const o2 = (t3) => {
    if (!t3) return;
    ...
    if (o3) {
      const a3 = {}, n3 = (t4, o4) => {
        ...
        } else if (1 === o4.paramType || o4.event === rs.CHART_MOUNTED || o4.event === rs.CHART_BEFORE_MOUNT) {
          const n5 = pp(e2, o4);
          n5 && (a3[t4] = n5);
        }
      };
  ...
  }, { getDataTable: a2 } = ls(), n2 = [], l2 = [], i2 = Or(), r2 = (e3) => {
  ...
  for (const o3 of e2) if (t2 && (o3.chart.isResource = true), r2(o3), o3.chart.type === ns.MfCombine) for (const e3 of o3.chart.options.components) t2 && (e3.chart.isResource = true), r2(e3);
  ...
  for (const e3 of l2) e3.data.priority > u2 && (s2.length > 0 && (await Promise.all(s2), s2 = []), u2 = e3.data.priority), o2(e3), s2.push(a2(e3).then((t3) => {
    e3.type && tp[e3.type]?.initData(e3, t3);
  }));
  return s2;
}
function hp(e2) {
...
function gp(e2, t2) {
  ...
  for (const e3 of o2) {
    if (hp(e3.chart), e3.chart.type === ns.MfCombine) for (const t3 of e3.chart.options.components) hp(t3.chart);
...
function mp(e2) {
  if (!Or().getIsResource) return;
  const t2 = (e3) => {
    delete e3.isResource, 0 === e3.data.type && (e3.data.id = "");
  };
  for (const o2 of e2) if (t2(o2.chart), o2.chart.type === ns.MfCombine) for (const e3 of o2.chart.options.components) t2(e3.chart);
}
function yp(e2) {
  return 0 === e2.data.type ? e2.data.dataSet : e2.data.result;
}
const vp = Or(), Cp = (e2) => e2 <= 0, bp = (e2, t2) => e2 >= t2.length - 1, Mp = (e2, t2, o2) => {
  const a2 = e2[t2];
  e2[t2] = e2[o2], e2[o2] = a2;
}, kp = et("screen-shortcut", { state: () => ({ showMenu: false, copyData: null, alignType: Ip.GROUP }), getters: { getShowMenu() {
  ...
  vp.getSelectArea?.components.length > 0 ? this.copyData = qe(vp.getSelectArea?.components) : vp.getCurComponent && (this.copyData = qe(vp.getCurComponent));
}, pasteComponent() {
  if (this.copyData) {
    if (Ye(this.copyData)) this.copyData.forEach((e2) => {
      const t2 = lp(e2, e2.chartContain.dropInfo.x + 10, e2.chartContain.dropInfo.y + 10);
      t2 && vp.addComponent(t2, 0);
    });
    else {
      const e2 = this.copyData.chartContain.dropInfo.x + 10, t2 = this.copyData.chartContain.dropInfo.y + 10, o2 = lp(this.copyData, e2, t2);
      o2 && (vp.addComponent(o2, 0), vp.setCurComponent(o2, 0));
    }
    vp.setUndoRedoData("组件复制");
  }
}, moveComponent(e2, t2) {
  vp.getCurComponent && ("x" === e2 && (vp.getCurComponent.chartContain.dropInfo.x = vp.getCurComponent.chartContain.dropInfo.x + t2), "y" === e2 && (vp.getCurComponent.chartContain.dropInfo.y = vp.getCurComponent.chartContain.dropInfo.y + t2), t2 >= 10 && vp.setUndoRedoData("组件移动"));
}, upComponent() {
  Cp(vp.getCurComponentIndex) || (Mp(vp.getComponentList, vp.getCurComponentIndex, vp.getCurComponentIndex - 1), vp.setCurComponentIndex(vp.getCurComponentIndex - 1), vp.setUndoRedoData("组件上移一层"));
}, downComponent() {
  bp(vp.getCurComponentIndex, vp.getComponentList) || (Mp(vp.getComponentList, vp.getCurComponentIndex, vp.getCurComponentIndex + 1), vp.setCurComponentIndex(vp.getCurComponentIndex + 1), vp.setUndoRedoData("组件下移一层"));
}, topComponent() {
  vp.getCurComponent && !Cp(vp.getCurComponentIndex) && (vp.getComponentList.splice(vp.getCurComponentIndex, 1), vp.getComponentList.unshift(vp.getCurComponent), vp.setCurComponentIndex(0), vp.setUndoRedoData("组件置顶"));
}, bottomComponent() {
  vp.getCurComponent && !bp(vp.getCurComponentIndex, vp.getComponentList) && (vp.getComponentList.splice(vp.getCurComponentIndex, 1), vp.getComponentList.push(vp.getCurComponent), vp.setCurComponentIndex(vp.getComponentList.length - 1), vp.setUndoRedoData("组件置底"));
}, composeComponent() {
  if (vp.getSelectArea.components.length <= 1) return;
  const e2 = [];
  vp.getSelectArea.components.forEach((t2) => {
    if (t2.chart.type === ns.MfCombine) {
      const o2 = { ...t2.chartContain.dropInfo }, a2 = t2.chart.options.components, n2 = vp.getScreenCanvas?.getBoundingClientRect();
      a2.forEach((e3) => {
        Sp(e3, n2, o2, vp.getScale);
      }), e2.push(...t2.chart.options.components);
    } else e2.push(t2);
  }), C(() => {
    const t2 = rp({ type: ns.MfCombine, name: "组合" });
    null !== t2 && (t2.chartContain.dropInfo = Object.assign(t2.chartContain.dropInfo, vp.getSelectArea.dropInfo), t2.chart.options.components = e2, xp(t2), vp.batchDeleteComponent(vp.getSelectArea.components), vp.addComponent(t2, 0), vp.cleanSelectArea(), vp.setCurComponent(t2, 0), vp.setUndoRedoData("组件组合"));
  });
}, decomposeComponent() {
  if (!vp.getCurComponent) return;
  if (vp.getCurComponent.chart.type !== ns.MfCombine) return;
  vp.getCurComponent.chartContain.show || (vp.getCurComponent.chartContain.show = true);
  const e2 = vp.getCurComponent.chart.options.components, t2 = [];
  for (const o2 of e2) o2.chartContain.show || (o2.chartContain.show = true, t2.push(o2));
  C(() => {
    const o2 = { ...vp.getCurComponent.chartContain.dropInfo };
    if (!vp.getScreenCanvas) return;
    const a2 = vp.getScreenCanvas.getBoundingClientRect(), n2 = vp.getCurComponentIndex;
    vp.deleteComponent(n2);
    for (let t3 = e2.length - 1; t3 >= 0; t3--) Sp(e2[t3], a2, o2, vp.getScale), vp.addComponent(e2[t3], n2), vp.getSelectArea.components.push(e2[t3]);
    t2.forEach((e3) => {
      e3.chartContain.show = false;
    }), $p(vp.getSelectArea.components), vp.setUndoRedoData("组件拆分");
  });
}, lockComponent() {
  ...
  vp.getSelectArea?.components.length > 0 && vp.getSelectArea?.components.forEach((t2) => {
    t2.chartContain.lock = e2;
  }), vp.getCurComponent && (vp.getCurComponent.chartContain.lock = e2), vp.setUndoRedoData(e2 ? "组件锁定" : "组件解锁");
}, hideComponent() {
  ...
  }), vp.getCurComponent && (vp.getCurComponent.chartContain.show = e2), vp.setUndoRedoData(e2 ? "组件显示" : "组件隐藏");
}, deleteComponent() {
  vp.deleteComponent(), vp.setUndoRedoData("组件删除");
}, selectAllComponent() {
  $p(vp.getComponentList);
}, screenSave() {
  op.emit(Qr.SCREEN_SAVE);
}, screenPreview() {
  op.emit(Qr.SCREEN_PREVIEW);
} } });
function wp(e2) {
  return 100 * e2 + "%";
}
function xp(e2) {
  ...
  e2.chart.options.components.forEach((e3) => {
    e3.chartContain.groupStyle = sp(e3, [Gr.x, Gr.y]), e3.chartContain.groupStyle.left = wp((e3.chartContain.dropInfo.x - t2.x) / t2.width), e3.chartContain.groupStyle.top = wp((e3.chartContain.dropInfo.y - t2.y) / t2.height), e3.chartContain.groupStyle.width = wp(e3.chartContain.dropInfo.width / t2.width), e3.chartContain.groupStyle.height = wp(e3.chartContain.dropInfo.height / t2.height);
  });
}
function Sp(e2, t2, o2, a2) {
  ...
  e2.chartContain.dropInfo.rotate = es(e2.chartContain.dropInfo.rotate + o2.rotate), e2.chartContain.dropInfo.width = Math.abs(Math.round(Number.parseFloat(e2.chartContain.groupStyle.width) / 100 * o2.width)), e2.chartContain.dropInfo.height = Math.abs(Math.round(Number.parseFloat(e2.chartContain.groupStyle.height) / 100 * o2.height)), e2.chartContain.dropInfo.x = Math.abs(Math.round((i2 - e2.chartContain.dropInfo.width * a2 / 2) / a2)), e2.chartContain.dropInfo.y = Math.abs(Math.round((r2 - e2.chartContain.dropInfo.height * a2 / 2) / a2)), e2.chartContain.groupStyle = {};
}
function _p(e2) {
  const t2 = Or(), o2 = [], { x: a2, y: n2 } = e2;
  return t2.getComponentList.forEach((t3) => {
    const { x: l2, y: i2, width: r2, height: s2 } = cp(t3.chartContain.dropInfo);
...
function $p(e2) {
  ...
  e2.forEach((e3) => {
    const t3 = cp(e3.chartContain.dropInfo);
...
function Lp(e2) {
  ...
  t2.cleanCurComponent(), t2.setSelectedComponents(e2), t2.getSelectArea.components.length > 1 && $p(t2.getSelectArea.components);
}
var Ip = ((e2) => (e2.GROUP = "Group", e2.CANVAS = "Canvas", e2))(Ip || {});
const Dp = (e2, t2) => {
  function o2(o3) {
    ...
    }), $p(e2.getSelectArea.components), e2.setUndoRedoData("x" === o3 ? "横向分布" : "纵向分布");
  }
  function a2() {
  ...
  function l2(t3) {
    const o3 = a2();
    n2().forEach((e3) => t3(e3, o3)), $p(e2.getSelectArea.components), e2.setUndoRedoData("对齐");
  }
  function i2() {
  ...
  const o2 = 150;
  return { KeysEnum: { ArrowUp: He(() => t2.moveComponent("y", -1), o2), ArrowDown: He(() => t2.moveComponent("y", 1), o2), ArrowLeft: He(() => t2.moveComponent("x", -1), o2), ArrowRight: He(() => t2.moveComponent("x", 1), o2), Delete: Oe(() => t2.deleteComponent(), o2) }, CtrlKeysEnum: { c: Oe(() => t2.copyComponent(), o2), v: Oe(() => t2.pasteComponent(), o2), ArrowUp: He(() => t2.moveComponent("y", -10), o2), ArrowDown: He(() => t2.moveComponent("y", 10), o2), ArrowLeft: He(() => t2.moveComponent("x", -10), o2), ArrowRight: He(() => t2.moveComponent("x", 10), o2), g: Oe(() => t2.composeComponent(), o2), l: Oe(() => t2.lockComponent(), o2), h: Oe(() => t2.hideComponent(), o2), a: Oe(() => t2.selectAllComponent(), o2), p: Oe(() => t2.screenPreview(), o2), s: Oe(() => t2.screenSave(), o2), z: Oe(() => e2.undo(), o2) }, ShiftKeysEnum: { ArrowUp: He(() => t2.upComponent(), o2), ArrowDown: He(() => t2.downComponent(), o2) }, CtrlShiftKeysEnum: { ArrowUp: Oe(() => t2.topComponent(), o2), ArrowDown: Oe(() => t2.bottomComponent(), o2), g: Oe(() => t2.decomposeComponent(), o2), l: Oe(() => t2.unLockComponent(), o2), h: Oe(() => t2.showComponent(), o2), z: Oe(() => e2.redo(), o2) }, NoneShortcutKeyEnum: Dp(e2, t2) };
}, Np = { class: "title" }, Bp = to(e({ __name: "ConfigWrapper", props: { iconfont: { type: String, default: void 0 }, icon: { type: String, default: void 0 }, title: { type: String, default: "" }, allowDelete: { type: Boolean, default: true }, full: { type: Boolean, default: false }, highlight: { type: Boolean, default: false }, queryMode: { type: Boolean, default: false } }, emits: ["deleteConfig", "execute"], setup(e2, { emit: t2 }) {
  const o2 = t2, a2 = Se().getThemeColor, { prefixCls: n2 } = xe("config-wrapper"), l2 = Ve(), i2 = _r(), y2 = m(false);
  function v2(e3) {
    o2("deleteConfig", e3);
  }
  function C2() {
  ...
  return (t3, o3) => (s(), r("div", { class: c([d(n2), d(i2).getShowData && d(i2).getShowDataLevel === d(l2) ? "active" : ""]), style: u(e2.highlight ? { border: `1px solid ${d(a2)}` } : "") }, [d(i2).getShowData && d(i2).getShowDataLevel === d(l2) ? (s(), r("div", { key: 0, class: "select", style: u({ backgroundColor: d(a2) }) }, [g(d(Be), { icon: "ant-design:check-outlined", color: "#FFFFFF", size: 14 })], 4)) : f("", true), void 0 !== e2.iconfont ? (s(), h(d(Be), { key: 1, color: d(a2), icon: `iconfont:${e2.iconfont}`, size: 14 }, null, 8, ["color", "icon"])) : f("", true), void 0 !== e2.icon ? (s(), h(d(Be), { key: 2, color: d(a2), icon: e2.icon, size: 14 }, null, 8, ["color", "icon"])) : f("", true), p("div", Np, S(e2.title), 1), e2.allowDelete && !e2.queryMode ? (s(), h(d(J), { key: 3, type: "vertical" })) : f("", true), e2.queryMode ? f("", true) : (s(), h(d(G), { key: 4, title: "删除" }, { default: M(() => [e2.allowDelete ? (s(), h(d(ee), { key: 0, size: "small", type: "link", onClick: v2 }, { icon: M(() => [g(d(ft))]), _: 1 })) : f("", true)]), _: 1 })), p("div", { class: c(["config", e2.full ? "full" : ""]) }, [R(t3.$slots, "config", {}, void 0, true)], 2), g(d(G), { title: "执行" }, { default: M(() => [g(d(ee), { size: "small", type: "link", style: { margin: "6px" }, onMouseenter: o3[0] || (o3[0] = (e3) => y2.value = true), onMouseleave: o3[1] || (o3[1] = (e3) => y2.value = false), onClick: C2 }, { icon: M(() => [!d(i2).getShowData || d(i2).getShowDataLevel !== d(l2) || y2.value ? (s(), h(d(ht), { key: 0 })) : (s(), h(d(gt), { key: 1 }))]), _: 1 })]), _: 1 })], 6));
} }), [["__scopeId", "data-v-c5dcf0b5"]]), Rp = to(e({ __name: "SelectFields", props: { tableFields: { type: Object, require: true, default: void 0 }, fields: { type: Object, require: true, default: void 0 }, queryMode: { type: Boolean, default: false } }, emits: ["fieldChange"], setup(e2, { emit: t2 }) {
  const o2 = e2, l2 = t2, i2 = m([]), p2 = m(false), { prefixCls: f2 } = xe("select-fields"), C2 = a(() => o2.tableFields ? o2.tableFields?.length < 10 ? 30 * o2.tableFields?.length : 300 : 30), b2 = m(false);
  n(() => o2.tableFields, (e3) => {
  ...
  return n(() => o2.sqlQuery?.sourceTable, (e3) => {
    ...
    o2.sqlQuery?.sqlQuery && o2.sqlQuery?.sourceTable && Dr(o2.sqlQuery).then((e3) => {
  ...
  }), (t3, a3) => (s(), h(Bp, { iconfont: "icon-mfish-table", title: "数据源", "allow-delete": false, onExecute: y2, "query-mode": e2.queryMode }, { config: M(() => [g(d(r2), { class: c(d(u2)), compact: "" }, { default: M(() => [g(d(l2), { value: e2.sqlQuery?.sourceTable, disabled: "" }, null, 8, ["value"]), g(Rp, $(t3.$attrs, { "table-fields": p2.value, fields: o2.sqlQuery?.fields, onFieldChange: v2, "query-mode": e2.queryMode }), null, 16, ["table-fields", "fields", "query-mode"])]), _: 1 }, 8, ["class"])]), _: 1 }, 8, ["query-mode"]));
} }), [["__scopeId", "data-v-1d0129ce"]]), Op = async (e2, t2) => {
  const o2 = _r(), a2 = [];
  ...
  for (; ; ) return o2.has(t2) ? (t2.endsWith(`_${e2}`) && (t2 = t2.slice(0, Math.max(0, t2.length - `${e2}`.length - 1))), Ap(++e2, `${t2}_${e2}`, o2)) : t2;
}, jp = (e2) => e2?.table && e2.condition?.field?.colName && e2.condition?.joinedField?.colName, zp = (e2) => {
  const t2 = { fields: [], table: "", tableAlias: "", condition: { field: { tableAlias: "结果集" }, operation: "=", joinedField: {} }, type: "left" };
  e2 && e2.push(t2);
}, Ep = to(e({ __name: "ConfigButton", props: { level: { type: Number, default: 1 }, icon: { type: String, default: void 0 }, iconfont: { type: String, default: void 0 }, title: { type: String, default: "按钮" } }, setup(e2) {
  const t2 = e2, { prefixCls: o2 } = xe("config-button"), n2 = Se().getThemeColor, l2 = _r(), i2 = a(() => t2.level < l2.getLevel ? 12 : 14);
  return (t3, a2) => (s(), h(d(ee), $(t3.$attrs, { size: "small", shape: "round", class: d(o2), style: { "--hover-color--": d(n2) }, type: "link" }), { icon: M(() => [e2.iconfont ? (s(), h(d(Be), { key: 0, icon: `iconfont:${e2.iconfont}`, style: { "margin-right": "8px" }, size: i2.value }, null, 8, ["icon", "size"])) : (s(), h(d(Be), { key: 1, icon: e2.icon, size: i2.value }, null, 8, ["icon", "size"]))]), default: M(() => [p("span", { class: c(e2.level < d(l2).getLevel ? "show-title" : ""), style: u({ fontSize: `${i2.value}px` }) }, S(e2.title), 7)]), _: 1 }, 16, ["class", "style"]));
} }), [["__scopeId", "data-v-749209b7"]]), Fp = { key: 0, class: "placeholder" }, Pp = { class: "condition" }, Hp = e({ __name: "TableJoins", props: { sqlQuery: { type: Object, require: true }, level: { type: Number, default: 1 }, configMitt: { type: Object }, queryMode: { type: Boolean, default: false } }, emits: ["joinChange"], setup(e2, { emit: t2 }) {
  const o2 = e2, l2 = t2, i2 = te.Item, C2 = H, { prefixCls: _2 } = xe("table-joins"), $2 = _r(), L2 = m([]), I2 = m([]), D2 = m([]), T2 = Se().getThemeColor, N2 = { left: "左联", inner: "内联", right: "右联", full: "全联" }, B2 = a(() => {
    if (1 === $2.getSourceType || o2.queryMode) return false;
    if (o2.sqlQuery && o2.sqlQuery?.joins) {
      for (const e3 of o2.sqlQuery.joins) if (!jp(e3)) return false;
    }
    return Nr(o2.sqlQuery, o2.level);
  });
  ...
  const R2 = a(() => {
    ...
    return [];
  }), q2 = (e3) => I2.value.length > e3 ? I2.value[e3] : [], O2 = (e3) => {
    ...
    return [];
  }, j2 = (e3, t3) => q2(e3).find((e4) => e4.value === t3[0]) ? t3[1] : "", z2 = (e3) => D2.value.length > e3 ? b(D2.value[e3]) : [], E2 = (e3) => {
    if (!o2.sqlQuery?.joins) return [];
  ...
  return (t3, a2) => {
    ...
    return s(), r("div", { class: c(d(_2)) }, [(s(true), r(y, null, v(e2.sqlQuery?.joins, (t4, c2) => (s(), h(Bp, { iconfont: "icon-mfish-join", title: "关联", key: c2, "query-mode": e2.queryMode, onDeleteConfig: (e3) => function(e4) {
      L2?.value.splice(e4, 1), l2("joinChange", [...L2.value]);
    }(c2), onExecute: (e3) => function(e4, t5) {
      Br(o2.configMitt, { ...o2.sqlQuery, joins: L2.value.slice(0, t5 + 1), aggregates: [], groups: [], customColumns: [], filters: [], orders: [], limit: void 0 }, e4);
    }(e3, c2) }, { config: M(() => [0 === c2 ? (s(), h(d(C2), { key: 0, class: "left-table", value: e2.sqlQuery?.sourceTable, disabled: "" }, null, 8, ["value"])) : (s(), h(d(C2), { key: 1, class: "left-table", value: t4.condition?.field.tableAlias, disabled: "" }, null, 8, ["value"])), g(d(V), { class: "condition", placement: "bottom", arrow: { pointAtCenter: true }, disabled: e2.queryMode }, { overlay: M(() => [g(d(te), { onClick: (e3) => function(e4, t5) {
      t5.type = e4.key;
    ...
    }, ["prevent"])) }, [x(S(N2[t4.type]) + " ", 1), g(d(Be), { icon: `iconfont:icon-mfish-${t4.type}-join`, color: d(T2), size: 14 }, null, 8, ["icon", "color"])])]), _: 2 }, 1032, ["disabled"]), g(n2, { class: "right-table", compact: "" }, { default: M(() => {
      return [g(d(oe), { disabled: e2.queryMode, "show-search": "", "allow-clear": false, class: "input-select", "option-filter-prop": "label", value: [O2(c2) || ""], options: R2.value, onChange: (e3) => function(e4, t5) {
        ...
        Ai({ sourceId: _r().getSourceId, tableName: t5.table, sourceType: 0 }).then((e5) => {
          $2.addTableFieldsMap(o3, e5), a4(e5);
        }).finally(() => U2());
      }(e3, t4) }, { displayRender: M(({ selectedOptions: e3 }) => [e3[0] ? (s(), h(d(G), { key: 1, title: e3[0]?.label }, { default: M(() => [x(S(Q2(c2, e3[0]?.value)), 1)]), _: 2 }, 1032, ["title"])) : (s(), r("div", Fp, "选择关联表"))]), _: 2 }, 1032, ["disabled", "value", "options", "onChange"]), g(Rp, { "query-mode": e2.queryMode, "table-fields": (a3 = t4.table, a3 && $2.getTableFieldsMap.has(a3) ? $2.getTableFieldsMap.get(a3) : []), fields: t4.fields, onFieldChange: (e3) => function(e4, t5) {
      ...
      var a3;
    }), _: 2 }, 1024), p("span", { class: "condition", style: u({ color: d(T2) }) }, "当", 4), g(d(oe), { disabled: e2.queryMode, "show-search": "", "allow-clear": false, class: "input-select", "option-filter-prop": "label", options: q2(c2), value: A2(c2), placeholder: `${e2.sqlQuery?.sourceTable}字段`, onChange: (e3, o3) => function(e4, t5, o4) {
      o4.condition && (o4.condition.field = e4[1].realValue, o4.condition.field.tableAlias = e4[0].value, P2(t5));
    }(o3, c2, t4) }, { displayRender: M(({ labels: e3 }) => [g(d(G), { title: j2(c2, e3) }, { default: M(() => [x(S(j2(c2, e3)), 1)]), _: 2 }, 1032, ["title"])]), _: 2 }, 1032, ["disabled", "options", "value", "placeholder", "onChange"]), p("div", Pp, [g(d(Be), { icon: "iconfont:icon-mfish-equals", color: d(T2), size: 14 }, null, 8, ["color"])]), g(d(K), { disabled: e2.queryMode, "show-search": "", "allow-clear": false, class: "input-select", "option-filter-prop": "label", value: E2(c2), options: z2(c2), placeholder: `${t4.tableAlias || "关联表"}字段`, onSelect: (e3, o3) => function(e4, t5, o4) {
      o4.condition && (o4.condition.joinedField = e4.realValue, P2(t5));
    }(o3, c2, t4) }, null, 8, ["disabled", "value", "options", "placeholder", "onSelect"])]), _: 2 }, 1032, ["query-mode", "onDeleteConfig", "onExecute"]))), 128)), B2.value ? (s(), h(Ep, { key: 0, class: "config-button", title: "关联", iconfont: "icon-mfish-join", level: 0, onClick: F2 })) : f("", true)], 2);
  };
} }), Qp = to(Hp, [["__scopeId", "data-v-d960e99c"]]), Gp = e({ __name: "TableLimit", props: { sqlQuery: { type: Object, require: true }, configMitt: { type: Object }, queryMode: { type: Boolean, default: false } }, emits: ["limitChange"], setup(e2, { emit: t2 }) {
  ...
  function c2(e3) {
    Br(o2.configMitt, { ...o2.sqlQuery, limit: l2.value }, e3);
  }
  return n(() => o2.sqlQuery?.limit, (e3) => {
    e3 && (l2.value = e3);
  }, { immediate: true }), (t3, o3) => void 0 !== e2.sqlQuery?.limit ? (s(), h(Bp, { key: 0, "query-mode": e2.queryMode, icon: "ant-design:column-height-outlined", title: "行数", onDeleteConfig: u2, onExecute: c2 }, { config: M(() => [g(d(Q), { disabled: e2.queryMode, "default-value": l2.value, "addon-after": "行", step: "10", onChange: i2, onPressEnter: r2, onBlur: r2 }, null, 8, ["disabled", "default-value"])]), _: 1 }, 8, ["query-mode"])) : f("", true);
} }), Up = [{ field: "id", label: "唯一ID", component: "Input", show: false }, { field: "name", label: "参数名称", component: "Input", dynamicRules: () => [{ required: true, validator: (e2, t2) => /^[\u4E00-\u9FA5\w]*$/.test(t2) ? Promise.resolve() : Promise.reject("错误：参数格式不正确！字符只允许中文、字符、数字、下划线!"), trigger: "blur" }] }, { field: "required", label: "是否必须", component: "RadioButtonGroup", defaultValue: 1, componentProps: { options: [{ label: "是", value: 1 }, { label: "否", value: 0 }] }, required: true }, { field: "defaultValue", label: "默认值", component: "Input" }, { field: "remark", label: "参数描述", component: "InputTextArea" }], Vp = { name: "ApiParamsModal", components: { BasicModal: bt, BasicForm: wt }, emits: ["addSuccess", "updateSuccess", "register", "variableChange"], setup(e2, { emit: t2 }) {
  const o2 = m(true), [n2, { resetFields: l2, setFieldsValue: i2, validate: r2 }] = xt({ name: "model_form_item", labelWidth: 100, baseColProps: { span: 24 }, schemas: Up, showActionButtonGroup: false, autoSubmitOnEnter: true }), s2 = m(), [u2, { setModalProps: c2, closeModal: p2 }] = Mt(async (e3) => {
    l2().then(), c2({ confirmLoading: false, width: "600px" }), o2.value = !!e3?.isUpdate, d(o2) && (s2.value = e3.index, i2({ ...e3.param }).then());
  }), f2 = _r(), h2 = a(() => d(o2) ? "编辑参数" : "新增参数"), { createMessage: g2 } = _e();
...
const Wp = to(Vp, [["render", function(e2, t2, o2, a2, n2, l2) {
...
}]]), Yp = { key: 0 }, Zp = e({ __name: "FilterVariable", props: { value: { type: String }, queryMode: { type: Boolean, default: false } }, emits: ["variableChange"], setup(e2, { emit: t2 }) {
  const o2 = e2, a2 = t2, l2 = K.Option, i2 = Se().getThemeColor, { prefixCls: C2 } = xe("filter-variable"), [b2, { openModal: w2 }] = kt(), _2 = _r(), $2 = m();
  n(() => o2.value, (e3) => {
    $2.value = e3;
  }, { immediate: true });
  ...
  return (t3, o3) => (s(), r(y, null, [g(d(K), { value: $2.value, "onUpdate:value": o3[0] || (o3[0] = (e3) => $2.value = e3), onChange: N2, disabled: e2.queryMode }, { dropdownRender: M(({ menuNode: e3 }) => [g(L2, { vnodes: e3 }, null, 8, ["vnodes"]), g(d(J), { style: { margin: "4px 0" } }), p("div", { style: { padding: "4px 8px", cursor: "pointer" }, onClick: I2 }, [g(d(Be), { icon: "ant-design:plus-outlined", color: d(i2) }, null, 8, ["color"]), p("span", { style: u(`color:${d(i2)};margin-left:6px`) }, "增加变量参数", 4)])]), default: M(() => [(s(true), r(y, null, v(d(_2).getVariables, (t4, o4) => (s(), h(d(l2), { label: t4.name, value: t4.name, key: o4 }, { default: M(() => [p("div", { class: c(`${d(C2)}-select-option`) }, [x(S(t4.name) + " ", 1), p("div", null, S(t4.remark), 1), e2.queryMode ? f("", true) : (s(), r("div", Yp, [g(d(Be), { icon: "ant-design:edit-outlined", color: d(i2), onClick: k((e3) => function(e4, t5) {
  ...
  }(o4), ["stop"]) }, null, 8, ["color", "onClick"])]))], 2)]), _: 2 }, 1032, ["label", "value"]))), 128))]), _: 1 }, 8, ["value", "disabled"]), g(Wp, { onRegister: d(b2), onAddSuccess: D2, onUpdateSuccess: T2, onVariableChange: B2 }, null, 8, ["onRegister"])], 64));
} }), Kp = to(Zp, [["__scopeId", "data-v-68d91e73"]]), Xp = e({ __name: "TableField", props: { tableFields: { type: Object }, value: { type: Object }, queryMode: { type: Boolean, default: false } }, emits: ["change"], setup(e2, { emit: t2 }) {
  const o2 = e2, a2 = t2, l2 = m({ dataType: "STRING" }), i2 = m([]), r2 = Se().getThemeColor, u2 = m();
  function c2(e3) {
    u2.value = ti(e3);
  }
...
} }), Jp = e({ __name: "FilterValue", props: { fieldType: { type: String, default: "STRING" }, valueType: { type: String, default: "0" }, operator: { type: String, default: "" }, tableFields: { type: Array, default: () => [] }, queryMode: { type: Boolean, default: false } }, emits: ["typeChange", "variableChange", "fieldChange"], setup(e2, { emit: t2 }) {
  const o2 = e2, a2 = t2, l2 = H, i2 = ae.Group, u2 = ae.Button, p2 = ne.ItemRest, { prefixCls: y2 } = xe("filter-value"), v2 = m("0"), C2 = _r();
  function b2() {
  ...
  function S2(e3) {
    e3 === oi.isNull || e3 === oi.isNotNull ? (p2.value = false, n2.value = "") : p2.value = true, "DATE" === u2.field?.dataType && (e3 === oi.between ? n2.value = n2.value ? `${n2.value}`.split(",") : [] : n2.value = n2.value ? `${n2.value}`.split(",")[0] : "");
  }
  async function _2() {
    try {
  ...
  return (t3, o3) => (s(), h(d(bt), $(t3.$attrs, { onRegister: d(C2), title: "设置过滤条件", onOk: _2 }), { default: M(() => [g(d(ne), { ref_key: "formRef", ref: i2, model: n2, "label-col": { span: 6 }, "wrapper-col": { span: 16 } }, { default: M(() => [g(d(a2), { label: "字段", name: "field", rules: [{ required: true, message: "请选择字段！" }] }, { default: M(() => [g(Xp, { "query-mode": e2.queryMode, "table-fields": r2.value, value: u2.field, onChange: w2 }, null, 8, ["query-mode", "table-fields", "value"])]), _: 1 }), g(d(a2), { label: "条件", name: "operator", rules: [{ required: true, message: "请选择条件！" }] }, { default: M(() => [g(d(K), { disabled: e2.queryMode, "show-search": "", "allow-clear": false, class: "input-select", "option-filter-prop": "label", value: n2.operator, "onUpdate:value": o3[0] || (o3[0] = (e3) => n2.operator = e3), options: c2.value, placeholder: "选择条件", onSelect: S2 }, null, 8, ["disabled", "value", "options"])]), _: 1 }), p2.value ? (s(), h(d(a2), { key: 0, label: "值", name: "value", rules: [{ required: "STRING" !== u2.field?.dataType, message: "请选择值！" }] }, { default: M(() => [g(ef, { "query-mode": e2.queryMode, placeholder: "输入值", value: n2.value, "onUpdate:value": o3[1] || (o3[1] = (e3) => n2.value = e3), "value-type": u2.valueType, "field-type": u2.field?.dataType, operator: n2.operator, "table-fields": r2.value, onTypeChange: L2, onVariableChange: I2, onFieldChange: D2 }, null, 8, ["query-mode", "value", "value-type", "field-type", "operator", "table-fields"])]), _: 1 }, 8, ["rules"])) : f("", true)]), _: 1 }, 8, ["model"])]), _: 1 }, 16, ["onRegister"]));
} }), of = e({ __name: "TableFilter", props: { sqlQuery: { type: Object }, level: { type: Number, default: 1 }, configMitt: { type: Object }, queryMode: { type: Boolean, default: false } }, emits: ["filterChange"], setup(e2, { emit: t2 }) {
  const o2 = e2, l2 = t2, i2 = m([]), u2 = a(() => void 0 === o2.sqlQuery?.filters && Nr(o2.sqlQuery, o2.level) && !o2.queryMode);
  n(() => o2.sqlQuery?.filters, (e3) => {
    i2.value = e3 ? [...e3] : [];
  }, { immediate: true });
  const c2 = Se().getThemeColor, p2 = m([{ name: "添加", icon: "icon-mfish-left-bracket", color: c2, value: { type: "0", name: "(", value: "(" } }, { name: "添加", icon: "icon-mfish-right-bracket", color: c2, value: { type: "0", name: ")", value: ")" } }]), y2 = { type: "0", slot: true, hideClose: true, value: "and" }, [v2, { openModal: C2 }] = kt();
  function b2() {
    l2("filterChange");
  }
  function w2() {
  ...
  function I2(e3) {
    Br(o2.configMitt, { ...o2.sqlQuery, filters: i2.value, aggregates: [], groups: [], customColumns: [], orders: [], limit: void 0 }, e3);
  }
  return (t3, o3) => (s(), r("div", null, [void 0 !== e2.sqlQuery?.filters ? (s(), h(Bp, { key: 0, "query-mode": e2.queryMode, icon: "ant-design:filter-outlined", title: "过滤", onDeleteConfig: b2, full: true, onExecute: I2 }, { config: M(() => [g(d(Ct), { "query-mode": e2.queryMode, items: i2.value, menus: p2.value, "add-title": "添加过滤条件", onAddBlock: x2, onEditBlock: S2, onCloseBlock: $2, onDragChange: L2 }, { tag: M(({ index: t4, element: o4 }) => [g(d(re), { disabled: e2.queryMode, style: { "margin-right": "8px" }, checked: o4?.value, "checked-children": "且", "un-checked-children": "或", "checked-value": "and", "un-checked-value": "or", size: "small", onChange: (e3) => function(e4, t5) {
    i2.value && i2.value?.length > t5 && (i2.value[t5].value = e4, setTimeout(() => {
      i2.value && l2("filterChange", [...i2.value]);
    }, 300));
  }(e3, t4) }, null, 8, ["disabled", "checked", "onChange"])]), _: 1 }, 8, ["query-mode", "items", "menus"])]), _: 1 }, 8, ["query-mode"])) : f("", true), u2.value ? (s(), h(Ep, { key: 1, class: "config-button", title: "过滤", icon: "ant-design:filter-outlined", level: 0, onClick: k(w2, ["prevent"]) })) : f("", true), g(tf, { "query-mode": e2.queryMode, onRegister: d(v2), onSubmit: _2 }, null, 8, ["query-mode", "onRegister"])]));
} }), af = e({ __name: "AggregateModal", props: { queryMode: { type: Boolean, default: false } }, emits: ["submit", "register"], setup(e2, { emit: t2 }) {
  const o2 = t2, a2 = ne.Item, n2 = l({ field: void 0, aggFunc: "" }), i2 = m(), r2 = m([]), u2 = l({ field: { dataType: "STRING" } }), c2 = Object.keys(ai).map((e3) => ({ value: e3, label: ai[e3] })), p2 = m(true), y2 = m(), [v2, { setModalProps: C2, closeModal: b2 }] = Mt(async (e3) => {
    if (C2({ confirmLoading: false, width: "600px" }), Op(void 0, e3.sqlQuery).then((e4) => {
      ...
      r2.value = e4;
    }), e3.isUpdate) return n2.field = [e3.field.tableAlias, e3.field.colName], n2.aggFunc = e3.aggFunc, u2.field = e3.field, p2.value = ai[n2.aggFunc] !== ai.count, void (y2.value = e3.index);
  ...
  function w2(e3) {
    p2.value = ai[e3] !== ai.count;
  }
  async function x2() {
    try {
      const e3 = await i2.value?.validateFields();
      b2(), o2("submit", y2.value, { ...u2, name: ai[n2.aggFunc] + (ai[n2.aggFunc] === ai.count ? "" : `(${u2.field?.colName})`), aggFunc: e3.aggFunc });
    } catch (e3) {
    }
  }
...
} }), nf = e({ __name: "GroupModal", props: { queryMode: { type: Boolean, default: false } }, emits: ["submit", "register"], setup(e2, { emit: t2 }) {
  const o2 = t2, a2 = ne.Item, n2 = l({ field: void 0 }), i2 = l({ field: { dataType: "STRING" } }), r2 = m(), u2 = m([]), c2 = m(), [p2, { setModalProps: f2, closeModal: y2 }] = Mt(async (e3) => {
    if (f2({ confirmLoading: false, width: "600px" }), Op(void 0, e3.sqlQuery).then((e4) => {
      u2.value = e4;
...
} }), lf = e({ __name: "TableAggregate", props: { sqlQuery: { type: Object }, configMitt: { type: Object }, queryMode: { type: Boolean, default: false } }, emits: ["aggregateChange", "groupChange", "deleteAggregate"], setup(e2, { emit: t2 }) {
  const o2 = e2, a2 = t2, l2 = Se().getThemeColor, { prefixCls: i2 } = xe("table-aggregate"), u2 = m(), y2 = m(), [v2, { openModal: C2 }] = kt(), [b2, { openModal: k2 }] = kt();
  async function w2() {
    a2("deleteAggregate");
  }
  function x2() {
  ...
  function N2(e3) {
    Br(o2.configMitt, { ...o2.sqlQuery, aggregates: u2.value, groups: y2.value, orders: [], limit: void 0 }, e3);
  }
  return n(() => o2.sqlQuery?.aggregates, (e3) => {
    u2.value = e3 ? [...e3] : [];
  }, { immediate: true }), n(() => o2.sqlQuery?.groups, (e3) => {
    y2.value = e3 ? [...e3] : [];
  }, { immediate: true }), (t3, o3) => (s(), r("div", { class: c(d(i2)) }, [void 0 !== e2.sqlQuery?.aggregates ? (s(), h(Bp, { key: 0, "query-mode": e2.queryMode, iconfont: "icon-mfish-sum", title: "聚合", onDeleteConfig: w2, onExecute: N2, full: true }, { config: M(() => [g(d(Ct), { "query-mode": e2.queryMode, items: u2.value, "add-title": "添加聚合指标", onAddBlock: x2, onEditBlock: _2, onDragChange: $2 }, null, 8, ["query-mode", "items"]), g(d(Be), { class: "groupIcon", icon: "ant-design:group-outlined", color: d(l2), style: { "font-size": "14px" } }, null, 8, ["color"]), o3[0] || (o3[0] = p("div", { class: "group" }, "分组", -1)), g(d(Ct), { "query-mode": e2.queryMode, items: y2.value, "add-title": "添加分组指标", onAddBlock: L2, onEditBlock: D2, onDragChange: T2 }, null, 8, ["query-mode", "items"])]), _: 1 }, 8, ["query-mode"])) : f("", true), g(af, { "query-mode": e2.queryMode, onRegister: d(v2), onSubmit: S2 }, null, 8, ["query-mode", "onRegister"]), g(nf, { "query-mode": e2.queryMode, onRegister: d(b2), onSubmit: I2 }, null, 8, ["query-mode", "onRegister"])], 2));
} }), rf = to(lf, [["__scopeId", "data-v-a3fb541b"]]), sf = "其他", uf = e({ __name: "OrderModal", emits: ["submit", "register"], setup(e2, { emit: t2 }) {
  const o2 = t2, a2 = ne.Item, n2 = l({ field: void 0 }), i2 = l({ field: { dataType: "STRING" }, order: "ASC" }), r2 = m(), u2 = m([]), c2 = m(), p2 = _r(), [f2, { setModalProps: y2, closeModal: v2 }] = Mt(async (e3) => {
    if (y2({ confirmLoading: false, width: "600px" }), Tr(e3.sqlQuery) ? Oi({ sourceId: p2.getSourceId, sourceType: p2.getSourceType, sqlQuery: e3.sqlQuery }).then((e4) => {
      u2.value = [{ value: sf, label: sf, realValue: sf, children: e4.map((e5) => ({ value: e5.colName, label: e5.colName + (e5.comment ? `[${e5.comment}]` : ""), realValue: { ...e5 } })) }];
    }) : u2.value = await Op(void 0, e3.sqlQuery), e3.isUpdate) return n2.field = [e3.field.tableAlias, e3.field.colName], i2.field = e3.field, void (c2.value = e3.index);
    n2.field = [], c2.value = void 0, i2.field = {};
  });
  function C2(e3) {
    e3.tableAlias && e3.colName && (n2.field = [e3.tableAlias, e3.colName], i2.field = e3);
  ...
  async function b2() {
    try {
      await r2.value?.validateFields(), v2(), i2.field.tableAlias === sf && (i2.field.tableAlias = ""), o2("submit", c2.value, { ...i2, name: i2.field?.colName });
    } catch (e3) {
    }
  }
  return (e3, t3) => (s(), h(d(bt), $(e3.$attrs, { onRegister: d(f2), title: "选择排序字段", onOk: b2 }), { default: M(() => [g(d(ne), { ref_key: "formRef", ref: r2, model: n2, "label-col": { span: 6 }, "wrapper-col": { span: 16 } }, { default: M(() => [g(d(a2), { label: "字段", name: "field", rules: [{ required: true, message: "请选择字段！" }] }, { default: M(() => [g(Xp, { "table-fields": u2.value, value: i2.field, onChange: C2 }, null, 8, ["table-fields", "value"])]), _: 1 })]), _: 1 }, 8, ["model"])]), _: 1 }, 16, ["onRegister"]));
} }), cf = e({ __name: "TableOrder", props: { sqlQuery: { type: Object, default: void 0 }, configMitt: { type: Object, default: void 0 }, queryMode: { type: Boolean, default: false } }, emits: ["orderChange"], setup(e2, { emit: t2 }) {
  const o2 = e2, a2 = t2, l2 = m([]), [i2, { openModal: u2 }] = kt();
  function c2() {
  ...
  function b2(e3) {
    Br(o2.configMitt, { ...o2.sqlQuery, orders: l2.value, limit: void 0 }, e3);
  }
  return n(() => o2.sqlQuery?.orders, (e3) => {
    l2.value = e3 || [];
  }, { immediate: true }), (t3, o3) => (s(), r("div", null, [void 0 !== e2.sqlQuery?.orders ? (s(), h(Bp, { key: 0, "query-mode": e2.queryMode, icon: "ant-design:sort-ascending-outlined", title: "排序", onDeleteConfig: v2, full: true, onExecute: b2 }, { config: M(() => [g(d(Ct), { "query-mode": e2.queryMode, items: l2.value, "add-title": "添加排序指标", onAddBlock: c2, onEditBlock: p2, onDragChange: y2 }, { tagIcon: M(({ element: e3 }) => [g(d(Be), { style: { "font-weight": "bold", "font-size": "20px" }, icon: "ASC" === e3.order ? "ant-design:sort-descending-outlined" : "ant-design:sort-ascending-outlined" }, null, 8, ["icon"])]), _: 1 }, 8, ["query-mode", "items"])]), _: 1 }, 8, ["query-mode"])) : f("", true), g(uf, { onRegister: d(i2), onSubmit: C2 }, null, 8, ["onRegister"])]));
} }), df = e({ __name: "FieldColModal", emits: ["submit", "register"], setup(e2, { emit: t2 }) {
  const o2 = t2, a2 = ne.Item, n2 = l({ field: void 0 }), i2 = m(), r2 = m(), u2 = m([]), c2 = m(), [p2, { setModalProps: f2, closeModal: y2 }] = Mt(async (e3) => {
    if (f2({ confirmLoading: false, width: "600px" }), Op(void 0, e3.sqlQuery).then((e4) => {
      u2.value = e4;
    }), e3.isUpdate) {
  ...
  async function C2() {
    try {
      await r2.value?.validateFields(), y2(), o2("submit", c2.value, { type: ni.field, value: i2.value, name: i2.value?.colName });
    } catch (e3) {
    }
  }
  return (e3, t3) => (s(), h(d(bt), $(e3.$attrs, { onRegister: d(p2), title: "选择字段", onOk: C2 }), { default: M(() => [g(d(ne), { ref_key: "formRef", ref: r2, model: n2, "label-col": { span: 6 }, "wrapper-col": { span: 16 } }, { default: M(() => [g(d(a2), { label: "字段", name: "field", rules: [{ required: true, message: "请选择字段！" }] }, { default: M(() => [g(Xp, { "table-fields": u2.value, value: i2.value, onChange: v2 }, null, 8, ["table-fields", "value"])]), _: 1 })]), _: 1 }, 8, ["model"])]), _: 1 }, 16, ["onRegister"]));
} }), pf = ["onClick"], ff = ["onClick"], hf = e({ __name: "FormulaManagement", props: { isShow: { type: Boolean, default: false }, curKey: { type: String, default: "" } }, emits: ["select"], setup(e2, { emit: t2 }) {
  const o2 = e2, l2 = t2, i2 = se.Item, C2 = ue.Item, b2 = m([]), { prefixCls: k2 } = xe("formula-management"), w2 = m(null), _2 = Se().getThemeColor, $2 = m([]), L2 = m(false), I2 = m([]), D2 = m(), T2 = m(0.4), B2 = m([]), R2 = m(), q2 = a(() => {
  ...
  function A2(e3, t3) {
    O2(t3.node.dataRef, t3.node?.parent?.node);
  }
  function j2(e3) {
    if (!e3) return O2(), void ($2.value = []);
    $2.value = [e3];
    const t3 = e3.split(",");
    if (t3 && 2 === t3.length) return w2.value?.setExpandedKeys([t3[0]]), void O2(w2.value?.getSelectedNode(e3), w2.value?.getSelectedNode(t3[0]));
    O2(w2.value?.getSelectedNode(e3));
  }
  n(() => o2.isShow, (e3) => {
    e3 && setTimeout(() => {
      j2(o2.curKey);
  ...
  }), (e3, t3) => (s(), h(d(_t), { "default-size": T2.value, min: 0.1, max: 0.5, class: c(d(k2)) }, { 1: M(() => [g(d(St), { title: "", toolbar: "", search: "", ref_key: "asyncTreeRef", ref: w2, "tree-wrapper-class-name": "h-[calc(100%-35px)] overflow-auto", "selected-keys": $2.value, "onUpdate:selectedKeys": t3[0] || (t3[0] = (e4) => $2.value = e4), "tree-data": B2.value || [], onSelect: A2 }, null, 8, ["selected-keys", "tree-data"])]), 2: M(() => [p("div", { class: c(`${d(k2)}-right h-full pt-2`) }, [g(d(se), { separator: ">", class: "breadcrumb" }, { default: M(() => [(s(true), r(y, null, v(R2.value, (e4, t4) => (s(), h(d(i2), { key: t4 }, { default: M(() => [g(d(Be), { icon: e4.icon }, null, 8, ["icon"]), p("a", { onClick: (t5) => j2(e4.key), class: "fw-bold text-decoration-none" }, S(e4.title), 9, pf)]), _: 2 }, 1024))), 128))]), _: 1 }), D2.value && "" === D2.value.parentId ? (s(), h(d(vt), { key: 0 }, { default: M(() => [g(d(ce), { class: "ml-6" }, { default: M(() => [(s(true), r(y, null, v(z2.value, (e4, t4) => (s(), h(d(de), { span: 22, key: t4, class: c(`${d(k2)}-card`) }, { default: M(() => [p("div", { class: c(`${d(k2)}-card-item`), onClick: (t5) => j2(e4.key) }, [p("div", { class: c(`${d(k2)}-card-item-info`) }, [g(d(G), { title: e4.formulaInfo?.cnName }, { default: M(() => [p("span", { style: u(`color: ${d(_2)};font-weight: bold`) }, [g(d(Be), { icon: "ant-design:function-outlined", color: d(_2) }, null, 8, ["color"]), x(" " + S(e4.formulaInfo?.cnName) + " (" + S(e4.formulaInfo?.enName) + ") ", 1)], 4)]), _: 2 }, 1032, ["title"]), g(d(G), { title: e4.formulaInfo?.description }, { default: M(() => [p("span", null, S(e4.formulaInfo?.description), 1)]), _: 2 }, 1032, ["title"])], 2)], 10, ff)]), _: 2 }, 1032, ["class"]))), 128))]), _: 1 })]), _: 1 })) : f("", true), D2.value && D2.value.formulaInfo ? (s(), h(d(vt), { key: 1 }, { default: M(() => [g(d(ue), { class: "pt-3 pr-2", column: 1, bordered: true }, { default: M(() => [g(d(C2), { label: "描述" }, { default: M(() => [x(S(q2.value?.description), 1)]), _: 1 }), g(d(C2), { label: "参数" }, { default: M(() => [(s(true), r(y, null, v(q2.value?.paramDesc, (e4, t4) => (s(), r("div", { key: t4 }, S(e4), 1))), 128))]), _: 1 }), g(d(C2), { label: "结果" }, { default: M(() => [x(S(q2.value?.returnDesc), 1)]), _: 1 })]), _: 1 })]), _: 1 })) : f("", true)], 2)]), _: 1 }, 8, ["default-size", "class"]));
} }), gf = to(hf, [["__scopeId", "data-v-6dc5ed8e"]]), mf = { key: 0, class: "content" }, yf = { key: 1, class: "content placeholder" }, vf = to(e({ __name: "FormulaSelect", props: { formulaKey: { type: String, default: "" }, queryMode: { type: Boolean, default: false } }, emits: ["formulaChange"], setup(e2, { emit: t2 }) {
  const o2 = e2, l2 = t2, i2 = a(() => Se().getDarkMode.value === Ge.DARK), { prefixCls: u2 } = xe("select-formula"), f2 = m(false);
  let y2;
  const { createMessage: v2 } = _e(), C2 = _r(), k2 = m(), w2 = (e3) => {
    f2.value = e3;
  };
  function _2() {
  ...
  }, { immediate: true }), (t3, o3) => (s(), h(d(V), { disabled: e2.queryMode, trigger: ["click"], open: f2.value, onOpenChange: w2, placement: "bottom", arrow: { pointAtCenter: true }, "overlay-style": { width: "500px" } }, { overlay: M(() => [g(d(te), null, { default: M(() => [g(d(te).Item, { disabled: "", style: { padding: "0", cursor: "default" } }, { default: M(() => [g(gf, { style: { height: "350px" }, "is-show": f2.value, "cur-key": k2.value?.key, onSelect: L2 }, null, 8, ["is-show", "cur-key"]), p("div", { class: c(`${d(u2)}-ok-button`) }, [g(d(ee), { class: "mr-2", onClick: _2 }, { default: M(() => o3[0] || (o3[0] = [x("清空")])), _: 1, __: [0] }), g(d(ee), { type: "primary", onClick: $2 }, { default: M(() => o3[1] || (o3[1] = [x("确定")])), _: 1, __: [1] })], 2)]), _: 1 })]), _: 1 })]), default: M(() => [g(d(ee), { class: c(`${d(u2)}-drop-button`) }, { default: M(() => [k2.value?.cnName ? (s(), r("span", mf, S(k2.value?.cnName), 1)) : (s(), r("span", yf, "选择公式")), g(d(Be), { size: 12, icon: f2.value ? "ant-design:up-outlined" : "ant-design:down-outlined", color: i2.value ? "rgba(255, 255, 255, 0.25)" : "rgba(0, 0, 0, 0.25)" }, null, 8, ["icon", "color"])]), _: 1 }, 8, ["class"])]), _: 1 }, 8, ["disabled", "open"]));
} }), [["__scopeId", "data-v-4015d717"]]), Cf = "displayName", bf = { config: { type: Object, default: () => ({ type: ni.param, value: "" }) }, comType: { type: String, default: "string" }, paramType: { type: String, default: "string" }, paramValue: { type: String, default: "" }, showRadio: { type: Boolean, default: true }, sqlQuery: { type: Object }, index: { type: Number, default: 0 }, queryMode: { type: Boolean, default: false } }, Mf = (e2) => {
  if (!e2) return { type: ni.formula, value: null };
  const t2 = (e2?.targetParam ? JSON.parse(e2.targetParam) : []).map((e3) => e3.paramType.startsWith("list") ? { type: ni.param, value: [{ type: ni.param, value: "" }] } : { type: ni.param, value: "" });
  return { type: ni.formula, value: { key: e2.key, cnName: e2.cnName, enName: e2.enName, targetObject: e2.targetObject, param: t2 } };
}, kf = e({ __name: "FormulaCompact", props: bf, emits: ["typeChange", "formulaSelect", "inputChange"], setup(e2, { emit: t2 }) {
  const o2 = e2, a2 = t2, { prefixCls: l2 } = xe("formula-compact"), i2 = ae.Group, u2 = ae.Button, y2 = m("string"), v2 = m("string"), C2 = m(), b2 = m(ni.param), k2 = m(), w2 = m([]), x2 = m(), S2 = m(false), _2 = m([]), $2 = m();
  function L2(e3) {
    a2("formulaSelect", e3 || "");
  }
  function I2() {
  ...
  function D2(e3) {
    e3.tableAlias && e3.colName && (x2.value = e3, a2("inputChange", { ...e3, [Cf]: "colName" }));
  }
  function T2(e3, t3) {
  ...
  return n(() => o2.config, (e3) => {
    e3 && (b2.value = e3.type, e3.type === ni.formula ? k2.value = e3.value?.key : C2.value = e3.value);
    ...
    if (e3 === ni.param && "list" === y2.value) {
      const e4 = /@\{(?<value>.*?)\}/u, t3 = o2.paramValue?.replace(e4, "$<value>");
      S2.value = "fieldName" === t3, S2.value && o2.sqlQuery ? Op(void 0, o2.sqlQuery).then((e5) => {
        ...
        if (_2.value = [], e5 && e5.value) {
          ...
          e5.value.forEach((e6) => {
            const o3 = e6[t4];
            _2.value.push({ label: o3, value: o3, realValue: { ...e6, [Cf]: t4 } });
          }), o2.config?.value && ($2.value = o2.config.value[o2.config.value[Cf]]);
        }
      });
    }
  }, { immediate: true }), (e3, t3) => (s(), r("div", { class: c(d(l2)) }, [o2.showRadio ? (s(), h(d(i2), { key: 0, disabled: o2.queryMode, class: "radio-group", value: b2.value, "onUpdate:value": t3[0] || (t3[0] = (e4) => b2.value = e4), "button-style": "solid", onChange: N2 }, { default: M(() => ["number" === v2.value ? (s(), h(d(u2), { key: 0, value: d(ni).param }, { default: M(() => [g(d(G), { placement: "left", title: "数字" }, { default: M(() => [g(d(Be), { icon: "ant-design:field-number-outlined" })]), _: 1 })]), _: 1 }, 8, ["value"])) : (s(), h(d(u2), { key: 1, value: d(ni).param }, { default: M(() => [g(d(G), { placement: "left", title: "字符" }, { default: M(() => [g(d(Be), { icon: "ant-design:field-string-outlined" })]), _: 1 })]), _: 1 }, 8, ["value"])), g(d(u2), { value: d(ni).formula }, { default: M(() => [g(d(G), { placement: "left", title: "公式" }, { default: M(() => [g(d(Be), { icon: "ant-design:function-outlined" })]), _: 1 })]), _: 1 }, 8, ["value"])]), _: 1 }, 8, ["disabled", "value"])) : f("", true), p("div", { class: c(o2.showRadio ? "formula-select" : "formula-select-show") }, [b2.value === d(ni).param && "string" === y2.value ? (s(), h(d(H), { key: 0, disabled: o2.queryMode, value: C2.value, "onUpdate:value": t3[1] || (t3[1] = (e4) => C2.value = e4), class: "input-com", placeholder: "请输入字符常量", onChange: I2 }, null, 8, ["disabled", "value"])) : b2.value === d(ni).param && "number" === y2.value ? (s(), h(d(Q), { key: 1, disabled: o2.queryMode, value: C2.value, "onUpdate:value": t3[2] || (t3[2] = (e4) => C2.value = e4), class: "input-com", placeholder: "请输入数字常量", onChange: I2 }, null, 8, ["disabled", "value"])) : b2.value === d(ni).param && "list" === y2.value && S2.value ? (s(), h(Xp, { key: 2, "query-mode": o2.queryMode, class: "select-com", "table-fields": w2.value, value: x2.value, onChange: D2 }, null, 8, ["query-mode", "table-fields", "value"])) : b2.value !== d(ni).param || "list" !== y2.value || S2.value ? (s(), h(vf, { key: 4, "query-mode": o2.queryMode, "formula-key": k2.value, onFormulaChange: L2 }, null, 8, ["query-mode", "formula-key"])) : (s(), h(d(K), { key: 3, disabled: o2.queryMode, class: "select-com", value: $2.value, "onUpdate:value": t3[3] || (t3[3] = (e4) => $2.value = e4), options: _2.value, placeholder: "请选择参数", onSelect: T2 }, null, 8, ["disabled", "value", "options"]))], 2)], 2));
} }), wf = to(kf, [["__scopeId", "data-v-8048866c"]]), xf = { style: { "margin-top": "6px", display: "flex", "justify-content": "flex-end" } }, Sf = e({ __name: "FormulaAround", emits: ["submit"], setup(e2, { emit: t2 }) {
  ...
  return (e3, t3) => (s(), h(d(V), { trigger: ["click"], open: l2.value, placement: "bottom", arrow: { pointAtCenter: true }, onOpenChange: u2 }, { overlay: M(() => [g(d(te), null, { default: M(() => [g(d(te).Item, { disabled: "", style: { padding: "0", cursor: "default" } }, { default: M(() => [g(d(pe), { title: "上层公式设置", bordered: false, style: { width: "240px" } }, { default: M(() => [g(vf, { onFormulaChange: c2 }), g(d(K), { style: { width: "100%", "margin-top": "6px" }, placeholder: "选择作为哪个参数", class: "select-com", value: n2.value, "onUpdate:value": t3[0] || (t3[0] = (e4) => n2.value = e4), options: a2.value }, null, 8, ["value", "options"]), p("div", xf, [g(d(ee), { size: "small", onClick: f2 }, { default: M(() => t3[1] || (t3[1] = [x(" 取消")])), _: 1, __: [1] }), g(d(ee), { size: "small", type: "primary", style: { "margin-left": "6px" }, onClick: y2 }, { default: M(() => t3[2] || (t3[2] = [x(" 确定")])), _: 1, __: [2] })])]), _: 1 })]), _: 1 })]), _: 1 })]), default: M(() => [g(d(ee), { style: { "margin-top": "-6px" }, size: "small", type: "link" }, { icon: M(() => [g(d(G), { placement: "left", title: "添加外层公式，当前层作为公式参数" }, { default: M(() => [g(d(Be), { icon: "ant-design:node-collapse-outlined" })]), _: 1 })]), _: 1 })]), _: 1 }, 8, ["open"]));
} }), _f = { class: "formula" }, $f = { class: "title" }, Lf = { key: 0, class: "add" }, If = e({ __name: "FormulaConfig", props: bf, emits: ["formulaChange"], setup(e2, { emit: t2 }) {
  const o2 = e2, a2 = t2, i2 = m(), u2 = m(), { prefixCls: C2 } = xe("formula-config"), b2 = _r(), k2 = m(), _2 = l({ type: ni.param, value: "" }), $2 = m(0), L2 = (e3) => e3.startsWith("list"), I2 = (e3) => k2?.value?.value?.param && Array.isArray(k2?.value.value.param[e3].value) ? k2?.value.value.param[e3].value : [0];
  function D2(e3) {
  ...
  return n(() => o2.config, (e3) => {
    ...
    return s(), r("div", { class: c(d(C2)) }, [p("div", _f, [g(wf, { "query-mode": o2.queryMode, class: "main", index: o2.index, "com-type": o2.comType, "param-type": o2.paramType, "param-value": o2.paramValue, config: k2.value, "sql-query": o2.sqlQuery, "show-radio": o2.showRadio, onInputChange: D2, onFormulaSelect: N2, onTypeChange: B2 }, null, 8, ["query-mode", "index", "com-type", "param-type", "param-value", "config", "sql-query", "show-radio"]), k2.value?.type !== d(ni).formula || o2.queryMode ? f("", true) : (s(), h(Sf, { key: 0, onSubmit: R2 }))]), (s(true), r(y, null, v(i2.value, (e4, l2) => (s(), r("div", { key: `${$2.value}${l2}` }, [(s(true), r(y, null, v(I2(l2), (t4, i3) => (s(), r("div", { class: "param", key: i3 }, [g(d(Be), { class: "icon", icon: "ant-design:arrow-up-outlined" }), p("span", $f, [g(d(G), { placement: "left", title: u2.value ? u2.value[l2] : "" }, { default: M(() => [x(S(e4.name), 1)]), _: 2 }, 1032, ["title"])]), g(n2, { "query-mode": o2.queryMode, index: i3, "com-type": e4.comType, "param-type": e4.paramType, "param-value": e4.value, config: L2(e4.paramType) && k2.value?.value?.param && k2.value?.value?.param[l2]?.value ? k2.value?.value?.param[l2]?.value[i3] : k2.value?.value?.param[l2], "sql-query": o2.sqlQuery, onFormulaChange: (t5) => function(e5, t6, o3, n3) {
      k2.value?.type === ni.formula && k2.value.value && k2.value.value.param && (L2(o3) ? k2.value.value.param[t6].value ? k2.value.value.param[t6].value[n3] = e5 : k2.value.value.param[t6].value = [e5] : k2.value.value.param[t6] = e5), a2("formulaChange", k2.value);
    }(t5, l2, e4.paramType, i3) }, null, 8, ["query-mode", "index", "com-type", "param-type", "param-value", "config", "sql-query", "onFormulaChange"]), i3 > 0 ? (s(), h(d(ee), { key: 0, size: "small", type: "link", onClick: (e5) => function(e6, t5) {
      k2.value?.type === ni.formula && k2.value.value && k2.value.value.param && (k2.value.value.param[e6].value.splice(t5, 1), $2.value++, a2("formulaChange", k2.value));
    }(l2, i3) }, { icon: M(() => [g(d(Be), { icon: "ant-design:delete-outlined" })]), _: 2 }, 1032, ["onClick"])) : f("", true)]))), 128)), L2(e4.paramType) ? (s(), r("div", Lf, [g(d(ee), { type: "primary", onClick: (e5) => function(e6) {
      k2.value && (k2.value.value.param[e6].value ? k2.value.value.param[e6].value.push({ type: ni.param, value: "" }) : k2.value.value.param[e6].value = [{ type: ni.param, value: "" }]);
    }(l2) }, { default: M(() => t3[0] || (t3[0] = [x("增加参数")])), _: 2, __: [0] }, 1032, ["onClick"])])) : f("", true)]))), 128))], 2);
  };
} }), Df = to(If, [["__scopeId", "data-v-aad21227"]]), Tf = e({ __name: "ParamsModal", emits: ["register", "submit"], setup(e2, { emit: t2 }) {
  const o2 = t2, [a2, { resetFields: n2, setFieldsValue: l2, validate: i2, resetSchema: r2 }] = xt({ name: "model_form_item", labelWidth: 180, baseColProps: { span: 24 }, showActionButtonGroup: false, autoSubmitOnEnter: true }), u2 = _r(), c2 = m(), [p2, { setModalProps: f2, closeModal: y2 }] = Mt(async (e3) => {
  ...
  return (e3, t3) => (s(), h(d(bt), $(e3.$attrs, { onRegister: d(p2), title: "输入参数值", onOk: v2 }), { default: M(() => [g(d(wt), { onRegister: d(a2), onSubmit: v2 }, null, 8, ["onRegister"])]), _: 1 }, 16, ["onRegister"]));
  ...
  const o2 = e2, a2 = t2, l2 = Se().getThemeColor, i2 = _r(), c2 = m(false);
  n(() => o2.open, (e3) => {
    c2.value = e3, e3 && p2();
  });
  const p2 = Te((e3) => {
    let t3 = 1, a3 = 10;
    e3 && (t3 = e3.current, a3 = e3.pageSize), qi({ sourceId: i2.getSourceId, sourceType: i2.getSourceType, sqlQuery: o2.curQuery || {}, params: o2.curParams }, { pageNum: t3, pageSize: a3 }).then((e4) => {
  ...
  }, 200), [y2, { setColumns: v2, setTableData: C2, setPagination: b2 }] = $t({ bordered: true, showIndexColumn: false, isCanResizeParent: true, canResize: true, onChange: p2 }), k2 = () => {
    c2.value = false, a2("close", true);
  };
  return (e3, t3) => (s(), h(d(fe), { title: "测试结果", placement: "bottom", open: c2.value, onClose: k2, height: 450 }, { default: M(() => [g(d(Lt), { onRegister: d(y2) }, { bodyCell: M(({ column: e4, record: t4 }) => [e4?.key === d(Lr) ? (s(), r("div", { key: 0, style: u({ color: d(l2) }) }, S(t4[d(Lr)]), 5)) : f("", true)]), _: 1 }, 8, ["onRegister"])]), _: 1 }, 8, ["open"]));
} }), Bf = e({ __name: "FormulaColModal", props: { queryMode: { type: Boolean, default: false } }, emits: ["submit", "register"], setup(e2, { emit: t2 }) {
  const o2 = t2, a2 = m(), n2 = m(), i2 = m(), u2 = l({ type: ni.formula, value: null });
  let c2;
  const p2 = m(0), v2 = _r(), [C2, { openModal: b2 }] = kt(), k2 = m(), w2 = m(), S2 = m(false), _2 = (e3) => {
    let t3 = "";
    return Array.isArray(e3.value) ? (e3.value.forEach((e4) => {
      t3 += `${_2(e4)},`;
    }), t3.endsWith(",") ? t3 = t3.slice(0, Math.max(0, t3.lastIndexOf(","))) : t3) : e3.type === ni.formula && e3.value ? (t3 = `${e3.value.cnName}(`, e3.value.param.forEach((e4) => {
      t3 += `${_2(e4)},`;
    }), t3.endsWith(",") && (t3 = t3.slice(0, Math.max(0, t3.lastIndexOf(",")))), t3 += ")", t3) : (e3.type === ni.param && (t3 += e3.value instanceof Object ? e3.value[e3.value[Cf]] : e3.value), t3);
  ...
  function B2() {
    k2.value = { schema: c2.schema, sourceTable: c2.sourceTable, joins: c2.joins, sqlQuery: c2?.sqlQuery, customColumns: [{ name: Lr, expressions: [{ type: ni.formula, value: u2.value }] }] };
    const e3 = Rr(k2.value || {});
    e3 && e3.size > 0 ? b2(true, { params: e3, level: 1 }) : R2({});
  }
  function R2(e3) {
    w2.value = e3, S2.value = true;
  }
  return (t3, o3) => (s(), r(y, null, [g(d(bt), $(t3.$attrs, { onRegister: d(L2), title: "公式配置", onOk: T2 }), { centerFooter: M(() => [g(d(ee), { onClick: B2, type: "primary", danger: "" }, { default: M(() => o3[1] || (o3[1] = [x("测试")])), _: 1, __: [1] })]), default: M(() => [i2.value ? (s(), h(d(he), { key: 0, class: "mb-2", message: i2.value, type: "info", "show-icon": "" }, { icon: M(() => [g(d(Be), { icon: "ant-design:function-outlined" })]), _: 1 }, 8, ["message"])) : f("", true), (s(), h(Df, { "query-mode": e2.queryMode, key: p2.value, "sql-query": d(c2), "show-radio": false, config: u2, onFormulaChange: N2 }, null, 8, ["query-mode", "sql-query", "config"])), g(Nf, { "cur-query": k2.value, "cur-params": w2.value, open: S2.value, onClose: o3[0] || (o3[0] = () => S2.value = false) }, null, 8, ["cur-query", "cur-params", "open"])]), _: 1 }, 16, ["onRegister"]), g(Tf, { onRegister: d(C2), onSubmit: R2 }, null, 8, ["onRegister"])], 64));
  ...
  const o2 = e2, a2 = t2, n2 = m(true), i2 = Se().getThemeColor, { prefixCls: u2 } = xe("col-modal"), f2 = m([]), C2 = l({ formula: { key: "formula", name: "公式", icon: "icon-mfish-sum", color: "#cd201f" }, add: { key: "add", name: "加号", icon: "icon-mfish-add", color: i2 }, subtract: { key: "subtract", name: "减号", icon: "icon-mfish-subtract", color: i2, slot: true }, multiply: { key: "multiply", name: "乘号", icon: "icon-mfish-multiply", color: i2, slot: true }, divide: { key: "divide", name: "除号", icon: "icon-mfish-divide", color: i2, slot: true }, leftBracket: { key: "leftBracket", name: "左括号", icon: "icon-mfish-left-bracket", color: i2, slot: true }, rightBracket: { key: "rightBracket", name: "右括号", icon: "icon-mfish-right-bracket", color: i2, slot: true } }), b2 = l({ formula: { type: ni.formula, value: "" }, add: { type: ni.operator, name: "add", value: "+", slot: true }, subtract: { type: ni.operator, name: "subtract", value: "-", slot: true }, multiply: { type: ni.operator, name: "multiply", value: "*", slot: true }, divide: { type: ni.operator, name: "divide", value: "/", slot: true }, leftBracket: { type: ni.operator, name: "leftBracket", value: "(", slot: true }, rightBracket: { type: ni.operator, name: "rightBracket", value: ")", slot: true } }), k2 = [{ field: "customCol", label: "自定义列", component: "Input", slot: "customCol" }, { field: "name", label: "名称", component: "Input", componentProps: { disabled: o2.queryMode, placeholder: "请输入名称", maxlength: 30, showCount: true }, required: true }], [S2, { resetFields: _2, setFieldsValue: L2, validate: I2 }] = xt({ name: "model_form_item", labelWidth: 100, baseColProps: { span: 24 }, schemas: k2, showActionButtonGroup: false, autoSubmitOnEnter: true }), [D2, { openModal: T2 }] = kt(), [N2, { openModal: B2 }] = kt(), R2 = m();
  const [O2, { setModalProps: A2, closeModal: j2 }] = Mt(async (e3) => {
    _2().then(), A2({ confirmLoading: false, width: "600px", maskClosable: false }), n2.value = !!e3?.isUpdate, d(n2) ? (R2.value = e3.index, L2({ ...e3.customCol }).then(), f2.value = JSON.parse(JSON.stringify(e3.customCol.expressions))) : (R2.value = void 0, f2.value = []);
  }), [z2, { openModal: E2 }] = kt(), F2 = m(), P2 = m(), H2 = m(false);
  async function Q2() {
    try {
      const e3 = await I2();
  ...
  function U2(e3, t3, a3) {
    t3.type !== ni.operator ? (a3(false, t3), B2(true, { index: e3, sqlQuery: { ...o2.sqlQuery }, isUpdate: false })) : a3(true, t3);
  }
  function V2() {
    T2(true, { sqlQuery: { ...o2.sqlQuery }, isUpdate: false });
  }
  function W2(e3, t3) {
    t3.type === ni.field && T2(true, { index: e3, field: t3, sqlQuery: { ...o2.sqlQuery }, isUpdate: true }), t3.type === ni.formula && B2(true, { index: e3, formulaConfig: t3, sqlQuery: { ...o2.sqlQuery }, isUpdate: true });
  }
  function Y2(e3, t3) {
  ...
  function K2() {
    F2.value = { schema: o2.sqlQuery?.schema, sourceTable: o2.sqlQuery?.sourceTable, joins: o2.sqlQuery?.joins, sqlQuery: o2.sqlQuery?.sqlQuery, customColumns: [{ name: Lr, expressions: f2.value }] };
    const e3 = Rr(F2.value || {});
  ...
  return (t3, a3) => {
    const n3 = w("AButton");
    return s(), r(y, null, [g(d(bt), $(t3.$attrs, { onRegister: d(O2), title: "自定义列配置", onOk: Q2 }), { centerFooter: M(() => [g(n3, { onClick: K2, type: "primary", danger: "" }, { default: M(() => a3[1] || (a3[1] = [x("测试")])), _: 1, __: [1] })]), default: M(() => [g(d(wt), { onRegister: d(S2), onSubmit: Q2 }, { customCol: M(() => [g(d(Ct), { "query-mode": e2.queryMode, "add-title": "添加字段", items: f2.value, onBlockBuild: U2, onAddBlock: V2, onEditBlock: W2 }, q({ tag: M(({ element: e3 }) => [p("div", { class: c(`${d(u2)}-slot-tag`) }, [g(d(Re), { icon: C2[e3.name].icon, color: d(i2) }, null, 8, ["icon", "color"])], 2)]), _: 2 }, [e2.queryMode ? void 0 : { name: "headerBar", fn: M(({ headerCallBack: e3 }) => [(s(true), r(y, null, v(C2, (t4, a4) => (s(), h(d(G), { key: a4, title: t4.name }, { default: M(() => [g(d(ge), { style: { cursor: "pointer" }, color: t4.color, draggable: "true", onClick: (a5) => function(e4, t5) {
      t5.type !== ni.operator ? B2(true, { index: void 0, sqlQuery: o2.sqlQuery, isUpdate: false }) : e4(t5);
    }(e3, b2[t4.key]), onDragstart: (e4) => function(e5, t5) {
      e5.dataTransfer.setData("Text", JSON.stringify(b2[t5.key])), e5.stopPropagation();
    }(e4, t4) }, { icon: M(() => [g(d(Re), { icon: t4.icon }, null, 8, ["icon"])]), _: 2 }, 1032, ["color", "onClick", "onDragstart"])]), _: 2 }, 1032, ["title"]))), 128))]), key: "0" }]), 1032, ["query-mode", "items"])]), _: 1 }, 8, ["onRegister"]), g(df, { onRegister: d(D2), onSubmit: Y2 }, null, 8, ["onRegister"]), g(Bf, { "query-mode": e2.queryMode, onRegister: d(N2), onSubmit: Z2 }, null, 8, ["query-mode", "onRegister"]), g(Nf, { "cur-query": F2.value, "cur-params": P2.value, open: H2.value, onClose: a3[0] || (a3[0] = () => H2.value = false) }, null, 8, ["cur-query", "cur-params", "open"])]), _: 1 }, 16, ["onRegister"]), g(Tf, { onRegister: d(z2), onSubmit: X2 }, null, 8, ["onRegister"])], 64);
  };
} }), qf = to(Rf, [["__scopeId", "data-v-93a6f597"]]), Of = e({ __name: "TableCustomCol", props: { sqlQuery: { type: Object }, level: { type: Number, default: 1 }, configMitt: { type: Object }, queryMode: { type: Boolean, default: false } }, emits: ["customColChange"], setup(e2, { emit: t2 }) {
  const o2 = e2, l2 = t2, i2 = m([]);
  function u2() {
    l2("customColChange");
  }
  const [c2, { openModal: p2 }] = kt(), y2 = a(() => void 0 === o2.sqlQuery?.customColumns && Nr(o2.sqlQuery, o2.level) && !o2.queryMode);
  function v2() {
  ...
  function x2(e3) {
    Br(o2.configMitt, { ...o2.sqlQuery, customColumns: i2.value, aggregates: [], groups: [], orders: [], limit: void 0 }, e3);
  }
  function S2(e3, t3) {
  ...
  return n(() => o2.sqlQuery?.customColumns, (e3) => {
    i2.value = e3 ? [...e3] : [];
  }, { immediate: true }), (t3, o3) => (s(), r("div", null, [void 0 !== e2.sqlQuery?.customColumns ? (s(), h(Bp, { key: 0, "query-mode": e2.queryMode, icon: "ant-design:insert-row-right-outlined", title: "自定义列", onDeleteConfig: u2, onExecute: x2, full: true }, { config: M(() => [g(d(Ct), { "query-mode": e2.queryMode, items: i2.value, "add-title": "添加自定义列", onAddBlock: v2, onEditBlock: C2, onDragChange: b2 }, null, 8, ["query-mode", "items"])]), _: 1 }, 8, ["query-mode"])) : f("", true), y2.value ? (s(), h(Ep, { key: 1, class: "config-button", title: "自定义列", icon: "ant-design:insert-row-right-outlined", level: 0, onClick: k(w2, ["prevent"]) })) : f("", true), g(qf, { "query-mode": e2.queryMode, onRegister: d(c2), "sql-query": e2.sqlQuery, onSubmit: S2 }, null, 8, ["query-mode", "onRegister", "sql-query"])]));
} }), Af = { class: "btn-bar" }, jf = to(e({ __name: "ApiConfig", props: { configKey: { type: Number, default: 0, require: true }, sqlQuery: { type: Object, require: true, default: void 0 }, level: { type: Number, default: 1 }, configMitt: { type: Object, default: void 0 }, queryMode: { type: Boolean, default: false } }, emits: ["configChange"], setup(e2, { emit: t2 }) {
  const o2 = e2, n2 = t2, { sqlQuery: l2 } = O(o2), { prefixCls: i2 } = xe("api-config"), y2 = Se().getThemeColor, v2 = m(), C2 = m(), { focused: b2 } = Ne(v2), { focused: M2 } = Ne(C2), k2 = _r(), x2 = m(false), S2 = a(() => {
    if (1 === k2.getSourceType || o2.queryMode) return false;
    if (o2.sqlQuery && o2.sqlQuery?.joins) {
      for (const e3 of o2.sqlQuery.joins) if (!jp(e3)) return false;
    }
    return true;
  }), _2 = a(() => (void 0 === o2.sqlQuery?.filters || Nr(o2.sqlQuery, o2.level)) && !o2.queryMode), $2 = a(() => (void 0 === o2.sqlQuery?.customColumns || Nr(o2.sqlQuery, o2.level)) && !o2.queryMode), L2 = a(() => (void 0 === o2.sqlQuery?.aggregates || Nr(o2.sqlQuery, o2.level)) && !o2.queryMode), I2 = a(() => void 0 === o2.sqlQuery?.orders && !o2.queryMode), D2 = a(() => void 0 === o2.sqlQuery?.limit && !o2.queryMode);
  function T2(e3) {
  ...
  async function W2(e3) {
    ...
    if (Nr(o2.sqlQuery, o2.level)) return t3 = await H2(), void a2(t3, true);
  ...
  function K2() {
    o2.configMitt?.emit(Ir.QUERY_SQL, o2.sqlQuery), M2.value = false;
  }
  function X2() {
    Br(o2.configMitt, o2.sqlQuery, o2.level), b2.value = false;
  }
  return (t3, o3) => {
    const a2 = w("ApiConfig", true);
    return s(), r("div", { class: c([d(i2), e2.level < d(k2).getLevel ? "inner" : "", d(k2).getShowData && d(k2).getShowDataLevel === e2.level ? "active" : ""]) }, [d(k2).getShowData && d(k2).getShowDataLevel === e2.level ? (s(), r("div", { key: 0, class: "select", style: u({ backgroundColor: d(y2) }) }, [g(d(Be), { icon: "ant-design:check-outlined", color: "#FFFFFF", size: 12 })], 4)) : f("", true), void 0 !== d(l2)?.sqlQuery ? (s(), h(a2, { key: 1, "config-key": e2.configKey, "query-mode": e2.queryMode, "sql-query": d(l2).sqlQuery, level: e2.level - 1, "config-mitt": e2.configMitt, onConfigChange: F2 }, null, 8, ["config-key", "query-mode", "sql-query", "level", "config-mitt"])) : f("", true), (s(), h(qp, { key: e2.configKey + d(k2).getFieldsChange, class: "config", "sql-query": d(l2), "query-mode": e2.queryMode, "config-mitt": e2.configMitt, onFieldChange: T2 }, null, 8, ["sql-query", "query-mode", "config-mitt"])), (s(), h(Qp, { level: e2.level, key: e2.configKey, class: "config", "sql-query": d(l2), "config-mitt": e2.configMitt, "query-mode": e2.queryMode, onJoinChange: N2 }, null, 8, ["level", "sql-query", "config-mitt", "query-mode"])), (s(), h(of, { "query-mode": e2.queryMode, level: e2.level, key: e2.configKey, class: "config", "sql-query": d(l2), "config-mitt": e2.configMitt, onFilterChange: B2 }, null, 8, ["query-mode", "level", "sql-query", "config-mitt"])), (s(), h(Of, { "query-mode": e2.queryMode, level: e2.level, key: e2.configKey, class: "config", "sql-query": d(l2), "config-mitt": e2.configMitt, onCustomColChange: R2 }, null, 8, ["query-mode", "level", "sql-query", "config-mitt"])), (s(), h(rf, { "query-mode": e2.queryMode, key: e2.configKey, class: "config", "sql-query": d(l2), "config-mitt": e2.configMitt, onAggregateChange: q2, onGroupChange: A2, onDeleteAggregate: j2 }, null, 8, ["query-mode", "sql-query", "config-mitt"])), (s(), h(cf, { "query-mode": e2.queryMode, key: e2.configKey, class: "config", "sql-query": d(l2), "config-mitt": e2.configMitt, onOrderChange: z2 }, null, 8, ["query-mode", "sql-query", "config-mitt"])), (s(), h(Gp, { "query-mode": e2.queryMode, key: e2.configKey, class: "config", "sql-query": d(l2), "config-mitt": e2.configMitt, onLimitChange: E2 }, null, 8, ["query-mode", "sql-query", "config-mitt"])), p("div", Af, [S2.value ? (s(), h(Ep, { key: 0, class: "config-button", title: "关联", iconfont: "icon-mfish-join", level: e2.level, onClick: Q2 }, null, 8, ["level"])) : f("", true), _2.value ? (s(), h(Ep, { key: 1, class: "config-button", title: "过滤", icon: "ant-design:filter-outlined", level: e2.level, onClick: G2 }, null, 8, ["level"])) : f("", true), $2.value ? (s(), h(Ep, { key: 2, class: "config-button", title: "自定义列", icon: "ant-design:insert-row-right-outlined", level: e2.level, onClick: U2 }, null, 8, ["level"])) : f("", true), L2.value ? (s(), h(Ep, { key: 3, class: "config-button", title: "聚合", iconfont: "icon-mfish-sum", level: e2.level, onClick: V2 }, null, 8, ["level"])) : f("", true), I2.value ? (s(), h(Ep, { key: 4, class: "config-button", title: "排序", icon: "ant-design:sort-ascending-outlined", level: e2.level, onClick: Y2 }, null, 8, ["level"])) : f("", true), D2.value ? (s(), h(Ep, { key: 5, class: "config-button", title: "行数", icon: "ant-design:column-height-outlined", level: e2.level, onClick: Z2 }, null, 8, ["level"])) : f("", true), g(Ep, { class: "config-button", ref_key: "querySqlRef", ref: C2, style: u({ fontWeight: "bold", border: `1px dashed ${d(y2)}` }), title: "查看SQL", icon: "ant-design:console-sql-outlined", level: e2.level, onClick: K2 }, null, 8, ["style", "level"]), g(Ep, { class: "config-button", style: u({ fontWeight: "bold", border: `1px dashed ${d(y2)}` }), ref_key: "queryDataRef", ref: v2, title: "执行", onMouseenter: o3[0] || (o3[0] = (e3) => x2.value = true), onMouseleave: o3[1] || (o3[1] = (e3) => x2.value = false), icon: !d(k2).getShowData || d(k2).getShowDataLevel !== e2.level || x2.value ? "ant-design:caret-right-outlined" : "ant-design:check-outlined", level: e2.level, onClick: X2 }, null, 8, ["style", "icon", "level"])])], 2);
  };
} }), [["__scopeId", "data-v-32695b70"]]), zf = { key: 0 }, Ef = { key: 1 }, Ff = e({ __name: "SqlQueryModal", setup(e2) {
  const t2 = m(500), o2 = m(""), [a2, { setModalProps: n2 }] = Mt(async (e3) => {
    n2({ width: "800px" }), o2.value = e3;
  }), { copy: l2, copied: i2 } = De({ legacy: true });
  function c2(e3) {
    t2.value = e3;
  }
  return (e3, n3) => {
    const p2 = w("AButton");
    return s(), h(d(bt), $(e3.$attrs, { onRegister: d(a2), title: "SQL查看", "show-ok-btn": false, "cancel-text": "关闭", onHeightChange: c2 }), { insertFooter: M(() => [g(p2, { onClick: n3[0] || (n3[0] = (e4) => d(l2)(o2.value)), color: d(i2) ? "success" : "primary" }, { default: M(() => [d(i2) ? (s(), r("span", Ef, "已拷贝")) : (s(), r("span", zf, "拷贝"))]), _: 1 }, 8, ["color"])]), default: M(() => [g(d(It), { style: u({ height: `${t2.value}px` }), mode: d(Dt).JS, value: o2.value, "onUpdate:value": n3[1] || (n3[1] = (e4) => o2.value = e4), readonly: true }, null, 8, ["style", "mode", "value"])]), _: 1 }, 16, ["onRegister"]);
  };
} });
const Pf = ["src"], Hf = to(e({ name: "PreviewFrame", __name: "PreviewFrame", props: { frameSrc: Nt.string.def(""), frameWidth: Nt.number.def(400), frameHeight: Nt.number.def(300) }, emits: ["message"], setup(e2, { expose: t2, emit: o2 }) {
  const n2 = e2, l2 = o2, r2 = m(true), f2 = m(), { prefixCls: g2 } = xe("preview-frame"), y2 = a(() => ({ width: `${n2.frameWidth}px`, height: `${n2.frameHeight}px` }));
  function v2() {
    r2.value = false;
  }
  const C2 = (e3) => {
  ...
  } }), (t3, o3) => (s(), h(d(vt), null, { default: M(() => [p("iframe", { style: u(y2.value), src: e2.frameSrc, class: c(`${d(g2)}__main`), ref_key: "frameRef", ref: f2, onLoad: v2 }, null, 46, Pf)]), _: 1 }));
} }), [["__scopeId", "data-v-1bb3fd25"]]);
const Qf = { class: "share-title" }, Gf = e({ __name: "ScreenPreview", props: { fixScale: { type: Number, default: 1 }, borderRadius: { type: String } }, setup(e2) {
  const t2 = e2, { prefixCls: n2 } = xe("screen-preview"), { token: C2 } = P.useToken(), b2 = a(() => C2.value.colorBorder), k2 = o("fitDomRef"), w2 = m([]), { getAntdLocale: x2 } = Tt(), $2 = Or(), { antTheme: L2 } = Le(a(() => $2.getTheme)), I2 = m(false), D2 = m(""), T2 = m(window.innerWidth), N2 = m(window.innerHeight), B2 = m(), R2 = a(() => [...$2.getComponentList].reverse()), q2 = l({ width: 0, height: 0 }), O2 = a(() => ({ ...ap($2.getCanvasConfig), width: $2.getCanvasConfig.width * q2.width * t2.fixScale + "px", height: $2.getCanvasConfig.height * q2.height * t2.fixScale + "px" })), E2 = (e3) => up({ ...e3.chartContain.dropInfo }), F2 = (e3) => {
  ...
  const { register: U2, unRegister: V2 } = function() {
    const { getDataTable: e3 } = ls(), t3 = [], o2 = Or();
    return { register: async function() {
      const a2 = (o3) => {
        o3.data.autoRefresh?.open && o3.data.autoRefresh.interval && t3.push(setInterval(() => {
          e3(o3).then((e4) => {
            o3.type && tp[o3.type]?.initData(o3, e4);
          });
        }, 60 * o3.data.autoRefresh.interval * 1e3));
      };
      t3.length = 0;
      for (const e4 of o2.getComponentList) if (a2(e4.chart), e4.chart.type === ns.MfCombine) for (const t4 of e4.chart.options.components) a2(t4.chart);
    }, unRegister: function() {
  ...
  return i(() => {
    setTimeout(() => {
      $2.loadingScreen().then(() => {
        ...
        for (let e4 = 0; e4 < R2.value.length; e4++) {
          ...
          op.on(Qr.EVENT_ANIMATION, (o4) => {
            Jd(o4, t3, w2.value[e4]);
          });
          const o3 = t3.chart.animations?.filter((e5) => !e5.events || 0 === e5.events.length);
          !o3 || 0 === o3.length || w2.value.length <= e4 || (R2.value[e4].chartContain.show && R2.value[e4].chart.type !== ns.MfCombine && (R2.value[e4].chartContain.show = false, n3.set(e4, R2.value[e4])), setTimeout(() => {
            n3.has(e4) && (n3.get(e4).chartContain.show = true), Xd(w2.value[e4], o3).then();
          }, 600));
        }
      });
    }, 10), op.on(Qr.SHOW_MODAL, (e3) => {
    ...
    G2 && G2();
  }), (t3, o2) => (s(), h(d(me), { locale: d(x2), theme: d(L2) }, { default: M(() => [g(d(vt), { class: c(d(n2)), style: u({ borderRadius: e2.borderRadius }) }, { default: M(() => [p("div", { class: "screen-wrap", style: u(O2.value) }, [p("div", { class: "screen-canvas", ref_key: "fitDomRef", ref: k2 }, [d($2).getCanvasConfig.watermark ? (s(), h(d(ye), { key: 0, content: d($2).getCanvasConfig.watermark, style: u({ position: "absolute", width: `${d($2).getCanvasConfig.width}px`, height: `${d($2).getCanvasConfig.height}px` }) }, null, 8, ["content", "style"])) : f("", true), (s(true), r(y, null, v(R2.value, (e3, t4) => A((s(), r("div", { class: c(["chart-contain", F2(e3)]), style: u(E2(e3)), key: e3?.chart.id }, [p("div", { ref_for: true, ref: (e4) => function(e5, t5) {
    w2.value[t5] = e5;
  }(e4, t4), style: u(H2(e3)) }, [(s(), h(j(e3?.chart && e3.chart.type && d(tp)[e3.chart.type].component), { class: "component", id: `com${e3?.chart.id}`, chart: e3?.chart, "chart-contain": e3?.chartContain }, null, 8, ["id", "chart", "chart-contain"]))], 4)], 6)), [[z, void 0 === e3.chart?.showHide?.show && e3.chartContain?.show || void 0 !== e3.chart?.showHide?.show && e3.chart?.showHide?.show]])), 128))], 512)], 4)]), _: 1 }, 8, ["class", "style"]), g(d(ve), { open: I2.value, "onUpdate:open": o2[0] || (o2[0] = (e3) => I2.value = e3), "get-container": Q2, width: T2.value + 48, footer: null, mask: true, centered: true }, { title: M(() => [p("div", Qf, S(B2.value), 1)]), default: M(() => [g(Hf, { "frame-src": D2.value, "frame-width": T2.value, "frame-height": N2.value }, null, 8, ["frame-src", "frame-width", "frame-height"])]), _: 1 }, 8, ["open", "width"])]), _: 1 }, 8, ["locale", "theme"]));
} }), Uf = to(Gf, [["__scopeId", "data-v-4def8031"]]), Vf = e({ __name: "ScreenSizeConfig", setup(e2) {
  const t2 = m("1k"), o2 = m([{ value: "720", label: "1280*720" }, { value: "1k", label: "1920*1080" }, { value: "2k", label: "2048*1152" }, { value: "4k", label: "4096*2160" }, { value: "自定义", label: "自定义" }]), a2 = Or(), l2 = (e3) => {
  ...
  return (t3, o3) => (s(), r("div", { class: c(d(i2)) }, [p("div", { class: c(["group-title", [e2.allowCollapse ? "collapsed" : ""]]), onClick: y2 }, [p("div", Yf, [e2.allowCheck ? (s(), h(d(W), { key: 0, checked: n2.value, "onUpdate:checked": o3[0] || (o3[0] = (e3) => n2.value = e3), onClick: o3[1] || (o3[1] = k(() => {
  }, ["stop"])), onChange: v2 }, null, 8, ["checked"])) : f("", true), e2.tooltip ? (s(), h(d(G), { key: 1, title: e2.tooltip }, { default: M(() => [x(S(e2.title), 1)]), _: 1 }, 8, ["title"])) : (s(), r("span", Zf, S(e2.title), 1)), !e2.allowCheck && e2.allowCollapse || e2.allowCheck && e2.titleCheck ? (s(), h(d(Be), { key: 3, icon: `ant-design:${l2.value ? "up" : "down"}-outlined`, size: 12 }, null, 8, ["icon"])) : f("", true)]), p("div", { onClick: o3[2] || (o3[2] = k(() => {
  }, ["stop"])) }, [R(t3.$slots, "action", {}, void 0, true)]), e2.allowCopy ? (s(), r("div", Kf, [g(d(G), { placement: "top", title: `复制${e2.title}` }, { default: M(() => [g(d(Be), { "aria-hidden": "false", size: 12, icon: "iconfont:icon-mfish-copy", onClick: k(C2, ["stop"]) })]), _: 1 }, 8, ["title"]), g(d(G), { placement: "top", title: `粘贴${e2.title}` }, { default: M(() => [g(d(Be), { "aria-hidden": "false", size: 12, icon: "iconfont:icon-mfish-paste", onClick: k(b2, ["stop"]) })]), _: 1 }, 8, ["title"])])) : f("", true)], 2), g(d(Rt), { show: !e2.allowCheck && l2.value || e2.allowCheck && l2.value && e2.titleCheck }, { default: M(() => [p("div", Xf, [R(t3.$slots, "default", {}, void 0, true)])]), _: 3 }, 8, ["show"])], 2));
} }), eh = to(Jf, [["__scopeId", "data-v-9aa90d73"]]), th = { class: "table-bordered" }, oh = e({ __name: "GlobalParamsConfig", setup(e2) {
  const { prefixCls: t2 } = xe("global-params-config"), o2 = Or(), a2 = [{ title: "参数名", dataIndex: "name", align: "center" }, { title: "默认值", dataIndex: "defaultValue", align: "center" }];
  return (e3, n2) => (s(), r("div", { class: c(d(t2)) }, [g(eh, { title: "全局变量", tooltip: "用于大屏全局接收外部参数" }, { default: M(() => [p("div", th, [g(d(be), { size: "small", columns: a2, "data-source": d(o2).getCanvasConfig?.globalParams, bordered: "", pagination: false }, null, 8, ["data-source"])])]), _: 1 })], 2));
} }), ah = { class: "title" }, nh = { class: "title" }, lh = { class: "title" }, ih = e({ __name: "CanvasConfig", setup(e2) {
  const { prefixCls: t2 } = xe("canvas-config"), o2 = Or(), a2 = m(), n2 = [{ title: "深色", value: Ge.DARK }, { title: "浅色", value: Ge.LIGHT }, { title: "跟随平台", value: Ge.SYSTEM }], u2 = l({ [Hr.FIT_BOX]: { name: "全屏铺满", icon: "carbon:fit-to-screen", description: "全屏铺满（设备与配置大屏比例不同时会变形）" }, [Hr.FIT_ORIGINAL]: { name: "原始比例", icon: "carbon:center-to-fit", description: "原始比例（保持配置大屏比例，不填充空白处）" }, [Hr.FIT_WIDTH_SCALE]: { name: "横向适配", icon: "carbon:fit-to-width", description: "横向适配-保持纵横比（横向铺满，允许纵向滚动，保持原有画布比例）" }, [Hr.FIT_HEIGHT_SCALE]: { name: "纵向适配", icon: "carbon:fit-to-height", description: "纵向适配-保持纵横比（纵向铺满，允许横向滚动，保持原有画布比例）" }, [Hr.FIT_WIDTH]: { name: "横向适配", icon: "carbon:fit-to-width", description: "横向适配（横向铺满，允许纵向滚动，会变形）" }, [Hr.FIT_HEIGHT]: { name: "纵向适配", icon: "carbon:fit-to-height", description: "纵向适配（纵向铺满，允许横向滚动，会变形）" } });
  function f2(e3) {
    C2(e3), a2.value = e3;
  }
  function C2(e3) {
  ...
  return i(() => {
    ...
    const i2 = w("AInput");
    return s(), h(d(vt), null, { default: M(() => [p("div", { class: c(d(t2)) }, [l2[8] || (l2[8] = p("div", { class: "title" }, "大屏名称", -1)), g(i2, { value: d(o2).getCanvasConfig.screenName, "onUpdate:value": l2[0] || (l2[0] = (e4) => d(o2).getCanvasConfig.screenName = e4), "show-count": "", maxlength: 30 }, null, 8, ["value"]), l2[9] || (l2[9] = p("div", { class: "title" }, " 描述", -1)), g(d(Me), { value: d(o2).getCanvasConfig.screenDesc, "onUpdate:value": l2[1] || (l2[1] = (e4) => d(o2).getCanvasConfig.screenDesc = e4), "show-count": "", maxlength: 200 }, null, 8, ["value"]), l2[10] || (l2[10] = p("div", { class: "title" }, "大屏尺寸", -1)), g(Wf), p("div", ah, [l2[5] || (l2[5] = x(" 主题 ")), g(d(Z), { size: "small", value: d(o2).getCanvasConfig.theme, "onUpdate:value": l2[2] || (l2[2] = (e4) => d(o2).getCanvasConfig.theme = e4) }, { default: M(() => [(s(), r(y, null, v(n2, (e4, t3) => g(d(Y), { key: t3, value: e4.value }, { default: M(() => [x(S(e4.title), 1)]), _: 2 }, 1032, ["value"])), 64))]), _: 1 }, 8, ["value"])]), l2[11] || (l2[11] = p("div", { class: "title" }, "背景颜色", -1)), g(d(Eo), { value: a2.value, onConfirmChange: f2, onInputChange: C2 }, null, 8, ["value"]), l2[12] || (l2[12] = p("div", { class: "title" }, "背景图片", -1)), g(d(Bt), { "file-keys": d(o2).getCanvasConfig.backgroundImage, accepts: ".jpg,.jpeg,.png,.svg", "button-text": "上传背景", "max-count": 1, "is-pic": true, onSuccess: b2, onRemove: k2 }, null, 8, ["file-keys"]), p("div", nh, [l2[6] || (l2[6] = x(" 水印 ")), p("div", null, [g(i2, { value: d(o2).getCanvasConfig.watermark, "onUpdate:value": l2[3] || (l2[3] = (e4) => d(o2).getCanvasConfig.watermark = e4), placeholder: "清空无水印", "show-count": "", maxlength: 50 }, null, 8, ["value"])])]), p("div", lh, [l2[7] || (l2[7] = x(" 适配方式 ")), p("div", null, [g(d(Z), { value: d(o2).getCanvasConfig.fitType, "onUpdate:value": l2[4] || (l2[4] = (e4) => d(o2).getCanvasConfig.fitType = e4) }, { default: M(() => [(s(true), r(y, null, v(Object.keys(u2), (e4) => (s(), h(d(G), { key: e4, placement: "bottomRight", title: u2[e4].description }, { default: M(() => [g(d(Y), { class: c(`${d(t2)}-radio-button`), value: e4 }, { default: M(() => [g(d(Be), { icon: u2[e4].icon }, null, 8, ["icon"])]), _: 2 }, 1032, ["class", "value"])]), _: 2 }, 1032, ["title"]))), 128))]), _: 1 }, 8, ["value"])])]), g(oh)], 2)]), _: 1 });
  };
} }), rh = to(ih, [["__scopeId", "data-v-c7f8401a"]]), sh = { class: "header" }, uh = e({ name: "FrameConfig", __name: "index", props: { collapsed: Nt.bool.def(false) }, setup(e2, { expose: t2 }) {
  const o2 = e2, { prefixCls: l2 } = xe("frame-config"), i2 = a(() => [l2, { collapsed: o2.collapsed }]), u2 = m(), y2 = m(), v2 = m(), C2 = Or(), b2 = m();
  function k2(e3) {
    if (e3) {
      const t3 = tp[e3.chart.type].config;
      u2.value = Object.keys(t3).filter((e4) => t3[e4].show).map((e4) => t3[e4]);
    } else u2.value = [{ value: "画布配置", payload: { icon: "carbon:gui-management", component: T(rh) } }];
  ...
  return n([() => C2.getCurComponent, () => C2.getCurComponent?.chart?.options?.selectIndex], ([e3, t3]) => {
    ...
    b2.value = e3;
  } }), (e3, t3) => (s(), r("div", { class: c(i2.value) }, [p("div", sh, [g(d(U), { value: y2.value, options: u2.value, onChange: w2, block: "" }, { label: M(({ value: e4, payload: t4 }) => [t4.icon ? (s(), h(d(Be), { key: 0, icon: t4.icon }, null, 8, ["icon"])) : f("", true), x(" " + S(e4), 1)]), _: 1 }, 8, ["value", "options"])]), (s(), h(j(v2.value), { "select-data": b2.value }, { "data-select": M(() => [R(e3.$slots, "data-select", { curSelect: b2.value })]), _: 3 }, 8, ["select-data"]))], 2));
} }), ch = { class: "action" }, dh = { class: "ac-int" }, ph = e({ __name: "CanvasRulerLine", props: { lineValue: { type: Number, default: 0 }, lineList: { type: Array, default: () => [] } }, setup(e2) {
  const t2 = e2, { lineList: o2 } = t2, a2 = Or(), { prefixCls: n2 } = xe("canvas-ruler-line"), l2 = Se().getThemeColor;
  return (e3, t3) => (s(), r("div", { class: c(d(n2)) }, [(s(true), r(y, null, v(d(o2), (e4, t4) => (s(), r("div", { class: c(["line-item", { x: "x" === e4.position, y: "y" === e4.position }]), style: u({ left: "x" === e4.position ? `${Math.round(e4.offset * d(a2).getScale)}px` : 0, top: "y" === e4.position ? `${Math.round(e4.offset * d(a2).getScale)}px` : 0 }), key: `${e4}${t4}` }, [p("div", ch, [g(d(Be), { class: "ac-close", icon: "ant-design:close-outline", color: d(l2), onClick: (a3) => function(e5, t5) {
    o2.splice(t5, 1);
  }(e4.position, t4) }, null, 8, ["color", "onClick"]), p("span", dh, S(e4.value), 1)])], 6))), 128))], 2));
} }), fh = to(ph, [["__scopeId", "data-v-91bb6b29"]]), hh = { id: "horn", class: "horn" }, gh = ["id", "width", "height"], mh = { class: "indicator-value" }, yh = ["id", "height", "width"], vh = { class: "indicator-value" }, Ch = e({ __name: "CanvasRuler", props: { rulerXLeft: { type: Number, default: 0 }, rulerYTop: { type: Number, default: 0 }, rulerX: { type: Number, default: 5e3 }, rulerY: { type: Number, default: 3e3 }, drift: { type: Number, default: 0 }, rulerHeight: { type: Number, default: 28 }, rulerWidth: { type: Number, default: 38 } }, setup(e2) {
  const t2 = e2, l2 = Or(), { prefixCls: i2 } = xe("canvas-ruler"), f2 = m(0), y2 = m(true), v2 = T({}), b2 = T({}), k2 = a(() => `${t2.rulerXLeft}px`), w2 = a(() => `${t2.rulerYTop}px`), x2 = o("rulerXRef"), _2 = o("rulerYRef"), $2 = a(() => Se().getDarkMode.value === Ge.DARK ? "#999" : "#666");
  n(() => Se().getDarkMode.value, (e3, t3) => {
    e3 !== t3 && C(() => {
      j2();
    });
  }), n([() => l2.getScale, () => t2.rulerX, () => t2.rulerY], ([e3, t3, o2], [a2, n2, l3]) => {
    a2 ? (e3 !== a2 && C(() => j2()), t3 !== n2 && C(() => P2()), o2 !== l3 && C(() => H2())) : C(() => j2());
  }, { immediate: true });
  ...
  return (t3, o2) => (s(), r("div", { class: c(d(i2)) }, [p("div", hh, [g(d(G), { title: (y2.value ? "隐藏" : "显示") + "辅助线" }, { default: M(() => [y2.value ? (s(), h(d(Be), { key: 0, class: "icon", icon: "ant-design:eye-outlined", onClick: Q2 })) : (s(), h(d(Be), { key: 1, class: "icon", icon: "ant-design:eye-invisible-outlined", onClick: Q2 }))]), _: 1 }, 8, ["title"])]), p("div", { class: "ruler rulerX", style: u({ width: `${L2.value}px`, left: k2.value }) }, [p("canvas", { ref_key: "rulerXRef", ref: x2, id: `${d(i2)}-x`, width: L2.value, height: e2.rulerHeight, onMousemove: D2, onMouseleave: N2, onClick: B2 }, null, 40, gh), p("div", { class: "indicator x", style: u(v2.value) }, [p("div", mh, S(f2.value), 1)], 4), A(g(fh, { "line-value": f2.value, "line-list": d(l2).getGuideLine.lineListY }, null, 8, ["line-value", "line-list"]), [[z, y2.value]])], 4), p("div", { class: "ruler rulerY", style: u({ height: `${I2.value}px`, top: w2.value }) }, [p("canvas", { ref_key: "rulerYRef", ref: _2, id: `${d(i2)}-y`, height: I2.value, width: e2.rulerWidth, onMousemove: R2, onMouseleave: q2, onClick: O2 }, null, 40, yh), p("div", { class: "indicator y", style: u(b2.value) }, [p("div", vh, S(f2.value), 1)], 4), A(g(fh, { "line-value": f2.value, "line-list": d(l2).getGuideLine.lineListX }, null, 8, ["line-value", "line-list"]), [[z, y2.value]])], 4)], 2));
} }), bh = to(Ch, [["__scopeId", "data-v-eeb86e63"]]), Mh = e({ __name: "CanvasBackground", props: { drift: { type: Number, default: 0 }, width: { type: Number, default: 9e3 }, height: { type: Number, default: 6e3 }, ruler: { type: Object, default: () => ({ height: 28, width: 38 }) } }, setup(e2) {
  const t2 = e2, o2 = Or(), n2 = kp(), l2 = a(() => t2.width * o2.getScale + "px"), i2 = a(() => t2.height * o2.getScale + "px"), { prefixCls: p2 } = xe("canvas-background");
  function f2(e3) {
    o2.cleanCurComponent(), 2 !== e3.button && n2.setShowMenu(false);
  }
  return (t3, a2) => (s(), r("div", { class: c(d(p2)), style: u({ width: l2.value, height: i2.value, padding: `${e2.drift * d(o2).getScale}px 0 0 ${e2.drift * d(o2).getScale}px` }), onMousedown: f2 }, [R(t3.$slots, "default", {}, void 0, true)], 38));
} }), kh = to(Mh, [["__scopeId", "data-v-2efcd248"]]), wh = { class: "scale-add-reduce" }, xh = { class: "scale-lock" }, Sh = e({ __name: "CanvasScale", props: { scale: { type: Number, default: 100 }, minScale: { type: Number, default: 20 }, maxScale: { type: Number, default: 500 } }, emits: ["addScale", "reduceScale", "changeScale", "autoScale"], setup(e2, { emit: t2 }) {
  const o2 = t2, a2 = m(true), { prefixCls: n2 } = xe("canvas-scale"), l2 = () => {
    o2("reduceScale");
  }, i2 = () => {
    o2("addScale");
  }, u2 = (e3) => {
  ...
  return (t3, o3) => (s(), r("div", { class: c(d(n2)) }, [p("div", wh, [g(d(Be), { class: "scale-icon", title: "放大", size: 18, icon: "ant-design:plus-outlined", onClick: i2 }), g(d(Q), { size: "small", bordered: false, value: e2.scale, max: e2.maxScale, min: e2.minScale, step: 5, controls: false, formatter: (e3) => `${Math.round(100 * e3)}`, onChange: u2 }, null, 8, ["value", "max", "min", "formatter"]), o3[2] || (o3[2] = p("div", { class: "scale-sign" }, "%", -1)), g(d(Be), { class: "scale-icon", title: "缩小", size: 18, icon: "ant-design:minus-outlined", onClick: l2 })]), p("div", xh, [g(d(G), { title: a2.value ? "锁定自动比例" : "解锁自动比例", placement: "right" }, { default: M(() => [a2.value ? (s(), h(d(Be), { key: 0, icon: "ant-design:unlock-outlined", onClick: o3[0] || (o3[0] = (e3) => f2(false)) })) : (s(), h(d(Be), { key: 1, icon: "ant-design:lock-outlined", onClick: o3[1] || (o3[1] = (e3) => f2(true)) }))]), _: 1 }, 8, ["title"])])], 2));
} }), _h = e({ __name: "ShortcutViewModal", setup(e2) {
  const [t2, { setModalProps: o2 }] = Mt(async () => {
    o2({ confirmLoading: false, width: "800px", cancelText: "关闭", showOkBtn: false });
  }), [a2] = $t({ columns: [{ title: "功能", dataIndex: "label", width: 120 }, { title: "Win快捷键", dataIndex: "win", width: 120 }, { title: "Mac快捷键", dataIndex: "mac", width: 120 }], dataSource: [{ label: "拖拽 / 放大缩小画布", win: "CTRL + 🖱️拖动 / 滚动", mac: "⌘ + 🖱️拖动 / 滚动", macSource: true }, { label: "向 上/右/下/左 移动", win: "CTRL + ↑ / → / ↓ / ←", mac: "⌘ + ↑" }, { label: "锁定/解锁", win: "CTRL + L / CTRL + SHIFT+ L", mac: "⌘ + L / ⌘ + ⇧ + L" }, { label: "隐藏/显示", win: "CTRL + H / CTRL + SHIFT + H", mac: "⌘ + H / ⌘ + ⇧ + H" }, { label: "组合/拆分", win: "CTRL + G / CTRL + SHIFT + G", mac: "⌘ + G / ⌘ + SHIFT + G" }, { label: "后退/前进", win: "CTRL + Z / CTRL + SHIFT + Z", mac: "⌘ + Z / ⌘ + ⇧ + Z" }, { label: "复制/粘贴", win: "CTRL + C / V", mac: "⌘ + C / V" }, { label: "多选", win: "拖动🖱️框选 / CTRL + 🖱️点击", mac: "拖动🖱️框选 / ⌘ + 🖱️点击" }, { label: "保存", win: "CTRL + S", mac: "⌘ + S" }, { label: "预览", win: "CTRL + P", mac: "⌘ + P" }, { label: "全选", win: "CTRL + A", mac: "⌘ + A" }, { label: "删除", win: "Delete", mac: "Delete" }], bordered: true, showIndexColumn: false, pagination: false, canResize: false, size: "small" });
  return (e3, o3) => (s(), h(d(bt), { onRegister: d(t2), title: "快捷操作" }, { default: M(() => [g(d(Lt), { onRegister: d(a2) }, null, 8, ["onRegister"])]), _: 1 }, 8, ["onRegister"]));
} }), $h = to(e({ __name: "CanvasOperator", setup(e2) {
  const { prefixCls: t2 } = xe("canvas-operator"), [o2, { openModal: a2 }] = kt();
  function n2() {
    a2(true, {});
  }
  return (e3, a3) => (s(), r(y, null, [p("div", { class: c(d(t2)) }, [g(d(G), { title: "快捷方式", placement: "right" }, { default: M(() => [g(d(Be), { icon: "carbon:table-shortcut", onClick: n2 })]), _: 1 })], 2), g(_h, { onRegister: d(o2) }, null, 8, ["onRegister"])], 64));
} }), [["__scopeId", "data-v-2148ffa6"]]), Lh = { lt: function(e2, t2, o2, a2, n2, l2) {
  const { symmetricPoint: i2 } = n2;
  let r2 = Kr(t2, i2), s2 = Zr(t2, r2, -e2.rotate), u2 = Zr(i2, r2, -e2.rotate), c2 = u2.x - s2.x, d2 = u2.y - s2.y;
  if (a2) {
    c2 / d2 > o2 ? (s2.x += Math.abs(c2 - d2 * o2), c2 = d2 * o2) : (s2.y += Math.abs(d2 - c2 / o2), d2 = c2 / o2);
    const t3 = Zr(s2, r2, e2.rotate);
    r2 = Kr(t3, i2), s2 = Zr(t3, r2, -e2.rotate), u2 = Zr(i2, r2, -e2.rotate), c2 = u2.x - s2.x, d2 = u2.y - s2.y;
  }
  c2 > 10 && d2 > 10 && (e2.width = Math.round(c2 / l2), e2.height = Math.round(d2 / l2), e2.x = Math.round(s2.x / l2), e2.y = Math.round(s2.y / l2));
}, t: Ih, rt: function(e2, t2, o2, a2, n2, l2) {
  const { symmetricPoint: i2 } = n2;
  let r2 = Kr(t2, i2), s2 = Zr(t2, r2, -e2.rotate), u2 = Zr(i2, r2, -e2.rotate), c2 = s2.x - u2.x, d2 = u2.y - s2.y;
  if (a2) {
    c2 / d2 > o2 ? (s2.x -= Math.abs(c2 - d2 * o2), c2 = d2 * o2) : (s2.y += Math.abs(d2 - c2 / o2), d2 = c2 / o2);
    const t3 = Zr(s2, r2, e2.rotate);
    r2 = Kr(t3, i2), s2 = Zr(t3, r2, -e2.rotate), u2 = Zr(i2, r2, -e2.rotate), c2 = s2.x - u2.x, d2 = u2.y - s2.y;
  }
  c2 > 10 && d2 > 10 && (e2.width = Math.round(c2 / l2), e2.height = Math.round(d2 / l2), e2.x = Math.round(u2.x / l2), e2.y = Math.round(s2.y / l2));
}, r: Dh, rb: function(e2, t2, o2, a2, n2, l2) {
  const { symmetricPoint: i2 } = n2;
  let r2 = Kr(t2, i2), s2 = Zr(i2, r2, -e2.rotate), u2 = Zr(t2, r2, -e2.rotate), c2 = u2.x - s2.x, d2 = u2.y - s2.y;
  if (a2) {
    c2 / d2 > o2 ? (u2.x -= Math.abs(c2 - d2 * o2), c2 = d2 * o2) : (u2.y -= Math.abs(d2 - c2 / o2), d2 = c2 / o2);
    const t3 = Zr(u2, r2, e2.rotate);
    r2 = Kr(t3, i2), s2 = Zr(i2, r2, -e2.rotate), u2 = Zr(t3, r2, -e2.rotate), c2 = u2.x - s2.x, d2 = u2.y - s2.y;
  }
  c2 > 10 && d2 > 10 && (e2.width = Math.round(c2 / l2), e2.height = Math.round(d2 / l2), e2.x = Math.round(s2.x / l2), e2.y = Math.round(s2.y / l2));
}, b: Ih, lb: function(e2, t2, o2, a2, n2, l2) {
  const { symmetricPoint: i2 } = n2;
  let r2 = Kr(t2, i2), s2 = Zr(i2, r2, -e2.rotate), u2 = Zr(t2, r2, -e2.rotate), c2 = s2.x - u2.x, d2 = u2.y - s2.y;
  if (a2) {
    c2 / d2 > o2 ? (u2.x += Math.abs(c2 - d2 * o2), c2 = d2 * o2) : (u2.y -= Math.abs(d2 - c2 / o2), d2 = c2 / o2);
    const t3 = Zr(u2, r2, e2.rotate);
    r2 = Kr(t3, i2), s2 = Zr(i2, r2, -e2.rotate), u2 = Zr(t3, r2, -e2.rotate), c2 = s2.x - u2.x, d2 = u2.y - s2.y;
  }
  c2 > 10 && d2 > 10 && (e2.width = Math.round(c2 / l2), e2.height = Math.round(d2 / l2), e2.x = Math.round(u2.x / l2), e2.y = Math.round(s2.y / l2));
}, l: Dh };
function Ih(e2, t2, o2, a2, n2, l2) {
  const { symmetricPoint: i2, curPoint: r2 } = n2, s2 = Zr(t2, r2, -e2.rotate), u2 = Zr({ x: r2.x, y: s2.y }, r2, e2.rotate), c2 = Math.hypot(u2.x - i2.x, u2.y - i2.y), d2 = u2.x - (u2.x - i2.x) / 2, p2 = u2.y + (i2.y - u2.y) / 2;
  let f2 = e2.width;
...
function Dh(e2, t2, o2, a2, n2, l2) {
  const { symmetricPoint: i2, curPoint: r2 } = n2, s2 = Zr(t2, r2, -e2.rotate), u2 = Zr({ x: s2.x, y: r2.y }, r2, e2.rotate), c2 = Math.hypot(u2.x - i2.x, u2.y - i2.y), d2 = u2.x - (u2.x - i2.x) / 2, p2 = u2.y + (i2.y - u2.y) / 2;
  let f2 = e2.height;
...
const Th = ["onMousedown"], Nh = { key: 2, class: "cover-layer-out" }, Bh = { key: 0, class: "cover-hover" }, Rh = e({ name: "ChartContain", __name: "index", props: { comOption: { require: true, type: Object, default: () => ({ chart: {}, chartContain: {} }) }, index: { require: true, type: Number, default: 0 } }, setup(e2) {
  t((e3) => ({ "15d86f1b": g2.value }));
  const n2 = e2, { token: l2 } = P.useToken(), g2 = a(() => l2.value.colorInfoBg), C2 = a(() => l2.value.colorBorder), b2 = o("chartContainRef"), M2 = o("chartItemRef"), { prefixCls: w2 } = xe("chart-contain"), x2 = { points: ["lt", "t", "rt", "r", "rb", "b", "lb", "l"], initAngle: { lt: 0, t: 45, rt: 90, r: 135, rb: 180, b: 225, lb: 270, l: 315 }, pointCursor: [{ start: 338, end: 23, cursor: "nw" }, { start: 23, end: 68, cursor: "n" }, { start: 68, end: 113, cursor: "ne" }, { start: 113, end: 158, cursor: "e" }, { start: 158, end: 203, cursor: "se" }, { start: 203, end: 248, cursor: "s" }, { start: 248, end: 293, cursor: "sw" }, { start: 293, end: 338, cursor: "w" }] }, S2 = m(false), _2 = m(false), $2 = m({}), L2 = Or(), I2 = kp(), D2 = a(() => [w2, { active: q2.value || O2.value }]), T2 = a(() => !S2.value && !_2.value && n2.comOption.chart?.id === L2.getCurHoverComponentId || S2.value && L2.getCurComponent?.chart.id === n2.comOption?.chart?.id), N2 = a(() => up({ ...n2.comOption.chartContain.dropInfo })), B2 = a(() => {
    const e3 = sp(n2.comOption);
    return e3.borderColor || (e3.borderColor = C2.value), Qe(e3, ["transform"]);
  }), q2 = a(() => L2.getCurComponent?.chart.id === n2.comOption?.chart.id), O2 = a(() => L2.getSelectArea.components.map((e3) => e3?.chart.id).includes(n2.comOption?.chart?.id));
  function j2(e3) {
  ...
  async function E2(e3) {
    ...
    const l3 = setInterval(() => {
      ...
      d(S2) ? L2.setUndoRedoData("组件移动") : L2.cleanSelectArea(), S2.value = false, clearInterval(l3), t2 && $p(L2.getSelectArea.components), op.emit(Qr.UN_MOVE), document.removeEventListener("mousemove", i2), document.removeEventListener("mouseup", r2), $2.value = F2(n2.comOption);
    };
    document.addEventListener("mousemove", i2), document.addEventListener("mouseup", r2);
  }
  function F2(e3) {
    if (!e3) return;
    const { pointCursor: t2, initAngle: o2, points: a2 } = x2, n3 = es(e3.chartContain.dropInfo.rotate), l3 = {};
    let i2 = -1;
    return a2.forEach((e4) => {
      const a3 = es(o2[e4] + n3), r2 = t2.length;
      for (; ; ) {
        i2 = (i2 + 1) % r2;
        const o3 = t2[i2];
  ...
  function U2(e3, t2) {
    ...
    let u2 = true;
    const c2 = function() {
      if (n2.comOption.chart.type !== ns.MfCombine) return false;
      for (const e4 of n2.comOption.chart.options.components) if (![0, 90, 180, 360].includes(es(e4.chartContain.dropInfo.rotate))) return true;
      return false;
    }(), p2 = He((t3) => {
      ...
      !function(e4, t4, o3, a3, n4, l4, i4) {
        Lh[e4](t4, o3, a3, n4, l4, i4);
      }(e3, o2, n3, i3, c2, { center: a2, curPoint: r2, symmetricPoint: s2 }, L2.getScale), j2(o2), op.emit(Qr.MOVE, false);
    }, 2), f2 = () => {
      d(S2) && L2.setUndoRedoData("组件大小调整"), S2.value = false, op.emit(Qr.UN_MOVE), document.removeEventListener("mousemove", p2), document.removeEventListener("mouseup", f2);
    };
  ...
  function V2(e3) {
    ...
    const s2 = He((e4) => {
      ...
      i2.rotate = Math.round(r2 + t3 - l3), j2(i2);
    }, 10), u2 = () => {
  ...
  return i(() => {
    n2.comOption && ($2.value = F2(n2.comOption)), op.on(Qr.MOVE, () => {
      n2.comOption && L2.getCurComponent && n2.comOption.chart.id !== L2.getCurComponent.chart.id && !d(_2) && (_2.value = true);
    }), op.on(Qr.UN_MOVE, () => {
      _2.value = false;
    }), op.on(Qr.PLAY_ANIMATION, () => {
      n2.comOption.chart.id === L2.getCurComponent?.chart.id && void 0 === n2.comOption.chart.options?.selectIndex && Xd(M2.value, n2.comOption.chart.animations).then(() => {
        op.emit(Qr.PLAY_ANIMATION_COMPLETE);
      });
    }), op.on(Qr.EVENT_ANIMATION, (e3) => {
      M2.value && Jd(e3, n2.comOption, M2.value);
    }), op.on(Qr.STOP_ANIMATION, () => {
  ...
  }), (t2, o2) => (s(), r("div", { ref_key: "chartContainRef", ref: b2, class: c(D2.value), style: u(N2.value), onMousedown: k(E2, ["stop"]), onMouseenter: k(H2, ["prevent", "stop"]), onMouseleave: k(Q2, ["prevent"]) }, [(s(true), r(y, null, v(q2.value && !e2.comOption.chartContain.lock && e2.comOption.chartContain.show ? x2.points : [], (e3) => (s(), r("div", { key: e3, class: "contain-point", style: u(G2(e3)), onMousedown: k((t3) => U2(e3, t3), ["stop", "prevent"]) }, null, 44, Th))), 128)), A(p("div", { ref_key: "chartItemRef", ref: M2, class: "chart-item", style: u(B2.value) }, [R(t2.$slots, "default", {}, void 0, true)], 4), [[z, void 0 === e2.comOption.chart?.showHide?.show && e2.comOption.chartContain?.show || void 0 !== e2.comOption.chart?.showHide?.show && e2.comOption.chart?.showHide?.show]]), q2.value && !e2.comOption.chartContain.lock && e2.comOption.chartContain.show ? (s(), h(d(Be), { key: 0, class: "icon-rotate", icon: "ant-design:sync-outlined", size: "20", onMousedown: k(V2, ["prevent", "stop"]) })) : f("", true), e2.comOption.chartContain.lock && e2.comOption.chartContain.show ? (s(), h(d(Be), { key: 1, class: "icon-lock", icon: "ant-design:lock-outlined" })) : f("", true), q2.value || O2.value || T2.value ? (s(), r("div", Nh, [T2.value ? (s(), r("div", Bh)) : f("", true)])) : f("", true)], 38));
} }), qh = to(Rh, [["__scopeId", "data-v-2f19c653"]]), Oh = "横上", Ah = "横中", jh = "横下", zh = "纵左", Eh = "纵中", Fh = "纵右", Ph = { class: "pos" }, Hh = e({ name: "ChartNearLine", __name: "index", setup(e2) {
  const t2 = l({ adsorb: 10, near: 2, lineStatus: { [Oh]: { show: false, pos: 0, node: null }, [Ah]: { show: false, pos: 0, node: null }, [jh]: { show: false, pos: 0, node: null }, [zh]: { show: false, pos: 0, node: null }, [Eh]: { show: false, pos: 0, node: null }, [Fh]: { show: false, pos: 0, node: null } } }), o2 = Or(), { prefixCls: a2 } = xe("near-line");
  function n2(e3, { x: o3, y: a3, bottom: n3, right: l2, halfWidth: i2, halfHeight: r2 }) {
    ...
    return { y: [{ isNear: u2(e3.y, a3), isAdsorb: f2(e3.y, a3), lineNode: d2[Oh].node, line: Oh, dragShift: a3, lineShift: a3 }, { isNear: u2(e3.bottom, a3), isAdsorb: f2(e3.bottom, a3), lineNode: d2[Oh].node, line: Oh, dragShift: a3 - e3.height, lineShift: a3 }, { isNear: u2(e3.y + c2, a3 + r2), isAdsorb: f2(e3.y + c2, a3 + r2), lineNode: d2[Ah].node, line: Ah, dragShift: a3 + r2 - c2, lineShift: a3 + r2 }, { isNear: u2(e3.y, n3), isAdsorb: f2(e3.y, n3), lineNode: d2[jh].node, line: jh, dragShift: n3, lineShift: n3 }, { isNear: u2(e3.bottom, n3), isAdsorb: f2(e3.bottom, n3), lineNode: d2[jh].node, line: jh, dragShift: n3 - e3.height, lineShift: n3 }], x: [{ isNear: u2(e3.x, o3), isAdsorb: f2(e3.x, o3), lineNode: d2[zh].node, line: zh, dragShift: o3, lineShift: o3 }, { isNear: u2(e3.right, o3), isAdsorb: f2(e3.right, o3), lineNode: d2[zh].node, line: zh, dragShift: o3 - e3.width, lineShift: o3 }, { isNear: u2(e3.x + s2, o3 + i2), isAdsorb: f2(e3.x + s2, o3 + i2), lineNode: d2[Eh].node, line: Eh, dragShift: o3 + i2 - s2, lineShift: o3 + i2 }, { isNear: u2(e3.x, l2), isAdsorb: f2(e3.x, l2), lineNode: d2[Fh].node, line: Fh, dragShift: l2, lineShift: l2 }, { isNear: u2(e3.right, l2), isAdsorb: f2(e3.right, l2), lineNode: d2[Fh].node, line: Fh, dragShift: l2 - e3.width, lineShift: l2 }] };
  }
  function u2(e3, o3) {
    ...
    op.on(Qr.MOVE, (e3) => {
      !async function(e4) {
        ...
        e4 ? o2.getSelectArea.components.forEach((e5) => {
          a3.push({ com: e5, style: cp(e5.chartContain.dropInfo) });
        }) : a3.push({ com: o2.getCurComponent, style: cp(o2.getCurComponent.chartContain.dropInfo) });
        g2().then(), a3.forEach((a4) => {
          !function(e5, a5) {
            ...
            l2(n2(e5.style, { x: o2.getCanvasConfig.width / 2, y: o2.getCanvasConfig.height / 2, bottom: 0, right: 0, halfWidth: 0, halfHeight: 0 })), l2(n2(e5.style, { x: 0, y: 0, bottom: o2.getCanvasConfig.height, right: o2.getCanvasConfig.width, halfWidth: 0, halfHeight: 0 })), o2.getGuideLine.lineListX.forEach((t3) => {
              ...
              const i2 = cp(t3.chartContain.dropInfo), { x: r2, y: s2, bottom: u3, right: c2 } = i2, d2 = i2.width / 2, p2 = i2.height / 2;
  ...
  }), (e3, o3) => (s(), r("div", { class: c(d(a2)) }, [(s(true), r(y, null, v(Object.keys(t2.lineStatus), (e4) => A((s(), r("div", { key: e4, ref_for: true, ref: (o4) => t2.lineStatus[e4].node = o4, class: c(["line", e4.includes("横") ? "xLine" : "yLine"]) }, [p("div", Ph, S(t2.lineStatus[e4].pos), 1)], 2)), [[z, t2.lineStatus[e4].show || false]])), 128))], 2));
} }), Qh = to(Hh, [["__scopeId", "data-v-88d9d34f"]]), Gh = e({ name: "ChartMultiSelect", __name: "index", props: { selectArea: { type: Object, default: () => {
} } }, setup(e2) {
  const t2 = Or(), { prefixCls: o2 } = xe("select-area");
...
} }), Uh = to(Gh, [["__scopeId", "data-v-e9844525"]]), Vh = e({ name: "ScreenCanvas", __name: "index", setup(e2) {
  const { prefixCls: t2 } = xe("screen-canvas"), n2 = Or(), p2 = kp(), m2 = o("screenCanvasRef"), { getAntdLocale: C2 } = Tt(), { antTheme: b2 } = Le(a(() => n2.getTheme)), w2 = a(() => [...n2.getComponentList].reverse()), x2 = l({ canvasX: 0, canvasY: 0, area: { show: false, x: 0, y: 0, width: 0, height: 0 }, comMoving: false, isUp: false }), S2 = a(() => ({ ...ap(n2.getCanvasConfig), transform: `scale(${n2.getScale})` })), _2 = (e3) => {
    if (e3.chart?.events?.emits && e3.chart.events.emits.length > 0) {
      for (const t3 of e3.chart.events.emits) if (t3 === rs.CHART_CLICK || t3 === rs.CHART_DBLCLICK) return "allow-pointer";
    }
  };
  i(() => {
    n2.setScreenCanvas(m2.value ?? null), op.on(Qr.MOVE, () => {
      x2.comMoving || (x2.comMoving = true);
    }), op.on(Qr.UN_MOVE, () => {
      x2.comMoving = false;
    }), op.on(Qr.CREATE_CHART, (e3) => {
      L2(e3, 0, 0);
    });
  });
  const $2 = (e3) => {
    p2.setShowMenu(false);
    const t3 = e3.dataTransfer?.getData(jr);
  ...
  }, ["prevent", "stop"])), onMousedown: I2, style: u(S2.value) }, [g(d(me), { locale: d(C2), theme: d(b2) }, { default: M(() => [d(n2).getCanvasConfig.watermark ? (s(), h(d(ye), { key: 0, content: d(n2).getCanvasConfig.watermark, style: u({ position: "absolute", width: `${d(n2).getCanvasConfig.width}px`, height: `${d(n2).getCanvasConfig.height}px` }) }, null, 8, ["content", "style"])) : f("", true), (s(true), r(y, null, v(w2.value, (e4, t3) => (s(), h(qh, { key: e4.chart?.id + d(n2).getKey, index: w2.value.length - t3 - 1, "com-option": e4, class: c(_2(e4)) }, { default: M(() => [(s(), h(j(e4.chart.type && d(tp)[e4.chart.type].component), { class: c(["component", { move: x2.comMoving && e4.chart.id !== d(n2).getCurComponent?.chart.id }]), id: `com${e4.chart.id}`, chart: e4.chart, "chart-contain": e4.chartContain }, null, 8, ["class", "id", "chart", "chart-contain"]))]), _: 2 }, 1032, ["index", "com-option", "class"]))), 128))]), _: 1 }, 8, ["locale", "theme"]), g(Uh, { "select-area": x2.area }, null, 8, ["select-area"]), g(Qh)], 38));
} }), Wh = to(Vh, [["__scopeId", "data-v-9395352a"]]), Yh = 0.2, Zh = e({ name: "FrameCanvas", __name: "index", props: { width: { type: Number, default: 5e3 }, height: { type: Number, default: 3e3 } }, setup(e2) {
  const t2 = m(true), { prefixCls: a2 } = xe("frame-canvas"), l2 = m(0), u2 = m(0), f2 = m(1e3), h2 = m(1), y2 = Or(), v2 = o("frameCanvasRef"), C2 = o("dropContentRef");
  let b2 = [0, 0], w2 = [0, 0];
  ...
  const x2 = He(() => {
    t2.value && (!function() {
      if (d(t2) && v2.value) {
        ...
        h2.value = n2 > a3 ? Number.parseFloat((o2 * a3 / y2.getCanvasConfig.width).toFixed(2)) : Number.parseFloat((t3 / a3 / y2.getCanvasConfig.height).toFixed(2)), d(h2) > 5 && (h2.value = 5), d(h2) < Yh && (h2.value = Yh);
      }
      y2.setScale(d(h2));
    }(), D2());
  }, 30);
  ...
  const $2 = He((e3) => {
    ...
    h2.value = "add" === e3 ? Math.min(5, Number.parseFloat((t3 + 0.05).toFixed(2))) : Math.max(Yh, Number.parseFloat((t3 - 0.05).toFixed(2))), D2(), y2.setScale(h2.value);
  }, 100), L2 = He((e3) => {
  ...
  return (o2, n2) => (s(), r("div", { class: c(d(a2)), ref_key: "frameCanvasRef", ref: v2 }, [g(bh, { "ruler-x-left": l2.value, "ruler-y-top": u2.value, drift: f2.value, "ruler-x": e2.width, "ruler-y": e2.height }, null, 8, ["ruler-x-left", "ruler-y-top", "drift", "ruler-x", "ruler-y"]), p("div", { ref_key: "dropContentRef", ref: C2, class: "drop-content", onScroll: S2 }, [g(kh, { width: e2.width, height: e2.height, drift: f2.value }, { default: M(() => [g(Wh, { draggable: "true", onDragstart: k(T2, ["prevent", "stop"]) })]), _: 1 }, 8, ["width", "height", "drift"])], 544), g($h), g(Sh, { scale: h2.value, "min-scale": 20, "max-scale": 500, onAddScale: n2[0] || (n2[0] = (e3) => d($2)("add")), onReduceScale: n2[1] || (n2[1] = (e3) => d($2)("reduce")), onChangeScale: d(L2), onAutoScale: n2[2] || (n2[2] = (e3) => t2.value = e3) }, null, 8, ["scale", "min-scale", "max-scale", "onChangeScale"])], 2));
} }), Kh = to(Zh, [["__scopeId", "data-v-de9ae999"]]), Xh = { key: 0, class: "header" }, Jh = { class: "title" }, eg = { class: "content" }, tg = to(e({ name: "FrameBox", __name: "index", props: { showHeader: { type: Boolean, default: true }, icon: { type: String, default: "" }, iconSize: { type: Number, default: 14 }, title: { type: String, default: "" }, radioData: { type: Array, default: () => [] }, span: { type: Number, default: 8 }, chart: { type: Object } }, emits: ["radioChange", "titleClick"], setup(e2, { emit: t2 }) {
  const o2 = e2, a2 = t2, { prefixCls: n2 } = xe("frame-box"), l2 = m();
  i(() => {
  ...
  return (t3, a3) => (s(), r("div", { class: c(d(n2)) }, [e2.showHeader ? (s(), r("div", Xh, [p("div", { class: "icon-title", onClick: C2 }, [e2.icon ? (s(), h(d(Be), { key: 0, class: "title", icon: e2.icon, size: e2.iconSize }, null, 8, ["icon", "size"])) : f("", true), p("div", Jh, S(e2.title), 1)]), g(d(Z), { class: "radio-group", size: "small", value: l2.value, "onUpdate:value": a3[0] || (a3[0] = (e3) => l2.value = e3), onChange: u2 }, { default: M(() => [(s(true), r(y, null, v(o2.radioData, (e3) => (s(), h(d(G), { key: e3.value, title: e3.tooltip }, { default: M(() => [g(d(Y), { class: "radio-button", value: e3.value }, { default: M(() => [g(d(Be), { icon: e3.icon }, null, 8, ["icon"])]), _: 2 }, 1032, ["value"])]), _: 2 }, 1032, ["title"]))), 128))]), _: 1 }, 8, ["value"])])) : f("", true), p("div", eg, [(s(), h(j(e2.chart))), R(t3.$slots, "default", {}, void 0, true)])], 2));
} }), [["__scopeId", "data-v-8929fb33"]]), og = { class: "item-content" }, ag = ["src"], ng = { class: "title", style: { flex: "1" } }, lg = { class: "iconContain" }, ig = e({ __name: "LayerItem", props: { parentItem: { type: Object }, item: { type: Object, required: true }, showEye: { type: Boolean, default: true }, showLock: { type: Boolean, default: true }, index: { type: Number }, expand: { type: Boolean, default: false }, externalSelect: { type: Boolean, default: false }, isSelect: { type: Boolean, default: false } }, emits: ["expand", "select", "enter", "leave"], setup(e2, { emit: o2 }) {
  t((e3) => ({ "56d1bd00": C2.value }));
  const n2 = e2, l2 = o2, { prefixCls: i2 } = xe("layer-item"), u2 = Or(), g2 = Ar(), y2 = a(() => u2.getChartImg(n2.item.chart.type)), { token: v2 } = P.useToken(), C2 = a(() => v2.value.colorInfoBg), b2 = Se().getThemeColor, M2 = a(() => [i2, { select: n2.externalSelect && n2.isSelect || !n2.externalSelect && (u2.getCurComponent?.chart.id === n2.item?.chart.id || u2.getSelectArea.components.map((e3) => e3.chart.id).includes(n2.item?.chart.id) || u2.getCurConfigComponent?.chart.id === n2.item?.chart.id), hover: u2.getCurHoverComponentId === n2.item?.chart.id || u2.getCurHoverComponentId === n2.parentItem?.chart.id && n2.parentItem?.chart?.options?.hoverId === n2.item?.chart.id }]), k2 = m(false);
  function w2() {
  ...
  return (t2, o3) => (s(), r("div", { class: c(M2.value), onMouseenter: w2, onMouseleave: x2, onMousedown: $2 }, [p("div", og, [e2.item.chart.type === d(ns).MfCombine ? (s(), r("div", { key: 0, onClick: _2 }, [n2.expand ? (s(), h(d(mt), { key: 0, class: "group", "two-tone-color": d(b2) }, null, 8, ["two-tone-color"])) : (s(), h(d(yt), { key: 1, class: "group", "two-tone-color": d(b2) }, null, 8, ["two-tone-color"]))])) : (s(), r("img", { key: 1, class: "img", alt: "组件", src: y2.value }, null, 8, ag)), p("span", ng, S(e2.item?.chart.name), 1), p("div", lg, [!e2.showEye || d(g2).getLayerCollapsed || !k2.value && e2.item.chartContain.show ? f("", true) : (s(), h(d(Be), { key: 0, class: "icon", icon: e2.item.chartContain.show ? "ant-design:eye-outlined" : "ant-design:eye-invisible-outlined", onClick: o3[0] || (o3[0] = () => e2.item.chartContain.show = !e2.item.chartContain.show) }, null, 8, ["icon"])), e2.showLock && !d(g2).getLayerCollapsed && (k2.value || e2.item.chartContain.lock) ? (s(), h(d(Be), { key: 1, class: "icon", icon: e2.item.chartContain.lock ? "ant-design:lock-outlined" : "ant-design:unlock-outlined", onClick: o3[1] || (o3[1] = () => e2.item.chartContain.lock = !e2.item.chartContain.lock) }, null, 8, ["icon"])) : f("", true)])])], 34));
} }), rg = to(ig, [["__scopeId", "data-v-925f45da"]]), sg = e({ __name: "LayerGroup", props: { item: { type: Object, required: true }, expand: { type: Boolean, default: false }, showEye: { type: Boolean, default: true }, showLock: { type: Boolean, default: true }, showChildEye: { type: Boolean, default: true }, showChildLock: { type: Boolean, default: true }, externalSelect: { type: Boolean, default: false }, isSelect: { type: Boolean, default: false }, childSelect: { type: String, default: "" } }, emits: ["parentClick", "childClick"], setup(e2, { emit: t2 }) {
  const o2 = e2, l2 = t2, { prefixCls: i2 } = xe("layer-group"), u2 = Or(), p2 = Ar(), f2 = a(() => ({ "child-item": true, collapsed: p2.getLayerCollapsed })), C2 = m(o2.expand);
  function b2() {
  ...
  return n(() => o2.expand, (e3) => C2.value = e3), (t3, o3) => (s(), r("div", { class: c(d(i2)), onMouseenter: b2, onMouseleave: k2 }, [g(rg, { item: e2.item, onExpand: x2, expand: C2.value, "show-eye": e2.showEye, "show-lock": e2.showLock, "external-select": e2.externalSelect, "is-select": e2.isSelect, onClick: S2 }, null, 8, ["item", "expand", "show-eye", "show-lock", "external-select", "is-select"]), g(d(Rt), { show: C2.value, class: c(f2.value) }, { default: M(() => [(s(true), r(y, null, v(e2.item.chart.options.components, (t4, o4) => (s(), h(rg, { key: t4.chart.id, "parent-item": e2.item, item: t4, index: o4, "external-select": e2.externalSelect, "is-select": e2.childSelect === t4.chart.id, "show-eye": e2.showChildEye, "show-lock": e2.showChildLock, onSelect: (t5) => function(e3, t6) {
...
} }), ug = to(sg, [["__scopeId", "data-v-b118e01c"]]), cg = ["onDrop"], dg = e({ name: "FrameLayer", __name: "index", props: { collapsed: Nt.bool.def(false) }, setup(e2) {
  const t2 = e2, { prefixCls: o2 } = xe("frame-layer"), n2 = a(() => [o2, { collapsed: t2.collapsed }]), l2 = [{ value: "collapse", icon: "iconfont:icon-mfish-collapse-all", tooltip: "折叠分组" }, { value: "expand", icon: "iconfont:icon-mfish-expand-all", tooltip: "展开分组" }], r2 = a(() => f2.getComponentList);
  i(() => {
    const e3 = f2.getComponentList.map((e4) => e4.chart.type).join(",");
    Yi({ types: e3, pageNum: 1, pageSize: 1e4 }).then((e4) => {
      e4 && e4.list?.length > 0 && e4.list.forEach((e5) => {
        f2.hasChartImg(e5.type) || pt(dt(e5?.picKey), { errorMessageMode: "none" }).then((t3) => {
          f2.setChartImg(e5.type, t3);
        });
      });
    });
  });
  const u2 = m(false), f2 = Or(), y2 = Ar(), v2 = (e3) => {
  ...
  function b2(e3, t3) {
    e3.stopPropagation();
    const o3 = e3.dataTransfer?.getData(jr);
    if (o3) {
  ...
  return (e3, t3) => (s(), h(tg, { class: c(n2.value), title: d(y2).getLayerCollapsed ? "" : "图层", icon: "iconfont:icon-mfish-layer", "icon-size": d(y2).getLayerCollapsed ? 16 : 12, "radio-data": l2, onTitleClick: C2, onDrop: t3[1] || (t3[1] = (e4) => b2(e4, r2.value.length)), onRadioChange: x2 }, { default: M(() => [0 === r2.value.length ? (s(), h(d(ke), { key: 0, "image-style": d(y2).getLayerCollapsed ? { height: "20px" } : {}, description: "暂无图层", image: d(ot) }, null, 8, ["image-style", "image"])) : (s(), h(d(vt), { key: 1 }, { default: M(() => [g(d(qt), { "item-key": "id", modelValue: r2.value, "onUpdate:modelValue": t3[0] || (t3[0] = (e4) => r2.value = e4), animation: "300", "ghost-class": "ghost", onChange: v2 }, { item: M(({ index: e4, element: t4 }) => [p("div", { onDrop: (t5) => b2(t5, e4), style: { "margin-top": "4px" } }, [t4.chart.type !== d(ns).MfCombine ? (s(), h(rg, { key: 0, item: t4, onMousedown: (o3) => k2(t4, e4, o3), onEnter: (e5) => {
    return o3 = t4, void f2.setCurHoverComponent(o3?.chart.id);
    var o3;
  }, onLeave: w2 }, null, 8, ["item", "onMousedown", "onEnter"])) : (s(), h(ug, { key: 1, item: t4, expand: u2.value, "show-child-lock": false, onMousedown: (o3) => k2(t4, e4, o3) }, null, 8, ["item", "expand", "onMousedown"]))], 40, cg)]), _: 1 }, 8, ["modelValue"])]), _: 1 }))]), _: 1 }, 8, ["class", "title", "icon-size"]));
} }), pg = to(dg, [["__scopeId", "data-v-3b66e5f3"]]), fg = { class: "menu-item-hide-title" }, hg = { class: "title" }, gg = to(e({ __name: "ChartMenus", props: { itemHeight: { type: Number }, width: { type: Number, default: 68 }, collapsed: { type: Boolean, default: false }, horizontal: { type: Boolean, default: false }, hideTitle: { type: Boolean, default: false }, backgroundColor: { type: Object, default: () => ({ light: "transparent", dark: "transparent" }) }, menus: { type: Array, default: () => [] } }, emits: ["menuClick", "menuChange"], setup(e2, { emit: o2 }) {
  ...
  const { prefixCls: f2 } = xe("chart-menus"), C2 = m([]), { backgroundColor: b2, width: k2 } = l2, w2 = a(() => b2.light), x2 = a(() => b2.dark), _2 = (e3) => {
    i2("menuClick", e3, d(C2)), i2("menuChange", e3.key, e3.item.id);
  };
  return (t2, o3) => (s(), r("div", { class: c(d(f2)) }, [g(d(te), { class: c({ collapsed: e2.collapsed }), "selected-keys": C2.value, "onUpdate:selectedKeys": o3[0] || (o3[0] = (e3) => C2.value = e3), onClick: _2 }, { default: M(() => [g(d(vt), null, { default: M(() => [(s(true), r(y, null, v(e2.menus, (t3) => (s(), h(d(we), { id: t3.name, key: t3.value, style: u({ height: `${e2.itemHeight}px` }) }, { default: M(() => [e2.hideTitle ? (s(), h(d(G), { key: 0, title: t3.name, placement: "right" }, { default: M(() => [p("div", fg, [g(d(Be), { style: u({ lineHeight: `${e2.itemHeight}px` }), icon: t3.icon }, null, 8, ["style", "icon"])])]), _: 2 }, 1032, ["title"])) : (s(), r("div", { key: 1, class: c(e2.horizontal ? "menu-item-horizontal" : "menu-item-vertical") }, [g(d(Be), { icon: t3.icon }, null, 8, ["icon"]), p("div", hg, S(t3.name), 1)], 2))]), _: 2 }, 1032, ["id", "style"]))), 128))]), _: 1 })]), _: 1 }, 8, ["class", "selected-keys"])], 2));
} }), [["__scopeId", "data-v-dea86191"]]), mg = ["onDragstart", "onDblclick"], yg = { class: "box-header" }, vg = { class: "box-header-text" }, Cg = { class: "box-content" }, bg = ["src"], Mg = e({ __name: "ChartItems", props: { charts: { type: Array, default: () => [] }, mode: { type: String, default: "single" } }, setup(e2) {
  const t2 = e2, { prefixCls: o2 } = xe("chart-items"), a2 = Or();
  return n(() => t2.charts, (e3) => {
    e3.forEach((e4) => {
      const t3 = e4?.type;
      t3 && e4?.picKey && !a2.hasChartImg(t3) && pt(dt(e4?.picKey), { errorMessageMode: "none" }).then((e5) => {
        a2.setChartImg(t3, e5);
      });
    });
  }, { immediate: true }), (t3, n2) => (s(), h(d(vt), { class: c(d(o2)) }, { default: M(() => [p("div", { class: c(["items", "single" === e2.mode ? e2.mode : [e2.mode, "miniAnimation"]]) }, [(s(true), r(y, null, v(e2.charts, (t4) => {
    return s(), r("div", { draggable: "true", key: t4.type, class: "item-box", onDragstart: (e3) => function(e4, t5) {
      const o4 = rp(t5);
      if (!o4) return;
      const a3 = e4.currentTarget;
      "img" === e4.target.className ? e4.dataTransfer?.setDragImage(a3, e4.offsetX + 15, e4.offsetY + 32) : e4.dataTransfer?.setDragImage(a3, e4.offsetX, e4.offsetY), e4.dataTransfer?.setData(jr, JSON.stringify(o4));
    }(e3, t4), onDblclick: (e3) => function(e4) {
      const t5 = rp(e4);
      t5 && op.emit(Qr.CREATE_CHART, t5);
    }(t4) }, [p("div", yg, [g(d(Rl), { disabled: true, size: "single" === e2.mode ? "small" : "mini" }, null, 8, ["size"]), p("span", vg, S(t4.name), 1)]), p("div", Cg, [p("img", { class: "img", alt: "组件图片", src: (o3 = t4.type, a2.getChartImg(o3)) }, null, 8, bg)])], 40, mg);
    var o3;
  }), 128))], 2)]), _: 1 }, 8, ["class"]));
} }), kg = to(Mg, [["__scopeId", "data-v-74651cba"]]), wg = e({ name: "FrameChart", __name: "index", props: { collapsed: Nt.bool.def(false) }, setup(e2) {
  const t2 = e2, o2 = Ar(), n2 = m(), l2 = m(), r2 = m(""), u2 = m(/* @__PURE__ */ new Map()), f2 = a(() => l2.value && l2.value.length > 0 ? l2.value.map((e3) => ({ name: e3.categoryName, icon: e3.icon, value: e3.id, children: e3.children })) : []), y2 = a(() => {
    if (n2.value) {
  ...
  i(() => {
    Ot("screen_charts_type", "down").then((e3) => {
  ...
  function $2(e3, t3) {
    let o3;
    o3 = "所有" === t3 ? y2.value.find((t4) => t4.value === e3).parentId : e3, r2.value = o3, u2.value.has(o3) || Yi({ category: o3, pageNum: 1, pageSize: 1e3 }).then((e4) => {
      u2.value.set(o3, e4.list);
    });
  }
  return (t3, o3) => (s(), h(tg, { title: "组件", icon: "iconfont:icon-mfish-chart", "icon-size": 12, "radio-data": b2, onRadioChange: w2, class: c(k2.value), onTitleClick: S2 }, { default: M(() => [p("div", { class: c(`${d(v2)}-content`) }, [g(gg, { menus: f2.value, "item-height": 58, "background-color": { light: "#f0f2f5", dark: "#232324" }, onMenuClick: x2, onMenuChange: _2 }, null, 8, ["menus"]), p("div", { class: c(["charts", { collapsed: e2.collapsed }]) }, [g(gg, { menus: y2.value, collapsed: e2.collapsed, "hide-title": true, "background-color": { light: "#f8f8f9", dark: "#1e1e1f" }, horizontal: true, "item-height": 28, width: 40, onMenuChange: $2 }, null, 8, ["menus", "collapsed"]), g(kg, { mode: C2.value, charts: u2.value.get(r2.value) }, null, 8, ["mode", "charts"])], 2)], 2)]), _: 1 }, 8, ["class"]));
} }), xg = to(wg, [["__scopeId", "data-v-3b8e6093"]]), Sg = ["onClick"], _g = { class: "name" }, $g = { class: "short-cut" }, Lg = e({ __name: "SubMenu", props: { subData: { type: Array, default: () => [] } }, setup(e2, { expose: t2 }) {
  const { prefixCls: a2 } = xe("frame-shortcut"), n2 = m(false), l2 = m(0), i2 = o("subMenuRef"), C2 = Se().getThemeColor;
  return t2({ show: function(e3) {
    n2.value = true;
  }, hidden: function() {
    n2.value = false;
  } }), (t3, o2) => (s(), h(F, { name: "shortcut" }, { default: M(() => [A(p("div", { ref_key: "subMenuRef", ref: i2, class: c(d(a2)), style: u({ top: `${l2.value}px`, left: "160px" }) }, [(s(true), r(y, null, v(e2.subData, (e3) => (s(), r(y, { key: e3.name }, [p("div", { class: "menu-item", onClick: e3.action }, [g(d(Be), { icon: e3.icon, color: d(C2) }, null, 8, ["icon", "color"]), p("span", _g, S(e3.name), 1), p("span", $g, S(e3.shortcut), 1)], 8, Sg), e3.divider ? (s(), r("div", { key: 0, class: c(`${d(a2)}__divider`) }, null, 2)) : f("", true)], 64))), 128))], 6), [[z, n2.value]])]), _: 1 }));
} }), Ig = to(Lg, [["__scopeId", "data-v-f4307b07"]]), Dg = { class: "short-cut" }, Tg = { class: "short-cut" }, Ng = { class: "short-cut" }, Bg = ["onClick"], Rg = { class: "name" }, qg = { class: "short-cut" }, Og = e({ name: "FrameShortcut", __name: "index", props: { menuTop: { type: Number, default: 0 }, menuLeft: { type: Number, default: 0 }, isUp: { type: Boolean, default: false } }, setup(e2, { expose: t2 }) {
  const n2 = e2, i2 = a(() => ({ top: `${n2.menuTop}px`, left: `${n2.menuLeft}px` })), m2 = Se().getThemeColor, { prefixCls: b2 } = xe("frame-shortcut"), w2 = kp(), x2 = Or(), _2 = o("frameShortcutRef"), $2 = o("canvasAlignMenuRef"), L2 = o("selectAlignMenuRef"), I2 = o("fitMenuRef"), { NoneShortcutKeyEnum: D2 } = Tp(x2, w2);
  t2({ getBoundingClientRect: () => _2.value?.getBoundingClientRect() });
  const T2 = a(() => x2.getSelectArea.components.length > 1), N2 = l([{ icon: "iconfont:icon-mfish-copy", name: "复制", shortcut: "CTRL+C", action: w2.copyComponent, show: a(() => x2.getCurComponent || x2.getSelectArea.components.length > 0) }, { icon: "iconfont:icon-mfish-paste", name: "粘贴", shortcut: "CTRL+V", action: w2.pasteComponent, show: a(() => w2.getCopyData), divider: true }, { icon: "iconfont:icon-mfish-up-level", name: "上移一层", shortcut: "SHIFT+↑", action: w2.upComponent, show: a(() => x2.getCurComponent && !d(T2)) }, { icon: "iconfont:icon-mfish-down-level", name: "下移一层", shortcut: "SHIFT+↓", action: w2.downComponent, show: a(() => x2.getCurComponent && !d(T2)) }, { icon: "iconfont:icon-mfish-up-level", name: "置顶", shortcut: "CTRL+SHIFT+↑", action: w2.topComponent, show: a(() => x2.getCurComponent && !d(T2)) }, { icon: "iconfont:icon-mfish-bottom-level", name: "置底", shortcut: "CTRL+SHIFT+↓", action: w2.bottomComponent, show: a(() => x2.getCurComponent && !d(T2)), divider: true }, { icon: "iconfont:icon-mfish-combine", name: "组合", shortcut: "CTRL+G", action: w2.composeComponent, show: a(() => x2.getSelectArea.components.length > 1) }, { icon: "iconfont:icon-mfish-un-combine", name: "拆分", shortcut: "CTRL+SHIFT+G", action: w2.decomposeComponent, show: a(() => x2.getCurComponent?.chart.type === ns.MfCombine && !d(T2)) }, { icon: "ant-design:lock-outlined", name: "锁定", shortcut: "CTRL+L", action: w2.lockComponent, show: a(() => {
  ...
  function q2() {
    z2(Ip.CANVAS), $2.value?.show(n2.isUp);
  }
  function O2() {
    $2.value?.hidden();
  }
  function A2() {
    z2(Ip.GROUP), L2.value?.show(n2.isUp);
  }
  function j2() {
    L2.value?.hidden();
  }
  function z2(e3) {
    w2.setAlignType(e3), 0 === x2.getSelectArea.components.length && x2.getCurComponent && w2.setAlignType(Ip.CANVAS);
  }
  function E2() {
    I2.value?.show();
  }
  ...
  }, ["stop"])) }, [d(x2).getCurComponent || d(x2).getSelectArea.components.length > 1 ? (s(), r("div", { key: 0, class: "menu-item", onMouseenter: q2, onMouseleave: O2, onMouseup: t3[0] || (t3[0] = k(() => {
  }, ["stop"])) }, [g(d(Be), { icon: "iconfont:icon-mfish-left-align", color: d(m2) }, null, 8, ["color"]), t3[4] || (t3[4] = p("span", { class: "name" }, "画布对齐", -1)), p("span", Dg, [g(d(Be), { icon: "ant-design:right-outlined" })]), g(Ig, { ref_key: "canvasAlignMenuRef", ref: $2, "sub-data": B2 }, null, 8, ["sub-data"])], 32)) : f("", true), d(x2).getSelectArea.components.length > 1 ? (s(), r("div", { key: 1, class: "menu-item", onMouseenter: A2, onMouseleave: j2, onMouseup: t3[1] || (t3[1] = k(() => {
  }, ["stop"])) }, [g(d(Be), { icon: "iconfont:icon-mfish-left-align", color: d(m2) }, null, 8, ["color"]), t3[5] || (t3[5] = p("span", { class: "name" }, "选中对齐", -1)), p("span", Tg, [g(d(Be), { icon: "ant-design:right-outlined" })]), g(Ig, { ref_key: "selectAlignMenuRef", ref: L2, "sub-data": B2 }, null, 8, ["sub-data"])], 32)) : f("", true), d(x2).getCurComponent ? (s(), r("div", { key: 2, class: "menu-item", onMouseenter: E2, onMouseleave: P2, onMouseup: t3[2] || (t3[2] = k(() => {
  }, ["stop"])) }, [g(d(Be), { icon: "carbon:fit-to-screen", color: d(m2) }, null, 8, ["color"]), t3[6] || (t3[6] = p("span", { class: "name" }, "撑满", -1)), p("span", Ng, [g(d(Be), { icon: "ant-design:right-outlined" })]), g(Ig, { ref_key: "fitMenuRef", ref: I2, "sub-data": R2 }, null, 8, ["sub-data"])], 32)) : f("", true), d(x2).getCurComponent || d(x2).getSelectArea.components.length > 1 ? (s(), r("div", { key: 3, class: c(`${d(b2)}__divider`) }, null, 2)) : f("", true), (s(true), r(y, null, v(N2, (e4) => (s(), r(y, { key: e4.name }, [e4.show ? (s(), r("div", { key: 0, class: "menu-item", onClick: (t4) => {
    return o2 = e4.action, w2.setShowMenu(false), void C(() => {
      o2();
    });
    var o2;
  } }, [g(d(Be), { icon: e4.icon, color: d(m2) }, null, 8, ["icon", "color"]), p("span", Rg, S(e4.name), 1), p("span", qg, S(e4.shortcut), 1)], 8, Bg)) : f("", true), e4.show && e4.divider ? (s(), r("div", { key: 1, class: c(`${d(b2)}__divider`) }, null, 2)) : f("", true)], 64))), 128))], 38)) : f("", true)]), _: 1 }));
} }), Ag = to(Og, [["__scopeId", "data-v-2dcb7415"]]);
export {
  fa as $,
  Ka as A,
  Xo as B,
