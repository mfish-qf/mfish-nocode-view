...
const le = ["onClick"], ie = H(e({ __name: "EventListen", setup(e2) {
  ...
  return n(() => m2.getCurConfigComponent.chart.id, (e3) => {
    U[m2.getCurConfigComponent.chart.type].customEvents?.forEach((e4) => {
  ...
  }, { immediate: true }), (e3, t2) => (o(), a("div", { class: l(i(p2)) }, [(o(true), a(u, null, s(Object.keys(d2.value), (e4) => (o(), a("div", { class: "check-item", key: e4 }, [c(i($), { size: "small", checked: d2.value[e4], "onUpdate:checked": (t3) => d2.value[e4] = t3, "checked-children": "开", "un-checked-children": "关", onChange: C2 }, null, 8, ["checked", "onUpdate:checked"]), r("div", { class: l(d2.value[e4] ? "checked" : ""), onClick: (t3) => function(e5) {
    d2.value[e5] = !d2.value[e5], C2();
  }(e4) }, v(i(S)(e4, i(m2).getCurConfigComponent?.chart.type)), 11, le)]))), 128))], 2));
  ...
  n(() => _2.getCurConfigComponent?.chart.id, (e3) => {
    p2.value = false, e3 && (y2.value = u2.page?.value, y2.name = u2.page?.name, y2.img = u2.page?.img);
  }, { immediate: true });
  const w2 = (e3) => {
    p2.value = e3;
  };
  ...
  function x2(e3) {
    if (p2.value = false, y2.value = e3?.id ?? void 0, y2.name = e3?.name ?? void 0, y2.img = e3?.thumbnail ?? void 0, e3.canvasConfig) {
      const t2 = JSON.parse(e3.canvasConfig);
      y2.width = t2?.width, y2.height = t2?.height;
    }
    s2("ok", y2);
  }
  ...
  return n(() => m2.getCurConfigComponent, (e3) => {
    e3 && (g2.value = te(e3.chart.jump?.params) || []);
    ...
    g2.value.some((n3, a2) => n3.name === e5.name && a2 !== t4) ? e5.errorInfo = "参数名不能重复" : delete e5?.errorInfo;
  ...
  function E2(e3) {
    if (p2.getCurConfigComponent.chart.jump = { ...p2.getCurConfigComponent.chart.jump, open: e3 }, e3) {
      let e4 = p2.getCurConfigComponent.chart.events?.emits;
      e4 || (e4 = []);
  ...
  function R2(e3) {
    p2.getCurConfigComponent.chart.jump = { ...p2.getCurConfigComponent.chart.jump, page: e3 }, S2.value = e3?.width, H2.value = e3?.height;
  }
  function $2() {
  ...
  n(() => m2.getCurConfigComponent?.chart.id, () => {
    p2.value = false;
  }), n(() => u2.event, () => {
    k2.value = x2();
  ...
  function x2() {
    v2.getCurConfigComponent.chart.showHide?.conditions || (v2.getCurConfigComponent.chart.showHide.conditions = []), v2.getCurConfigComponent.chart.showHide.conditions.push({ event: {}, condition: "eq", value: "" });
  }
  return (e3, t2) => {
    const p3 = w("AInput");
    return o(), a("div", { class: l(i(n2)) }, [c(z, { title: "条件显隐", "allow-check": true, tooltip: "组件开启条件显示隐藏功能。开启后当事件触发时满足下面设置条件时显示，否则隐藏。", "title-check": i(v2).getCurConfigComponent.chart.showHide?.open, onChecked: b2 }, { default: d(() => [(o(true), a(u, null, s(i(v2).getCurConfigComponent.chart.showHide?.conditions, (e4, n3) => (o(), a("div", { class: "condition", key: n3, onMouseenter: (e5) => g2.value = n3, onMouseleave: t2[0] || (t2[0] = (e5) => g2.value = -1) }, [r("div", Ue, [t2[1] || (t2[1] = y(" 触发 ")), c(xe, { "filter-self": true, event: e4.event, onOk: (e5) => function(e6, t3) {
      ...
      v2.getCurConfigComponent.chart.showHide.conditions.splice(e6, 1), delete v2.getCurConfigComponent.chart.showHide?.show;
    }(n3) }, null, 8, ["onClick"])) : h("", true)], 40, Ie))), 128)), r("div", { class: "add-button", onClick: x2 }, [c(i(G), { icon: "ant-design:plus-outlined" }), t2[4] || (t2[4] = y(" 添加显隐条件 "))])]), _: 1 }, 8, ["title-check"])], 2);
  };
